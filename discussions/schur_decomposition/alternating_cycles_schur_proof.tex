...
\title{Alternating Cycles of Length \texorpdfstring{$2\pmod 4$}{2 mod 4}:\\
A Schur--Complement Proof}
\author{}
\date{}
...
Schur-complement factorization produces a scalar excursion series whose
coefficients form a decreasing nonnegative sequence; the parity
$m\equiv1\pmod 2$ then gives the required sign after logarithmic cyclic
assembly.
\end{abstract}
...
\[
  ...
  \qquad x_{r+1}:=x_1.
\]
This definition is used below also for the signed kernel $2W-1$.  For
...
\end{align*}
Thus the colors alternate red, blue, red, blue around the cycle.
\end{definition}
...
The strengthened inequality also gives quantitative quasirandomness.
We use the cut norm
\[
  \|K\|_{\square}
  :=
...
\[
  ...
  \qquad 0\le a<2^n,
\]
with the usual harmless modification at the endpoint $1$, and let
...
\begin{equation}
  ...
  \label{eq:HS-budget}
\end{equation}
Thus Theorem~\ref{thm:main} follows from the abstract matrix theorem in
the next section.

\section{The abstract rank-one matrix inequality}
...
The proof occupies the next four sections.  Put
\begin{equation}
  L:=(P+X)(P-X).
  \label{eq:def-L}
\end{equation}
...
\section{The rank-two Schur determinant}

All power-series identities in this section may be read either formally
or as Taylor expansions for $z$ sufficiently close to zero.  Since all
matrices are finite-dimensional, the two interpretations are equivalent
...
\begin{lemma}[Rank-two determinant factorization]
...
\section{The excursion coefficients}

Diagonalize $X$ in an orthonormal eigenbasis:
\[
  Xv_i=\lambda_i v_i,
  \qquad
  \omega_i:=|\ip{e}{v_i}|^2.
\]
Eigenvalues are indexed with multiplicity.  Then
\begin{equation}
  \omega_i\ge0,
  \qquad
  \sum_i\omega_i=1,
...
\]
The function $f$ is increasing on $\R$, so this divided difference is
nonnegative.  If $x=-y$, the continuous value is
...
\begin{lemma}[Monotone excursion coefficients]
...
\end{align*}
where the last inequality follows because the eigenvalues are indexed
...
The following purely one-dimensional lemma is the parity step of the
proof.
...
\section{Trace extraction and proof of the matrix theorem}
...
Take minus the logarithm of the determinant factorization
\eqref{eq:det-factor}:
\begin{equation}
  -\log\det(I-zL)
  =
...
The proof actually supplies an explicit nonnegative defect.

\begin{proposition}[Exact matrix defect]
...
\section{Proof of the graphon theorem}

\begin{proof}[Proof of Theorem~\ref{thm:main}]
By Lemma~\ref{lem:step-reduction}, first assume that $W$ is a symmetric
step graphon.  Work on the finite-dimensional space $E$ from
Section~2, and define $P$ and $X$ as in \eqref{eq:color-coordinate}.
The Hilbert--Schmidt budget \eqref{eq:HS-budget} gives
\[
  \Tr(X^2)\le1.
\]
...
\section{A fixed-density consequence}

Although the theorem is an unrestricted semi-inducibility result, the
signed-cycle defect also gives a simple density-sensitive bound.
...
\section{Why the congruence class
\texorpdfstring{$2\pmod 4$}{2 mod 4} is essential}

The argument uses that the paired length $m$ is odd.  The opposite
parity genuinely behaves differently.

Let $[0,1]=A\sqcup B$ modulo null sets, where $A$ and $B$ are
measurable sets of measure $1/2$, and let
\[
  \varphi:=\one_A-\one_B,
\]
and let $W$ be the complete bipartite graphon associated with this
partition.  Then
\[
  2W-1=-\varphi\otimes\varphi.
\]
Put
\[
  Q:=\varphi\otimes\varphi.
\]
The rank-one projections $P=\one\otimes\one$ and $Q$ have orthogonal
ranges.  Thus $X=-Q$ and
\[
  (P+X)(P-X)=(P-Q)(P+Q)=P-Q.
\]
...
\begin{equation}
  \Tr\left(\bigl((P+X)(P-X)\bigr)^m\right)
  =\Tr((P-Q)^m)=1+(-1)^m.
  \label{eq:parity-example}
\end{equation}
If $m$ is odd, this trace is $0$.  If $m$ is even, it is $2$, and hence
\[
  \Alt_{2m}(W)=2\cdot4^{-m},
\]
twice the random-coloring value.  Thus the theorem cannot extend from
cycles of length $4k+2$ to cycles of length $4k$.

The same example is informative for odd $m$: there
$\Tr\bigl(((P+X)(P-X))^m\bigr)=0$ while $\Tr(X^{2m})=\Tr(Q)=1$, so
equality holds in the strengthened inequality
\eqref{eq:main-strengthened} with the two terms in the extreme
position opposite to the constant graphon $W\equiv1/2$, where
$4^m\Alt_{2m}(W)=1$ and the signed-cycle term vanishes.  The
strengthened inequality is therefore sharp at both ends, and neither
term can be dropped or given a larger coefficient.

At the level of Lemma~\ref{lem:odd-log}, this dichotomy is exact.  When
$m$ is odd, every coefficient $[z^{m-2r}]G(-z)^r$ has a minus sign.  If
$m$ is even, those coefficients have a plus sign instead.
...
\item Pass to a step graphon so that all operators are finite matrices.
\item Use the signed color coordinate
\[
  T_W=\frac{P+X}{2},
  \qquad
  T_{1-W}=\frac{P-X}{2},
\]
where $P$ is the projection onto constants and
$\Tr(X^2)\le1$.
\item Express the alternating density as
\[
  4^m\Alt_{2m}(W)
  =\Tr\left(\bigl((P+X)(P-X)\bigr)^m\right).
\]
\item Strip off the rank-one state $P$ by the rank-two determinant
factorization
\[
  \det(I-z(P+X)(P-X))
  =\det(I+zX^2)(1-zF(z)).
\]
\item Diagonalize $X$ and write
\[
  F(z)=\sum_{n\ge0}(-1)^n\beta_nz^n.
\]
The Hilbert--Schmidt budget and the recurrence for the divided-difference
polynomials $c_n$ imply
\[
  1=\beta_0\ge\beta_1\ge\beta_2\ge\cdots\ge0.
\]
\item Successive differences $d_n=\beta_n-\beta_{n+1}$ factor the scalar
return series as
\[
  (1+z)(1-zF(z))=1-z^2G(-z),
  \qquad G(z)=\sum d_nz^n.
\]
\item For odd $m$, logarithmic cyclic assembly gives a negative sign to
every nonconstant contribution.  Equivalently,
\[
  1-\Tr(L^m)-\Tr(X^{2m})
  =
  m\sum_{r=1}^{(m-1)/2}\frac1r[z^{m-2r}]G(z)^r\ge0.
\]
\end{enumerate}

Thus the proof is a direct instance of the principle
\[
\begin{gathered}
  \text{separate a distinguished state}
  \;\longrightarrow\;
  \text{encode returns by a Schur complement}\\
  \longrightarrow\;
  \text{use parity in logarithmic cyclic assembly}.
\end{gathered}
\]
