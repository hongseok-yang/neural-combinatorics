...
the dual certificate lambda = min(1, 2 rho xi) pays: R_m <= C_m psi(xi, rho).
...
(e) kappa -> 0 bottom-out (k1 = 0 boxes): pointwise, R_m > 0 forces
    ...
    payment >= c0_lo kappa^2 m (case I; the box is arranged to be case I).
    Since exp(-a/kappa)/kappa^2 is increasing in kappa for kappa <= a/2, a single
    check at kappa = k2 certifies all kappa in (0, k2].
...
class Box:
    ...
    def __init__(self, e1, e2, k1, k2):
        ...
        self.f_lo = max(self.a_lo - self.L_up, self.d_lo + self.delta_lo / 2)
...
def bottom_out(B: Box, depth: int) -> bool:
    ...
    if not (k2 <= a / 2):               # monotonicity range of exp(-a/k)/k^2
        return subdivide(B.e1, B.e2, B.k1, k2, depth)
