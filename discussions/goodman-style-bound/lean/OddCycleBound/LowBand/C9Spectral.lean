...
theorem c9_cycle_bound_of_mass_bound
  ...
  exact LowBand.C9.cycle_bound_of_negative_mass_bound
...
threshold and still leaves nonnegative square budget, then the cubic capacity
...
polynomial estimates and the algebraic `ell`-reduction in `LowBand.C9`. -/
theorem c9LinearScalarCapacityExclusion :
    C9LinearScalarCapacityExclusion := by
  intro p ell z hgt hle hpell hz0 hztarget hbudget
  let q : Real := 1 - p
  let eps : Real := p - 1 / 2
  ...
  have hDeltaAlpha : 0 <= p - ell ^ 2 - alpha ^ 2 := by
    ...
    nlinarith
  have hcapacity_lt :=
    LowBand.C9.cubic_capacity_strict_decreases_of_ninth
      (S := p - ell ^ 2) halpha0 hz0 halpha_lt_z_ninth hbudget
  have hell :=
    LowBand.C9.ell_reduction_bound
      (p := p) (q := q) (ell := ell) (alpha0 := alpha0) (alpha := alpha)
      hp0 halpha00 halpha0_le_p hpell halpha09 halpha9 hDeltaAlpha
  have hend :=
    LowBand.C9.endpoint_scalar_bound
...
facts used in the proof of the negative-mass estimate: the square budget, the
...
This is the integral `0 <= W <= 1` part of the Hilbert-Schmidt square-budget
...
This is the square-budget estimate in kernel-norm form.  The identity
...
/-- Countable spectral trace data before the elementary graphon square-budget
...
structure C9BudgetTraceSpectralData
    (W : Omega -> Omega -> Real) (mu : Measure Omega) where
  eigen : Nat -> Real
  summable_square : Summable fun n : Nat => eigen n ^ 2
  square_budget :
...
theorem finite_energy_budget_of_row_energy_identity
    {E : Type*} [NormedAddCommGroup E] [InnerProductSpace Real E]
    {row : Omega -> E}
    {mode : Nat -> E}
...
This is the abstract Hilbert-space bridge behind the graphon square budget:
...
/-- A finite-energy bound on the listed modes gives the finite square-budget
...
/-- No-diagonal compact-action data with only the square budget, not the
...
structure C9CanonicalL2CompactActionBudgetTraceSpectralDataNoDiag
    [IsProbabilityMeasure mu]
    (hW : IsGraphon W mu) where
  mode : Nat -> Lp Real 2 mu
  eigen : Nat -> Real
...
/-- No-diagonal compact-action data with only the square budget and no
...
structure C9CanonicalL2CompactActionBudgetSpectralDataNoDiag
    [IsProbabilityMeasure mu]
    (hW : IsGraphon W mu) where
  mode : Nat -> Lp Real 2 mu
  eigen : Nat -> Real
...
/-- No-diagonal compact-action data with finite square budgets and no
cube/ninth trace assumptions.

This is the finite-estimate version of
`C9CanonicalL2CompactActionBudgetSpectralDataNoDiag`: Lean derives square
summability and the infinite budget from the initial-segment inequalities,
then derives cube/ninth trace identities from the action expansion. -/
structure C9CanonicalL2CompactActionFiniteBudgetSpectralDataNoDiag
    [IsProbabilityMeasure mu]
    (hW : IsGraphon W mu) where
  mode : Nat -> Lp Real 2 mu
  eigen : Nat -> Real
...
This is the most local square-budget interface in this file: it does not
...
structure C9CanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiag
    [IsProbabilityMeasure mu]
    (hW : IsGraphon W mu) where
  mode : Nat -> Lp Real 2 mu
  eigen : Nat -> Real
...
`∑ eigen n² <= edgeDensity`: the finite square budget is derived from
...
kernel, viewed in `L²`, have square budget and represent the finite energy of
...
/-- Forget the square trace identity, keeping only the square budget needed by
the C9 low-band argument. -/
def toCompactActionBudgetTraceSpectralDataNoDiag
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionTraceSpectralDataNoDiag hW) :
    C9CanonicalL2CompactActionBudgetTraceSpectralDataNoDiag hW where
  mode := S.mode
  eigen := S.eigen
  mode_orthonormal := S.mode_orthonormal
  principal_max := S.principal_max
...
namespace C9CanonicalL2CompactActionBudgetSpectralDataNoDiag

/-- The eigenvalues listed in no-trace budget compact-action graphon data are
bounded by one. -/
theorem abs_eigen_le_one
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionBudgetSpectralDataNoDiag hW) :
    ∀ n, |S.eigen n| <= 1 :=
  abs_eigen_le_one_of_graphon_action_expansion
    (mu := mu) (W := W) S.mode S.eigen
    S.mode_orthonormal S.action_expansion

/-- The no-trace budget compact-action expansion propagates through every
...
the no-trace budget compact-action interface. -/
theorem hasSum_integral_graphon_row_weighted_inner_sq
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionBudgetSpectralDataNoDiag hW)
...
no-trace budget compact-action data. -/
theorem trace_compPow_hasSum_eigen_pow
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionBudgetSpectralDataNoDiag hW)
...
/-- Convert no-trace budget compact-action data into the existing budget trace
...
end C9CanonicalL2CompactActionBudgetSpectralDataNoDiag

namespace C9CanonicalL2CompactActionFiniteBudgetSpectralDataNoDiag

/-- Finite square-budget no-trace compact-action data imply the infinite
budget no-trace package. -/
def toCompactActionBudgetSpectralDataNoDiag
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionFiniteBudgetSpectralDataNoDiag hW) :
    C9CanonicalL2CompactActionBudgetSpectralDataNoDiag hW :=
  let hsq :=
    summable_square_and_tsum_le_of_sum_range_square_le
      S.eigen S.finite_square_budget
  {
    mode := S.mode
    eigen := S.eigen
    mode_orthonormal := S.mode_orthonormal
...
end C9CanonicalL2CompactActionFiniteBudgetSpectralDataNoDiag

namespace C9CanonicalL2CompactActionEnergySpectralDataNoDiag

/-- Energy compact-action data imply the finite square-budget no-trace
package. -/
def toCompactActionFiniteBudgetSpectralDataNoDiag
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionEnergySpectralDataNoDiag hW) :
    C9CanonicalL2CompactActionFiniteBudgetSpectralDataNoDiag hW where
  mode := S.mode
  eigen := S.eigen
  mode_orthonormal := S.mode_orthonormal
  principal_max := S.principal_max
  ...
  finite_square_budget :=
    sum_range_eigen_sq_le_of_action_eigen_expansion_and_energy_bound
      (L2Kernel.kernelOpCLM (mu := mu) hW) S.mode S.eigen
      S.mode_orthonormal S.action_expansion S.finite_energy_budget

end C9CanonicalL2CompactActionEnergySpectralDataNoDiag

namespace C9CanonicalL2CompactActionBudgetTraceSpectralDataNoDiag

/-- The eigenvalues listed in budget no-diagonal compact-action graphon data
are bounded by one. -/
theorem abs_eigen_le_one
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionBudgetTraceSpectralDataNoDiag hW) :
    ∀ n, |S.eigen n| <= 1 :=
  abs_eigen_le_one_of_graphon_action_expansion
    (mu := mu) (W := W) S.mode S.eigen
    S.mode_orthonormal S.action_expansion
...
/-- If the canonical graphon operator is compact, the eigenvalue list in
budget no-diagonal compact-action data tends to zero. -/
theorem eigen_tendsto_zero
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionBudgetTraceSpectralDataNoDiag hW)
...
/-- Every listed spectral value in budget no-diagonal compact-action data is an
actual eigenvalue of the canonical graphon operator. -/
theorem hasEigenvalue
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
...
/-- In positive density, the top listed coefficient in budget no-diagonal
compact-action data is nonnegative. -/
theorem principal_nonneg_of_edgeDensity_pos
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
...
/-- Budget compact-action data give the graphon-facing budget trace package in
positive density. -/
def toBudgetTraceSpectralData
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionBudgetTraceSpectralDataNoDiag hW)
    (hp : 0 < edgeDensity W mu) :
    C9BudgetTraceSpectralData W mu where
  eigen := S.eigen
  summable_square := S.summable_square
  square_budget := S.square_budget
  trace_cube := S.trace_cube
...
end C9CanonicalL2CompactActionBudgetTraceSpectralDataNoDiag

namespace C9CanonicalL2CompactActionBudgetSpectralDataNoDiag

/-- In positive density, no-trace budget compact-action data give the
graphon-facing budget trace package. -/
def toBudgetTraceSpectralData
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionBudgetSpectralDataNoDiag hW)
    (hp : 0 < edgeDensity W mu) :
    C9BudgetTraceSpectralData W mu :=
  (S.toCompactActionBudgetTraceSpectralDataNoDiag
    (mu := mu) (hW := hW)).toBudgetTraceSpectralData hp

end C9CanonicalL2CompactActionBudgetSpectralDataNoDiag

namespace C9CanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiag

/-- The eigenvalues listed in finite-budget no-diagonal compact-action graphon
data are bounded by one. -/
theorem abs_eigen_le_one
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiag hW) :
    ∀ n, |S.eigen n| <= 1 :=
  abs_eigen_le_one_of_graphon_action_expansion
    (mu := mu) (W := W) S.mode S.eigen
    S.mode_orthonormal S.action_expansion
...
/-- If the canonical graphon operator is compact, the eigenvalue list in
finite-budget no-diagonal compact-action data tends to zero. -/
theorem eigen_tendsto_zero
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiag hW)
...
/-- Every listed spectral value in finite-budget no-diagonal compact-action
...
/-- Finite square-budget compact-action data imply the infinite square-budget
package.
...
def toCompactActionBudgetTraceSpectralDataNoDiag
    ...
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiag hW) :
    C9CanonicalL2CompactActionBudgetTraceSpectralDataNoDiag hW :=
  let hsq :=
    summable_square_and_tsum_le_of_sum_range_square_le
      S.eigen S.finite_square_budget
  {
...
/-- In positive density, finite square-budget compact-action data give the
graphon-facing budget trace package. -/
def toBudgetTraceSpectralData
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    ...
    (hp : 0 < edgeDensity W mu) :
    C9BudgetTraceSpectralData W mu :=
  S.toCompactActionBudgetTraceSpectralDataNoDiag.toBudgetTraceSpectralData hp

end C9CanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiag
...
/-- Energy compact-action data imply the finite square-budget package.

The square budget is proved, not copied: diagonal action turns
`‖T mode_n‖²` into `eigen_n²`, and the finite-energy hypothesis supplies the
kernel-side bound. -/
def toCompactActionFiniteBudgetTraceSpectralDataNoDiag
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionEnergyTraceSpectralDataNoDiag hW) :
    C9CanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiag hW where
  mode := S.mode
  eigen := S.eigen
  mode_orthonormal := S.mode_orthonormal
  principal_max := S.principal_max
...
def toBudgetTraceSpectralData
    ...
    (hp : 0 < edgeDensity W mu) :
    C9BudgetTraceSpectralData W mu :=
  S.toCompactActionFiniteBudgetTraceSpectralDataNoDiag.toBudgetTraceSpectralData hp
...
identities and the row norm budget are graphon kernel facts already proved in
`L2Kernel`. -/
...
/-- Finite initial square budgets for padded core data.
...
/-- Square summability and the infinite square budget follow from the finite
initial square budgets for padded core data. -/
theorem summable_square_and_square_budget
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionPaddedCoreSpectralDataNoDiag hW) :
  ...
  summable_square_and_tsum_le_of_sum_range_square_le
    S.eigen (S.finite_square_budget (mu := mu) (hW := hW))
...
theorem summable_square
    ...
    (S : C9CanonicalL2CompactActionPaddedCoreSpectralDataNoDiag hW) :
    Summable (fun n : Nat => S.eigen n ^ 2) :=
  (S.summable_square_and_square_budget (mu := mu) (hW := hW)).1

/-- Infinite square budget for padded core data. -/
theorem square_budget
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionPaddedCoreSpectralDataNoDiag hW) :
    (∑' n : Nat, S.eigen n ^ 2) <= edgeDensity W mu :=
  (S.summable_square_and_square_budget (mu := mu) (hW := hW)).2
...
theorem abs_eigen_le_one
  ...
  have hsq_le_one :
      S.eigen n ^ 2 <= 1 :=
    hsq_le_edge.trans
      ((S.square_budget (mu := mu) (hW := hW)).trans (edgeDensity_le_one hW))
...
/-- Padded compact-action data give the graphon-facing budget trace package in
positive density. -/
def toBudgetTraceSpectralData
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
...
/-- Finite initial square budgets for direct-principal padded core data. -/
theorem finite_square_budget
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiag hW) :
...
/-- Square summability and the infinite square budget follow from the finite
initial square budgets for direct-principal padded core data. -/
theorem summable_square_and_square_budget
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiag hW) :
...
/-- Infinite square budget for direct-principal padded core data. -/
theorem square_budget
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiag hW) :
...
/-- Direct-principal padded compact-action data give the graphon-facing budget
trace package. -/
def toBudgetTraceSpectralData
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
...
def toCompactActionEnergySpectralDataNoDiag
  ...
  action_expansion := S.action_expansion
  finite_energy_budget :=
    finite_energy_budget_of_row_energy_identity
      (mu := mu) S.mode_orthonormal
      S.row_finite_integrable S.row_norm_integrable
      S.row_energy S.row_norm_budget
...
/-- Row-energy compact-action data imply the finite square-budget package. -/
def toCompactActionFiniteBudgetTraceSpectralDataNoDiag
    [IsProbabilityMeasure mu]
    {hW : IsGraphon W mu}
    (S : C9CanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiag hW) :
    C9CanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiag hW :=
  S.toCompactActionEnergyTraceSpectralDataNoDiag.toCompactActionFiniteBudgetTraceSpectralDataNoDiag

/-- Row-energy compact-action data imply the graphon-facing budget trace
...
def toBudgetTraceSpectralData
    ...
    (hp : 0 < edgeDensity W mu) :
    C9BudgetTraceSpectralData W mu :=
  S.toCompactActionEnergyTraceSpectralDataNoDiag.toBudgetTraceSpectralData hp
...
This discharges the row integrability, row norm budget, and finite row-energy
...
/-- Bounded-representative compact-action data imply the graphon-facing budget
...
def toBudgetTraceSpectralData
    ...
    (hp : 0 < edgeDensity W mu) :
    C9BudgetTraceSpectralData W mu :=
  S.toCompactActionRowEnergyTraceSpectralDataNoDiag.toBudgetTraceSpectralData hp
...
The only extra input is `IsGraphon W mu`, used to prove the square budget from
...
namespace C9BudgetTraceSpectralData
...
def toC9SpectralData
    [IsProbabilityMeasure mu]
    (S : C9BudgetTraceSpectralData W mu) (hW : IsGraphon W mu) :
    C9SpectralData W mu :=
...
end C9BudgetTraceSpectralData

namespace C9RawTraceSpectralData

/-- Raw trace data imply budget trace data by applying the elementary graphon
...
eigenvalue data with the square budget, Rayleigh principal bound, and cube/ninth
...
graphon square-budget inequality. -/
...
def C9GraphonRawTraceSpectralDataForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega) : Prop :=
  IsGraphon W mu ->
  1 / 2 < edgeDensity W mu ->
  edgeDensity W mu <= 1003 / 2000 ->
...
/-- Budget trace spectral data for the C9 low band only.

This is weaker than raw trace data: the square trace identity is replaced by
the square budget `∑ λ_n^2 <= edgeDensity W μ`. -/
def C9GraphonBudgetTraceSpectralDataForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega) : Prop :=
  IsGraphon W mu ->
  1 / 2 < edgeDensity W mu ->
  edgeDensity W mu <= 1003 / 2000 ->
  Nonempty (C9BudgetTraceSpectralData W mu)
...
def C9GraphonCanonicalL2CompactActionTraceSpectralDataForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
def C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
square budget, not the square trace identity. -/
def C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
    edgeDensity W mu <= 1003 / 2000 ->
    Nonempty (C9CanonicalL2CompactActionBudgetTraceSpectralDataNoDiag hW)

/-- Low-band compact-action spectral data with a square budget and no
...
def C9GraphonCanonicalL2CompactActionBudgetSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
    edgeDensity W mu <= 1003 / 2000 ->
    Nonempty (C9CanonicalL2CompactActionBudgetSpectralDataNoDiag hW)

/-- Low-band compact-action finite-budget spectral data with no cube/ninth
trace assumptions. -/
def C9GraphonCanonicalL2CompactActionFiniteBudgetSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
    edgeDensity W mu <= 1003 / 2000 ->
    Nonempty (C9CanonicalL2CompactActionFiniteBudgetSpectralDataNoDiag hW)

/-- Low-band compact-action finite-energy spectral data with no cube/ninth
trace assumptions. -/
def C9GraphonCanonicalL2CompactActionEnergySpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
assumptions. -/
def C9GraphonCanonicalL2CompactActionRowEnergySpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
def C9GraphonCanonicalL2CompactActionCoreSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
Unlike `C9GraphonCanonicalL2CompactActionCoreSpectralDataNoDiagForLowBand`,
this allows zero padding and is therefore compatible with finite spectra and
finite-dimensional graphon `L²` spaces. -/
def C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
condition. -/
def C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
into that subspace. -/
def C9GraphonCanonicalL2CompactActionNonzeroHilbertBasisSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
def C9GraphonCanonicalL2CompactActionZeroOrthogonalHilbertBasisSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
`(eigenspace T 0)ᗮ`. -/
def C9GraphonCanonicalL2CompactActionZeroOrthogonalHilbertBasisEigenDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
ordering. -/
def C9GraphonCanonicalL2CompactActionZeroOrthogonalHilbertBasisEigenPrincipalBoundDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
def C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
def C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenRayleighDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
def C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenOpNormDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
assumptions. -/
def C9GraphonCanonicalL2CompactActionGoodRowSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
budgets instead of an infinite square summability/budget field. -/
def C9GraphonCanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
    edgeDensity W mu <= 1003 / 2000 ->
    Nonempty (C9CanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiag hW)
...
finite-square-budget package: Lean derives the finite eigenvalue-square budget
from this energy bound. -/
def C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
def C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
rows. -/
def C9GraphonCanonicalL2CompactActionGoodRowTraceSpectralDataNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
def C9GraphonCanonicalL2CompactActionTraceSpectralDataHasSumNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
Compared with
`C9GraphonCanonicalL2CompactActionTraceSpectralDataHasSumNoDiagForLowBand`,
...
def C9GraphonCanonicalL2CompleteCompactActionTraceSpectralDataHasSumNoDiagForLowBand
    (W : Omega -> Omega -> Real) (mu : Measure Omega)
    [IsProbabilityMeasure mu] : Prop :=
  ∀ hW : IsGraphon W mu,
    1 / 2 < edgeDensity W mu ->
...
bound, while the square budget is derived separately from graphon boundedness. -/
...
argument is used. -/
def C9GraphonRawTraceSpectralDataLowBandTheorem : Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
    IsGraphon W mu ->
...
/-- Global low-band budget graphon spectral theorem needed for C9.

Compared with `C9GraphonRawTraceSpectralDataLowBandTheorem`, this removes the
square trace identity from the external theorem statement. -/
def C9GraphonBudgetTraceSpectralDataLowBandTheorem : Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
    IsGraphon W mu ->
...
`L²` graphon operator. -/
def C9GraphonCanonicalL2CompactActionTraceSpectralDataLowBandTheorem : Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
    ∀ hW : IsGraphon W mu,
...
diagonal field. -/
def C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagLowBandTheorem : Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
    ∀ hW : IsGraphon W mu,
...
/-- Global low-band compact-action budget theorem without an explicit diagonal
field. -/
def C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
/-- Global low-band compact-action budget theorem without cube/ninth trace
assumptions.  The trace identities are derived from the action expansion. -/
def C9GraphonCanonicalL2CompactActionBudgetSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
/-- Global low-band compact-action finite-budget theorem without cube/ninth
trace assumptions. -/
def C9GraphonCanonicalL2CompactActionFiniteBudgetSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
trace assumptions. -/
def C9GraphonCanonicalL2CompactActionEnergySpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
assumptions. -/
def C9GraphonCanonicalL2CompactActionRowEnergySpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
def C9GraphonCanonicalL2CompactActionCoreSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
def C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
def C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
def C9GraphonCanonicalL2CompactActionNonzeroHilbertBasisSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
subspace `(eigenspace T 0)ᗮ`. -/
def C9GraphonCanonicalL2CompactActionZeroOrthogonalHilbertBasisSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
subspace `(eigenspace T 0)ᗮ`. -/
def C9GraphonCanonicalL2CompactActionZeroOrthogonalHilbertBasisEigenDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
def C9GraphonCanonicalL2CompactActionZeroOrthogonalHilbertBasisEigenPrincipalBoundDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
bound. -/
def C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
def C9GraphonCanonicalL2CompactLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
def C9GraphonHilbertSchmidtFiniteRankApproxLowBandTheorem :
    Prop :=
  ∀ {Omega : Type u} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
theorem C9GraphonHilbertSchmidtFiniteRankApproxLowBandTheorem.proved :
    C9GraphonHilbertSchmidtFiniteRankApproxLowBandTheorem.{u} := by
...
theorem C9GraphonCanonicalL2CompactLowBandTheorem.of_hilbertSchmidt_finiteRank_approx
    (happrox : C9GraphonHilbertSchmidtFiniteRankApproxLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
  exact
...
theorem C9GraphonCanonicalL2CompactLowBandTheorem.proved :
    C9GraphonCanonicalL2CompactLowBandTheorem.{u} :=
  C9GraphonCanonicalL2CompactLowBandTheorem.of_hilbertSchmidt_finiteRank_approx
    C9GraphonHilbertSchmidtFiniteRankApproxLowBandTheorem.proved
...
def C9GraphonCanonicalL2CompactPositiveNormEndpointLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
def C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenRayleighDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
norm. -/
def C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenOpNormDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
assumptions. -/
def C9GraphonCanonicalL2CompactActionGoodRowSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
/-- Global low-band compact-action finite-budget theorem without an explicit
diagonal field. -/
def C9GraphonCanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
field.  The finite eigenvalue-square budget is derived from its finite
operator-energy estimates. -/
def C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
def C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
the listed modes. -/
def C9GraphonCanonicalL2CompactActionGoodRowTraceSpectralDataNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
stated directly as `HasSum`s. -/
def C9GraphonCanonicalL2CompactActionTraceSpectralDataHasSumNoDiagLowBandTheorem : Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
    ∀ hW : IsGraphon W mu,
...
def C9GraphonCanonicalL2CompleteCompactActionTraceSpectralDataHasSumNoDiagLowBandTheorem :
    Prop :=
  ∀ {Omega : Type*} [MeasurableSpace Omega]
    {mu : Measure Omega} [IsProbabilityMeasure mu]
    {W : Omega -> Omega -> Real},
...
/-- Raw low-band trace data imply the weaker budget low-band trace package. -/
theorem C9GraphonBudgetTraceSpectralDataForLowBand.of_raw
    [IsProbabilityMeasure mu]
    (hspec : C9GraphonRawTraceSpectralDataForLowBand W mu) :
    C9GraphonBudgetTraceSpectralDataForLowBand W mu := by
  intro hW hgt hle
  exact ⟨(Classical.choice (hspec hW hgt hle)).toBudgetTraceSpectralData hW⟩
...
completeness. -/
theorem C9GraphonCanonicalL2CompactActionTraceSpectralDataForLowBand.of_hilbert
    [IsProbabilityMeasure mu]
    (hspec : C9GraphonCanonicalL2HilbertTraceSpectralDataFor W mu) :
    C9GraphonCanonicalL2CompactActionTraceSpectralDataForLowBand W mu := by
  intro hW _hgt _hle
...
theorem C9GraphonCanonicalL2CompactActionTraceSpectralDataForLowBand.of_no_diag
    [IsProbabilityMeasure mu]
    (hspec : C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagForLowBand
      W mu) :
    C9GraphonCanonicalL2CompactActionTraceSpectralDataForLowBand W mu := by
  intro hW hgt hle
  exact
...
theorem C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagForLowBand.of_hasSum_no_diag
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionTraceSpectralDataHasSumNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagForLowBand W mu := by
  intro hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toCompactActionTraceSpectralDataNoDiag⟩

/-- No-diagonal compact-action trace data imply the weaker budget package by
forgetting the square trace identity. -/
theorem C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand.of_no_diag
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toCompactActionBudgetTraceSpectralDataNoDiag⟩

/-- No-trace budget compact-action data imply the existing budget trace
package: cube and ninth trace identities are derived by term-by-term
integration of the row-coordinate expansion. -/
theorem C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand.of_budget_no_trace
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionBudgetSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
...
/-- Finite-budget no-trace compact-action data imply the infinite-budget
no-trace compact-action package. -/
theorem C9GraphonCanonicalL2CompactActionBudgetSpectralDataNoDiagForLowBand.of_finite_budget_no_trace
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionFiniteBudgetSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionBudgetSpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toCompactActionBudgetSpectralDataNoDiag⟩

/-- Energy no-trace compact-action data imply the finite-budget no-trace
compact-action package. -/
theorem C9GraphonCanonicalL2CompactActionFiniteBudgetSpectralDataNoDiagForLowBand.of_energy_no_trace
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionEnergySpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionFiniteBudgetSpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toCompactActionFiniteBudgetSpectralDataNoDiag⟩

/-- Row-energy no-trace compact-action data imply energy no-trace data. -/
theorem C9GraphonCanonicalL2CompactActionEnergySpectralDataNoDiagForLowBand.of_row_energy_no_trace
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionRowEnergySpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionEnergySpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
...
theorem C9GraphonCanonicalL2CompactActionCoreSpectralDataNoDiagLowBandTheorem.of_global
    (hspec :
      C9GraphonCanonicalL2CompactActionCoreSpectralDataNoDiagTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionCoreSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW _hgt _hle
  exact @hspec Omega _ mu _ W hW

/-- The global padded pure compact-action theorem implies its low-band form. -/
theorem C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.of_global
    (hspec :
      C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW _hgt _hle
  exact @hspec Omega _ mu _ W hW

/-- The global Rayleigh dense-eigenmode theorem implies its low-band form. -/
theorem C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenRayleighDataNoDiagLowBandTheorem.of_global
    (hspec :
      C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenRayleighDataNoDiagTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenRayleighDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW _hgt _hle
  exact @hspec Omega _ mu _ W hW

/-- The global op-norm dense-eigenmode theorem implies its low-band form. -/
theorem C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenOpNormDataNoDiagLowBandTheorem.of_global
    (hspec :
      C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenOpNormDataNoDiagTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenOpNormDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW _hgt _hle
  exact @hspec Omega _ mu _ W hW
...
theorem C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.of_positiveNormEndpoint
    (hspec :
      C9GraphonCanonicalL2CompactPositiveNormEndpointLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
...
package. -/
theorem C9GraphonCanonicalL2CompactPositiveNormEndpointLowBandTheorem.of_compact
    (hcompact :
      C9GraphonCanonicalL2CompactLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactPositiveNormEndpointLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
...
theorem C9GraphonCanonicalL2CompactPositiveNormEndpointLowBandTheorem.proved :
    C9GraphonCanonicalL2CompactPositiveNormEndpointLowBandTheorem.{u} :=
  C9GraphonCanonicalL2CompactPositiveNormEndpointLowBandTheorem.of_compact
    C9GraphonCanonicalL2CompactLowBandTheorem.proved

/-- Low-band compactness alone supplies the dense principal-bound eigenmode
theorem used by the C9 pipeline. -/
theorem C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.of_compact
    (hcompact :
      C9GraphonCanonicalL2CompactLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.{u} :=
  C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.of_positiveNormEndpoint
    (C9GraphonCanonicalL2CompactPositiveNormEndpointLowBandTheorem.of_compact hcompact)

/-- The dense principal-bound eigenmode package is proved in the C9 low band
from the compactness theorem. -/
theorem C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.proved :
    C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.{u} :=
  C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.of_compact
    C9GraphonCanonicalL2CompactLowBandTheorem.proved

/-- The older full-orthonormal core theorem implies the padded low-band core
theorem. -/
theorem C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.of_core
    (hspec :
      C9GraphonCanonicalL2CompactActionCoreSpectralDataNoDiagLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
  exact
...
core theorem. -/
theorem C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.of_nonzero_hilbertBasis
    (hspec :
      C9GraphonCanonicalL2CompactActionNonzeroHilbertBasisSpectralDataNoDiagLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toPaddedCoreSpectralDataNoDiag⟩
...
core theorem. -/
theorem C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.of_zero_orthogonal_hilbertBasis
    (hspec :
      C9GraphonCanonicalL2CompactActionZeroOrthogonalHilbertBasisSpectralDataNoDiagLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toPaddedCoreSpectralDataNoDiag⟩
...
core theorem. -/
theorem C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.of_zero_orthogonal_eigenBasis
    (hspec :
      C9GraphonCanonicalL2CompactActionZeroOrthogonalHilbertBasisEigenDataNoDiagLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toPaddedCoreSpectralDataNoDiag⟩
...
theorem C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.of_zero_orthogonal_eigenBasis_principal_bound
    (hspec :
      C9GraphonCanonicalL2CompactActionZeroOrthogonalHilbertBasisEigenPrincipalBoundDataNoDiagLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
  exact
...
theorem C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.of_zero_orthogonal_orthonormal_eigen_principal_bound
    (hspec :
      C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
...
padded compact-action package needed by the low-band C9 budget route. -/
theorem C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.proved :
    C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.{u} :=
  C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.of_zero_orthogonal_orthonormal_eigen_principal_bound
    C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenPrincipalBoundDataNoDiagLowBandTheorem.proved

/-- Rayleigh-form dense countable orthonormal eigenmode data imply the
direct-principal padded low-band core theorem. -/
theorem C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.of_zero_orthogonal_orthonormal_eigen_rayleigh
    (hspec :
      C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenRayleighDataNoDiagLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toPaddedCorePrincipalBoundSpectralDataNoDiag⟩
...
direct-principal padded low-band core theorem. -/
theorem C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.of_zero_orthogonal_orthonormal_eigen_opNorm
    (hspec :
      C9GraphonCanonicalL2CompactActionZeroOrthogonalOrthonormalEigenOpNormDataNoDiagLowBandTheorem.{u}) :
    C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.{u} := by
  intro Omega _ mu _ W hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toPaddedCorePrincipalBoundSpectralDataNoDiag⟩

/-- Padded pure compact-action spectral data imply the budget trace package
consumed by the C9 scalar certificate. -/
theorem C9GraphonBudgetTraceSpectralDataForLowBand.of_padded_core_no_trace
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionPaddedCoreSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonBudgetTraceSpectralDataForLowBand W mu := by
  intro hW hgt hle
  have hp : 0 < edgeDensity W mu := by linarith
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toBudgetTraceSpectralData hp⟩

/-- Direct-principal padded pure compact-action spectral data imply the budget
trace package consumed by the C9 scalar certificate. -/
theorem C9GraphonBudgetTraceSpectralDataForLowBand.of_padded_core_principal_bound_no_trace
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonBudgetTraceSpectralDataForLowBand W mu := by
  intro hW hgt hle
  exact
...
global low-band budget trace theorem needed by C9. -/
theorem C9GraphonBudgetTraceSpectralDataLowBandTheorem.of_padded_core_principal_bound_no_trace
    (hspec :
      C9GraphonCanonicalL2CompactActionPaddedCorePrincipalBoundSpectralDataNoDiagLowBandTheorem.{u}) :
...
theorem C9GraphonCanonicalL2CompactActionRowEnergySpectralDataNoDiagForLowBand.of_core_no_trace
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionCoreSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionRowEnergySpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
...
theorem C9GraphonCanonicalL2CompactActionRowEnergySpectralDataNoDiagForLowBand.of_good_row_no_trace
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionGoodRowSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionRowEnergySpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
...
/-- Finite-budget compact-action trace data imply the infinite-budget
compact-action interface. -/
theorem C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand.of_finite_budget
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
...
/-- Energy compact-action trace data imply the finite-budget compact-action
interface.  This is where the abstract Hilbert-space energy/eigenvalue bridge
enters the graphon low-band pipeline. -/
theorem C9GraphonCanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiagForLowBand.of_energy
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
  exact
    ⟨(Classical.choice (hspec hW hgt hle)).toCompactActionFiniteBudgetTraceSpectralDataNoDiag⟩

/-- Energy compact-action trace data also imply the infinite-budget
compact-action interface. -/
theorem C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand.of_energy
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand
      W mu :=
  C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand.of_finite_budget
    (C9GraphonCanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiagForLowBand.of_energy
      hspec)

/-- Row-energy compact-action trace data imply the finite-energy compact-action
interface. -/
theorem C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand.of_row_energy
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
...
/-- Row-energy compact-action trace data imply the finite-budget compact-action
interface. -/
theorem C9GraphonCanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiagForLowBand.of_row_energy
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiagForLowBand
      W mu :=
  C9GraphonCanonicalL2CompactActionFiniteBudgetTraceSpectralDataNoDiagForLowBand.of_energy
    (C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand.of_row_energy
      hspec)

/-- Row-energy compact-action trace data imply the infinite-budget
compact-action interface. -/
theorem C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand.of_row_energy
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand
      W mu :=
  C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand.of_energy
    (C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand.of_row_energy
      hspec)
...
theorem C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagForLowBand.of_no_diag
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagForLowBand
        W mu) :
...
compact-action interface. -/
theorem C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagForLowBand.of_good_row
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionGoodRowTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagForLowBand
      W mu := by
  intro hW hgt hle
...
compact-action interface. -/
theorem C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand.of_good_row
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionGoodRowTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand
      W mu :=
  C9GraphonCanonicalL2CompactActionEnergyTraceSpectralDataNoDiagForLowBand.of_row_energy
    (C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagForLowBand.of_good_row
      hspec)

/-- Bounded-representative compact-action trace data imply the infinite-budget
compact-action interface. -/
theorem C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand.of_good_row
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionGoodRowTraceSpectralDataNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand
      W mu :=
  C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand.of_row_energy
    (C9GraphonCanonicalL2CompactActionRowEnergyTraceSpectralDataNoDiagForLowBand.of_good_row
      hspec)
...
theorem C9GraphonCanonicalL2CompactActionTraceSpectralDataHasSumNoDiagForLowBand.of_no_diag
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagForLowBand
        W mu) :
...
theorem C9GraphonCanonicalL2CompleteCompactActionTraceSpectralDataHasSumNoDiagForLowBand.of_hasSum_no_diag
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionTraceSpectralDataHasSumNoDiagForLowBand
        W mu) :
    C9GraphonCanonicalL2CompleteCompactActionTraceSpectralDataHasSumNoDiagForLowBand
      W mu := by
  intro hW hgt hle
  exact
...
theorem C9GraphonCanonicalL2CompleteCompactActionTraceSpectralDataHasSumNoDiagForLowBand.of_no_diag
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagForLowBand
        W mu) :
...
field. -/
theorem C9GraphonCanonicalL2CompactActionTraceSpectralDataHasSumNoDiagForLowBand.of_complete
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompleteCompactActionTraceSpectralDataHasSumNoDiagForLowBand
        W mu) :
...
theorem C9GraphonRawTraceSpectralDataForLowBand.of_complete_compact_action
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompleteCompactActionTraceSpectralDataHasSumNoDiagForLowBand
        W mu) :
    C9GraphonRawTraceSpectralDataForLowBand W mu := by
  intro hW hgt hle
  have hp : 0 < edgeDensity W mu := by linarith
  exact
...
theorem C9GraphonRawTraceSpectralDataForLowBand.of_compact_action_hasSum_noDiag
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionTraceSpectralDataHasSumNoDiagForLowBand
        W mu) :
...
theorem C9GraphonRawTraceSpectralDataForLowBand.of_compact_action_noDiag
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionTraceSpectralDataNoDiagForLowBand
        W mu) :
...
/-- Budget compact-action data imply the low-band budget trace theorem needed
by C9. -/
theorem C9GraphonBudgetTraceSpectralDataForLowBand.of_compact_action_budget_noDiag
    [IsProbabilityMeasure mu]
    (hspec :
      C9GraphonCanonicalL2CompactActionBudgetTraceSpectralDataNoDiagForLowBand
        W mu) :
...
identities.  The elementary square budget is then derived in Lean from
...
/-- The tail square budget after removing the principal mode. -/
...
square mass is bounded by the remaining tail budget. -/
...
/-- The same remaining-budget estimate with the graphon square budget and the
...
/-- A signed cubic-tail upper bound from a positive-square budget and a
...
budgets, yields the scalar inequality that the C9 closure certificate must
contradict. -/
...
theorem linearTriangleClosureEstimate_of_scalarCapacityExclusion
  ...
  have hB0 : 0 <= edgeDensity W mu - S.principal ^ 2 - z ^ 2 := by
    ...
    have htail_budget := S.tailSquareMass_le_edge_sub_principal_sq
    linarith
  have hcapacity :=
