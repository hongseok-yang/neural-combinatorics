import OddCycleBound.RegionII.CenteredKernel

/-!
# The centered Hilbert--Schmidt budget

This file proves the analytic identity behind the Region-II block decomposition.
...
The pointwise bound `0 <= W <= 1` then gives the sharp budget
...
private lemma centeredKernel_row_square
  ...
  have hr : Good r :=
    good_sub hrow
      (HighDensity.good_const (Omega := Omega) (degree W mu x))
...
/-- The sharp Hilbert--Schmidt budget for the centered compression. -/
theorem centeredKernel_hilbertSchmidt_budget (hW : IsGraphon W mu) :
    kernelSqNorm mu (centeredKernel W mu) +
        2 * ‖HighDensity.centeredDegreeL2 hW‖ ^ 2 <=
      edgeDensity W mu * (1 - edgeDensity W mu) := by
  have hid := centeredKernel_energy_identity hW
  have hsq := kernelSqNorm_le_edgeDensity hW
  unfold HighDensity.centeredDegreeL2
