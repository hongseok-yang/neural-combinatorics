...
/-!
# Refined trace bound at a Region-II frontier
...
power is charged to the Hilbert--Schmidt radius left after that frontier
...
/-- Spectral form of the refined frontier trace estimate.  The possible
zero eigenspace is absent from both sides and therefore needs no special
treatment. -/
theorem centered_trace_frontier_lower_bound
  ...
  have hpoint : forall j : CenteredEigenIndex hW,
    ...
    · simp only [if_neg hji, sub_zero]
      ...
      have hodd := odd_pow_le_q_pow_mul_sq
        (frontierSafeRadius_nonneg hW i) hle hm hm3
...
degree budget pays twice for the coupling vector. -/
theorem centered_trace_frontier_defect_lower_bound
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    {m : Nat} (hm : Odd m) (hm3 : 3 <= m) :
    let alpha := complementEigenvalue hW i
    let L := frontierSafeRadius hW i
  ...
  let alpha := complementEigenvalue hW i
  let L := frontierSafeRadius hW i
  have htrace := centered_trace_frontier_lower_bound hW i hm hm3
  have hbudget := centeredTraceSq_add_degree_budget hW
  have hLsq := frontierSafeRadius_sq hW i
  have hrem :
      centeredTraceSq hW - alpha ^ 2 <=
  ...
  have hpowNonneg : 0 <= L ^ (m - 2) :=
    pow_nonneg (frontierSafeRadius_nonneg hW i) _
