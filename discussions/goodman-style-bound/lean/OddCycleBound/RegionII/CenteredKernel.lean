import OddCycleBound.RegionII.CenteredOperator

/-!
# Kernel realization of the centered graphon compression

The Region-II square budget is an `L²(Omega × Omega)` statement.  This file
...
lemma good_centeredInput {f : Omega -> Real} (hf : Good f) :
  ...
  exact good_sub hf (HighDensity.good_const (Omega := Omega) (mean mu f))
  ...
  have hP : centerProjection (Omega := Omega) (mu := mu)
    ...
    calc
      centerProjection (Omega := Omega) (mu := mu)
          ...
          goodL2 (mu := mu)
            (good_sub hf
              (HighDensity.good_const (Omega := Omega) (mean mu f))) :=
  ...
  exact MemLp.toLp_congr
    (good_memLp_two
      (good_sub (good_kernelOp hW hfc)
        (HighDensity.good_const (Omega := Omega)
          (mean mu (kernelOp W mu fc)))))
