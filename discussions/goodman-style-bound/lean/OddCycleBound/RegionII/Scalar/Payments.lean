import OddCycleBound.RegionII.Scalar.ThreeGeometric
import OddCycleBound.RegionII.DirectedKernel

/-! # Scalar payment bounds for Zone C -/
...
theorem k_eq_directedKernel (lambda : Real) (hlambda : -P.p <= lambda)
    (hden : P.p + lambda ≠ 0) :
    P.k lambda = RegionII.directedKernel P.p P.m lambda := by
  unfold k
  symm
  exact RegionII.directedKernel_eq_div P.m_odd
    (le_trans (by norm_num) P.m_ge_fifteen) hlambda hden
...
theorem k_alpha_le_k_L : P.k P.alpha <= P.k P.L := by
  ...
  exact RegionII.directedKernel_anti_on_nonneg P.p_pos P.L_nonneg
    P.L_lt_alpha.le P.m_odd (le_trans (by norm_num) P.m_ge_fifteen)
...
theorem x_pow_le_x_pow_fourteen :
    P.x ^ (P.m - 1) <= P.x ^ 14 := by
  have hm15 := P.m_ge_fifteen
...
/-- Explicit m-free lower bound used to choose the Zone-C dual branch. -/
theorem rhoLo_le_rho : P.rhoLo <= P.rho := by
  have hAB : (1 - P.x ^ 14) / (1 + P.x) <= P.A / P.B := by
    ...
    have hfirst : (1 - P.x ^ 14) / (1 + P.x) <=
        (1 - P.x ^ (P.m - 1)) / (1 + P.x) :=
      (div_le_div_iff_of_pos_right hden).2 (by linarith [P.x_pow_le_x_pow_fourteen])
...
/-- The first Zone-C dual payment, using the explicit lower comparison
`rhoLo <= rho` but retaining the exact Huber minimum on the right. -/
theorem psi_ge_rhoLo_branch
  ...
  have hfrac :
    ...
    exact mul_le_mul_of_nonneg_left
      (by nlinarith [P.rhoLo_le_rho]) (sq_nonneg lambda)
  have hmono :
      huberDual P.xi P.rhoLo lambda <= huberDual P.xi P.rho lambda := by
    unfold huberDual
    linarith
  have hxiDen : 1 + 2 * P.xi ≠ 0 := by linarith [P.xi_pos]
  have hrhoLo : P.rhoLo ≠ 0 := P.rhoLo_pos.ne'
  have hvalue :
      huberDual P.xi P.rhoLo lambda =
...
/-- The linear dual payment forced once the explicit lower comparison crosses
the Huber transition gate. -/
theorem psi_ge_linear_rhoLo_branch
    (hgate : 1 < 2 * P.rhoLo * P.xi) :
    P.xi * (4 * P.xi + 1) / (4 * P.xi + 2) <=
      psi P.xi P.rho := by
  have hcross : 2 < 4 * P.xi * P.rho := by
    have hlo := P.rhoLo_le_rho
    have hxi := P.xi_pos
...
/-- Exact normalization used in the second Zone-C payment branch. -/
theorem C_mul_xi :
...
/-- Exact normalization used in the first Zone-C payment branch. -/
...
/-- The dimensionless payment coefficient used by the first moderate-Zone-C
certificate branch. -/
def paymentCoeffI : Real :=
  2 * (1 - P.x ^ 14) * (1 - P.y ^ (P.m - 1)) * P.kappa ^ 2 *
      (1 + 4 * P.xi) /
    ((1 + P.x) * (1 + P.y) * (1 + 2 * P.xi))

/-- The dimensionless payment coefficient used by the linear moderate-Zone-C
certificate branch. -/
def paymentCoeffII : Real :=
  Real.sqrt (2 * P.alpha) * (1 - P.y ^ (P.m - 1)) * P.f * P.d *
      (4 * P.xi + 1) /
    (P.alpha ^ 3 * (1 + P.y) * (4 * P.xi + 2))

theorem normalized_payment_pos :
    0 < P.alpha ^ 3 * P.p ^ (P.m - 2) :=
  mul_pos (pow_pos P.alpha_pos _) (pow_pos P.p_pos _)

/-- First half of the manuscript's m-free-gated payment estimate. -/
theorem paymentCoeffI_le_normalized_psi
    (hgate : 2 * P.rhoLo * P.xi <= 1) :
    (P.m : Real) * P.paymentCoeffI <=
  ...
  have hpsi := mul_le_mul_of_nonneg_left
    (P.psi_ge_rhoLo_branch hgate) P.C_pos.le
  apply (le_div_iff₀ P.normalized_payment_pos).2
  calc
    (P.m : Real) * P.paymentCoeffI *
        (P.alpha ^ 3 * P.p ^ (P.m - 2)) =
        ((P.m : Real) * P.p ^ (P.m - 2) *
...
/-- Second half of the manuscript's m-free-gated payment estimate. -/
theorem paymentCoeffII_le_normalized_psi
    (hgate : 1 < 2 * P.rhoLo * P.xi) :
    (P.m : Real) * P.paymentCoeffII <=
      P.C * psi P.xi P.rho / (P.alpha ^ 3 * P.p ^ (P.m - 2)) := by
  have hyDen : 0 < 1 + P.y := by linarith [P.y_nonneg]
  ...
  have hB := mul_le_mul_of_nonneg_right P.B_lower hscale
  have hpsi := mul_le_mul_of_nonneg_left
    (P.psi_ge_linear_rhoLo_branch hgate) P.C_pos.le
  apply (le_div_iff₀ P.normalized_payment_pos).2
  calc
    (P.m : Real) * P.paymentCoeffII *
        (P.alpha ^ 3 * P.p ^ (P.m - 2)) =
