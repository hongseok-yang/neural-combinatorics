...
theorem chart_frontier_quadratic_nonpos
...
noncomputable def AdmissibleParams.ofChart
    ...
    (hkq : kappa < kappaQ e)
    (hm : Odd m) (hm15 : 15 <= m) : AdmissibleParams where
  q := chartQ e kappa
  alpha := chartAlpha e
  m := m
  ...
  alpha_gt_q := by
    ...
    nlinarith
  alpha_le_radius := by
    apply alpha_le_frontierRadius_of_quadratic
    · have := chart_q_gt_third he hkq
      linarith
    · exact le_of_lt (chart_alpha_pos hethird)
    · exact chart_frontier_quadratic_nonpos he hkmax
  m_odd := hm
  m_ge_fifteen := hm15
