import OddCycleBound.RegionII.Scalar.FrontierAlgebra
import Mathlib.Topology.Order.Compact

/-!
# Exact Huber minimum: compactness and dual certificates

The scalar payment is defined as the actual infimum of the corrected Huber
...
theorem continuous_huberObjective (xi rho : Real) :
    Continuous (huberObjective xi rho) := by
  unfold huberObjective
  fun_prop

theorem huberObjective_image_nonempty (xi rho : Real) :
    (huberObjective xi rho '' Icc (0 : Real) 1).Nonempty := by
  exact ⟨huberObjective xi rho 0, ⟨0, by constructor <;> norm_num, rfl⟩⟩

theorem huberObjective_nonneg {xi rho v : Real}
    (hrho : 0 ≤ rho) :
    0 ≤ huberObjective xi rho v := by
  unfold huberObjective
  exact add_nonneg (mul_nonneg hrho (sq_nonneg v)) (le_max_right _ _)

theorem huberObjective_image_bddBelow {xi rho : Real}
    (hrho : 0 ≤ rho) :
    BddBelow (huberObjective xi rho '' Icc (0 : Real) 1) := by
  refine ⟨0, ?_⟩
  intro y hy
  rcases hy with ⟨v, hv, rfl⟩
  exact huberObjective_nonneg hrho

/-- The `sInf` in the definition of `psi` is attained on `[0,1]`. -/
theorem exists_huber_minimizer (xi rho : Real) :
    ∃ v ∈ Icc (0 : Real) 1,
      psi xi rho = huberObjective xi rho v := by
  have hcompact : IsCompact
      (huberObjective xi rho '' Icc (0 : Real) 1) :=
    isCompact_Icc.image (continuous_huberObjective xi rho)
  have hmem := hcompact.sInf_mem (huberObjective_image_nonempty xi rho)
  rcases hmem with ⟨v, hv, hvalue⟩
  refine ⟨v, hv, ?_⟩
  unfold psi
  exact hvalue.symm

theorem psi_le_huberObjective {xi rho v : Real}
    (hrho : 0 ≤ rho) (hv : v ∈ Icc (0 : Real) 1) :
    psi xi rho ≤ huberObjective xi rho v := by
  unfold psi
  exact csInf_le (huberObjective_image_bddBelow hrho) ⟨v, hv, rfl⟩

theorem psi_nonneg {xi rho : Real} (hrho : 0 <= rho) :
    0 <= psi xi rho := by
  rcases exists_huber_minimizer xi rho with ⟨v, hv, hvalue⟩
  rw [hvalue]
  exact huberObjective_nonneg hrho
...
/-- Pointwise weak duality for the corrected Huber objective. -/
theorem huberDual_le_huberObjective
    {xi rho lambda v : Real}
    (hrho : 0 ≤ rho)
    (hlambda : lambda ∈ Icc (0 : Real) 1) :
    huberDual xi rho lambda ≤ huberObjective xi rho v := by
  ...
  · have hrho0 : rho = 0 := by nlinarith [hlambda.1]
    ...
    rw [hrho0, hlambda0]
    simp [huberDual, huberObjective, le_max_right]
  · have ha0 : 0 < rho + lambda := by
    ...
    unfold huberDual huberObjective
    calc
...
the true Huber minimum. -/
theorem huberDual_le_psi
    {xi rho lambda : Real}
    (hrho : 0 ≤ rho)
    (hlambda : lambda ∈ Icc (0 : Real) 1) :
    huberDual xi rho lambda ≤ psi xi rho := by
  unfold psi
  apply le_csInf (huberObjective_image_nonempty xi rho)
  intro y hy
  rcases hy with ⟨v, hv, rfl⟩
  exact huberDual_le_huberObjective hrho hlambda

/-- The `lambda = 1` certificate used in Zones B and C. -/
theorem psi_ge_dual_one {xi rho : Real} (hrho : 0 ≤ rho) :
    xi - 1 / (4 * (rho + 1)) ≤ psi xi rho := by
  simpa [huberDual] using
    (huberDual_le_psi (xi := xi) (rho := rho) (lambda := 1) hrho
...
theorem psi_eq_linear_branch
  ...
  have hupper := psi_le_huberObjective (xi := xi) (rho := rho) hrho hvstar
  rw [huberObjective, max_eq_left hactive] at hupper
  have hvalue :
...
/-- The `lambda = 2*rho*xi` certificate used in the small-frontier zone. -/
...
theorem psi_eq_sqrt_branch
  ...
  have hpoint : ∀ v ∈ Icc (0 : Real) 1,
      rho * vMinus xi ^ 2 ≤ huberObjective xi rho v := by
    intro v hv
  ...
  have hupper := psi_le_huberObjective (xi := xi) (rho := rho) hrho
    (show vMinus xi ∈ Icc (0 : Real) 1 from ⟨hvm.1, hvm.2.trans (by norm_num)⟩)
  have hzero : xi - vMinus xi + vMinus xi ^ 2 = 0 := by linarith [hvmEq]
  rw [huberObjective, hzero, max_self, add_zero] at hupper
  rcases exists_huber_minimizer xi rho with ⟨v, hv, hpsi⟩
  have hlower := hpoint v hv
  apply le_antisymm hupper
  calc
    rho * vMinus xi ^ 2 ≤ huberObjective xi rho v := hlower
    _ = psi xi rho := hpsi.symm

/-- In the Region-II parameter range, the dual maximum is attained and pays
exactly `psi`. -/
theorem exists_huberDual_eq_psi
    {xi rho : Real} (hrho : 0 < rho) (hxi0 : 0 ≤ xi) :
    ∃ lambda ∈ Icc (0 : Real) 1,
      huberDual xi rho lambda = psi xi rho := by
...
/-- Exact dual formula for the corrected Huber payment on its Region-II
domain. -/
theorem psi_eq_sSup_huberDual
    {xi rho : Real} (hrho : 0 < rho) (hxi0 : 0 ≤ xi) :
    psi xi rho = sSup (huberDual xi rho '' Icc (0 : Real) 1) := by
  have hnonempty : (huberDual xi rho '' Icc (0 : Real) 1).Nonempty :=
    ⟨huberDual xi rho 0, ⟨0, by constructor <;> norm_num, rfl⟩⟩
  have hbdd : BddAbove (huberDual xi rho '' Icc (0 : Real) 1) := by
    refine ⟨psi xi rho, ?_⟩
    intro y hy
    rcases hy with ⟨lambda, hlambda, rfl⟩
    exact huberDual_le_psi hrho.le hlambda
  obtain ⟨lambda, hlambda, heq⟩ := exists_huberDual_eq_psi hrho hxi0
  apply le_antisymm
  · rw [← heq]
