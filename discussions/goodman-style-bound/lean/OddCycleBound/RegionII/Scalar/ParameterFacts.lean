...
/-!
# Elementary facts about admissible Region-II parameters

These lemmas centralize the sign and square-root facts used by the Huber
reduction and all scalar zones.
-/
...
/-- The explicit frontier radius is strictly below `1/2` throughout Region
II. -/
theorem frontierRadius_lt_half {q : Real}
    (hq0 : 0 <= q) (hqhalf : q < 1 / 2) :
    frontierRadius q < 1 / 2 := by
...
/-- The frontier radius is nonnegative. -/
theorem frontierRadius_nonneg {q : Real} (hq : 0 <= q) :
    0 <= frontierRadius q := by
  have hrad : 0 <= q ^ 2 + 4 * q := by positivity
...
/-- The explicit radius is the positive root of the frontier quadratic. -/
theorem frontierRadius_quadratic_eq {q : Real} (hq : 0 <= q) :
    frontierRadius q ^ 2 + q * frontierRadius q - q = 0 := by
...
theorem alpha_lt_half : P.alpha < 1 / 2 :=
  P.alpha_le_radius.trans_lt
    (frontierRadius_lt_half P.q_nonneg P.q_lt_half)

theorem alpha_lt_p : P.alpha < P.p :=
  lt_trans P.alpha_lt_half P.p_gt_half

theorem frontier_quadratic_nonpos :
    P.alpha ^ 2 + P.q * P.alpha - P.q <= 0 := by
  let r := frontierRadius P.q
  have hr0 : 0 <= r := frontierRadius_nonneg P.q_nonneg
  have hroot : r ^ 2 + P.q * r - P.q = 0 :=
    frontierRadius_quadratic_eq P.q_nonneg
...
theorem L_radicand_nonneg :
  ...
  have hquad := P.frontier_quadratic_nonpos
  dsimp [p] at *
...
theorem k_alpha_pos : 0 < P.k P.alpha := by
  ...
  exact quotientKernel_pos P.p_pos P.alpha_nonneg P.alpha_lt_p
    (le_trans (by norm_num : 2 <= 15) P.m_ge_fifteen)
