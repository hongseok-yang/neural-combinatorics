import OddCycleBound.RegionII.Frontier
import OddCycleBound.RegionII.Scalar.FrontierAlgebra

/-!
# Forced variance at a Region-II frontier eigenvalue
...
/-- A positive complement-frontier eigenvalue forces the square of the
...
theorem exists_frontierEigenfunction_with_abs_sq_ge
...
/-- The unique frontier eigenvalue is a global upper bound for the quadratic
...
theorem complementCompression_quadratic_le_frontier
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
  ...
  have hle : ∀ j : CenteredEigenIndex hW,
    ...
    have hjfront :
      ...
      hfront.trans hij
    have heq := complement_frontier_unique hW hqthird hfront hjfront
    subst j
...
lemma forced_variance_of_coupling
    {q alpha a G : Real}
...
/-- Graphon forced variance at a Region-II frontier eigenvalue, together
with the strict upper bound needed to clear its denominator. -/
theorem frontier_forced_variance_and_lt_half
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    (hqthird : (1 : Real) / 3 < 1 - edgeDensity W mu)
    (hfront : 1 - edgeDensity W mu < complementEigenvalue hW i) :
    complementEigenvalue hW i *
  ...
  rcases exists_frontierEigenfunction_with_abs_sq_ge hW i halphaPos with
  ...
  have hbodyUpper :
      inner Real hvec (centeredGraphonOp hU hvec) <=
        alpha * (1 - a ^ 2) := by
    have htop := complementCompression_quadratic_le_frontier
  ...
  have hcoupling : (alpha - q) * a ^ 2 <=
    ...
    nlinarith
  constructor
  · exact forced_variance_of_coupling hfront halphaPos ha0 haSq haSqOne hcoupling
  · simpa [alpha] using halphaHalf

/-- The forced degree-variance lower bound at a Region-II frontier
eigenvalue. -/
theorem frontier_forced_variance
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    (hqthird : (1 : Real) / 3 < 1 - edgeDensity W mu)
    (hfront : 1 - edgeDensity W mu < complementEigenvalue hW i) :
    complementEigenvalue hW i *
        ...
        (2 * (1 - 2 * complementEigenvalue hW i)) <=
      ‖centeredDegreeL2 hW‖ ^ 2 :=
  (frontier_forced_variance_and_lt_half hW i hqthird hfront).1

/-- A frontier eigenvalue is strictly smaller than (1/2). -/
theorem complement_frontier_lt_half
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    (hqthird : (1 : Real) / 3 < 1 - edgeDensity W mu)
    (hfront : 1 - edgeDensity W mu < complementEigenvalue hW i) :
    complementEigenvalue hW i < (1 : Real) / 2 :=
  (frontier_forced_variance_and_lt_half hW i hqthird hfront).2

/-- A single frontier atom and the centered degree vector obey the global
Hilbert--Schmidt budget. -/
theorem frontier_spectral_budget
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW) :
...
/-- Forced variance and the Hilbert--Schmidt budget give the quadratic
frontier ceiling. -/
theorem frontier_quadratic_ceiling
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    (hqthird : (1 : Real) / 3 < 1 - edgeDensity W mu)
    (hfront : 1 - edgeDensity W mu < complementEigenvalue hW i) :
    complementEigenvalue hW i ^ 2 +
  ...
  apply Scalar.quadratic_ceiling_of_forced_variance
    hqthird hfront
    (complement_frontier_lt_half hW i hqthird hfront)
    (frontier_forced_variance hW i hqthird hfront)
  have hbudget := frontier_spectral_budget hW i
  convert hbudget using 1 <;> ring

/-- Explicit square-root form of the frontier ceiling. -/
theorem frontier_radius_ceiling
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    (hqthird : (1 : Real) / 3 < 1 - edgeDensity W mu)
    (hfront : 1 - edgeDensity W mu < complementEigenvalue hW i) :
    complementEigenvalue hW i <=
      Scalar.frontierRadius (1 - edgeDensity W mu) := by
  apply Scalar.frontierRadius_of_forced_variance
    hqthird hfront
    (complement_frontier_lt_half hW i hqthird hfront)
    (frontier_forced_variance hW i hqthird hfront)
  have hbudget := frontier_spectral_budget hW i
