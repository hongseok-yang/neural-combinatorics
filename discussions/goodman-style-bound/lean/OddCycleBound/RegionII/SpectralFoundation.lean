import OddCycleBound.RegionII.HilbertSchmidtBudget
import OddCycleBound.RegionII.BoundedKernelL2
import OddCycleBound.LowBand.C9Spectral

/-!
# Reusable compact self-adjoint spectral data for Region II
...
kernel.  The centered Region-II kernel is signed.  This file extracts the
...
theorem centeredEigenmode_orthonormal (hW : IsGraphon W mu) :
  ...
  exact nonzeroEigenmode_orthonormal
    (OddCycleBound.HighDensity.centeredGraphonOp hW)
    (centeredGraphonOp_isCompact hW)
    (OddCycleBound.HighDensity.centeredGraphonOp_isSymmetric hW)

theorem centeredEigenmode_diagonal (hW : IsGraphon W mu)
    (i : CenteredEigenIndex hW) :
    OddCycleBound.HighDensity.centeredGraphonOp hW (centeredEigenmode hW i) =
...
theorem centeredGraphonOp_action_expansion (hW : IsGraphon W mu)
    ...
    HasSum
      ...
      (OddCycleBound.HighDensity.centeredGraphonOp hW f) := by
  exact compactSelfAdjoint_action_expansion
    (OddCycleBound.HighDensity.centeredGraphonOp hW)
    (centeredGraphonOp_isCompact hW)
    (OddCycleBound.HighDensity.centeredGraphonOp_isSymmetric hW) f

/-- Every finite collection of centered eigenvalues obeys the signed-kernel
Hilbert--Schmidt square budget. -/
theorem centeredEigenvalue_finite_square_budget (hW : IsGraphon W mu)
  ...
  let row : Omega → Lp Real 2 mu := fun x =>
    OddCycleBound.LowBand.L2Kernel.goodL2 (mu := mu)
      (OddCycleBound.LowBand.L2Kernel.goodK_row hK x)
  have hdiagEnergy :
      s.sum (fun i => centeredEigenvalue hW i ^ 2) =
  ...
  have hrowInt : Integrable (fun x : Omega => inner Real (row x) (row x)) mu := by
    simpa [hK, row] using
      (OddCycleBound.LowBand.L2Kernel.integrable_goodK_row_inner_self
        (mu := mu) (centeredKernel_goodK hW))
  have hpoint : ∀ x : Omega,
  ...
  calc
    ...
    _ = kernelSqNorm mu (centeredKernel W mu) := by
      simpa [hK, row] using
        (OddCycleBound.LowBand.L2Kernel.integral_goodK_row_inner_self_eq_kernelSqNorm
          (mu := mu) (centeredKernel_goodK hW))
...
theorem centeredEigenvalue_square_summable (hW : IsGraphon W mu) :
  ...
  intro s
  simpa using centeredEigenvalue_finite_square_budget hW s

/-- The Region-II square trace is the sum of the squares of all nonzero
...
/-- Spectral form of the centered variance budget from the Region-II paper:
`Tr(A²) + 2 ‖g‖₂² ≤ p q`. -/
theorem centeredTraceSq_add_degree_budget (hW : IsGraphon W mu) :
    centeredTraceSq hW +
        2 * ‖OddCycleBound.HighDensity.centeredDegreeL2 hW‖ ^ 2 ≤
  ...
  calc
    ...
    _ ≤ edgeDensity W mu * (1 - edgeDensity W mu) :=
      centeredKernel_hilbertSchmidt_budget hW
