...
/-!
# Intermediate-region master defect
...
the intermediate region proof: all remaining obligations concern the scalar coefficients
and the envelope estimates.
-/
...
leading_eigenvalue mode is retained exactly and its orthogonal complement pays at the
safe spectral endpoint. -/
theorem graphon_leading_eigenvalue_defect_lower_bound_directed
  ...
  have htrace := centered_trace_leading_eigenvalue_defect_lower_bound
    hW i hm hm3
  have hshift := graphonOneSidedShift_leading_eigenvalue_lower_bound
    hW i hp hm (by omega)
