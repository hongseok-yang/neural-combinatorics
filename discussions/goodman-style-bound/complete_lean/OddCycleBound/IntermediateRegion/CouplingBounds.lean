...
/-!
# LeadingEigenvalue eigenfunction coupling bounds

This file develops the exact complement-graphon block equation used by both
the direct and safe coupling bounds in the corrected the intermediate region proof.
-/
...
case split. -/
def leadingEigenvaluePos (phi : Omega -> Real) (x : Omega) : Real :=
  (|phi x| + phi x) / 2

noncomputable def leadingEigenvalueAbsMean (mu : Measure Omega)
    (phi : Omega -> Real) : Real :=
  ∫ x, |phi x| ∂mu

noncomputable def leadingEigenvalueShapeZ (mu : Measure Omega)
    (phi : Omega -> Real) : Real :=
  leadingEigenvalueAbsMean mu phi ^ 2

noncomputable def leadingEigenvalueShapeB (mu : Measure Omega)
    (phi : Omega -> Real) : Real :=
  ∫ x, |phi x| * phi x ∂mu

/-- Component of `|phi|` orthogonal to the constant and leading_eigenvalue modes. -/
def leadingEigenvalueResidual (a b : Real) (phi : Omega -> Real) : Omega -> Real :=
  (fun x => |phi x|) - (fun _ => a) - b • phi

lemma good_leadingEigenvaluePos {phi : Omega -> Real} (hphi : Good phi) :
    Good (leadingEigenvaluePos phi) := by
  let h := good_smul ((2 : Real)⁻¹)
    (good_add (good_abs hphi) hphi)
  convert h using 1
  funext x
  simp [leadingEigenvaluePos, Pi.smul_apply, div_eq_mul_inv]
  ring

lemma leadingEigenvaluePos_nonneg (phi : Omega -> Real) (x : Omega) :
    0 <= leadingEigenvaluePos phi x := by
  unfold leadingEigenvaluePos
  nlinarith [neg_le_abs (phi x)]

lemma le_leadingEigenvaluePos (phi : Omega -> Real) (x : Omega) :
    phi x <= leadingEigenvaluePos phi x := by
  unfold leadingEigenvaluePos
  nlinarith [le_abs_self (phi x)]

lemma good_leadingEigenvalueResidual {phi : Omega -> Real} (hphi : Good phi)
    (a b : Real) :
    Good (leadingEigenvalueResidual a b phi) := by
  unfold leadingEigenvalueResidual
  exact good_sub
    (good_sub (good_abs hphi) (good_const (Omega := Omega) a))
    (good_smul b hphi)

/-- Testing a graphon transform against the positive part gives the sharp
rank-one upper bound used in the direct coupling. -/
lemma integral_leadingEigenvaluePos_mul_kernelOp_le_sq
    (hU : IsGraphon W mu) {phi : Omega -> Real} (hphi : Good phi) :
    (∫ x, leadingEigenvaluePos phi x * kernelOp W mu phi x ∂mu) <=
      (∫ x, leadingEigenvaluePos phi x ∂mu) ^ 2 := by
  let hpos := good_leadingEigenvaluePos hphi
  have hrow : ∀ x : Omega,
      kernelOp W mu phi x <= ∫ y, leadingEigenvaluePos phi y ∂mu := by
    intro x
    calc
      kernelOp W mu phi x <=
          kernelOp W mu (leadingEigenvaluePos phi) x := by
        unfold kernelOp
        refine integral_mono (integrable_Uf hU hphi x)
          (integrable_Uf hU hpos x) ?_
        intro y
        exact mul_le_mul_of_nonneg_left
          (le_leadingEigenvaluePos phi y) (hU.nonneg x y)
      _ <= ∫ y, leadingEigenvaluePos phi y ∂mu := by
        unfold kernelOp
        refine integral_mono (integrable_Uf hU hpos x)
          hpos.integrable ?_
        intro y
        exact mul_le_of_le_one_left
          (leadingEigenvaluePos_nonneg phi y) (hU.le_one x y)
  have hintegrableLeft :
      Integrable (fun x => leadingEigenvaluePos phi x * kernelOp W mu phi x) mu :=
    (hpos.mul (good_kernelOp hU hphi)).integrable
  have hintegrableRight :
      Integrable
        (fun x => (∫ y, leadingEigenvaluePos phi y ∂mu) * leadingEigenvaluePos phi x) mu :=
    ((good_const (Omega := Omega) (∫ y, leadingEigenvaluePos phi y ∂mu)).mul
      hpos).integrable
  calc
    (∫ x, leadingEigenvaluePos phi x * kernelOp W mu phi x ∂mu) <=
        ∫ x, (∫ y, leadingEigenvaluePos phi y ∂mu) *
          leadingEigenvaluePos phi x ∂mu := by
      refine integral_mono hintegrableLeft hintegrableRight ?_
      intro x
      simpa [mul_comm] using
        (mul_le_mul_of_nonneg_left (hrow x)
          (leadingEigenvaluePos_nonneg phi x))
    _ = (∫ x, leadingEigenvaluePos phi x ∂mu) ^ 2 := by
...
`T_(1-W) phi = c 1 + alpha phi` for a centered leading_eigenvalue eigenmode. -/
theorem kernelOpCLM_compl_leading_eigenvalue_eq_hub_add
...
/-- Direct leading_eigenvalue coupling coupling, in denominator-free form:
`2 c sqrt(z) + 2 alpha b <= z - 2 alpha`. -/
theorem leading_eigenvalue_coupling_inner_bound
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    {phi : Omega -> Real} (hphi : Good phi)
    (hrep : centeredEigenmode hW i = goodL2 (mu := mu) hphi)
    (hmean : mean mu phi = 0)
  ...
  let b := leadingEigenvalueShapeB mu phi
  let c := inner Real (centeredDegreeL2 hU) (centeredEigenmode hW i)
  let hpos : Good (leadingEigenvaluePos phi) := good_leadingEigenvaluePos hphi
  let posVec := goodL2 (mu := mu) hpos
  have ha0 : 0 <= a := by
    dsimp [a, leadingEigenvalueAbsMean]
    exact integral_nonneg fun x => abs_nonneg (phi x)
  have hz : z = a ^ 2 := rfl
  have hsqrtZ : Real.sqrt z = a := by
    rw [hz, Real.sqrt_sq ha0]
  have hmeanPos : mean mu (leadingEigenvaluePos phi) = a / 2 := by
    unfold mean leadingEigenvaluePos
    ...
    simp [a, leadingEigenvalueAbsMean]
  have hinnerPosPhi :
    ...
    simp [b, leadingEigenvalueShapeB]
  have hinnerPosOne :
    ...
    exact hmeanPos
  have hblock := kernelOpCLM_compl_leading_eigenvalue_eq_hub_add
    hW i hphi hrep hmean
  have hpairBlock := congrArg (fun v => inner Real posVec v) hblock
  have hpairEq :
      (∫ x, leadingEigenvaluePos phi x *
  ...
  have hdom := integral_leadingEigenvaluePos_mul_kernelOp_le_sq
    (isGraphon_compl hW) hphi
  rw [hpairEq] at hdom
  have hposIntegral :
      (∫ x, leadingEigenvaluePos phi x ∂mu) = a / 2 := by
...
/-- The direct coupling for the opposite leading_eigenvalue orientation.  Together with
`leading_eigenvalue_coupling_inner_bound`, this permits choosing the sign for which the cubic
shape coefficient is nonnegative. -/
theorem leading_eigenvalue_coupling_inner_bound_neg
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    {phi : Omega -> Real} (hphi : Good phi)
    (hrep : centeredEigenmode hW i = goodL2 (mu := mu) hphi)
    (hmean : mean mu phi = 0)
  ...
  let hpos : Good (leadingEigenvaluePos psi) := good_leadingEigenvaluePos hpsi
  let posVec := goodL2 (mu := mu) hpos
  have ha0 : 0 <= a := by
    dsimp [a, leadingEigenvalueAbsMean]
    exact integral_nonneg fun x => abs_nonneg (phi x)
  ...
  have hsqrtZ : Real.sqrt z = a := by
    rw [hz, Real.sqrt_sq ha0]
  have hmeanPos : mean mu (leadingEigenvaluePos psi) = a / 2 := by
    unfold mean leadingEigenvaluePos
  ...
  have hinnerPosPhi :
    ...
    rw [inner_goodL2_eq_integral_mul]
    have hpoint :
        (fun x : Omega => leadingEigenvaluePos psi x * phi x) =
          fun x => (|phi x| * phi x - phi x * phi x) / 2 := by
      funext x
      simp [leadingEigenvaluePos, psi]
      ring
    rw [hpoint, integral_div]
    rw [integral_sub
      ((good_abs hphi).mul hphi).integrable
  ...
  have hpairBlock := congrArg (fun v => inner Real posVec v) hblock
  have hpairEq :
      (∫ x, leadingEigenvaluePos psi x * kernelOp (compl W) mu phi x ∂mu) =
        c * (a / 2) + alpha * ((b - 1) / 2) := by
    rw [hrep, kernelOpCLM_goodL2_eq_goodL2] at hpairBlock
    change inner Real posVec
        (goodL2 (mu := mu) (good_kernelOp (isGraphon_compl hW) hphi)) =
  ...
  have hkernelNeg : forall x : Omega,
    ...
    simp [psi]
  have hleftNeg :
      (∫ x, leadingEigenvaluePos psi x * kernelOp (compl W) mu psi x ∂mu) =
        -(∫ x, leadingEigenvaluePos psi x * kernelOp (compl W) mu phi x ∂mu) := by
    simp_rw [hkernelNeg]
    have hpoint : (fun x : Omega => leadingEigenvaluePos psi x *
        -kernelOp (compl W) mu phi x) =
      fun x => -(leadingEigenvaluePos psi x * kernelOp (compl W) mu phi x) := by
      funext x
      ring
  ...
  have hposIntegral : (∫ x, leadingEigenvaluePos psi x ∂mu) = a / 2 := by
    simpa [mean] using hmeanPos
  rw [hposIntegral] at hdom
  change -2 * c * Real.sqrt z - 2 * alpha * b <= z - 2 * alpha
  rw [hsqrtZ, hz]
...
/-- Safe leading_eigenvalue coupling coupling.  The first conjunct is the exact shape
bound `z+b^2+K=1`; the second is the denominator-free form of
`b c + <g_s,k> >= H`. -/
theorem leading_eigenvalue_coupling_orthogonal_bound
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    {phi : Omega -> Real} (hphi : Good phi)
    (hrep : centeredEigenmode hW i = goodL2 (mu := mu) hphi)
    (hmean : mean mu phi = 0)
    ...
    (hzalpha : 2 * complementEigenvalue hW i <=
      leadingEigenvalueShapeZ mu phi) :
    let hU := isGraphon_compl hW
    let alpha := complementEigenvalue hW i
  ...
  have hphiAbs : inner Real phiVec absVec = b := by
    ...
    simpa [b, leadingEigenvalueShapeB, mul_comm]
  ...
  have hkVecEq : kVec = absVec - a • one - b • phiVec := by
    ...
    have hkToLp :
      ...
      exact ae_of_all _ fun x => by
        simp [leadingEigenvalueResidual, Pi.smul_apply, smul_eq_mul]
    calc
  ...
  have hshape : z + b ^ 2 + K = 1 := by
    ...
    rw [hz]
    nlinarith
  have hkMean : mean mu (leadingEigenvalueResidual a b phi) = 0 := by
    have h := honeK
    dsimp [one, kVec] at h
...
/-- Cauchy--Schwarz form of the safe coupling.  It is stated without division
by `K`, so it includes the corrected degenerate case `K = 0`. -/
theorem leading_eigenvalue_coupling_orthogonal_bound_sq
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
    {phi : Omega -> Real} (hphi : Good phi)
    (hrep : centeredEigenmode hW i = goodL2 (mu := mu) hphi)
    (hmean : mean mu phi = 0)
  ...
  have hsafe := (leading_eigenvalue_coupling_orthogonal_bound hW i hphi hrep hmean hnorm
    halpha hzalpha).2
  change
    (alpha - q) * z + (alpha - L) * K <=
      2 * Real.sqrt z *
