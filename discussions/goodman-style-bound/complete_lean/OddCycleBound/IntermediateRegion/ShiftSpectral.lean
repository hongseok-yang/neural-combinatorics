...
residual, while orthogonality removes the distinguished leading_eigenvalue mode from the
...
polynomial on vectors orthogonal to the leading_eigenvalue mode. -/
theorem centeredReturnQuadratic_safe_lower_bound
    ...
    directedKernel p m (leadingEigenvalueSafeRadius hW i) * inner Real v v <=
  ...
  have hpoint : forall j : CenteredEigenIndex hW,
    ...
    · subst j
      simp [horth]
    · have habs := abs_complementEigenvalue_le_leadingEigenvalueSafeRadius hW hji
      have hmem : complementEigenvalue hW j ∈ Set.Icc (-L) L := by
        exact abs_le.mp habs
      have hkernel := directedKernel_safe_lower_bound hp
        (leadingEigenvalueSafeRadius_nonneg hW i) hmem hm hm2
      have hidentify :
  ...
  have hKle : K <= a0 := by
    ...
    exact directedKernel_le_zero_value hp
      (leadingEigenvalueSafeRadius_nonneg hW i) hm hm2
  have hresidual :
      (K - a0) * inner Real v v <=
        (K - a0) *
          (∑' j : CenteredEigenIndex hW,
...
/-- Linear-term lower bound with the leading_eigenvalue coupling split off exactly. -/
theorem graphon_oneSidedUCoeff_leading_eigenvalue_lower_bound
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
  ...
  have hsafe' :
      directedKernel p m (leadingEigenvalueSafeRadius hW i) * ‖gs‖ ^ 2 <=
        centeredReturnQuadratic hW p m gs := by
...
/-- The full one-sided graphon shift dominates the exact leading_eigenvalue contribution
and the safe-endpoint envelope_value for the orthogonal component. -/
theorem graphonOneSidedShift_leading_eigenvalue_lower_bound
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW)
  ...
  dsimp only
  have hcoeff := graphon_oneSidedUCoeff_leading_eigenvalue_lower_bound
