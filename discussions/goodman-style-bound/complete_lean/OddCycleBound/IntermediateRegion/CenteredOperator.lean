...
/-!
# The centered complement operator used in the intermediate region

This file packages the copied high-density projection operator for the
the intermediate region proof.  The first result closes an infrastructure gap left by the
...
approximation hypothesis exposed to downstream the intermediate region files. -/
...
self-adjoint facts consumed by the intermediate-region spectral layer. -/
