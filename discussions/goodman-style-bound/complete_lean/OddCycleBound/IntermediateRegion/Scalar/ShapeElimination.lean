...
/-!
# Exact Envelope shape elimination

This file contains the scalar geometry behind the corrected the intermediate region Envelope
reduction.  Its hypotheses are exactly the direct coupling, the squared safe
coupling, and the normalized leading_eigenvalue-shape bound.  In particular, the proof
...
/-- The safe-coupling threshold `H` from the corrected manuscript. -/
noncomputable def shapeThreshold
    (alpha q L z K : Real) : Real :=
  ((alpha - q) * z + (alpha - L) * K) / (2 * Real.sqrt z)

/-- The safe threshold retains a eigenvalue-gap term and half of the safe
spectral envelope_value. -/
...
/-- If the coupling coefficient is nonpositive, the safe coupling pays the
full product of the leading_eigenvalue and safe gaps. -/
theorem safe_envelope_value_of_nonpos_c
...
/-- For a positive coupling coefficient, the safe coupling pays twice the
safe gap times the positive part of the residual leading_eigenvalue demand.  The proof
contains the corrected `K = 0` branch. -/
theorem safe_envelope_value_of_pos_c
    {alpha q L z b K c gsSq : Real}
    (halpha : 0 < alpha)
    (hd : 0 <= alpha - q) (hf : 0 <= alpha - L)
    (hb : 0 <= b) (hK : 0 <= K) (hgs : 0 <= gsSq)
...
/-- Exact Envelope elimination for abstract leading_eigenvalue-shape data.  The parameters
`C`, `xi`, and `rho` are the manuscript normalizations. -/
theorem exact_envelope_shape_elimination
  ...
  · have hcpos : 0 < c := lt_of_not_ge hc
    ...
    have hdirectS : 2 * c * s + 2 * alpha * b <= z - 2 * alpha := by
      ...
      linarith
    have hbound : b ^ 2 + 2 * alpha * b + 2 * c * s <= e := by
      dsimp [e]
      nlinarith
    ...
    have hsafePay := safe_envelope_value_of_pos_c
    ...
    have hpsi := psi_le_envelopeObjective
      (xi := xi) (rho := rho) hrho hv
    calc
      C * psi xi rho <= C * envelopeObjective xi rho v :=
        mul_le_mul_of_nonneg_left hpsi (le_of_lt hC)
...
/-- Sign-free form of exact Envelope elimination.  The two direct inequalities
are precisely the positive-part estimates for the two orientations of the
leading_eigenvalue eigenfunction. -/
theorem exact_envelope_shape_elimination_two_sided
    {alpha q L A B c gsSq z b K : Real}
    (halpha : 0 < alpha) (halphaHalf : alpha < 1 / 2)
    (hqalpha : q < alpha) (hLalpha : L < alpha)
    (hA : 0 <= A) (hB : 0 < B)
  ...
  dsimp only
  by_cases hb : 0 <= b
  · exact exact_envelope_shape_elimination halpha halphaHalf hqalpha hLalpha
  ...
  · have hbneg : b < 0 := lt_of_not_ge hb
    ...
    have hpay := exact_envelope_shape_elimination
