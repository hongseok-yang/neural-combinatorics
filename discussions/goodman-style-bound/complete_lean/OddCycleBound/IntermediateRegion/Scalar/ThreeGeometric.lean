...
This is the density-independent analytic reduction used by the scalar
branch analysis of the intermediate region.
-/
...
theorem R_three_geometric :
  ...
  have hm2 : 2 <= P.m := le_trans (by norm_num) P.m_ge_nine
  have hm1 : 1 <= P.m := le_trans (by norm_num) P.m_ge_nine
...
/-- Dropping the two nonpositive safe-geometric terms leaves the leading_eigenvalue
...
theorem secant_power_gate (hR : 0 < P.R) (hm15 : 15 <= P.m) :
...
theorem secant_log_gate (hR : 0 < P.R) (hm15 : 15 <= P.m) :
    Real.log (1 + P.G2) <
  ...
  have hpow := P.secant_power_gate hR hm15
...
theorem secant_gate (hR : 0 < P.R) (hm15 : 15 <= P.m) :
    Real.log (1 + P.G2) <
      ((P.m - 2 : Nat) : Real) * Real.log (P.alpha / P.q) ∧
    ((P.m - 2 : Nat) : Real) * Real.log (P.alpha / P.q) <=
      ((P.m - 2 : Nat) : Real) * P.d / P.q := by
  refine ⟨P.secant_log_gate hR hm15, ?_⟩
