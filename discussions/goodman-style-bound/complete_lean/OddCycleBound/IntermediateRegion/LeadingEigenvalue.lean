...
/-!
# Intermediate-region leading_eigenvalue spectrum

This file starts the one-leading_eigenvalue reduction for the complement compression.
...
/-- A scalar odd-power estimate used in the no-leading_eigenvalue case. -/
...
II threshold `q = 1-p`. -/
theorem complement_leading_eigenvalue_unique
    (hW : IsGraphon W mu)
  ...
  classical
  by_contra hij
  have hbound := centeredEigenvalue_finite_square_bound hW ({i, j} : Finset _)
  ...
  have hsquares :
    ...
    have hfinite :
      ...
      simpa [hij, add_comm] using hbound
    have hkernel' :
...
theorem centered_trace_no_leading_eigenvalue_lower_bound
    (hW : IsGraphon W mu) {m : Nat}
    (hm : Odd m) (hm3 : 3 ≤ m)
    (hleading_eigenvalue : ∀ i : CenteredEigenIndex hW,
  ...
  have hpoint : ∀ i : CenteredEigenIndex hW,
    ...
    intro i
    have hv := odd_pow_le_q_pow_mul_sq hq0 (hleading_eigenvalue i) hm hm3
  ...
  have hsqbound : centeredTraceSq hW ≤ p * q := by
    have hbound := centeredTraceSq_add_degree_bound hW
    dsimp [p, q]
    nlinarith [sq_nonneg ‖centeredDegreeL2 hW‖]
  have hmul := mul_le_mul_of_nonneg_left hsqbound (pow_nonneg hq0 (m - 2))
...
/-- Completed no-leading_eigenvalue branch of the intermediate-region odd-cycle bound. -/
theorem cycle_bound_of_eigenvalues_le_q
    (hW : IsGraphon W mu) {m : Nat}
    (hm : Odd m) (hm3 : 3 ≤ m)
  ...
  have htrace := centered_trace_no_leading_eigenvalue_lower_bound hW hm hm3 hleading_eigenvalue
...
theorem centeredEigenmode_hasGoodRepresentative
    (hW : IsGraphon W mu) (i : CenteredEigenIndex hW) :
    OddCycleBound.Spectral.InfiniteSpectral.HasGoodRepresentative (mu := mu)
  ...
  exact
    OddCycleBound.Spectral.InfiniteSpectral.hasGoodRepresentative_of_nonzero_eigenmode_and_good_operator_image
      hf himage hdiag hne
