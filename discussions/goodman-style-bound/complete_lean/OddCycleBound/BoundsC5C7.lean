...
identity (`Necklace.lean`) with the positivity certificate (`MomentSOS.lean`) and the
...
/-- **`C₃` (Goodman's triangle bound) for all edge densities, fully integral-grounded.**
`t(C₃, 1−U) ≥ p³ − p(1−p)²` (`p = 1 − q`, `q = ∫∫U`).  This is the shortest necklace (length `2`):
`complTrace_necklace hU 1` expands `t(C₃, 1−U)` to the pure polynomial identity
`g₃ + 2·s₀ + (x₂ − c₃)` in the spectral moments, after which `s₀ ≥ 0`
(`specMoment_zero_nonneg`, the degenerate `Φ₃` certificate) and `x₂ − c₃ ≥ 0` (edge deletion,
`edge_deletion_general hU 1`) finish by linear arithmetic.  Mirrors `C5_integral` (paper §2). -/
theorem C3_integral (hU : IsGraphon U μ) :
    trace μ (compPow μ (compl U) 2) ≥ (1 - edgeDensity U μ) ^ 3 - (1 - edgeDensity U μ) * edgeDensity U μ ^ 2 := by
  ...
  have hed : trace μ (compPow μ U 2) ≤ pathDensity U μ 2 := edge_deletion_general hU 1
  have hcert : 0 ≤ specMoment U μ 0 := specMoment_zero_nonneg hU
  rw [complTrace_necklace hU 1]
  simp only [pairing_pathIter_complIter_closed hU, complMean_succ hU, complMean_zero, pathDensity_zero,
...
theorem C7_integral (hU : IsGraphon U μ) (hq : edgeDensity U μ ≤ 1 / 2) :
  ...
  have hcert := specMoment_sos_seven hU (edgeDensity U μ) (edgeDensity_nonneg hU) hq
