...
engine; `cert5_specMoment` and `cert7_specMoment` are then pure polynomial algebra in the `specMoment`.
-/
...
lemma cert5_specMoment (hU : IsGraphon U μ) (q : ℝ) :
...
lemma cert7_Lpart_specMoment (hU : IsGraphon U μ) (q : ℝ) (hq1 : q ≤ 1 / 2) :
...
lemma cert7_B_specMoment (hU : IsGraphon U μ) (q : ℝ) :
...
lemma cert7_specMoment (hU : IsGraphon U μ) (q : ℝ) (hq0 : 0 ≤ q) (hq1 : q ≤ 1 / 2) :
    0 ≤ 6 * specMoment U μ 4 + (12 * q - 7) * specMoment U μ 3 + (18 * q ^ 2 - 21 * q + 7) * specMoment U μ 2
        + (24 * q ^ 3 - 42 * q ^ 2 + 28 * q - 7) * specMoment U μ 1
  ...
  have hLpart := cert7_Lpart_specMoment hU q hq1
  have hB := cert7_B_specMoment hU q
  have hs0 := specMoment_zero_nonneg hU
