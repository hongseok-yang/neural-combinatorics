...
| finite Bernstein certs, all odd `9 ≤ m ≤ 61` (`prop:finite` = `Hfin`) | ✅ | in-kernel reflection: `HfinPolyReflect` (dense ℤ-BP `bpEval` homomorphism, `eqBP` soundness, Horner RHS `hornerBP`, `hsym`-closed-form LHS `dkClearedBP`) + generated `Hfin/` (**all 196 pairs**, 27 dispatchers, `Aggregate.finKernel_all`); each pair = the cleared depth-0 Bernstein identity checked by `decide +kernel` (no `native_decide`), nonneg structural.  Capstone `HighDensityLE61.odd_cycle_bound_le61`.  The >~500-bit large pairs (`43 ≤ m ≤ 61`) need a raised kernel memory guard — built serially at `-M 48000` (lakefile `weakLeanArgs`); e.g. `(61,2)` (799-bit coeffs) builds in ~3 min. |
