...
| finite Bernstein certs, odd `m ≤ 41` (`prop:finite` = `Hfin`) | ✅ (band `m ≤ 41`) | in-kernel reflection: `HfinPolyReflect` (dense ℤ-BP `bpEval` homomorphism, `eqBP` soundness, Horner RHS `hornerBP`, `hsym`-closed-form LHS `dkClearedBP`) + generated `Hfin/` (81 pairs, 17 dispatchers, `Aggregate.finKernel_all`); each pair = the cleared depth-0 Bernstein identity checked by `decide +kernel` (no `native_decide`), nonneg structural.  Capstone `HighDensityLE41.odd_cycle_bound_le41`.  **Deferred `43 ≤ m ≤ 61`:** certs proven valid + pass at higher `-M`, but >~500-bit cleared coeffs OOM the kernel `decide` at 16 GB (e.g. `(61,2)` needs `-M 48000`); build them serially to widen the band. |
