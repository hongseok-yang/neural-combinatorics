...
\[
 ...
 =-2a^2\ge-\frac12.
\]
Finally, \(A=P_{\one^\perp}T_UP_{\one^\perp}\) acts on \(\one^\perp\).  Thus,
for every \(f\in\one^\perp\), we have \(P_{\one^\perp}f=f\) and hence
\[
       \ip f{Af}=\ip f{T_Uf}.
\]
...
\[
 \norm A_{\mathrm{op}}
 =\sup_{\substack{f\in\one^\perp\\ \norm f=1}}
    \bigl|\ip f{Af}\bigr|.
\]
The upper and lower bounds just proved therefore give
\(\bigl|\ip f{Af}\bigr|\le\tfrac12\) for every unit \(f\in\one^\perp\), and
...
Let \(E_A\) be the spectral resolution of \(A\): for a Borel set
\(B\subseteq\R\), the operator \(E_A(B)\) is the orthogonal projection onto
the spectral subspace of \(A\) corresponding to eigenvalues in \(B\).
Define the finite positive measure
\begin{equation}\label{eq:mu-def}
       \mu(B)=\ip g{E_A(B)g}.
\end{equation}
Concretely, in the finite-rank case write
\[
       A=\sum_i\lambda_iP_i,
\]
...
Then
\[
 \mu(B)=\sum_{\lambda_i\in B}\ip g{P_i g}
       =\sum_{\lambda_i\in B}\norm{P_i g}^2.
\]
The last equality holds because \(P_i\) is an orthogonal projection:
\(\ip g{P_i g}=\ip{P_i g}{P_i g}\).  Thus \(\mu\) assigns to each
eigenvalue the nonnegative weight measuring how much of \(g\) lies in its
eigenspace.  By \Cref{lem:half-norm}, \(\mu\) is supported on
\([-1/2,1/2]\).  More generally, the spectral theorem gives
\[
       \ip g{F(A)g}
       =\int_{-1/2}^{1/2}F(\lambda)\dd\mu(\lambda)
\]
