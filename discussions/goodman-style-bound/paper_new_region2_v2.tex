...
\begin{proof}
Let \((\mathcal P_n)\) be refining finite measurable partitions generating
the Borel sigma-algebra, and put
\[
 W_n=\E[W\mid\mathcal P_n\times\mathcal P_n].
\]
Then \(W_n\) is a symmetric step graphon, \(0\le W_n\le1\), and
\(W_n\to W\) in \(L^2([0,1]^2)\).  Conditional expectation preserves the
integral, so \(t(K_2,W_n)=p\) exactly.  The associated integral operators
converge in Hilbert--Schmidt norm.  A telescoping expansion of
\(T_{W_n}^m-T_W^m\), followed by Schatten--H\"older, gives
\[
       \Tr(T_{W_n}^m)\longrightarrow\Tr(T_W^m).
\]
Since \(t(C_m,W)=\Tr(T_W^m)\), the desired inequality passes to the limit.
The graphon--operator dictionary used here is standard; see, for example,
Lov\'asz~\cite{Lovasz2012}.
\end{proof}

\begin{remark}[Formal versus analytic power series]
All coefficient extractions below may be read formally.  After the
finite-rank reduction, the relevant resolvents are rational functions
analytic near the origin, so ordinary determinant and logarithm identities
are also legitimate analytically.
\end{remark}
