...
\end{equation}
Equivalently, this identity holds coefficientwise as a formal power series
at \(z=0\).
...
We use the following sign change.  On the orthogonal decomposition
\(\mathcal E=\R\one\oplus\mathcal E_0\), define
\[
 \mathcal J(a\one+h)=a\one-h
 \qquad(a\in\R,\ h\in\mathcal E_0).
\]
Thus \(\mathcal J\) fixes the constant part of a function and reverses the
sign of its mean-zero part.  For \(a,b\in\R\) and
\(h,k\in\mathcal E_0\), orthogonality of the two summands gives
\[
\begin{aligned}
 \ip{\mathcal J(a\one+h)}{\mathcal J(b\one+k)}
 &=\ip{a\one-h}{b\one-k}\\
 &=ab+\ip h{k}
 =\ip{a\one+h}{b\one+k}.
\end{aligned}
\]
Thus \(\mathcal J\) preserves the inner product.  It also satisfies
\(\mathcal J^{-1}=\mathcal J\), so it is onto.  Hence \(\mathcal J\) is a
unitary (in this real setting, orthogonal) operator.  In block form,
\[
 \mathcal J=\begin{pmatrix}1&0\\0&-I\end{pmatrix},
 \qquad
 \mathcal JT_W\mathcal J^{-1}
 =\begin{pmatrix}p&g^*\\g&-A\end{pmatrix}
 =:M.
\]
Two operators related by such a unitary change of coordinates are called
\emph{unitarily equivalent}.  This changes only their coordinate
description: for every \(j\ge1\),
\[
       M^j=\mathcal JT_W^j\mathcal J^{-1}.
\]
For finite matrices, \(\Tr(BC)=\Tr(CB)\), as one sees directly by writing
both sides as \(\sum_{i,k}B_{ik}C_{ki}\).  Therefore
\[
 \Tr(M^j)
 =\Tr(\mathcal JT_W^j\mathcal J^{-1})
 =\Tr(T_W^j\mathcal J^{-1}\mathcal J)
 =\Tr(T_W^j).
\]
By \eqref{eq:finite-trace-cycle}, this says that \(M\) has exactly the same
cycle traces as \(T_W\), so we may use \(M\) in place of \(T_W\).
...
\eqref{eq:block-determinant-identity}.  All matrices here act on the
finite-dimensional space \(\mathcal E\).  Formally, the blocks
\(I\pm zA\) are invertible because their constant term is \(I\); analytically,
they are invertible for \(z\) sufficiently close to zero.
