...
\end{equation}
This cell-averaging operation is the conditional 
expectation of $W$ with respect to the sigma algebra
$A_n\otimes\mathcal A_n$, and it is usually 
denoted by the compact notation
\(\E[W\mid\mathcal A_n\otimes\mathcal A_n]\). Since \(0\le W\le1\),
...
\[
 ...
 =\frac{1}{2^{nm}}
   ...
   w^{(n)}_{a_ma_1}.
\]
Equivalently, the integral operator with kernel \(W_n\) has range in the
span of the \(2^n\) interval indicators, which explains the term
...
Finally, cycle densities are continuous under this convergence for an
elementary reason.  Put \(x_{m+1}=x_1\), and for a fixed
\((x_1,\ldots,x_m)\) write
\[
...
power series; ``formal'' does not mean heuristic.

The second interpretation is the usual \emph{analytic} one.  By
\Cref{lem:step-reduction}, it is enough during these calculations to work
with a step graphon.  Its operator is represented on its finite-dimensional
range by a finite matrix \(X\).  The determinant \(\det(I-zX)\) is then a
polynomial equal to \(1\) at \(z=0\), so it is nonzero for all sufficiently
small \(|z|\).  Consequently \((I-zX)^{-1}\) is a matrix of rational
functions analytic near \(0\), and the branch of
\(\log\det(I-zX)\) that vanishes at \(0\) is analytic there as well.  Their
ordinary Taylor expansions are exactly the formal series above.

The two readings therefore give the same coefficient identities.  One may
read the later determinant, resolvent, and logarithm manipulations as
ordinary finite-matrix generating-function arguments.  After those
identities prove the desired inequality for step graphons,
\Cref{lem:step-reduction} passes it to the original graphon; no
infinite-dimensional determinant theory is required.
\end{remark}
...
\end{aligned}
\]
Since \(C_3=K_3\), this is the claim.
\end{proof}
...
\begin{equation}\label{eq:path-recursion}
       ...
       \qquad (a_0,h_0)=(1,0).
\end{equation}
Repeated use of \eqref{eq:path-recursion} yields
...
\begin{proof}
Assume \eqref{eq:q-short}.  Expand the five factors \(1-U_e\) around the
cycle.  Every proper selected subgraph is a disjoint union of paths, and
homomorphism density is multiplicative over disjoint unions.  The relevant
counts are
\[
\begin{array}{c|c|c}
\text{number of selected edges}&\text{component lengths}&\text{multiplicity}\\
\hline
0&-&1\\
1&1&5\\
2&2&5\\
...
\begin{proof}
Assume \eqref{eq:q-short}.  The proper edge subsets of a labelled \(7\)-cycle
have the following run-length types.
\[
\begin{array}{c|l}
 k&\text{types and multiplicities for }k\text{ selected edges}\\
\hline
0&1\\
1&7(1)\\
2&7(2)+14(1+1)\\
...
\end{array}
\]
Here, for example, \(3+1\) denotes a disjoint union of a three-edge path and
an edge.  Expanding \(t(C_7,1-U)\), and using
\(c_7=t(C_7,U)\le x_6\), gives
