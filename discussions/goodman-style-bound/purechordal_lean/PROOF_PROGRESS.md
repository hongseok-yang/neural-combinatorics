...
| Certificate-level minimality | Expose the $k$-partite minimality as a `PureCliqueTreeDecomp` method `balancedMultipartite_minimal`, taking the certificate directly (no chordality/maximality hypotheses); the abstract theorem is a one-line corollary. | Medium | ✅ |
| Enumerated example catalog | Certify all $17$ non-clique pure chordal graphs on $\le 6$ vertices ($1+4+12$) via generated `PureCliqueTreeDecomp`s and prove $k$-partite `optimality` for each by applying `balancedMultipartite_minimal`. Generated by `gen_examples.py`. | Hard | ✅ |
| Leaf API layout | Keep both final theorems (`pureChordal_chromaticPolynomial_lower_bound`, `pureChordal_balancedMultipartite_minimal`) in `Main.lean`; collect the enumerated graph examples under `Examples/N4,N5,N6`; `Example.lean` is the hub importing `Main` and the example modules. | Medium | ✅ |
