...
% --- extra macros for the July 2026 sections (Layers 2--3) ---
...
\emph{Layer 2 (July 4, 2026, \S\ref{sec:setup2}--\S\ref{sec:general-pos}, \S\ref{sec:regionII}).}  The two-sided spectral-shift identity $t(C_m,W)+t(C_m,1-W)=p^m+q^m+S_m$ reorganises the problem exactly; after a single edge-deletion step it reduces, for every odd $m$ at once, to the nonnegativity of an explicit two-parameter family of diagonal kernels $\wtP_{m,r}(q,\ell)$ (via a mixture-of-diagonals theorem and a one-dimensional kernel representation).  Positivity is proved analytically in several regimes (the residual cases are completed in Layer~3 below), and the machinery yields new Region-II tools (an aggregate forced-coupling lemma; the failure of the naive total-coupling bound).
...
%======================================================================
%  THE JULY 2026 DEVELOPMENT: THE TWO-SIDED SPECTRAL-SHIFT METHOD, THE
%  HIGH-DENSITY THEOREM, THE RANK-TWO THEOREM, AND REGION II
%  (Claude, July 4, 2026; completed by GPT Pro, July 5, 2026)
%======================================================================

\bigskip
\begin{center}\rule{0.6\linewidth}{0.4pt}\end{center}
\paragraph{About the July 2026 development (\S\ref{sec:setup2} onward, i.e.\ Layers 2--3 of the abstract).}
The sections up to this point are the May 2026 state of the project.  The
remainder of the document is the July 2026 development, whose central outcome is the
\emph{high-density theorem} (\S\ref{sec:high-density-theorem}): Conjecture~\ref{conj:main} holds for every odd
...
\]
The May 2026 sections (Lemma~\ref{lem:compression} and Corollary~\ref{cor:norm-safe}) establish the compression bound $\sigma(A)\subseteq[-1/2,1/2]$ and the trace-square budget
\[
        \Tr(A^2)+2\norm g_2^2\le pq.
\]
...
\[
        \wtP_{m,r}(q,\ell)\ge0
        \qquad
        (1\le r\le(m-1)/2,\quad -1/2\le\ell\le1/2).
\]
\end{corollary}

\section{The diagonal family as a one-dimensional integral}\label{sec:kernel}

Fix $r\ge1$ and odd $n\ge1$, and put $m=n+2r$.  The diagonal family has a useful probabilistic form.  Let $\Xi\sim\Betad(r,r)$ and set
\[
        V_x=qx+\ell(1-x),
        \qquad
        W_x=px-\ell(1-x).
...
\begin{proof}
In~\eqref{eq:G-form}, write the $\Betad(r,r)$ density as
\[
...
\begin{proof}
The two formulae in item~\ref{rho-group} are algebraic rearrangements of~\eqref{eq:rho-def}.  For $u\le1/2$, the first formula is nonnegative because $n-1$ is even and $|1-u|\ge |u|$.  For $n/m\le u\le1$, the second formula is nonnegative.  If $u\ge1$, then $u^n-(u-1)^n\ge u^{n-1}$, so $\rho(u)\ge(m/n-1)u^{n-1}\ge0$.  If $u=-v\le0$, then $(1+v)^n-v^n\ge nv^{n-1}$, so $\rho(u)\ge(m-1)v^{n-1}\ge0$.  This proves item~\ref{rho-window}; item~\ref{rho-empty} follows because the possible negative window is empty when $n/m\le1/2$.
...
\]
The final product is nonnegative because $n-1$ is even and $|u|\ge|1-u|$ exactly when $u\ge1/2$.  Finally, item~\ref{rho-neg} follows by dropping the nonnegative term $(m/n)(1-u)^n$ from the second formula in item~\ref{rho-group}.
\end{proof}
...
\begin{proof}
It is enough to prove the bracket in~\eqref{eq:G-form} is nonnegative.  The hypothesis gives $\ell\ge q+r/m>q$ (as $r/m>0$); in particular $\ell-q>0$, and the function $V(x)=\ell+(q-\ell)x$ is positive on $[0,1]$.  (For $\ell\le0$ the statement is already part of Theorem~\ref{thm:pointwise}.)

First, $\E W_\Xi^n\ge0$.  Indeed, $W(x)=(p+\ell)(x-x_w)$ with $x_w=\ell/(p+\ell)\le1/2$, and the $\Betad(r,r)$ law is symmetric about $1/2$.  Pairing $x=1/2+t$ with $x=1/2-t$, the two values of $x-x_w$ have nonnegative sum and the larger absolute value is the positive one, so their odd $n$th powers sum to a nonnegative number.
...
pairing genuinely consumes partner mass.)  The proof given below, due to
...
The GPT~Pro checker verifies $\operatorname{StripCert}(61)$ using exact rational interval arithmetic.  Its July 5 run log records successful verification for all residual pairs with odd $5\le m\le61$.
\end{proposition}
...
\begin{enumerate}[label=\textup{(\alph*)}]
\item In \texttt{gptpro\_jul5/}, \texttt{clean\_two\_sided\_shift\_checker.py} verifies
...
\begin{align}
        ...
        \rho(u)&\ge \frac{u-\nu}{\nu}u^{n-1}
        &&\text{for }\nu<u\le1,\label{eq:rho-pos-tail}\\
        \rho(u)&\ge \frac mn(1-u)^n-u^{n-1}
        &&\text{for }u\ge0.\label{eq:rho-left-surplus}
\end{align}
\end{lemma}
...
\[
        \nu\rho(u)=(u-\nu)u^{n-1}+(1-u)^n\ge(u-\nu)u^{n-1},
\]
where the last step drops $(1-u)^n\ge0$, which is nonnegative precisely because $u\le1$ and $n$ is odd.  Finally, \eqref{eq:rho-left-surplus} holds for $u\ge0$: it is obtained from the definition of $\rho$ by discarding the term $(m/n)u^n$, which is nonnegative exactly when $u\ge0$ (again as $n$ is odd).  In the residual strip both bounds are applied only where they are valid: \eqref{eq:rho-pos-tail} at $u=\nu+e\le2\nu-\tfrac12\le\tfrac56<1$ in Lemma~\ref{lem:upper-reflection-tail}, and \eqref{eq:rho-left-surplus} at $u=q+s\ge0$ in Lemma~\ref{lem:left-surplus-tail}.
\end{proof}
...
\[
        ...
        +\frac{2a^2-\alpha}{2a}.
\]
Thus an asymmetric frontier eigenfunction with $\alpha$ close to its ceiling
...
Consequently
\[
        \frac{\norm g_2^2(p-\alpha_1)^2}{p\alpha_1(\alpha_1-q)^2}
        =9s+O(s^2)\to0.
\]
Thus no universal positive constant can make the rank-one lower bound
\[
        \norm g_2^2\gtrsim \frac{p\alpha_1(\alpha_1-q)^2}{(p-\alpha_1)^2}
\]
...
(iv) The certificate family of the high-density method (\S\ref{sec:setup2}--\S\ref{sec:high-density-theorem}) extends numerically to $q\le0.4117$
...
The following checks accompany the July 2026 sections (\S\ref{sec:setup2}--\S\ref{sec:regionII}).  All ``exact'' items use
