...
\title{Region II of the odd-cycle lower bound:\\
the scalar Huber inequality and its resolution\\[1mm]
\large A self-contained continuation of the July 11 baton note}
\author{Working record for T.K., H.Y., and collaborators\\[1mm]
\normalsize Reduction: GPT~Pro, July 11, 2026 (recapped here with full proofs);\\
\normalsize scalar theorem and zone proofs: Claude, July 11, 2026}
\date{}
...
\begin{abstract}
Let $W$ be a graphon with edge density $p$, and let $m\ge3$ be odd.  The
conjecture $t(C_m,W)\ge p^m-p(1-p)^{m-1}$ was previously proved for
$p\le1/2$ (trivial), $p\ge2/3$ (the high-density theorem), all odd $m\le13$,
and several structured families; the July~11 baton note of GPT~Pro reduced
the remaining range $\tfrac12<p<\tfrac23$ (``Region II'') rigorously to a
single scalar inequality, $R_m\le C_m\psi(\xi,\rho)$, in the three variables
$(q,\alpha,m)$: $q=1-p$, $\alpha$ the unique frontier eigenvalue of the
complement compression, $\psi$ an explicit piecewise Huber envelope.

This note proves that scalar inequality for all admissible $(q,\alpha)$ and
all odd $m\ge15$.  The proof splits the admissible domain into three zones
and, in each, certifies the Huber payment through an explicit value of its
dual representation; the delicate zones are the \emph{pinch}
($p\downarrow1/2$ with $m(1-2\alpha)\to0$, where the inequality is
asymptotically tight, with relative margin $\Theta(1/m)$) and the
\emph{Tur\'an corner} ($p\uparrow2/3$, where both sides vanish and the
domain constraint $q>1/3$ is essential).  All proofs are analytic with
explicit constants.  Combined with the earlier results, this closes
Region~II and hence the odd-cycle lower-bound conjecture in full:
\emph{for every graphon $W$ and every odd $m\ge3$,
$t(C_m,W)\ge p^m-p(1-p)^{m-1}$.}

For self-containedness, the one-frontier reduction of the baton note is
restated here with complete proofs (with three small expository gaps
repaired); the new material is the reparametrization and zone analysis of
\S\ref{sec:coords}--\S\ref{sec:assembly}.  A companion script re-verifies
every algebraic identity exactly and every inequality step numerically.
Verification status: the reduction and Zones A and C--small are fully
analytic; Zone B and Zone C--moderate are analytic up to two finite exact
rational certificates ($23$ and $3001$ boxes); the Tur\'an corner is
analytic.  The pinch and Zone B were each additionally proved or certified a
second time by an independent derivation.
\end{abstract}
...
\end{equation}
The consolidated record \texttt{paper\_new.tex} (July 5, 2026) proves
...
The July 11 baton note of GPT~Pro (\texttt{gptpro\_jul11/regionII\_baton\_note.tex})
reduces Region II rigorously to a single \emph{scalar} inequality --- the
...
\emph{Proof architecture.}  The scalar inequality is proved through the two
dual certificates $\lambda=1$ and $\lambda=\min(1,2\rho\xi)$ of the Huber
envelope $\psi$, on three zones that cover the admissible domain
(\S\ref{sec:coords}): the pinch zone A ($e=1-2\alpha\le\tfrac1{60}$,
$\xi\ge1$), where the inequality is asymptotically tight and the proof is
sharpest; zone B ($e\ge\tfrac1{60}$, $\xi\ge1$); and zone C ($\xi\le1$),
which contains the ultra-thin frontier scale and the Tur\'an corner
$p\uparrow\tfrac23$.  Every lemma is analytic, with explicitly displayed
constants; no step relies on floating-point computation.  As a safety net
against transcription and arithmetic slips, the companion script
\texttt{regionII\_scalar\_checker.py} re-verifies (i) all algebraic
identities exactly in rational/symbolic arithmetic and (ii) every displayed
inequality of every proof step numerically on dense adversarial grids
...
\begin{center}
\small
\begin{tabular}{p{0.30\linewidth}p{0.22\linewidth}p{0.4\linewidth}}
\hline
Result & Origin & Verification \\
\hline
One-frontier reduction (\S\ref{sec:reduction}) & GPT~Pro, July 11 & analytic; audited line-by-line (July 11), three expository gaps repaired here; identities re-checked in exact arithmetic \\
Chart, defect form, certificates (\S\ref{sec:coords}) & Claude, July 11 & analytic; identities exact-checked \\
Zone A = pinch (\S\ref{sec:zoneA}) & Claude & analytic; every step numerically re-verified; \emph{independent second proof} with exact rational constant certificates (\texttt{claude\_jul11/proof\_Z1.md}, \texttt{claude\_jul11/check\_Z1.py}) \\
Zone B (\S\ref{sec:zoneB}) & Claude & analytic $m$-maximization + exact rational box certificate (\texttt{zoneB\_certifier.py}); independently re-derived box certificate (\texttt{claude\_jul11/check\_ZB\_final.py}) \\
Zone C, $e\le1/60$ (\S\ref{sec:zoneC}) & Claude & analytic; every step numerically re-verified \\
Zone C, $e\ge1/60$ (\S\ref{subsec:zoneC-mod}) & Claude & analytic superstructure (three-geometric defect, secant gate, $m$-free payment gating) + exact rational box certificate (\texttt{zoneC\_certifier.py}); Tur\'an-corner lemma analytic, constants re-verified numerically \\
Assembly (\S\ref{sec:assembly}) & Claude & analytic \\
Global sweep (\S\ref{sec:verification}) & Claude (agents) & numerical only (safety net): $68{,}564$ admissible points, no violation, min margin $4.6\times10^{-6}$ \\
\hline
\end{tabular}
\end{center}
...
Fix $q\in(1/3,1/2)$.  If $\lambda_{\max}(A)\le q$, the graphon is
positive-spectrum-safe and \eqref{eq:conj} is already proved
(\texttt{paper\_new.tex}, \S ``General structural results'').  So assume $A$ has
an eigenvalue $\alpha>q$ with unit eigenfunction $\phi\perp\one$.
...
\]
whence $(\alpha-q)a^2\le2a\|g\|_2\sqrt{1-a^2}$ and
\[
        \|g\|_2^2\ge\frac{(\alpha-q)^2a^2}{4(1-a^2)}
        \ge\frac{\alpha(\alpha-q)^2}{2(1-2\alpha)},
\]
using $a^2\ge2\alpha$ and that $t\mapsto t/(1-t)$ is increasing.
...
Lemma~\ref{lem:safe} reads $bc\ge H\ge\frac{d\sqrt{2\alpha}}2>0$, so $c>0$;
in the variables $v$ introduced below, $c\beta(c)\ge bc\ge\frac{d\sqrt{2\alpha}}2$
forces $\xi-v+v^2\le0$, whence the payment already satisfies
$A_mc^2=C_m\rho v^2\ge C_m\psi$.  So assume $K>0$.)
...
\subsection{The admissible domain and the $(e,\kappa)$ chart}
...
\item \textbf{Zone C} (\S\ref{sec:zoneC}): $\xi\le1$ (any $e$); certificate
$\lambda=\min(1,2\rho\xi)$.  This zone contains the ultra-thin frontier scale
and the \emph{Tur\'an corner} $e\uparrow\tfrac13$, $\kappa\downarrow0$, where
$R_m\to0$ and the constraint $q>\tfrac13$ is essential.
\end{itemize}

\section{Zone A: $e\le1/60$, $\xi\ge1$}\label{sec:zoneA}
...
The second inequality is the certificate \eqref{eq:cert-II}; the content is the
first.  We may assume $R_m>0$.  Throughout this section put
\[
        z:=(m-2)u,
        \qquad
        ...
        \sigma_e:=(2.04\,e)^{6},
\]
and record the standing bounds, valid for $e\le\tfrac1{60}$, $\kappa\le1$:
\begin{equation}\label{eq:standing}
        \alpha\ge\tfrac{59}{120},\quad
        p\le\tfrac{1+3e}2\le\tfrac{21}{40},\quad
        x\ge\tfrac{1-e}{1+3e}\ge0.9365,\quad
        \frac e\alpha\le\frac{2e}{1-e}\le2.04\,e,\quad
...
$\tau^{m-1}<x+L^m/(p\alpha^{m-1})$.  Since $L^2\le\alpha e$,
\[
        \frac{L^m}{p\alpha^{m-1}}
        \le\frac{(\alpha e)^{m/2}}{p\,\alpha^{m-1}}
        =x\Bigl(\frac e\alpha\Bigr)^{m/2}
        \le x\,(2.04e)^{7.5}\le x\,\sigma_e\cdot(2.04e)^{1.5} .
\]
By Bernoulli, $\tau^{m-1}\ge1-(m-1)(1-\tau)=1-(m-1)d/\alpha$, so
\[
        (m-1)\frac d\alpha\ >\ 1-x\bigl(1+\sigma_e(2.04e)^{1.5}\bigr)
        \ \ge\ \frac{d+e}{p}-\sigma_e(2.04e)^{1.5}.
\]
Multiplying by $\alpha/e$ and using $\alpha(d+e)/(pe)=x(1+\kappa)$ together
with $\frac\alpha e\,\sigma_e(2.04e)^{1.5}
=\alpha\,2.04^{1.5}\sqrt e\;\sigma_e\le0.19\,\sigma_e\le\sigma_e$
(as $\alpha\le\tfrac12$, $e\le\tfrac1{60}$)
gives the first inequality of \eqref{eq:posA}.  Then
$\kappa(m-1-x)>x-\sigma_e$, so
$\kappa\ge(x-\sigma_e)/(m-1)\ge0.93/(m-1)$ by \eqref{eq:standing}.
\end{proof}

\begin{lemma}[Payment and defect reduction]\label{lem:reduceA}
Assume $R_m>0$, $e\le\tfrac1{60}$, $\xi\ge1$.  Then
\begin{equation}\label{eq:epsA}
        \eps\ \le\ \min\Bigl(\tfrac14,\ 0.278\,T\Bigr),
\end{equation}
and \eqref{eq:zoneA-target} is implied by
\begin{equation}\label{eq:reducedA}
        \sqrt{1-e}\;\frac{(1-\ell)(1-y^{m-1})(1-\eps)}{1+y}
        \ \ge\
        x^{m-2}\Bigl[\frac{m-1}{mx}-\frac{1+1/\kappa}{m}\Bigr]
        +\Lambda,
        \qquad
...
\begin{proof}
\eqref{eq:epsA}: $\xi\ge1$ means $d\ge e^2/(4\alpha^2)$, so
$\eps\le1/(4(1+\rho))\le\tfrac14$; and $\kappa\ge0.93/(m-1)$
(Lemma~\ref{lem:posA}) gives
$\eps\le\frac{(m-1)e}{0.93\cdot4(1-e)^2}\le0.278\,T$.

Payment: $B_m\ge m\,k_m(L)=\frac{m\,p^{m-1}(1-y^{m-1})}{p+L}$, $\sqrt{2\alpha}=\sqrt{1-e}$,
$f=\alpha(1-\ell)$, $p+L=p(1+y)$, so
\[
        \sqrt{2\alpha}B_mf\Bigl(d-\frac{e^2}{16\alpha^2(1+\rho)}\Bigr)
        \ \ge\
        \sqrt{1-e}\;m\,d\,\alpha\,p^{m-2}\,
        \frac{(1-\ell)(1-y^{m-1})(1-\eps)}{1+y}.
\]
Defect: by \eqref{eq:defect-form} and Bernoulli
($\tau^{m-1}\ge1-(m-1)d/\alpha$, and $\alpha-p=-(d+e)$, $p/\alpha=1/x$),
\[
        R_m\le\alpha^{m-1}\Bigl[\frac{(m-1)d}{x}-(d+e)\Bigr]+(\alpha e)^{m/2}.
\]
Dividing both bounds by $m\,d\,\alpha\,p^{m-2}>0$ and using
$\alpha^{m-2}/p^{m-2}=x^{m-2}$, $(d+e)/d=1+1/\kappa$ yields
\eqref{eq:reducedA}.  Finally
$\Lambda=x^{m-2}\,(e/\alpha)^{m/2-1}/(m\kappa)
\le x^{m-2}(2.04e)^{6.5}/0.93\le\sigma_e x^{m-2}$,
using $m\kappa\ge0.93$ and $m\ge15$.
\end{proof}

\begin{proof}[Proof of Lemma~\ref{lem:zoneA}]
Assume $R_m>0$ and abbreviate the two sides of \eqref{eq:reducedA} as
$\mathrm{LHS}\ge\mathrm{RHS}$.

\emph{Left side.}  Since $y\le\ell$ and $(1-\ell)/(1+\ell)\ge1-2\ell$, and
$\sqrt{1-e}\ge1-0.51e$ for $e\le0.0769$, and
$y^{m-1}\le\ell^{14}\le(2.04e)^7\le\sigma_e$,
\begin{equation}\label{eq:GammaA}
        \mathrm{LHS}\ \ge\ 1-\Gamma,
        \qquad
        \Gamma:=2\ell+\eps+0.51e+\sigma_e .
\end{equation}

\emph{Right side.}  If the bracket in \eqref{eq:reducedA} is negative, then
$\mathrm{RHS}\le\Lambda\le\sigma_e$ while $\mathrm{LHS}\ge1-\Gamma\ge0.36$
(using $2\ell\le0.369$, $\eps\le\tfrac14$, $e\le\tfrac1{60}$), and we are
done.  So assume the bracket is nonnegative.  Since
$\frac{m-1}{mx}=\frac1x-\frac1{mx}$ and $\frac1{mx}\ge\frac1m$,
\[
        \Bigl[\cdots\Bigr]\ \le\ \frac1x-\frac{2+1/\kappa}{m},
        \qquad
        \frac1x=1+\frac{2(1+\kappa)e}{1-e}\le1+2.034(1+\kappa)e .
\]
By \eqref{eq:E-a} with $n=m-2\ge13$ and
$\binom n2u^2=\frac{m-3}{2(m-2)}z^2\ge\frac6{13}z^2$,
\[
        x^{m-2}\le\frac1{1+z+\frac6{13}z^2}.
\]

\emph{Auxiliary bounds.}  From $u=(d+e)/p\ge\frac{2(1+\kappa)e}{1+3e}
\ge1.9048(1+\kappa)e$:
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=2.4em]
\item $2.034(1+\kappa)e\le\frac{2.034}{1.9048}\cdot\frac z{m-2}
\le\frac{1.0679}{13}z\le0.0822\,z$;
\item $T=me\le\frac{15}{13}(m-2)e\le\frac{15}{13}\cdot
\frac{z}{1.9048(1+\kappa)}\le0.6057\,z$, so by \eqref{eq:epsA}
$\eps\le0.278\,T\le0.1684\,z$;
\item from $m\le\frac{z}{1.9048(1+\kappa)e}+2$ and
$(1+\kappa)(2+1/\kappa)=3+2\kappa+\frac1\kappa\ge3+2\sqrt2\ge5.8284$
(minimum at $\kappa=1/\sqrt2$), together with $1+\kappa\le2$:
\[
        \frac{2+1/\kappa}{m}
        \ \ge\ \frac{1.9048\,(1+\kappa)(2+1/\kappa)\,e}{z+3.8096(1+\kappa)e}
        \ \ge\ \frac{11.08\,e}{z+7.8\,e}.
\]
\end{enumerate}

\emph{The battle inequality.}  Combining the last four displays,
\eqref{eq:reducedA} is implied by: for all $z>0$ and $0<e\le\tfrac1{60}$,
\begin{equation}\label{eq:battle}
        (1-\Gamma)\Bigl(1+z+\tfrac6{13}z^2\Bigr)
        \ \ge\ 1+0.0822\,z-\frac{11.08\,e}{z+7.8\,e}+\sigma_e ,
\end{equation}
with $\Gamma\le A_0+\min\bigl(\tfrac14,\,0.1684z\bigr)$,
$A_0:=2.8527\sqrt e+0.51e+\sigma_e$.
[Here $2\ell\le2\cdot1.4262\sqrt e\le2.8527\sqrt e$ by \eqref{eq:standing};
and $\sigma_e$ on the right absorbs $\Lambda\cdot(1+z+\tfrac6{13}z^2)
\le\sigma_e$ since $\Lambda\le\sigma_ex^{m-2}$ and
$x^{m-2}(1+z+\tfrac6{13}z^2)\le1$.]

Equivalently, writing $F(z,e)$ for the difference of the two sides of
\eqref{eq:battle} after moving everything to the left:
\[
        F=z\bigl(0.9178-\Gamma\bigr)+\tfrac6{13}(1-\Gamma)z^2
        +\frac{11.08\,e}{z+7.8\,e}-\Gamma-\sigma_e\ \ge\ 0 .
\]
We prove $F\ge0$ in three ranges of $z$.  Note $A_0\le0.3768$ and
$\sigma_e\le(0.034)^5\cdot 2.04e\le5\cdot10^{-8}e$ for $e\le\tfrac1{60}$.

\emph{Range 1: $0<z\le1$.}  Here $\Gamma\le A_0+0.1684z$.  Then
\[
        F\ \ge\ z\bigl(0.7494-A_0\bigr)
        +z^2\Bigl(\tfrac6{13}(1-A_0)-0.1684-\tfrac6{13}\cdot0.1684\,z\Bigr)
        +\frac{11.08e}{z+7.8e}-A_0-\sigma_e .
\]
Since $A_0\le0.3768$ and $z\le1$, the $z^2$-coefficient is at least
$0.4615\cdot0.6232-0.1684-0.0777=0.0415\ge0$, so it may be dropped, and
$0.7494-A_0\ge0.3726$:
\[
        F\ \ge\ h(z)-A_0-\sigma_e,
        \qquad
        h(z):=0.3726\,z+\frac{11.08\,e}{z+7.8\,e} .
\]
$h$ is convex with interior minimum at $z_*+7.8e=\sqrt{11.08e/0.3726}
=5.4532\sqrt e$ (interior since $5.4532\sqrt e>7.8e$ for $e\le\tfrac1{60}$),
of value
\[
        h(z_*)=2\sqrt{0.3726\cdot11.08\,e}-0.3726\cdot7.8\,e
        =4.0638\sqrt e-2.9063\,e .
\]
Thus $F\ge4.0638\sqrt e-2.9063e-2.8527\sqrt e-0.51e-2\sigma_e
=1.2111\sqrt e-3.4163e-2\sigma_e>0$, because
$3.4163\sqrt e\le0.4411<1.2111$ and $2\sigma_e\le10^{-7}e\ll\sqrt e$.

\emph{Range 2: $1\le z\le1.4845$.}  Still $\Gamma\le A_0+0.1684z$ (as
$0.1684z\le0.25$).  All occurrences of $A_0$ in $F$ carry a negative sign, so
the worst case is $A_0=0.3768$, and (keeping the cubic term exactly)
\[
        F\ \ge\ q(z):=0.3726\,z+0.1192\,z^2-0.0777\,z^3-0.3768-\sigma_e .
\]
$q'(z)=0.3726+0.2384z-0.2331z^2$ is decreasing on $[1,1.4845]$ with
$q'(1.4845)=0.2129>0$, so $q$ is increasing there and
$q(1)=0.0373-\sigma_e>0.037>0$.

\emph{Range 3: $z\ge1.4845$.}  Here $\Gamma\le A_0+\tfrac14\le0.6268$, so
\[
        F\ \ge\ 0.2910\,z+0.1722\,z^2-0.6268-\sigma_e ,
\]
which at $z=1.4845$ equals $0.1847-\sigma_e>0$ and is increasing in $z$.

In all ranges $F\ge0$, proving \eqref{eq:battle}, hence \eqref{eq:reducedA},
hence the lemma.
\end{proof}

\begin{remark}[Independent second proof of the pinch]\label{rmk:zoneA-alt}
The pinch was proved twice, independently, in this project.  The proof above
(hypotheses $e\le\tfrac1{60}$, $\xi\ge1$, all $T$) is the one we adopt.  A
second derivation (working note \texttt{claude\_jul11/proof\_Z1.md}) proves the same
$\lambda=1$ certificate on $\{T\le\tfrac12\}$ with \emph{no} $\xi$-restriction,
via a normalized ``ledger'' inequality, elimination of $m$ through
$Y=\sqrt{m^2e}$, and negative-discriminant quadratic certificates, with all
constants certified in exact rational arithmetic.  The two proofs overlap on
$\{e\le\tfrac1{60},\xi\ge1,T\le\tfrac12\}$ and their positivity thresholds
agree ($\kappa m>0.93$ here, $\kappa m>0.8787$ there, empirically
$\kappa m\gtrsim1.04$); both show $\{\xi\le1,\ T\le\tfrac12,\ R_m>0\}=\emptyset$.
\end{remark}
...
High-precision scans (\S\ref{sec:verification}) confirm the minimum relative
margin of the exact inequality over $\mathcal D$ behaves as $\approx2.60/m$:
the inequality is asymptotically tight along $m\to\infty$, so no proof with a
uniform relative constant is possible --- Range 1 above is forced to win by
exactly the $O(\sqrt e)$ and $O(1/m)$ amounts it does.
\end{remark}

\section{Zone B: $e\ge1/60$, $\xi\ge1$}\label{sec:zoneB}
...
$s\mapsto s(s+A+1)$ is increasing, so $s_*\le14-A$ iff
$(14-A)\cdot15\le(A+1)/\lambda$ --- note the direction: $s_*\le14-A$ iff
$(14-A)(15)\ge(A+1)/\lambda$, i.e.\ $15(14-A)\lambda\ge A+1$.  (iv): from
...
\ge\tfrac14(1-e^{-14u})\ge\underline\rho$, where
$\frac{\sqrt\alpha}{2\sqrt2f(1+x)}\ge\frac1{2\sqrt2\sqrt\alpha\,(1+x)}
\ge\tfrac14$ and $u\ge\frac{2(1+\kappa)e}{1+3e}\ge1.2407(1+\kappa)e$);
...
\section{Zone C: $\xi\le1$}\label{sec:zoneC}

\begin{lemma}[Zone C]\label{lem:zoneC}
Let $(q,\alpha,m)\in\mathcal D$ with $\xi\le1$ and $R_m>0$.  Then
$R_m\le C_m\psi(\xi,\rho)$; more precisely the certificate
$\lambda=\min(1,2\rho\xi)$ pays:
\[
        2\rho\xi\le1:\quad R_m\le2\alpha^3A_m\kappa^2\cdot\frac{1+4\xi}{1+2\xi},
        \qquad\qquad
        2\rho\xi>1:\quad R_m\le\sqrt{2\alpha}\,B_mfd\cdot\frac{4\xi+1}{4\xi+2}.
\]
\end{lemma}
...
\begin{proof}
Lemma~\ref{lem:posA} applies (its proof used only $e\le\tfrac1{60}$), so
$\kappa(m-1)>x(1+\kappa)-\sigma_e$ and $\kappa\ge0.93/(m-1)$.  Since
$\kappa\le\kappa_\xi=e/(1-e)^2$,
\begin{equation}\label{eq:C-mT}
        m-1\ \ge\ \frac{0.93(1-e)^2}{e}\ \ \Rightarrow\ \
        m\ge55,
        \qquad
        T=me\ >\ (m-1)e\ \ge\ 0.93(1-e)^2\ \ge\ 0.899 .
\end{equation}

\emph{The case $2\rho\xi>1$ is empty here.}  Since
$k_m(\alpha)\le k_m(L)$,
\[
        \frac{A_m}{B_m}
        \ \le\ \frac{2L^{m-2}}{m\,k_m(L)}+\frac{k_m(\alpha)}{k_m(L)}
        \ \le\ \frac{2y^{m-2}(1+y)}{m\,(1-y^{m-1})}+\frac{p+L}{p+\alpha}
        \ \le\ 10^{-38}+\frac{1+y}{1+x}\ \le\ 0.6117,
\]
using $k_m(L)=p^{m-1}(1-y^{m-1})/(p+L)$, the bound
$\frac{k_m(\alpha)}{k_m(L)}=\frac{(p^{m-1}-\alpha^{m-1})(p+L)}
{(p^{m-1}-L^{m-1})(p+\alpha)}\le\frac{p+L}{p+\alpha}$ (as $\alpha\ge L$),
then $y\le\ell\le0.1843$, $x\ge0.9365$ \eqref{eq:standing} and $m\ge55$
\eqref{eq:C-mT}.  Also
$\frac{\sqrt\alpha}{2\sqrt2f}\le\frac1{2\sqrt2\cdot0.8157\sqrt\alpha}
\le0.6182$ ($f=\alpha(1-\ell)\ge0.8157\alpha$,
$\alpha\ge\tfrac{59}{120}$).  Hence
$\rho\le0.6117\cdot0.6182\le0.3782$, so $2\rho\xi\le0.7564\,\xi<1$.

\emph{Case $2\rho\xi\le1$ (the whole zone).}  We show
$R_m\le2\alpha^3A_m\kappa^2$, which suffices since $(1+4\xi)/(1+2\xi)\ge1$.
Use $A_m\ge m\,k_m(\alpha)=m\,p^{m-2}\,(1-x^{m-1})/(1+x)$ and, by
\eqref{eq:C-mT},
\[
        (m-1)u\ \ge\ 1.9048\,(m-1)e\ \ge\ 1.9048\cdot0.899\ \ge\ 1.712,
        \qquad
        1-x^{m-1}\ \ge\ 1-e^{-1.712}\ \ge\ 0.8142 .
\]
Writing $t:=(m-1)\ln(1/\tau)-\ln(1/x)$, the defect form
\eqref{eq:defect-form} gives $\alpha-p\tau^{m-1}=\alpha(1-e^{-t})$, so
\[
        R_m\ \le\ \alpha^m\min(1,t_+)+L^m,
        \qquad
        t\ \le\ (m-1)\frac dq-u\ \le\ e\,(2.0351\,z-1.9048),
        \quad z:=\kappa m,
\]
using $\ln(1/\tau)\le\frac{1-\tau}\tau=\frac dq$, $\ln(1/x)\ge u\ge1.9048e$,
$\frac1q\le\frac1{\alpha-\kappa_\xi e}\le2.0351$, and $(m-1)d\le\kappa me=ze$.
Dividing by $\alpha^3p^{m-2}$ and using
$\alpha^m/(\alpha^3p^{m-2})=x^{m-2}/\alpha$ and
$L^m/(\alpha^3p^{m-2})=\ell^2y^{m-2}/\alpha$, the target
$R_m\le2\alpha^3A_m\kappa^2$ is implied by
\[
        x^{m-2}\min(1,t_+)+\ell^2y^{m-2}
        \ \le\ 0.8142\,\alpha\,\kappa^2m ,
\]
and $0.8142\,\alpha\ge0.40$ ($\alpha\ge0.4917$).  The $L$-term is
negligible: $\ell^2,y^2\le2.04e\le0.034$ and $m\ge55$ give
$\ell^2y^{m-2}\le0.034^{27.5}$, while
$\kappa^2m\ge(\kappa m)^2/m\ge0.865/m$, and
$m\,(0.034)^{(m-2)/2}$ is decreasing in $m$ with value $<10^{-37}$ at
$m=55$; so $\ell^2y^{m-2}\le10^{-37}\kappa^2m$, and it suffices to prove
\begin{equation}\label{eq:C-battle}
        x^{m-2}\min(1,t_+)\ \le\ 0.39\,\kappa^2m .
\end{equation}
Since $(m-2)u\ge1.9048\,\tfrac{m-2}{m}\,T\ge1.8355\,T$ ($m\ge55$), we have
$x^{m-2}\le e^{-1.8355T}$; and $\kappa^2m=z^2e/T$.

\emph{If $t\le1$:} using $t\le e(2.0351z-1.9048)_+$ and
$\frac{(2.0351z-1.9048)_+}{z^2}\le\frac{2.0351^2}{4\cdot1.9048}\le0.5436$
(vertex of the parabola $z\mapsto z^2\cdot0.5436-2.0351z+1.9048$),
\eqref{eq:C-battle} follows from
\[
        e^{-1.8355T}\cdot0.5436\,z^2e\ \le\ 0.39\,\frac{z^2e}{T}
        \quad\Longleftarrow\quad
        Te^{-1.8355T}\le0.717,
\]
which holds with room to spare:
$\max_{T>0}Te^{-1.8355T}=\tfrac1{1.8355\,\mathrm e}\le0.2004$.

\emph{If $t>1$:} then $e(2.0351z-1.9048)>1$ forces
$z\ge\frac{1/e-1.9048}{2.0351}\ge\frac{0.4753}{e}$ (using $e\le\tfrac1{60}$).
Then $0.39\,z^2e/T\ge0.39\cdot(0.4753)^2/(eT)=0.0881/(eT)$, so
\eqref{eq:C-battle} follows from $eTe^{-1.8355T}\le0.0881$, and indeed
$eTe^{-1.8355T}\le\tfrac1{60}\cdot0.2004\le0.0034$.
Thus \eqref{eq:C-battle} holds in both cases, completing Case
$2\rho\xi\le1$.
\end{proof}

\subsection{Moderate $e$ and the Tur\'an corner}\label{subsec:zoneC-mod}
...
\begin{proof}
(i) Divide $R_m=\alpha^m+L^m-pq^{m-1}$ by $\alpha^3p^{m-2}$ and use
$\alpha^m/(\alpha^3p^{m-2})=x^{m-2}/\alpha$,
$L^m/(\alpha^3p^{m-2})=\ell^2y^{m-2}/\alpha$,
$pq^{m-1}/(\alpha^3p^{m-2})=(pq/\alpha^3)s^{m-2}$; the coefficient identity is
$pq-\alpha^2=L^2$.  (ii) From (i),
$x^{m-2}>s^{m-2}+\ell^2s^{m-2}(1-(y/s)^{m-2})
\ge s^{m-2}(1+G_2)$ (using $m-2\ge13$ and $y/s<1$), so
$(m-2)\ln(x/s)>\ln(1+G_2)$; and $\ln(x/s)=\ln(\alpha/q)\le\frac{\alpha-q}q=\frac dq$.
...
$m\ge M_+:=2+\lceil q(G_2-G_2^2/2)/d\rceil$ (the secant gate with
$\ln(1+t)\ge t-t^2/2$), and all sufficiently large $m$ pass at once because
...
$\kappa\in(0,1]$: $3001$ boxes verified (of which $5$ by the analytic
...
largest explicitly checked cycle length $m=1716$.
\end{proof}

\subsection{The Tur\'an-corner sliver}

\begin{lemma}[Tur\'an corner]\label{lem:turan}
Lemma~\ref{lem:zoneC} holds when $\delta=\tfrac{1-3e}6<\tfrac1{2000}$
(equivalently $e>\tfrac13-\tfrac1{1000}$).
\end{lemma}

\begin{proof}
Here $0<d<\delta<\tfrac1{2000}$, $\alpha=\tfrac13+\delta$,
$e=\tfrac13-2\delta$, $q=\tfrac13+(\delta-d)$, $p=\tfrac23-(\delta-d)$, and
$\xi\le4.01\,d\le1$, so we are indeed in Zone C.  We record the standing
bounds (each a one-line consequence of $\delta\le\tfrac1{2000}$, $d<\delta$;
all are re-verified numerically in the companion checker):
\begin{gather}
        \tfrac12\le x\le\tfrac12+2.26\delta\le0.50113,\qquad
        x^{14}\le6.4\times10^{-5},\qquad y<\tfrac12,\notag\\
        q^2-L^2=3\alpha\delta-(2\alpha-e)d+2d^2\ \in\
        [\,0.66\,\delta,\ 1.003\,\delta\,],\qquad
        0.666\le q+L\le0.667,\notag\\
        f=\frac{3\alpha\delta+d(d+e)}{\alpha+L}\ \in\
        [\,1.498\,\delta,\ 2.004\,\delta\,],\qquad
        1-\ell=\frac f\alpha\le6.1\,\delta .\label{eq:corner-standing}
\end{gather}

\emph{Step 1 (forcing).}  $1-\tfrac ys=\tfrac{q-L}q\ge\frac{0.996\,\delta}{0.334}
\ge2.98\,\delta$ and $1-\tfrac ys\le4.52\,\delta$, so by
$1-(1-t)^{13}\ge13t-78t^2$ and $\ell^2\ge1-12.2\,\delta$,
\[
        G_2\ \ge\ (1-12.2\delta)\bigl(13\cdot2.98\delta-78\,(4.52\delta)^2\bigr)
        \ \ge\ 37\,\delta .
\]
By Lemma~\ref{lem:threegeo}(ii) and $\tfrac1q\le3$, $\ln(1+G_2)\ge
G_2(1-G_2/2)\ge36.9\,\delta$, hence
\begin{equation}\label{eq:corner-forcing}
        R_m>0\ \Longrightarrow\ (m-2)\,d\ >\ 12.3\,\delta .
\end{equation}

\emph{Step 2 (defect).}  By the mean value theorem on $[s,x]$ and $[y,s]$
($y=\ell x$),
\[
        \frac{\alpha R_m}{\alpha^3 p^{m-2}}
        \le(m-2)(x-s)x^{m-3}-\ell^2(m-2)(s-y)y^{m-3}
        =(m-2)\,x^{m-3}\,\frac Np,
\]
where $N:=d-(q^2-L^2)\,\ell^{m-1}/(q+L)$.  Insert the exact
$q^2-L^2$ from \eqref{eq:corner-standing}, drop $2d^2\ell^{m-1}/(q+L)\ge0$,
use $\ell^{m-1}\le1$ on the $d$-term and
$\ell^{m-1}\ge1-6.1(m-1)\delta$ on the $\delta$-term:
\[
        N\ \le\ d\Bigl(1+\frac{2\alpha-e}{q+L}\Bigr)
        -\frac{3\alpha\delta}{q+L}
        +\frac{3\alpha\,\delta\cdot6.1(m-1)\delta}{q+L}.
\]
Now the coefficient identity
$1+\frac{2\alpha-e}{q+L}-\frac{3\alpha}{q+L}=\frac{L-e-d}{q+L}$ and
$L-e\le(\tfrac13-\tfrac\delta2)-(\tfrac13-2\delta)=1.5\,\delta$ give
\[
        N\ \le\
        \Bigl(1+\frac{2\alpha-e}{q+L}\Bigr)(d-\delta)
        +\delta\cdot\frac{1.5\,\delta}{q+L}
        +9.2\,m\,\delta^2
        \ \le\ 10\,m\,\delta^2 ,
\]
since $d\le\delta$ kills the first term and $m\ge15$.  Hence, with
$\tfrac1{p\alpha}\le4.52$,
\begin{equation}\label{eq:corner-defect}
        \frac{R_m}{\alpha^3p^{m-2}}\ \le\ 45.2\,m\,(m-2)\,\delta^2\,x^{m-3}.
\end{equation}

\emph{Step 3 (payments).}  From Lemma~\ref{lem:threegeo}(iii) and
\eqref{eq:corner-standing} ($\kappa=d/e\ge3d$, $\sqrt{2\alpha}\ge0.816$,
$\alpha^3\le0.0372$, $\tfrac{1+4\xi}{1+2\xi}\ge1$,
$\tfrac{4\xi+1}{4\xi+2}\ge\tfrac12$):
\[
        2\rho_{\mathrm{lo}}\xi\le1:\quad
        \text{payment}\ \ge\ 7.9\,d^2\,m\cdot\alpha^3p^{m-2};
        \qquad
        2\rho_{\mathrm{lo}}\xi>1:\quad
        \text{payment}\ \ge\ 10.9\,\delta d\,m\cdot\alpha^3p^{m-2}.
\]

\emph{Step 4 (battles).}  Suppose $R_m>0$; insert the forcing
\eqref{eq:corner-forcing} into each payment and compare with
\eqref{eq:corner-defect}:
\[
        7.9\,d^2m\ \ge\ \frac{7.9\cdot12.3^2\,\delta^2m}{(m-2)^2}:
        \qquad
        45.2\,(m-2)^3x^{m-3}\ \le\ 45.2\cdot13^3\cdot0.50113^{12}\le25.1
        \ \le\ 7.9\cdot12.3^2=1195\,;
\]
\[
        10.9\,\delta dm\ \ge\ \frac{10.9\cdot12.3\,\delta^2m}{m-2}:
        \qquad
        45.2\,(m-2)^2x^{m-3}\ \le\ 45.2\cdot169\cdot0.50113^{12}\le1.92
        \ \le\ 10.9\cdot12.3=134\,,
\]
where both left sides are maximal at $m=15$
(the ratio of consecutive values is $<x\cdot(\tfrac{m-1}{m-2})^3<1$ for
$m\ge15$).  In both cases the payment exceeds the defect for every $m\ge15$,
whichever case of the $\rho_{\mathrm{lo}}$-gate holds pointwise.
\end{proof}
...
$m\le13$ the conjecture is proved in \texttt{paper\_new.tex} at all densities.
Let $m\ge15$.  If $\lambda_{\max}(A)\le q$, $W$ is positive-spectrum-safe and
the bound holds (\texttt{paper\_new.tex}, \S ``General structural results'').
...
$p\ge\tfrac23$ this is the high-density theorem of \texttt{paper\_new.tex}.
...
\item \emph{Analytic.}  The reduction (\S\ref{sec:reduction}), the
coordinates and defect identities (\S\ref{sec:coords}), Zone A
(\S\ref{sec:zoneA}), the $m$-maximization Lemma~\ref{lem:Kmax}, the Zone-C
small-$e$ lemma, and the assembly (\S\ref{sec:assembly}) are complete
pencil-and-paper proofs.  Every displayed numerical constant is verified to
its stated inequality inside the accompanying checker.
...
$3001$ boxes with a per-box finite $m$-scan plus gate and tail lemmas); no floating point enters the certified
...
\texttt{regionII\_scalar\_checker.py} re-verifies: (a) all algebraic
identities of the paper in exact rational/symbolic arithmetic; (b) every
inequality step of every zone chain pointwise on dense adversarial grids
(mpmath, $60$ digits), reporting the worst slack of each step; (c) a global
scan of the target $R_m\le C_m\psi$ and of both dual certificates.  In the
July 11 runs: no step of any chain was violated; the global scan (together
with the earlier cartography sweep of $68{,}564$ admissible points, $m$ up
to $10^7$, adaptive precision to $400$ digits) found no violation of the
target anywhere, with minimum relative margin $4.6\times10^{-6}$, attained
in the pinch corner, and the minimum-margin curve fitting
$\min\text{-margin}(m)\approx2.60/m$ --- consistent with
Remark~\ref{rmk:zoneA-tight}: the inequality is asymptotically tight, so
the zone proofs above are essentially forced to be as sharp as they are.
\end{enumerate}

\subsection*{How to audit}

\begin{enumerate}[label=\textup{(\alph*)}]
\item Read \S\ref{sec:reduction} against the baton note
(\texttt{gptpro\_jul11/regionII\_baton\_note.tex}); the three repairs are
Remarks~\ref{rmk:budget-sign}, \ref{rmk:knegative} and the $K=0$ case in
