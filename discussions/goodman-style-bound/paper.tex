...
We also record two complementary lines of \emph{general} results, valid for every odd cycle at once.  The first is operator-theoretic: writing $A$ for the compression of $T_{1-W}$ to the mean-zero subspace, a hub-coupling positivity lemma yields the universal lower bound $t(C_m,W)\ge p^m-\Tr(A^m)$, hence the conjecture whenever $\lambda_{\max}(A)\le1-p$ (the \emph{positive-spectrum-safe} criterion, which subsumes all $p$-regular graphons).  Beyond this regime we prove the conjecture for three families of complements: rank-one $1-W=u\otimes u$, equal disjoint clique unions, and the two-value (Razborov) complete multipartite family.  The second is variational: reframing the bound as a constrained minimisation $f_{C_m}(p)=\min\{t(C_m,W):t(K_2,W)=p\}$, we show $g_m$ is the ``Goodman envelope'' (the minimiser scallops strictly above it between Tur\'an points), the Tur\'an graphon is a strict local minimiser, and --- correcting a natural guess --- near $p=1/2$ the minimiser is near-bipartite rather than multipartite, which is why the residual $C_{13}$ interval resists the triangle-density methods.
...
The following statements are established in this note; all are rigorous (computer-assisted where noted) except the explicitly numerical observations in \textup{(viii)}.
\begin{enumerate}[label=\textup{(\roman*)}]
    ...
    \item For $C_{13}$, Conjecture~\ref{conj:main} holds for all graphons with $p\ge519/1000$ (path-certificate), for $1/2<p\le1/2+\delta_{13}$ with $\delta_{13}=0.0155929\ldots$ (spectral; a clean rational version covers $p\le51/100$), and trivially for $p\le1/2$; the residual interval $(1/2+\delta_{13},\,519/1000)$, of width $\approx0.0034$, is open.  A four-route closure attempt (\S\ref{subsec:C13-attempt}) provably falls short on this interval, and locates the minimiser there as near-bipartite (\S\ref{subsec:C13-minimiser}).  See Section~\ref{sec:C13}.
    ...
    \item \textup{(All-$k$ families.)}  Conjecture~\ref{conj:main} holds for every odd cycle when $1-W=u\otimes u$ is rank one (\S\ref{subsec:rank-one}), when $1-W$ is an \emph{equal} disjoint union of clique blocks (\S\ref{subsec:equal-clique}), and on the \emph{two-value} (Razborov) complete multipartite family --- two equal parts and one different part --- by Theorem~\ref{thm:two-value} (\S\ref{subsec:multipartite}).
...
The residual interval \eqref{eq:C13-residual} sits next to the triangle-density extremal problem, so it is natural to verify the triangle extremal family directly.  For $p\in[1/2,2/3]$ the Razborov--Reiher triangle minimiser is the complete tripartite graphon $W_c$ with parts $(c,b,b)$, $b=\tfrac{1-c}2$, $p=\tfrac12+c-\tfrac32c^2$, $c\in[0,1/3]$.
...
This is exactly the $r=2$, $m=13$ instance of the two-value multipartite Theorem~\ref{thm:two-value} of \S\ref{sec:variational}: $W_c=(c,b,b)$ is the two-value family with two equal parts $b$ and one part $c$.  The margin is small in the residual interval --- $t(C_{13},W_c)-g_{13}\approx1.9\times10^{-5}$ near $p=0.515$ --- a number that will matter in \S\ref{subsec:C13-attempt}.

\subsection{The residual interval: a closure attempt and the precise obstruction}\label{subsec:C13-attempt}

A dedicated attempt to close \eqref{eq:C13-residual} by four independent rigorous routes did \emph{not} succeed; each provably falls short, and the failure pins down the obstruction.  We record it because several byproducts are sharp and reusable.  (All claims below are checked in \texttt{odd\_cycle\_c13\_closure\_checker.py}.)

\emph{(a) The spectral window has a hard ceiling.}  The window of the triangle-density spectral method is exactly $\delta_{13}=0.0155929\ldots$, even after adjoining the support bound $\lambda_i\in[-\tfrac12,\tfrac12]$ (Lemma~\ref{lem:lambda-min} below) and the Sidorenko bound $t(C_4,W)\ge p^4$: at the binding configuration the relevant atom sits at $-\alpha_0$ with $\alpha_0=(pq^{12})^{1/13}$ strictly inside $(-\tfrac12,\tfrac12)$, so the support bound is slack.  The deficit at the path cut-off is genuine ($\Theta(p)-(p^3-\alpha_0^3+\Delta_0^{3/2})\approx-2.6\times10^{-4}$ at $\eps=0.019$).

\emph{(b) The path certificate provably fails inside the interval.}  The defect polynomial $\Phi_{13}$ of \S\ref{subsec:C13-path} is not nonnegative on the realisable region: the exact rational two-block witness with \emph{unequal} part measures $(39/100,\,61/100)$ and complement values $U=\bigl(\begin{smallmatrix}43/50&1/10\\1/10&41/50\end{smallmatrix}\bigr)$ has $q=120877/250000=0.483508$ (inside the interval) and $\Phi_{13}=-2.0758\times10^{-7}<0$, while the true defect $t(C_{13},W)-g_{13}=+1.105\times10^{-4}>0$ lives entirely in the discarded deletion term $x_{12}-c_{13}$.  Hence the path range cannot be pushed below $\rho_{13}=519/1000$ \emph{without retaining} the deletion gap.
...
\emph{(d) The minimiser is not the triangle-extremal family.}  As shown in \S\ref{subsec:C13-minimiser}, the actual $C_{13}$ minimiser in the residual interval is \emph{near-bipartite}, not the tripartite $W_c$; in particular $W_c$ is not even a stationary point of $t(C_{13},\cdot)$ there.  So a stability transfer anchored at the triangle-extremal family is geometrically misaimed.
...
\begin{proof}
Write $S=2W-1$, so $T_W=\tfrac12(J+T_S)$ with $J$ the rank-one averaging projection.  Since $|S|\le1$, Schur's test gives $\norm{T_S}\le1$, hence $T_W\succeq\tfrac12(J-I)\succeq-\tfrac12 I$.  The non-principal eigenvalues are eigenvalues of the compression $-A$ of $T_W$ to $\one^\perp$, and $\norm A\le\tfrac12$ by Lemma~\ref{lem:compression}.
\end{proof}

The common thread of \textup{(a)--(d)} is the \emph{aggregate-not-modewise} bottleneck of \S\ref{subsec:bottleneck}: every certificate built from marginal moments of the spectral measure of $A$ (plus the deletion bound) is defeated by an unrealisable spectral configuration that the $g$--$A$ coupling forbids but no marginal-moment constraint excludes.  The residual $C_{13}$ interval is thus a faithful, narrow instance of the general obstruction, and the most promising route is a \emph{deletion-retaining} certificate (lower-bound $x_{12}-c_{13}$ in the moment variables and add it back to $\Phi_{13}$, using the one-frontier-mode reduction).
...
The complete multipartite graphons $K(w)$ (parts $w=(w_1,\dots,w_k)$, $\sum w_i=1$; $W=1$ across parts, $0$ within) form the natural candidate family for $p$ bounded away from $1/2$, and contain the Tur\'an graphons.  (Near $p=1/2$ the true minimiser is instead near-bipartite, not multipartite; see \S\ref{subsec:C13-minimiser}.  The results of this subsection are statements about the multipartite \emph{family}, independent of whether it contains the global minimiser.)  Writing $s=\sqrt w$ and $D=\operatorname{diag}(w)$,
...
For every odd $m\ge3$ and $r\ge2$, the Tur\'an graphon $T_r$ is a strict local minimiser of $t(C_m,\cdot)$ at fixed $p=1-1/r$: the first variation along any nonzero edge-preserving admissible direction is strictly positive, with symmetric-direction coefficient $m/r^{m-1}>0$.
\end{proposition}
...
\subsection{The minimiser is not always multipartite: near $p=1/2$ it is near-bipartite}\label{subsec:C13-minimiser}

The complete multipartite picture of \S\ref{subsec:multipartite} is the high-density story.  Near $p=1/2$ it \emph{fails}: the global minimiser is near-bipartite, not multipartite, and in particular the triangle-extremal family is not stationary.
...
This is an exact computation (\texttt{c13\_extremal\_structure\_checker.py}); the KKT-violating direction ``add $b$--$b$ within-part edges, remove $c$--$b$ cross edges'' strictly decreases $t(C_{13})$.  Tracking it down, the actual minimiser in the residual interval \eqref{eq:C13-residual} is, to high accuracy and stably across $4,5,6$ blocks, a \emph{near-bipartite two-block graphon}: two parts of measure $\approx\tfrac12$, cross density $1$, one part carrying a small internal fill $\approx0.06$--$0.07$ and the other $\approx0$.  Its margin $t(C_{13})-g_{13}\approx4.7\times10^{-6}$ lies \emph{below} the triangle-extremal margin $\approx1.9\times10^{-5}$ of Theorem~\ref{thm:C13-tripartite}.

Two consequences.  First, near $p=1/2$ the odd-cycle minimiser is governed by the \emph{bipartite} extremal structure (complete bipartite has no odd cycles), not the tripartite triangle-extremal one --- so a stability transfer for $C_{13}$ anchored at the triangle-density extremal family is geometrically misaimed.  Second, the numerical claim that the $C_m$ minimiser is complete multipartite is \emph{false} near $p=1/2$: the true minimiser has fractional within-block structure.  Theorem~\ref{thm:two-value} remains valid as a statement about the multipartite \emph{family}, but it does not bound the (non-multipartite) global minimiser there.  The right local object near $p=1/2$ is the near-bipartite two-block family, anchored by Lemma~\ref{lem:lambda-min} ($\lambda_{\min}(T_W)\ge-\tfrac12$, with near-equality forcing near-bipartiteness); reducing the residual interval to this two-block family is the concrete next step.

\section{What remains}\label{sec:remains}

The full conjecture for $C_{2k+1}$ with $k\ge6$ remains open.  The obstruction can be stated either per-cycle (the hybrid of \S\ref{sec:C5}--\S\ref{sec:C13}) or uniformly in $m$ (the spectral bottleneck of \S\ref{sec:general}, or equivalently the extremal-structure problem of \S\ref{sec:variational}).  We collect what each viewpoint says.

\medskip\noindent\emph{The immediate gap: $C_{13}$.}
Only the residual interval $(1/2+\delta_{13},\,519/1000)$ of width $\approx0.0034$ separates the proved cases from $C_{13}$ (\S\ref{subsec:C13-gap}).  The obstruction is quantitative, but it is \emph{not} closed by sharpening either half of the hybrid: the closure attempt of \S\ref{subsec:C13-attempt} shows the spectral window has the hard ceiling $\delta_{13}=0.0155929<0.019$, and the path certificate provably fails inside the interval (an exact rational witness with $\Phi_{13}<0$).  The minimiser there is near-bipartite, not multipartite (\S\ref{subsec:C13-minimiser}), and the obstruction is the same coupling bottleneck as the general case, localised to a single frontier eigenvalue.  The two concrete routes that survive are: \textup{(a)} a \emph{deletion-retaining} certificate --- lower-bound the discarded gap $x_{12}-c_{13}$ in the moment variables $s_0,\dots,s_{10}$ and add it back to $\Phi_{13}$, then seek a sum-of-squares certificate using the one-frontier-mode reduction of \S\ref{subsec:C13-attempt}\textup{(c)}; or \textup{(b)} a near-bipartite reduction --- prove the residual-interval minimiser is the two-block near-bipartite family of \S\ref{subsec:C13-minimiser} (anchored by $\lambda_{\min}(T_W)\ge-\tfrac12$), reducing the interval to a low-dimensional inequality.
