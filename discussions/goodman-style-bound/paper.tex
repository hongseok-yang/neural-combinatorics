...
\begin{enumerate}[label=\textup{(\roman*)}]
    ...
    \item For $C_{13}$, Conjecture~\ref{conj:main} holds for all graphons with $p\ge519/1000$ (path-certificate), for $1/2<p\le51/100$ by a fully rational spectral certificate, and trivially for $p\le1/2$.  Numerically, the same spectral inequality appears to hold up to $1/2+\delta_{13}$ with $\delta_{13}=0.0155929\ldots$, leaving the narrower interval $(1/2+\delta_{13},\,519/1000)$, of width $\approx0.0034$, as the observed residual target.  A four-route closure attempt (\S\ref{subsec:C13-attempt}) falls short on this interval, and numerical optimisation locates the minimiser there as near-bipartite (\S\ref{subsec:C13-minimiser}).  See Section~\ref{sec:C13}.
...
For $C_{13}$ the path-certificate half of the hybrid still goes through and yields a rigorous, Bernstein-certified theorem on $p\ge519/1000$.  The triangle-density spectral half has a clean exact certificate on $1/2<p\le51/100$; a high-precision scalar computation indicates that the same argument should extend to $1/2<p\le1/2+\delta_{13}$ with $\delta_{13}\approx0.01559$, still short of the path-certificate cut-off.  Thus the fully certified arguments leave the interval $(51/100,519/1000)$, and the numerically indicated frontier leaves a much shorter residual interval.  This is the first cycle length at which the two halves of the hybrid fail to meet.  Along the way we prove a closed-form formula for the linear-part polynomial $P_q^{(m)}$ that holds for every odd $m\ge5$.
...
The exact rational version in Remark~\ref{rmk:C13-rational} below rigorously covers $1/2<p\le51/100$.  Since $1/2+\delta_{13}<519/1000$, the numerically indicated spectral frontier and the path-certificate range leave the residual interval
...
\end{equation}
as the narrow target not reached by the present hybrid.  Without the numerical endpoint, the rigorously certified gap is the larger interval $(51/100,519/1000)$.
...
\emph{(d) The minimiser is not the triangle-extremal family.}  As shown in \S\ref{subsec:C13-minimiser}, numerical optimisation in the residual interval finds a \emph{near-bipartite} minimiser, not the tripartite $W_c$; in particular $W_c$ is not even a stationary point of $t(C_{13},\cdot)$ there.  So a stability transfer anchored at the triangle-extremal family is geometrically misaimed.
...
The exact certificates in this note leave the interval $(51/100,\,519/1000)$; the sharper numerical frontier of the same spectral method leaves the shorter target interval $(1/2+\delta_{13},\,519/1000)$ of width $\approx0.0034$ (\S\ref{subsec:C13-gap}).  The obstruction is quantitative, but it is \emph{not} closed by sharpening either half of the hybrid: the closure attempt of \S\ref{subsec:C13-attempt} indicates the spectral window has a ceiling at $\delta_{13}=0.0155929<0.019$, and the path certificate provably fails inside the interval (an exact rational witness with $\Phi_{13}<0$).  The best numerical minimiser there is near-bipartite, not multipartite (\S\ref{subsec:C13-minimiser}), and the obstruction is the same coupling bottleneck as the general case, localised to a single frontier eigenvalue.  The two concrete routes that survive are: \textup{(a)} a \emph{deletion-retaining} certificate --- lower-bound the discarded gap $x_{12}-c_{13}$ in the moment variables $s_0,\dots,s_{10}$ and add it back to $\Phi_{13}$, then seek a sum-of-squares certificate using the one-frontier-mode reduction of \S\ref{subsec:C13-attempt}\textup{(c)}; or \textup{(b)} a near-bipartite reduction --- prove the residual-interval minimiser is the two-block near-bipartite family of \S\ref{subsec:C13-minimiser} (anchored by $\lambda_{\min}(T_W)\ge-\tfrac12$), reducing the interval to a low-dimensional inequality.
