...
\large Verified results for $C_3$, $C_5$, $C_7$, $C_9$, $C_{11}$, and regular graphons}
\author{Working note}
\date{May 29--30, 2026}
...
\begin{enumerate}[label=\textup{(\roman*)}]
    ...
    \item Conjecture~\ref{conj:main} holds for $C_{11}$ for all graphons.
\end{enumerate}
\end{theorem}

\begin{remark}[Scope]
This note does \emph{not} prove the conjecture for $C_{2k+1}$ with $k\ge6$.  The $C_9$ and $C_{11}$ proofs are hybrids: the complement path-certificate method handles $p\ge \rho_m$ (with $\rho_9=1003/2000$ and $\rho_{11}=103/200$), and the narrow remaining interval $1/2<p\le\rho_m$ is closed by a separate argument combining the sharp triangle-density theorem with a spectral moment inequality.  The two proofs are given in Sections~\ref{sec:C9} and~\ref{sec:C11}.  Neither half of either hybrid is known to extend to $C_{13}$ and beyond; see Section~\ref{sec:remains}.
\end{remark}
...
The full conjecture for $C_{2k+1}$ with $k\ge6$ remains open.  The hybrid strategy that closed $C_9$ and $C_{11}$ has two halves, and neither is known to extend to $C_{13}$ without new input.

\medskip\noindent\emph{The path-certificate half.}
The proofs for $C_5$, $C_7$, the $p\ge1003/2000$ range of $C_9$, and the $p\ge103/200$ range of $C_{11}$ all use the crude estimate
\[
        t(C_{2k+1},U)\le t(P_{2k},U),
\]
obtained by deleting one factor $U\le1$ from the cycle integrand.  The resulting defect $\Phi_{2k+1}$ is positive on $0\le q\le 1-\rho_{2k+1}$ for some cut-off $\rho_{2k+1}<1$, but \emph{not} all the way to $q=1/2$.  The cut-offs have so far stayed close to $1/2$ ($\rho_9=1003/2000$, $\rho_{11}=103/200$) but with no qualitative bound forcing this, and for $C_{13}$ and beyond the cut-off may move appreciably further from $1/2$.  A natural next target is a refined cycle-to-path inequality of the form
\[
...
The closure arguments in \S\ref{subsec:C9-gap} and \S\ref{subsec:C11-gap} both use the sharp triangle-density theorem to bound the negative $(2k+1)$-th-power mass $N_{2k+1}=\sum_{\lambda_i<0}|\lambda_i|^{2k+1}$ in terms of $\sum_{i\ge1}\lambda_i^3=t(K_3,W)-\lambda_0^3$, reducing to the scalar inequality $\Theta(p)-\ell^3\ge(S-\alpha^2)^{3/2}-\alpha^3$.  Two countervailing trends are at play as $k$ grows: the target slack $pq^{2k}$ shrinks, and the interval that the spectral closure needs to cover widens (width $3/2000$ for $C_9$, width $3/200$ for $C_{11}$, since the path-certificate cut-off $\rho_{2k+1}$ has moved further from $1/2$).  Both put pressure on the comparison.  The final scalar comparison already required a non-linear analysis for $C_{11}$ (the $t=\sqrt\eps$ substitution); for $C_{13}$ the combination may be too tight for any triangle-density input alone to close, in which case a colour-critical supersaturation theorem tailored to odd cycles, or a higher clique-density input, will be needed.

\medskip\noindent\emph{The overlap question.}
The conjecture is established for $C_m$ exactly when the path-certificate range $p\ge\rho_m$ and the triangle-density range $1/2<p\le\rho_m$ meet at the cut-off $\rho_m$.  They do for $C_9$ and $C_{11}$.  Whether they continue to overlap for $C_{13}$ is the key open question.
