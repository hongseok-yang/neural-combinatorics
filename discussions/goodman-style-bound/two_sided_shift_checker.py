#!/usr/bin/env python3
"""Exact checker for two_sided_shift_note.tex.

All checks are in exact rational arithmetic (fractions.Fraction) unless noted.
...
def main():
    ...
    ap.add_argument("--max-m", type=int, default=43)
    ap.add_argument("--strip", action="store_true",
                    help="run the strip-criterion interval verification")
    ap.add_argument("--strip-max-m", type=int, default=61)
    ap.add_argument("--q0-2-5", action="store_true",
                    help="run the Bernstein certificates on [0,2/5] as well")
    args = ap.parse_args()
    ...
    check6_certificates(args.max_m)
    if args.q0_2_5:
        check10_certificates_q0(args.max_m)
    if args.strip:
        check9_strip(args.strip_max_m)
