#!/usr/bin/env python3
"""Verification companion to regionII_corrected_solution.tex.

Checks for the corrected Region-II scalar Huber proof.
...
Notation follows regionII_corrected_solution.tex:
...
def exact_checks() -> None:
    ...
    # Directed constant checks used in the corrected analytic zones.
    # All terminating decimals are interpreted as the corresponding rationals.
    assert F(120, 59) < F(2034, 1000)
    assert F(1, 1) / (F(93, 100) * 4 * F(59, 60) ** 2) < F(2781, 10000)
    assert F(2781, 10000) * F(6058, 10000) < F(1686, 10000)
    assert 14 * F(2, 1) / (1 + 3 * F(2033, 10000)) > F(1737, 100)

    # Zone-A endpoint estimates involving square roots, checked by squaring
    # positive rational quantities.
    assert F(14262, 10000) ** 2 > F(120, 59)
    assert F(28527, 10000) ** 2 * F(1, 60) < (
        F(3768, 10000) - F(51, 100) * F(1, 60)
        - (F(51, 25) * F(1, 60)) ** 6
    ) ** 2
    assert (F(51, 25) * F(1, 60)) ** 6 < F(1, 10_000_000) * F(1, 60)

    # Turan-corner endpoint constants.
    assert 4 * (F(2003, 1994)) ** 2 < F(404, 100)
    assert F(3, 1) / (2 * (F(2, 3) - F(1, 2000))) < F(226, 100)
    assert F(2000, 2003) / F(2003, 6000) > F(299, 100)
    assert F(2003, 2000) * F(3000, 1997) / F(1, 3) < F(452, 100)
    delta0 = F(1, 2000)
    g2_ratio = (1 - F(121, 10) * delta0) * (
        13 * F(299, 100) - 78 * F(452, 100) ** 2 * delta0
    )
    assert g2_ratio > F(378, 10)
    assert F(378, 10) * (1 - F(294, 10000) / 2) > F(372, 10)
    assert 3 * F(2003, 6000) * F(602, 100) / F(1997, 3000) < F(906, 100)
    assert F(18000, 3997) < F(451, 100)
    assert F(95, 10) * F(451, 100) < 43
    assert (
        2 * (1 - F(64, 1_000_000)) * (1 - F(1, 2**14)) * 9
        / (F(50113, 100000) * F(3, 2))
        > F(79, 10)
    )
    assert F(816, 1000) ** 2 < F(2, 3)
    assert F(2003, 6000) ** 3 < F(373, 10000)
    assert (
        F(816, 1000) * (1 - F(1, 2**14)) * F(1497, 1000)
        / (2 * F(373, 10000) * F(3, 2))
        > F(109, 10)
    )
    assert 43 * 13**3 * F(50113, 100000) ** 12 < 24
    assert 43 * 13**2 * F(50113, 100000) ** 12 < F(183, 100)
    assert F(50113, 100000) * F(14, 13) ** 3 < 1

    # Remaining Zone-A constants.
    assert F(59, 63) > F(9365, 10000)
    assert F(1842, 10000) ** 2 > F(2, 59)
    assert (
        (F(59, 120) ** 2) * F(204, 100) ** 3 * F(1, 60)
        < F(19, 100) ** 2
    )
    assert F(59, 63) - (F(51, 25) * F(1, 60)) ** 6 > F(93, 100)
    assert F(204, 100) * F(1, 60) < F(19, 100) ** 2
    assert 1 - F(1, 60) >= (1 - F(51, 100) * F(1, 60)) ** 2
    assert F(2034, 1000) / (13 * F(40, 21)) < F(822, 10000)
    assert F(15, 1) / (13 * F(40, 21)) < F(6058, 10000)
    assert F(14142, 10000) ** 2 < 2
    assert F(40, 21) * F(58284, 10000) > F(1108, 100)
    assert F(78, 10) * 3 > 2 * F(1108, 100)
    c = F(6, 13)
    a0 = F(3768, 10000)
    aa = F(1686, 10000)
    assert c * (1 - a0) - aa - c * aa > F(41, 1000)
    assert 4 * F(3724, 10000) * F(1108, 100) > F(40626, 10000) ** 2
    assert F(3724, 10000) * F(78, 10) < F(29048, 10000)
    assert F(34148, 10000) ** 2 * F(1, 60) < F(441, 1000) ** 2
    assert c * (1 - a0) - aa > F(1190, 10000)
    assert c * aa < F(779, 10000)
    zcut = F(14827, 10000)
    qp = lambda z: F(3724, 10000) + F(2380, 10000) * z - F(2337, 10000) * z * z
    assert qp(1) > 0 and qp(zcut) > 0
    assert F(1686, 10000) * zcut < F(1, 4)
    assert (
        F(2910, 10000) * zcut + F(1722, 10000) * zcut**2
        - F(6268, 10000) > F(183, 1000)
    )

    # Remaining small-e Zone-C constants.
    assert F(93, 100) * F(59, 60) ** 2 > F(899, 1000)
    assert (
        2 * F(1842, 10000) ** 53 * (1 + F(1842, 10000))
        / (55 * (1 - F(1842, 10000) ** 54))
        < F(1, 10**38)
    )
    assert (1 + F(1842, 10000)) / (1 + F(9365, 10000)) < F(6117, 10000)
    assert 1 - F(1842, 10000) > F(8157, 10000)
    assert (
        F(6182, 10000) ** 2 * 8 * F(8157, 10000) ** 2 * F(59, 120)
        > 1
    )
    assert F(6117, 10000) * F(6182, 10000) + F(1, 10**38) < F(3782, 10000)
    assert F(19047, 10000) * F(899, 1000) > F(1712, 1000)

    def exp_partial(t: F, n: int) -> F:
        total = F(1)
        term = F(1)
    ...
    assert exp_partial(F(1712, 1000), 12) > 1 / (1 - F(8142, 10000))
    qlo = F(59, 120) - F(1, 3481)
    assert 1 / qlo < F(20351, 10000)
    assert F(8142, 10000) * F(59, 120) > F(4003, 10000)
    assert F(1842, 10000) ** 2 < F(34, 1000)
    assert F(93, 100) ** 2 > F(864, 1000)
    assert (
        F(34, 1000) * F(1842, 10000) ** 53
        < F(1, 10**37) * F(864, 1000) / 55
    )
    assert F(19047, 10000) * F(53, 55) > F(18354, 10000)
    assert F(20351, 10000) ** 2 / (4 * F(19047, 10000)) < F(5437, 10000)
    assert exp_partial(F(1), 10) > F(271828, 100000)
    assert 1 / (F(18354, 10000) * F(271828, 100000)) < F(2005, 10000)
    assert F(2005, 10000) < F(39, 100) / F(5437, 10000)
    assert 1 / F(20351, 10000) > F(4753, 10000)
    assert F(39, 100) * F(4753, 10000) ** 2 > F(881, 10000)
    assert F(2005, 10000) / 60 < F(34, 10000) < F(881, 10000)

    # Zone-B non-emptiness bracket.
    zone_poly = lambda t: 3 * t**3 - t**2 + 5 * t - 1
    assert zone_poly(F(2032, 10000)) < 0 < zone_poly(F(2033, 10000))

    report("directed constants in Zones A/B/C (exact rational)", True)
...
# corresponds to a display in regionII_corrected_solution.tex)
    ...
    for e in es:
        ...
        for kf in [mpf(f) / 100 for f in [0, 1, 10, 30, 50, 70, 90, 100]]:
            ...
            for m in [15, 21, 41, 101, 501, 5001, 100001, 2000001]:
                ...
                # epsA
                note("epsA", min(mpf(1) / 4, mpf("0.2781") * T) - eps)
                ...
                note("aux-b", mpf("0.1684") * z - mpf("0.2781") * T)
    ...
    for e in [mpf(10) ** x for x in [-8, -6, -4, -3, mpf("-2.2")]] + [E0]:
        ...
        for kf in [mpf(f) / 100 for f in [1, 10, 30, 60, 90, 100]]:
            ...
            for m in [max(15, m0 - 2), m0 + 2, 2 * m0 + 1, 10 * m0 + 1]:
                ...
                note("C-t-ub", e * (mpf("2.0351") * z - mpf("1.9047")) - t)
                note("C-battle", mpf("0.39") * kappa**2 * m
                     - exp(-mpf("1.8354") * T) * min(mpf(1), max(t, mpf(0))))
...
def corner_chain() -> None:
    ...
    for dexp in [mpf("-3.31"), -4, -5, -7, -9]:
        ...
        for df in [mpf(x) / 1000 for x in [50, 300, 600, 900, 999, 1000]]:
            ...
            note("T-q2L2-lo", (q * q - L * L) - (mpf(2) / 3) * delta)
            note("T-q2L2-up", mpf("1.0015") * delta - (q * q - L * L))
            note("T-f", V["f"] - mpf("1.497") * delta)
            note("T-f-up", mpf("2.005") * delta - V["f"])
            note("T-ell", (1 - ell) - 0)
            note("T-ell-up", mpf("6.02") * delta - (1 - ell))
            G2 = ell * ell * (1 - (y / s) ** 13)
            note("T-G2", G2 - mpf("37.8") * delta)
            for m in [15, 21, 61, 301, 5001]:
                W = quantities(e, kappa, m)
                R = W["R"]
                N = d - (q * q - L * L) * ell ** (m - 1) / (q + L)
                note("T-N", mpf("9.5") * m * delta**2 - N)
                D = (x ** (m - 2) + ell * ell * y ** (m - 2)
                     - (p * q / (a * a)) * s ** (m - 2)) / a
                if N > 0:
                    note("T-MVT", (m - 2) * x ** (m - 3) * N / (p * a) - D)
                if R > 0:
                    note("T-force", (m - 2) * d - mpf("12.4") * delta)
                    xi = W["xi"]
                    x14, y14 = x**14, y**14
