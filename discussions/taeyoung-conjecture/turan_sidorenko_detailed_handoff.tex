...
\title{Detailed Handoff Note: A Turan--Sidorenko Programme for Chromatic-Polynomial Lower Bounds}\author{Prepared for colleague review}\date{June 30, 2026}
...
\section*{Executive summary for colleague}
\addcontentsline{toc}{section}{Executive summary for colleague}
This document summarizes a research programme around the lower bound
\[
        t(H,W)\ge (1-p)^{v(H)}\chi_H\!\left(\frac1{1-p}\right),
        \qquad p=t(K_2,W),
\]
on the top Turan branch.  The central interpretation is that the right-hand side is the density predicted by an equivalence-relation graphon: vertices receive latent classes and edges appear exactly between distinct classes.  Thus the problem asks when \emph{class-based anticorrelation} is the strongest possible mechanism for suppressing the density of a non-bipartite graph $H$.

The original structural proposal was the \emph{odd-atomic} positive-core conjecture: graphs obtained by clique-gluing atoms that are cliques or odd cycles should satisfy the inequality.  The work so far supports this direction, but it also shows that the primitive conjecture should be revised.  The right induction invariant is not merely the unrooted inequality for each atom; it is a family of \emph{rooted product inequalities} controlling the covariance of rooted extension densities across clique separators.  This is the main conceptual development.

The most important current technical target is the first mixed gluing inequality
\[
        t(C_3\cup_{K_2}C_5,W)\ge (2p-1)t(C_5,W), \qquad p\ge \frac12.
\]
Equivalently, with $T_W$ the graphon operator and
\[
        K_W(x,y)=W(x,y)\bigl(T_W^2(x,y)-(2p-1)\bigr),
\]
the target is the smoothed Goodman trace inequality
\[
        \operatorname{Tr}(T_W^2K_WT_W^2)\ge0.
\]
This has been proved for several nontrivial graphon families: regular graphons, complete multipartite graphons, equal-weight bipodal graphons, rank-one graphons, one-sided threshold bipodal graphons, and Boolean bipodal graphons.  The full bipodal case has been reduced to a concrete four-variable polynomial inequality with an exact rank-two identity and a partial exact Bernstein certificate; the remaining gap lies in a very small boundary layer near complete/vanishing-block degeneracies.

The places where advice would be most valuable are: (i) prove or refute the smoothed Goodman trace inequality; (ii) complete the bipodal proof, especially the boundary-layer lemma; (iii) find a flag-algebra/SOS certificate for the $C_3$--$C_5$ target; (iv) assess whether a variational reduction to bipodal or threshold-bipodal extremizers is plausible; and (v) test whether odd wheels, thetas, pyramids, prisms, or even-hole-free non-odd-atomic atoms lie inside or outside the positive class.
...
\section{Further progress on the $C_3$--$C_5$ target}
\label{sec:further-SG2}

This section records progress obtained after the initial formulation of the smoothed Goodman trace inequality.  The results here are the most relevant for the next stage of the project.

\subsection{Equivalent finite and probabilistic forms}

The inequality
\begin{equation}\label{eq:SG2-updated}
  \Delta_2(W):=t(C_3\cup_{K_2}C_5,W)-(2p-1)t(C_5,W)\ge0
\end{equation}
is a linear quantum-graph inequality:
\[
  t(C_3\cup_{K_2}C_5,W)+t(C_5,W)
  \ge 2t(K_2,W)t(C_5,W).
\]
Since random-free graphons are dense for fixed finite homomorphism densities, a strict graphon counterexample would yield an arbitrarily close $0/1$ graphon counterexample.  Thus \eqref{eq:SG2-updated} can be attacked as a dense finite graph problem.

There is also a useful $C_5$-biased formulation.  Sample an ordered homomorphic $5$-cycle
\[
  X_0,X_1,X_2,X_3,X_4
\]
with probability proportional to
\[
  \prod_{i=0}^4 W(X_i,X_{i+1}),
\]
with indices modulo $5$, and then sample an independent $Z\in[0,1]$.  By symmetry over the five cycle edges, \eqref{eq:SG2-updated} is equivalent to
\begin{equation}\label{eq:C5-biased-Goodman}
  \mathbb E\left[\frac15\sum_{i=0}^4
  W(Z,X_i)W(Z,X_{i+1})\right]
  \ge 2p-1.
\end{equation}
For a $0/1$ graphon, let
\[
  S_Z:=\{i\in\mathbb Z/5\mathbb Z: Z\not\sim X_i\}.
\]
Let $e_{C_5}(S_Z)$ be the number of consecutive pairs inside $S_Z$ along the sampled cycle.  Then \eqref{eq:C5-biased-Goodman} is equivalent to
\begin{equation}\label{eq:covering-form}
  \mathbb E\bigl[2|S_Z|-e_{C_5}(S_Z)\bigr]\le 10(1-p).
\end{equation}
Thus the target says: in a $C_5$-biased cycle, the non-neighbourhood of a random outside vertex covers at most the globally expected fraction of cycle edges.  This is one of the cleanest combinatorial forms of the desired rooted anticorrelation principle.

\subsection{Slack-dominates-covariance decomposition}

Let
\[
  A(x,y):=T_W^2(x,y),\qquad B(x,y):=T_W^4(x,y),
\]
and use the edge-biased probability measure
\[
  d\mu_E(x,y)=\frac{W(x,y)}p\,dx\,dy.
\]
Then
\[
  \frac{t(K_3,W)}p=\mathbb E_{\mu_E}A,
  \qquad
  \frac{t(C_5,W)}p=\mathbb E_{\mu_E}B,
\]
and
\[
  \frac{t(C_3\cup_{K_2}C_5,W)}p=\mathbb E_{\mu_E}(AB).
\]
Therefore
\begin{equation}\label{eq:covariance-decomp}
  \Delta_2(W)=
  \frac{t(C_5,W)}p\Bigl(t(K_3,W)-p(2p-1)\Bigr)
  +p\,\operatorname{Cov}_{\mu_E}(A,B).
\end{equation}
The first term is the Goodman slack and is nonnegative.  The second term can be negative.  Hence the correct mechanism is not positive correlation, but
\[
        \text{Goodman slack dominates possible negative rooted covariance.}
\]
This is precisely the phenomenon expected in clique-separator gluing.

Positive covariance is false even for a simple bipodal graphon.  Take two parts of masses $3/4$ and $1/4$ and matrix
\[
  \begin{pmatrix}3/7&1\\1&0\end{pmatrix}.
\]
Then $p=69/112$ and exact calculation gives
\[
  \operatorname{Cov}_{\mu_E}(T_W^2,T_W^4)
  =-\frac{306423}{3373232128}<0,
\]
while
\[
  \Delta_2(W)=\frac{69804909}{6746464256}>0.
\]
This example is useful because it rules out any proof based only on rooted positive association.

\subsection{False stronger routes}

Several tempting strengthenings are false.

First, the Goodman defect kernel
\[
  K_W(x,y)=W(x,y)\bigl(T_W^2(x,y)-(2p-1)\bigr)
\]
is not copositive.  There is an equal-five-block $0/1$ graphon with adjacency matrix
\[
A=
\begin{pmatrix}
0&1&1&1&1\\
...
\end{pmatrix}
\]
for which $p=16/25$ and a nonnegative test function $f$ gives
\[
  \int K_W(x,y)f(x)f(y)\,dx\,dy=-\frac{16}{625}<0.
\]
For the same graphon, however,
\[
  \operatorname{Tr}(T_W^2K_WT_W^2)=\frac{64}{15625}>0.
\]
Thus the special smoothing by $T_W^2$ is essential.

Second, one might try to prove only that a $C_5$-rooted vertex is degree-biased in the favourable direction:
\[
  \int d(x)T_W^5(x,x)\,dx\ge p\,t(C_5,W),
  \qquad d(x)=\int W(x,y)\,dy.
\]
This is false.  A random-free tripodal example with masses
\[
  17/50,\quad 17/50,\quad 8/25
\]
and matrix
\[
M=
...
\end{pmatrix}
\]
has $p=833/1250$ and violates this vertex-bias inequality.  Hence the full correction term $e_{C_5}(S_Z)$ in \eqref{eq:covering-form} is essential.

\subsection{Additional proved families for $\Delta_2\ge0$}

Beyond the classes in Section~8, the following families have also been settled.

\begin{theorem}[One-sided threshold bipodal graphons]
Let $W_{s,a}$ be the two-block graphon with part masses $s,1-s$ and matrix
\[
  M_{s,a}=\begin{pmatrix}a&1\\1&0\end{pmatrix},
  \qquad 0\le s,a\le1.
\]
Then
\[
...
This is an exact computational proof.  One computes
\[
  p=s^2a+2s(1-s),
\]
and
\[
  \Delta_2(W_{s,a})=s^3aP(s,a)
\]
for an explicit polynomial $P$.  The boundary values are
\[
  P(0,a)=6,
\]
\[
...
\]
\[
  P(s,0)=(1-s)^2(20s^2-21s+6),
\]
and
\[
  P(s,1)=2(3-s)(1-s)^3,
\]
which are nonnegative on the square.  The resultant of $P$ and $\partial P/\partial s$ factors as
\[
  2a^4(a-2)(a-1)^4(a^4-5a^2+5)L(a),
\]
where $L$ is a degree-$16$ polynomial.  A Sturm check shows that $L$ has no real root in $[0,1]$, while $a^4-5a^2+5>0$ on $[0,1]$.  Hence the number of roots of $P(\cdot,a)$ in $s\in(0,1)$ is constant for $a\in(0,1)$.  Checking one sample value, for instance $a=1/2$, proves that no interior root exists.  Therefore $P\ge0$, and hence $\Delta_2\ge0$.  The accompanying script reproduces these symbolic checks.
\end{proof}

\begin{theorem}[Boolean bipodal graphons]
Let $W$ be bipodal with arbitrary part masses $s,1-s$ and matrix entries $a,b,c\in\{0,1\}$.  Then $\Delta_2(W)\ge0$.
\end{theorem}

\begin{proof}[Proof sketch]
All Boolean two-block cases can be checked exactly.  The main case not already covered by earlier families is the disjoint union of two cliques,
\[
  M=\begin{pmatrix}1&0\\0&1\end{pmatrix}.
\]
Writing $u=s(1-s)$, exact calculation gives
\[
  \Delta_2(W)=s(1-s)(18u^2-16u+3).
\]
Since $0\le u\le1/4$ and
\[
  18u^2-16u+3\ge 18\left(\frac14\right)^2-16\left(\frac14\right)+3=\frac18,
\]
this case is nonnegative.  The other Boolean cases reduce to formulas such as
\[
  M=\begin{pmatrix}1&0\\0&0\end{pmatrix}
  \quad\Longrightarrow\quad
  \Delta_2=s^5(1-s)(2s+1)\ge0,
\]
\[
  M=\begin{pmatrix}1&1\\1&0\end{pmatrix}
  \quad\Longrightarrow\quad
  \Delta_2=2s^3(3-s)(1-s)^3\ge0,
\]
and the symmetric counterparts.
\end{proof}

\subsection{Full bipodal reduction and current gap}

For a general bipodal graphon with masses $s,1-s$ and matrix
\[
  M=\begin{pmatrix}a&b\\b&c\end{pmatrix},
\]
the defect $\Delta_2$ is an explicit polynomial
\[
  \Delta_2(s,a,b,c)
\]
...
\[
  ...
  \qquad \deg_c=7.
\]
The two-dimensional graphon operator satisfies a rank-two Cayley--Hamilton identity.  Put
\[
  \tau:=sa+(1-s)c,
  \qquad
  \eta:=s(1-s)(ac-b^2).
...
Then
\[
  T_W^4=(\tau^2-\eta)T_W^2-\tau\eta T_W,
\]
and hence
\begin{equation}\label{eq:rank-two-updated}
  \boxed{
...
where
\[
  D_1:=\int WT_W^2\bigl(T_W^2-(2p-1)\bigr),
\]
and
\[
  R:=\int W^2\bigl(T_W^2-(2p-1)\bigr).
\]
The term $D_1$ is nonnegative for every graphon by Goodman plus Jensen.  Thus any bipodal counterexample would have to use the second term in \eqref{eq:rank-two-updated}.  This is a sharp localization of the algebraic difficulty.

A numerical Bernstein-subdivision search over the entire cube $[0,1]^4$ found no counterexample and terminated on a tree with $1423$ nodes and $712$ terminal boxes.  Rechecking those boxes with exact integer Bernstein arithmetic certifies $688$ of the $712$ leaves.  The remaining $24$ are not counterexamples; they are boundary-layer boxes where $\Delta_2$ vanishes on lower-dimensional faces and Bernstein coefficients remain slightly negative.  The unresolved boxes lie near the degeneracies
\[
  \{s=0,c=1\},
  \qquad
  \{s=1,a=1\},
  ...
  \{a=b=c=1\},
\]
and their symmetric variants.  For example,
\[
  \Delta_2(0,a,b,c)=c^5(1-c)^2\ge0,
\]
and symmetrically
\[
  \Delta_2(1,a,b,c)=a^5(1-a)^2\ge0.
\]
Near the complete graphon $a=b=c=1$, the quadratic part in the deficits
\[
  X=1-a,
  \qquad Y=1-b,
  \qquad Z=1-c
\]
is positive semidefinite but degenerate.  This explains why pure Bernstein subdivision stalls.  The current precise subtarget is therefore:
\begin{quote}
Prove a local boundary lemma for $\Delta_2$ near the complete-graphon and vanishing-block degeneracies.  Combining that lemma with the exact Bernstein certificate would prove the bipodal smoothed Goodman conjecture.
\end{quote}

\subsection{Updated status table for the smoothed Goodman target}

\[
\begin{array}{l|l}
\toprule
\text{Graphon class} & \text{Status for }\Delta_2(W)\ge0\\
\midrule
...
\text{general bipodal graphons} & \text{strong evidence; exact reduction; partial exact Bernstein certificate}\\
...
The handoff package includes two main scripts.
\begin{enumerate}[label=(\roman*)]
\item \texttt{bipodal\_delta2\_reduction.py}: derives $\Delta_2$ symbolically, verifies the rank-two identity, checks Boolean bipodal boundary cases, and checks the one-sided threshold family using the resultant/Sturm argument.
\item \texttt{bipodal\_delta2\_formalization.py}: derives $\Delta_2$, verifies the rank-two identity, builds the floating-point Bernstein subdivision tree, and rechecks the accepted boxes with exact integer/rational Bernstein arithmetic.  It reports the certified and unresolved boxes.
\end{enumerate}
These scripts are not a substitute for a polished proof, but they make the computational claims precise and reproducible.  The exact Bernstein part certifies only the boxes whose exact Bernstein coefficients are nonnegative; the remaining boundary-layer boxes are explicitly reported and remain the current formal gap.
...
\section{Specific questions for colleague review}
\label{sec:questions-for-colleague}

Here are the questions for which outside advice would be most helpful.

\begin{enumerate}[label=\textbf{Q\arabic*.},leftmargin=*]
\item \textbf{Is the smoothed Goodman trace inequality true?}  The target is
\[
  t(C_3\cup_{K_2}C_5,W)\ge(2p-1)t(C_5,W).
\]
It has survived many tests, but we do not yet have an arbitrary-graphon proof.

\item \textbf{Can one find a flag-algebra/SOS certificate?}  The equivalent quantum-graph inequality is
\[
  t(C_3\cup_{K_2}C_5,W)+t(C_5,W)
  \ge 2t(K_2,W)t(C_5,W).
\]
A certificate, even if initially machine-generated, would be extremely informative.

\item \textbf{Can the bipodal proof be completed?}  The full bipodal problem is now a four-variable polynomial inequality with an exact rank-two reduction and a partial exact Bernstein certificate.  The remaining gap is a local boundary lemma near complete and vanishing-block degeneracies.

\item \textbf{Is a variational reduction to bipodal or threshold-bipodal graphons plausible?}  If any counterexample to $\Delta_2\ge0$ can be compressed to a bipodal or one-sided threshold-bipodal counterexample, then the current bipodal evidence may be close to a full proof.

\item \textbf{What is the right rooted clique-density statement?}  For clique atoms, the unrooted clique-density theorem is insufficient for clique-cut induction.  We need rooted product inequalities for completing a rooted clique to larger cliques.

\item \textbf{What are the right boundary atoms?}  We need numerical and theoretical tests for odd wheels, theta graphs, pyramids, prisms, and even-hole-free but non-odd-atomic atoms.  These examples will determine whether odd-atomicity is the final boundary or only a clean positive core.
\end{enumerate}
