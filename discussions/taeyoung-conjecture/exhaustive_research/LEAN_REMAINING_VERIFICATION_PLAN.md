...
Atlas188 completed on 2026-09-23. Its three interval bounds and the full catalogue proof passed in a fresh 98-file build: 40m55s at 11.90 GiB. The installed recheck brings combined verification to 41m03s. The theorem has only the standard axioms; its example and classification are updated. Atlas171 is now the active row. See [the verification record](lean/docs/ATLAS188_VERIFICATION_PROGRESS.md).
...
| 17 | 171 | Same three-bound shape as 188. `Φ₁₇₁ = p(2p−1)(11p³−20p²+13p−3)`, whose cubic factor is increasing with value `−1/512` at `5/8`, so the target is nonpositive on `[1/2,5/8]` by an elementary sign lemma; the exact middle `[5/8,2/3]` and upper `[2/3,1]` certificates cover the rest. Neither the self-amalgam argument nor the transfer from 188 is needed. The transfer (spanning-subgraph map deleting edge `15`, with `Φ₁₈₈ − Φ₁₇₁ = p(p−1)(2p−1)²(3p−2) ≥ 0` on `[1/2,2/3]`) stays available should the middle piece overrun the envelope. |
...
| 19 | 174 | Same three-bound shape as 188. `Φ₁₇₄ = p(2p−1)(13p³−25p²+17p−4)`, whose cubic factor is increasing with value `−17/1728` at `7/12`, so an elementary sign lemma covers `[1/2,7/12]`; the exact middle `[7/12,2/3]` and upper `[2/3,1]` certificates cover the rest. No self-amalgam argument is needed. Its middle certificate has the weakest source diagonal-dominance margin of the four, so watch the compact face discovery. |
