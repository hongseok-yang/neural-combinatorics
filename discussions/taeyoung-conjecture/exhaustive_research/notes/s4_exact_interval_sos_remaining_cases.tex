...
\paragraph{Completion update, September 21, 2026.}
The two partial rows recorded in this historical note are now fully positive.
The exact induced rooted certificates in
\path{notes/atlas130_atlas203_complete_bounds.tex} prove Atlas 130 on
$[1/2,2/3]$ and Atlas 203 on $[2/3,3/4]$.  Together with the high intervals
proved here, they complete both required ranges.  The certificates and
theorems in the present note are unchanged.
...
153 & $[3/5,2/3]$ & \path{atlas153_exact_middle35_interval_sos.json} & $0.9977$\\
...
Atlas 153, 171, 174, and 188 were carried into Lean on 2026-09-23, each as a
sorry-free \verb|SatisfiesLowerBound| theorem whose \verb|#print axioms| gives
only \verb|propext|, \verb|Classical.choice|, and \verb|Quot.sound|.  Two
deviations from this note are worth stating.

First, none of the four Lean rows uses the self-amalgam theorem of
\path{notes/four_self_amalgam_partial_ranges.tex}, whose four proved ranges are
tabulated above.  For each of these graphs the catalogue target factors as
\[
 \Phi_H(p)=p\,(2p-1)\,Q_H(p),
\]
and each $Q_H'$ has negative discriminant, so $Q_H$ is strictly increasing.
With $a_H$ the left endpoint of the middle certificate,
\[
\begin{array}{c|c|c}
H&a_H&Q_H(a_H)\\ \hline
153 &3/5  &-1/125\\
...
\end{array}
\]
so $Q_H\le Q_H(a_H)<0$ on $[1/2,a_H]$, while $p\ge0$ and $2p-1\ge0$ there.
Hence $\Phi_H(p)\le0\le t(H,W)$ on $[1/2,a_H]$, by nonnegativity of the
homomorphism density alone.  Each Lean row proves this in a single file
\verb|AtlasNNN/Low.lean|, about forty lines, and joins it to the two exact
certificates by a case split at $a_H$.

Second, Atlas 153's middle interval in Lean is $[3/5,2/3]$, not the
$[37/60,2/3]$ that the campaign of 2026-08-25 produced.  The sign argument
above reaches only to the root $0.6068\ldots$ of $Q_{153}$, which lies below
$37/60=0.6166\ldots$, so the August interval would have left a genuine gap.
The interval SOS was therefore re-solved on the wider $[3/5,2/3]$, giving
\path{atlas153_exact_middle35_interval_sos.json} in the table above; that file,
not \path{atlas153_exact_middle_interval_sos.json}, is what the Lean row
carries.  The wider interval also keeps the affine denominator at $15$, matching
Atlas 188.

Each row is 98 Lean files and passes a fresh sequential rebuild within one
hour and 16 GiB; the four measurements are 40m52s to 42m16s at 11.88 to
11.91 GiB.  The per-row certificates and measurements are in
\path{lean/docs/ATLAS153_VERIFICATION_PROGRESS.md} and its three companions.
