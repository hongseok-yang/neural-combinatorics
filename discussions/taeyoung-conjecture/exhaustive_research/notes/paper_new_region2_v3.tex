...
\begin{enumerate}[label=\textbf{Section \arabic*.},leftmargin=4.6em,start=2]
  ...
  The theorem in this region is thereby reduced to the inequality $R_m\le \Omega_m\psi(\xi,\rho)$.
  ...
  Write
  \[
    \widehat Q_m=\frac{\Omega_m\rho\xi^2}{\Lambda^m},
    \qquad
    \widehat L_m=\frac{\Omega_m\xi}{\Lambda^m},
    \qquad
    \theta=\rho\xi=\frac{\widehat Q_m}{\widehat L_m}.
  \]
  Dividing the objective by $\xi$ and discarding $\lambda\xi$ from the denominator leaves a maximum in which only $\theta$ occurs,
  \[
    \frac{\Omega_m\psi(\xi,\rho)}{\Lambda^m}\ge\widehat L_m\,d(\theta),
    \qquad
  ...
  \[
    \delta_{\tilde p}=\frac {p}{\Lambda}-1,
    \qquad
    \zeta_q=\frac{M^2}{\Lambda(\Lambda-q)}
  \]
  determine $q$, $p$, $\Lambda$, and $M$, so we may replace $q$ and $\Lambda$ by $\delta_{\tilde p}$ and $\zeta_q$.
  % TODO: revise this region statement. The inequality zeta_q(zeta_q+delta_p)>=1 is no longer
  % proved in subsec:plt23-variables (disabled there inside \iffalse), because no later step uses
  % it; the proofs use (5.55), (5.56), (5.58) instead. The statement is also too weak: the two
  % conditions below do not imply q>1/3 (e.g. zeta_q=(sqrt5-1)/2, delta_p=1 gives q about 0.236).
  For every $m$, the pairs $(\delta_{\tilde p},\zeta_q)$ to be considered are those satisfying
  \[
    0<\delta_{\tilde p}<1,
    \qquad
    \zeta_q(\zeta_q+\delta_{\tilde p})\ge1
  \]
  (\Cref{subsec:plt23-variables}).
  Writing $\widehat R_m=R_m/\Lambda^m$, what has to be proved is that $\widehat R_m\le\widehat L_m\,d(\theta)$ holds in that region for every odd $m\ge9$.

  The region is divided only at $\delta_{\tilde p}=1/2$, and on each half two bounds are proved at once.
  \begin{itemize}[leftmargin=1.6em,topsep=2pt,itemsep=1pt]
    \item For $\delta_{\tilde p}\le1/2$: $2\widehat R_m\le\widehat Q_m$ and $\widehat R_m/\widehat Q_m+2\widehat R_m/\widehat L_m\le2$ (\Cref{prop:plt23-low-v}).
    \item For $\delta_{\tilde p}\ge1/2$: $\widehat R_m\le\widehat Q_m$ and $2\widehat R_m\le\widehat L_m$ (\Cref{prop:plt23-high-v-A,prop:plt23-high-v-B}).
  \end{itemize}
  In both halves the first bound settles $\theta\le1/2$ and the second settles $\theta\ge1/2$, which gives $\widehat R_m\le\widehat L_m\,d(\theta)$ (\Cref{prop:plt23-scalar}).
  All four bounds are proved by elementary analysis; the only inputs are two upper bounds for $\widehat R_m$, one bound for a product of elementary factors, and one comparison between a geometric sum and a power.
\end{enumerate}
...
\begin{lemma}[Eigenvalues above $q$]\label{lem:plt23-eigenvalues-above-q}
  There is at most one eigenvalue of $A$ that is larger than $q$, counted with multiplicity.
...
\end{equation}
where $M<q$ because $M^2<pq-q^2=q(1-2q)<q^2$ for $q>1/3$, and $\Lambda<1/2$ because $\Lambda$ is an eigenvalue of $A$, so that $\Lambda^2\le\Tr(A^2)\le pq=p(1-p)<1/4$ by \eqref{eq:plt23-HS-bound} and $p>1/2$.
...
Two normalizations are used throughout this subsection.
On the quantities to be compared, $\widehat\bullet$ denotes $\bullet/\Lambda^m$:
\begin{equation}\label{eq:plt23-AB-theta}
  \widehat R_m:=\frac{R_m}{\Lambda^m},
  \qquad
  \widehat Q_m:=\frac{\Omega_m\rho\xi^2}{\Lambda^m},
  \qquad
  \widehat L_m:=\frac{\Omega_m\xi}{\Lambda^m},
  ...
  \theta:=\rho\xi=\frac{\widehat Q_m}{\widehat L_m}.
\end{equation}
On the quantities with the same scale as $\Lambda$, $\tilde\bullet$ denotes $\bullet/\Lambda$:
\begin{equation}\label{eq:plt23-tilde-def}
  ...
  \qquad
  \delta_{\tilde q}:=1-\tilde q=\frac{\Lambda-q}{\Lambda},
  \qquad
  \delta_{\tilde p}:=\tilde p-1=\frac{p-\Lambda}{\Lambda}.
\end{equation}
These normalizations rewrite the maximum in \eqref{eq:plt23-psi-dual} as $\widehat L_m$ times a maximum over $\lambda$ in which the parameters enter only through $\theta$ and $\xi$; discarding $\xi$ there reduces \eqref{eq:plt23-scalar-target} to a single inequality.
...
\begin{proposition}[Reduction to a normalized inequality]\label{prop:plt23-reduction}
  ...
  \begin{equation}\label{eq:plt23-dtheta}
    ...
    \end{cases}
  \end{equation}
  If $\widehat R_m\le\widehat L_m\,d(\theta)$, then \eqref{eq:plt23-scalar-target} holds.
\end{proposition}

\begin{proof}
  Here $\xi$ and $\rho$ are positive by \eqref{eq:plt23-M-Lambda-order} and \eqref{eq:plt23-Lambdam-Mm-positive}, hence so are $\widehat Q_m$, $\widehat L_m$, and $\theta$.
  ...
  \]
  because $\theta+\lambda\xi\ge\theta$, so replacing $4(\theta+\lambda\xi)$ by $4\theta$ makes the subtracted term larger.
  Solving the last maximization over $\lambda$ shows that when $2\theta \le 1$, the maximum is attained at $\lambda=2\theta$ and equals $\theta$, while when $2\theta \ge 1$, the maximum is attained at $\lambda=1$ and equals $1-1/(4\theta)$; in both cases it is $d(\theta)$.
  Together with the hypothesis this gives
  \[
    \widehat R_m
    \le\widehat L_m\,d(\theta)
    \le\frac{\Omega_m\psi(\xi,\rho)}{\Lambda^m},
  \]
  and multiplying by $\Lambda^m$ gives \eqref{eq:plt23-scalar-target}.
\end{proof}

We first translate the existing relations between $p$, $q$, $\Lambda$, $M$ to after normalization.
The ordering $M<q<\Lambda<\frac12<p$ of \eqref{eq:plt23-M-Lambda-order}, together with $0<M$ from \Cref{lem:plt23-forced-variance}, gives
\begin{equation}\label{eq:plt23-tilde-order}
  0<\tilde M<\tilde q<1,
  \qquad
  1<\tilde p<2,
\end{equation}
where the upper bound on $\tilde p$ holds because $q>1/3$ and $\Lambda>q$ give $q+2\Lambda>3q>1$, that is, $p<2\Lambda$.
For $\delta_{\tilde p}$, these give
\begin{equation}\label{eq:plt23-zeta-v-consequences}
  0<\delta_{\tilde p}<1,
  \qquad
  \tilde M^2<\delta_{\tilde p},
\end{equation}
the first because $1<\tilde p<2$, the second because
\[
  \delta_{\tilde p}-\tilde M^2
  =\frac{(p\Lambda-\Lambda^2)-(pq-\Lambda^2)}{\Lambda^2}
  =\frac{p(\Lambda-q)}{\Lambda^2}>0.
\]

For $\tilde M$, the relation $M^2=pq-\Lambda^2$ gives
\[
  \tilde p=\frac{1+\tilde M^2}{\tilde q},
  \qquad\text{so that}\qquad
  p=\frac{(1+\tilde M^2)\Lambda}{\tilde q},
\]
so $1=p+q=(1+\tilde q^2+\tilde M^2)\Lambda/\tilde q$, that is,
\[
  \Lambda=\frac{\tilde q}{1+\tilde q^2+\tilde M^2},
  \qquad
  q=\frac{\tilde q^2}{1+\tilde q^2+\tilde M^2},
\]
so the quadratic inequality \eqref{eq:plt23-alpha-ceiling-poly} in $\Lambda$ is equivalent to
\begin{equation}\label{eq:plt23-frontier-chart}
  \tilde M^2\ge \tilde q(1-\tilde q)=\tilde q\delta_{\tilde q}.
\end{equation}
Likewise, since $q=\tilde q^2/(1+\tilde q^2+\tilde M^2)$, the bound $q>1/3$ is equivalent to $1+\tilde q^2+\tilde M^2<3\tilde q^2$, that is, to
\begin{equation}\label{eq:plt23-Mtilde-upper}
  \tilde M^2<2\tilde q^2-1.
\end{equation}
Together, \eqref{eq:plt23-frontier-chart} and \eqref{eq:plt23-Mtilde-upper} give $\tilde q-\tilde q^2<2\tilde q^2-1$, hence $3\tilde q^2-\tilde q-1>0$ and
\begin{equation}\label{eq:plt23-qtilde-lower}
  \tilde q>\frac{1+\sqrt{13}}6>\frac34.
\end{equation}

We now express $\widehat R_m$, $\widehat L_m$, and $\widehat Q_m$ in the normalized variables $\tilde q$, $\tilde M$, and $\tilde p$.
For $\widehat R_m$, the definition $R_m=\Lambda^m+M^m-pq^{m-1}$ with $M=\tilde M\Lambda$, $q=\tilde q\Lambda$, and $p=\tilde p\Lambda=(1+\tilde M^2)\Lambda/\tilde q$ gives
\begin{align}
  \widehat R_m
  &=1+\tilde M^m-\tilde p\tilde q^{m-1}
   =1+\tilde M^m-(1+\tilde M^2)\tilde q^{m-2}
  \nonumber\\
  &=1-\tilde q^{m-2}-\tilde M^2\bigl(\tilde q^{m-2}-\tilde M^{m-2}\bigr).
  \label{eq:plt23-F-formula}
\end{align}

For $\widehat L_m$, substituting the definition \eqref{eq:plt23-C-xi-rho} of $\Omega_m$ and the definition \eqref{eq:plt23-xi-def} of $\xi$,
\[
  \Omega_m\xi
  =\frac{M_m(\Lambda-M)\sqrt{2\Lambda}\,(1-2\Lambda)^2}{4\Lambda^2}
   \cdot\frac{4\Lambda^2(\Lambda-q)}{(1-2\Lambda)^2}
  =M_m(\Lambda-M)\sqrt{2\Lambda}\,(\Lambda-q),
\]
and substituting $\Lambda-q=\Lambda\delta_{\tilde q}$ and $\Lambda-M=\Lambda(1-\tilde M)$ gives
\begin{equation}\label{eq:plt23-Lhat-formula}
  \widehat L_m
  =\delta_{\tilde q}(1-\tilde M)\sqrt{2\Lambda}\,
   \frac{M_m}{\Lambda^{m-2}}.
\end{equation}
For $\widehat Q_m$, substituting the same definition of $\Omega_m$ and the definition \eqref{eq:plt23-rho-def} of $\rho$,
\[
  \Omega_m\rho
  =\frac{M_m(\Lambda-M)\sqrt{2\Lambda}\,(1-2\Lambda)^2}{4\Lambda^2}
   \cdot\frac{\Lambda_m}{M_m}\frac{\sqrt\Lambda}{2\sqrt2\,(\Lambda-M)}
  =\frac{\Lambda_m(1-2\Lambda)^2}{8\Lambda},
\]
...
\]
so that, again with $\Lambda-q=\Lambda\delta_{\tilde q}$,
\begin{equation}\label{eq:plt23-Qhat-formula}
  \widehat Q_m
  =\frac{2\Lambda^3\delta_{\tilde q}^2}{(1-2\Lambda)^2}\,
   \frac{\Lambda_m}{\Lambda^{m-2}}.
\end{equation}

Unlike \eqref{eq:plt23-F-formula}, these are not yet fully normalized: they retain $\Lambda$, $1-2\Lambda$, $\Lambda_m/\Lambda^{m-2}$, and $M_m/\Lambda^{m-2}$.
For $\Lambda$ and $1-2\Lambda$, since $p=\tilde p\Lambda$ and $q=(1+\tilde M^2)\Lambda/\tilde p$,
\[
  1=p+q
  =\left(\tilde p+\frac{1+\tilde M^2}{\tilde p}\right)\Lambda
  =\frac{\tilde p^2+1+\tilde M^2}{\tilde p}\,\Lambda,
\]
so that
\[
  \Lambda=\frac{\tilde p}{\tilde p^2+1+\tilde M^2},
  \qquad
  1-2\Lambda=\frac{\tilde p^2+1+\tilde M^2-2\tilde p}{\tilde p^2+1+\tilde M^2}
  =\frac{\delta_{\tilde p}^2+\tilde M^2}{\tilde p^2+1+\tilde M^2}.
\]
Substituting these into \eqref{eq:plt23-Lhat-formula} and \eqref{eq:plt23-Qhat-formula} gives
\begin{equation}\label{eq:plt23-QL-Delta}
  \widehat L_m
  =\delta_{\tilde q}(1-\tilde M)
   \sqrt{\frac{2\tilde p}{\tilde p^2+1+\tilde M^2}}\,
   \frac{M_m}{\Lambda^{m-2}},
  \qquad
  \widehat Q_m
  =\frac{2\tilde p^3\delta_{\tilde q}^2}
        {(\tilde p^2+1+\tilde M^2)(\delta_{\tilde p}^2+\tilde M^2)^2}\,
   \frac{\Lambda_m}{\Lambda^{m-2}},
\end{equation}
so that only $\Lambda_m/\Lambda^{m-2}$ and $M_m/\Lambda^{m-2}$ remain.

For these two, dividing the definitions of $\Lambda_m$ and $M_m$ by $\Lambda^{m-2}$ gives
\begin{equation}\label{eq:plt23-coeff-norm}
  \frac{\Lambda_m}{\Lambda^{m-2}}=2\tilde M^{m-2}+mK_\Lambda,
  \qquad
  \frac{M_m}{\Lambda^{m-2}}=2\tilde M^{m-2}+mK_M,
\end{equation}
where, by \eqref{eq:plt23-km-def}--\eqref{eq:plt23-Mm-def} and since $m-1$ is even,
\begin{align}
  K_\Lambda
  &:=\frac{k_m(\Lambda)}{\Lambda^{m-2}}
    =\frac{\tilde p^{m-1}-1}{\tilde p+1}
    =\delta_{\tilde p}
     \underbrace{\sum_{j=0}^{(m-3)/2}\tilde p^{2j}}_{\eqqcolon S_m},
  \label{eq:plt23-KA-def}\\
  K_M
  &:=\frac{k_m(M)}{\Lambda^{m-2}}
    =\frac{\tilde p^{m-1}-\tilde M^{m-1}}{\tilde p+\tilde M}
    =(\tilde p-\tilde M)
     \sum_{j=0}^{(m-3)/2}
       \tilde p^{m-3-2j}\tilde M^{2j}.
  \label{eq:plt23-KL-def}
\end{align}
Substituting these into \eqref{eq:plt23-QL-Delta},
\[
  \widehat L_m
  =\delta_{\tilde q}(1-\tilde M)
   \sqrt{\frac{2\tilde p}{\tilde p^2+1+\tilde M^2}}
   \bigl(2\tilde M^{m-2}+mK_M\bigr),
  \qquad
  \widehat Q_m
  =\frac{2\tilde p^3\delta_{\tilde q}^2
         \bigl(2\tilde M^{m-2}+m\delta_{\tilde p}S_m\bigr)}
        {(\tilde p^2+1+\tilde M^2)(\delta_{\tilde p}^2+\tilde M^2)^2},
\]
so that $\widehat R_m$, $\widehat L_m$, and $\widehat Q_m$ are functions of $\tilde q$, $\tilde M$, and $\tilde p$ alone.

Two inequalities will be applied to these expressions.
The first bounds $S_m$: it has $(m-1)/2$ summands whose geometric mean is $\tilde p^{(m-3)/2}$, so the arithmetic--geometric mean inequality gives
\begin{equation}\label{eq:plt23-SN-AMGM}
  S_m\ge \frac{m-1}{2}\tilde p^{(m-3)/2}.
\end{equation}
The second bounds the denominator common to $\widehat L_m$ and $\widehat Q_m$: since $\tilde M^2<\delta_{\tilde p}$ by \eqref{eq:plt23-zeta-v-consequences}, that is $1+\tilde M^2<\tilde p$,
\begin{equation}\label{eq:plt23-Delta-upper-sigma}
  \tilde p^2+1+\tilde M^2<\tilde p^2+\tilde p=\tilde p(\delta_{\tilde p}+2).
\end{equation}

Each of $\widehat R_m$, $\widehat Q_m$, and $\widehat L_m$ carries a factor $\delta_{\tilde q}$, common to both sides of every comparison below, and we divide it out.
For $\widehat R_m$, dividing \eqref{eq:plt23-F-formula} by $\delta_{\tilde q}$ and using $1-\tilde q^{m-2}\le(m-2)(1-\tilde q)=(m-2)\delta_{\tilde q}$,
\begin{equation}\label{eq:plt23-F-T}
  \frac{\widehat R_m}{\delta_{\tilde q}}
  \le(m-2)-\zeta_q\bigl(\tilde q^{m-2}-\tilde M^{m-2}\bigr)
  \eqqcolon T_m,
  \qquad
  \zeta_q:=\frac{\tilde M^2}{\delta_{\tilde q}}=\frac{M^2}{\Lambda(\Lambda-q)}.
\end{equation}
Thus $\widehat R_m\le0$ whenever $T_m\le0$, and then \eqref{eq:plt23-scalar-target} already holds because $\psi(\xi,\rho)\ge0$.
Whenever convenient below we may therefore assume
\begin{equation}\label{eq:plt23-F-positive}
  \widehat R_m>0,
\end{equation}
which by \eqref{eq:plt23-F-T} implies $T_m>0$.

The ratio $\zeta_q$ determines the tilde variables, since
\begin{equation}\label{eq:plt23-zeta-v-inverse}
  \delta_{\tilde q}=\frac{\delta_{\tilde p}}{\zeta_q+1+\delta_{\tilde p}},
  \qquad
  \tilde q=\frac{\zeta_q+1}{\zeta_q+1+\delta_{\tilde p}},
  \qquad
  \tilde M^2=\frac{\zeta_q\delta_{\tilde p}}{\zeta_q+1+\delta_{\tilde p}}.
\end{equation}
Indeed, $\tilde M^2=\zeta_q\delta_{\tilde q}$, while $\tilde p=(1+\tilde M^2)/\tilde q$ and $\tilde q=1-\delta_{\tilde q}$ give $1+\delta_{\tilde p}=(1+\zeta_q\delta_{\tilde q})/(1-\delta_{\tilde q})$, which is equivalent to the first identity in \eqref{eq:plt23-zeta-v-inverse}; the other two follow immediately.
Dividing \eqref{eq:plt23-frontier-chart} by $\delta_{\tilde q}$ gives $\zeta_q\ge\tilde q$, hence $\zeta_q>3/4$ by \eqref{eq:plt23-qtilde-lower}.
\iffalse
Substituting \eqref{eq:plt23-zeta-v-inverse} into \eqref{eq:plt23-frontier-chart} gives
\begin{equation}\label{eq:plt23-zeta-domain}
  \zeta_q(\zeta_q+\delta_{\tilde p})\ge1.
\end{equation}
\fi

For $\widehat Q_m$ and $\widehat L_m$, which carry the factor $\delta_{\tilde q}$ by \eqref{eq:plt23-QL-Delta}, we set
\[
  X:=\frac{\widehat Q_m}{\delta_{\tilde q}},
  \qquad
  Y:=\frac{\widehat L_m}{\delta_{\tilde q}},
  \qquad
  G:=\frac{\widehat R_m}{\delta_{\tilde q}},
\]
the last so that \eqref{eq:plt23-F-T} is $G\le T_m$.
...
\begin{lemma}[Defect bounds]\label{lem:plt23-T-bounds}
  For every admissible point and every odd $m\ge9$,
  \begin{align}
    T_m
    &\le (m-2)(1+\delta_{\tilde p})-\zeta_q\bigl(1-\delta_{\tilde p}^{(m-2)/2}\bigr),
    \label{eq:plt23-T-bound-one}\\
    T_m
    &<(m-2)\bigl(1-\tilde M^{m-1}\bigr).
    \label{eq:plt23-T-bound-two}
  \end{align}
\end{lemma}

\begin{proof}
  For the first inequality, Bernoulli gives $\tilde q^{m-2}=(1-\delta_{\tilde q})^{m-2}\ge1-(m-2)\delta_{\tilde q}$.
  Also $\tilde M^2<\delta_{\tilde p}$ by \eqref{eq:plt23-zeta-v-consequences}, hence $\tilde M^{m-2}<\delta_{\tilde p}^{(m-2)/2}$ and $\zeta_q\delta_{\tilde q}=\tilde M^2<\delta_{\tilde p}$.
  Therefore
  \begin{align*}
    T_m
    &=(m-2)-\zeta_q\bigl(\tilde q^{m-2}-\tilde M^{m-2}\bigr)\\
    &\le (m-2)-\zeta_q+(m-2)\zeta_q\delta_{\tilde q}+\zeta_q\tilde M^{m-2}\\
    &\le (m-2)(1+\delta_{\tilde p})-\zeta_q\bigl(1-\delta_{\tilde p}^{(m-2)/2}\bigr).
  \end{align*}

  For the second inequality, \eqref{eq:plt23-Mtilde-upper} gives
  \[
    \tilde M^2<2\tilde q^2-1<(2\tilde q-1)^2,
  \]
  the second inequality because their difference is $2(1-\tilde q)^2>0$ by \eqref{eq:plt23-tilde-order}.
  Since $2\tilde q-1>0$ by \eqref{eq:plt23-qtilde-lower}, this gives $\tilde M<2\tilde q-1$, equivalently
  \[
    \tilde q-\tilde M>1-\tilde q=\delta_{\tilde q}.
  \]
  Convexity of $x\mapsto x^{m-2}$ on $[0,\infty)$ gives
  \[
    \tilde q^{m-2}-\tilde M^{m-2}
    \ge (m-2)\tilde M^{m-3}(\tilde q-\tilde M)
    >(m-2)\delta_{\tilde q}\tilde M^{m-3}.
  \]
  Multiplying by $\zeta_q=\tilde M^2/\delta_{\tilde q}$ yields
  \[
    \zeta_q\bigl(\tilde q^{m-2}-\tilde M^{m-2}\bigr)>(m-2)\tilde M^{m-1},
  \]
...
The next lemma replaces all of the polynomial positivity checks that appeared in the former small-$\delta_{\tilde p}$ analysis.

\begin{lemma}[Uniform product bound]\label{lem:plt23-product}
  Let $m\ge9$ be odd.
  For every $0\le y\le1/\sqrt2$,
  \begin{equation}\label{eq:plt23-product}
    (1-y)(1-y+y^2)(1+y^2)^{m-4}
    \ge
    \frac{m-2}{m}\sqrt{1+\frac{y^2}{2}}.
  \end{equation}
\end{lemma}
...
\begin{proof}
  ...
  Write $\mathcal H_m$ for the quotient of the two sides of \eqref{eq:plt23-product},
  \[
    \mathcal H_m(y)
    :=\frac{m}{m-2}
      \frac{(1-y)(1-y+y^2)(1+y^2)^{m-4}}
           {\sqrt{1+y^2/2}},
  \]
  so that \eqref{eq:plt23-product} is the statement $\log\mathcal H_m(y)\ge0$.
  ...
  \[
    \Psi_m(y)
    \ge
    \frac{2(m-3)}{(m-2)^2}-2y+\alpha_my^2,
    \qquad
    \alpha_m:=\frac{31m-136}{32}.
  \]
  The quadratic on the right has global minimum $2(m-3)/(m-2)^2-1/\alpha_m$.
  This is nonnegative because
  \begin{align*}
    \frac{2(m-3)}{(m-2)^2}\ge\frac1{\alpha_m}
    &\iff
    2(m-3)(31m-136)\ge32(m-2)^2\\
    &\iff
    15m^2-165m+344\ge0,
  \end{align*}
  and the last expression is positive at $m=9$ and increasing for $m\ge9$.

  It remains to consider $1/4\le y\le1/\sqrt2$.
  For odd $m\ge9$,
  \[
    \frac{\mathcal H_{m+2}(y)}{\mathcal H_m(y)}
    =\frac{m^2-4}{m^2}(1+y^2)^2.
  \]
  When $y\ge1/4$, the right side is at least
  \[
    \frac{m^2-4}{m^2}\left(\frac{17}{16}\right)^2>1,
  \]
  because
  \[
    289\bigl(m^2-4\bigr)-256m^2
    =33m^2-1156>0
    \qquad (m\ge9).
  \]
  Thus it is enough to prove the claim for $m=9$, where \eqref{eq:plt23-Psi-lower} gives
  \[
    \Psi_9(y)=\frac{12}{49}-2y+\frac{37}{8}y^2-\frac52y^4.
  \]
  Now
  \[
    \Psi_9'(y)=-2+\frac{37}{4}y-10y^3,
    \qquad
    \Psi_9''(y)=\frac{37}{4}-30y^2.
  \]
  Hence $\Psi_9'$ first increases and then decreases on our interval, so its minimum there occurs at an endpoint.
  We have
  \[
    \Psi_9'\left(\frac14\right)=\frac5{32}>0,
    \qquad
    \Psi_9'\left(\frac1{\sqrt2}\right)
    =-2+\frac{17}{4\sqrt2}>0.
  \]
  Therefore $\Psi_9$ is increasing on $[1/4,1/\sqrt2]$, and
  \[
    \Psi_9(y)\ge\Psi_9\left(\frac14\right)=\frac{607}{25088}>0.
  \]
  This completes the proof.
\end{proof}

\subsection{The range \texorpdfstring{$0<\delta_{\tilde p}\le1/2$}{0<delta\_p<=1/2}}

We now prove two simultaneous bounds.
The first controls the quadratic scale $\widehat Q_m$, while the second keeps both $\widehat Q_m$ and $\widehat L_m$ at once.

\begin{proposition}[The low-$\delta_{\tilde p}$ bounds]\label{prop:plt23-low-v}
  Suppose $0<\delta_{\tilde p}\le1/2$.
  Then
  \begin{align}
    2\widehat R_m&\le\widehat Q_m,
    \label{eq:plt23-low-v-A}\\
    \frac{\widehat R_m}{\widehat Q_m}+2\frac{\widehat R_m}{\widehat L_m}&\le2.
    \label{eq:plt23-low-v-both}
  \end{align}
\end{proposition}

\begin{proof}
  If $\widehat R_m\le0$, both inequalities are immediate.
  Assume \eqref{eq:plt23-F-positive}.
  Put
  \[
    z:=\frac{\zeta_q}{\tilde p},
    \qquad
    c:=1-\delta_{\tilde p}^{(m-2)/2}.
  \]
  Since $\zeta_q>3/4$ by \eqref{eq:plt23-frontier-chart} and \eqref{eq:plt23-qtilde-lower}, while $\tilde p=1+\delta_{\tilde p}\le3/2$ by hypothesis,
  \[
    z>\frac12.
  \]
  Because $\zeta_q=\tilde pz$ and $1+\delta_{\tilde p}=\tilde p$, \eqref{eq:plt23-zeta-v-inverse} becomes
  \begin{equation}\label{eq:plt23-low-v-u-Mtilde}
    \delta_{\tilde q}=\frac{\delta_{\tilde p}}{\tilde p(1+z)},
    \qquad
    \tilde M^2=\frac{\delta_{\tilde p}z}{1+z}.
  \end{equation}
  ...
  \begin{equation}\label{eq:plt23-G-low-v}
    ...
    P:=(m-2)-cz.
  \end{equation}
  Since $\widehat R_m>0$, the left side is positive, so $P>0$.

  We first bound $X$ from below.
  From \eqref{eq:plt23-QL-Delta} and \eqref{eq:plt23-low-v-u-Mtilde},
  \begin{align*}
    X
    &=\frac{2\tilde p^3\delta_{\tilde q}}
            {(\tilde p^2+1+\tilde M^2)(\delta_{\tilde p}^2+\tilde M^2)^2}\,
      \frac{\Lambda_m}{\Lambda^{m-2}}\\
    &=\frac{2\tilde p^2(1+z)}
            {(\tilde p^2+1+\tilde M^2)\,\delta_{\tilde p}\,(\delta_{\tilde p}+\tilde pz)^2}\,
      \frac{\Lambda_m}{\Lambda^{m-2}}.
  \end{align*}
  Indeed,
  \[
    \delta_{\tilde p}^2+\tilde M^2
    =\delta_{\tilde p}\left(\delta_{\tilde p}+\frac{z}{1+z}\right)
    =\frac{\delta_{\tilde p}(\delta_{\tilde p}+\tilde pz)}{1+z}.
  \]
  By \eqref{eq:plt23-coeff-norm} and \eqref{eq:plt23-KA-def}, $\Lambda_m/\Lambda^{m-2}\ge m\delta_{\tilde p}S_m$.
  Using also \eqref{eq:plt23-Delta-upper-sigma},
  \begin{equation}\label{eq:plt23-X-low-v-1}
    X
    >\frac{2\tilde pmS_m(1+z)}
            {(\delta_{\tilde p}+2)(\delta_{\tilde p}+\tilde pz)^2}.
  \end{equation}
  We claim
  \[
    (\delta_{\tilde p}+2)(\delta_{\tilde p}+\tilde pz)^2
  ...
  \[
    \delta_{\tilde p}\tilde p^2z^2+2\tilde pz-\delta_{\tilde p}^2(\delta_{\tilde p}+2),
  \]
  which is positive because $z\ge1/2$ and $\tilde p>1$ by \eqref{eq:plt23-tilde-order}, hence $2\tilde pz\ge1$, while $\delta_{\tilde p}^2(\delta_{\tilde p}+2)\le5/8$ by hypothesis.
  Thus \eqref{eq:plt23-X-low-v-1} implies
  \begin{equation}\label{eq:plt23-X-low-v-2}
    X\ge\frac{mS_m}{\tilde p^2z}.
  \end{equation}
  Combining \eqref{eq:plt23-G-low-v}, \eqref{eq:plt23-X-low-v-2}, and \eqref{eq:plt23-SN-AMGM}, with $k=(m-3)/2\ge3$, gives
  \begin{equation}\label{eq:plt23-G-over-X}
    \frac GX
    \le
    \frac{\tilde p^3zP}{mS_m}
    \le
    \frac{2\tilde p^{3-k}zP}{(m-1)m}
    \le
    \frac{2zP}{(m-1)m},
  \end{equation}
  the last inequality because $\tilde p>1$ by \eqref{eq:plt23-tilde-order} and $k\ge3$.
  Since $zP=z\bigl((m-2)-cz\bigr)\le(m-2)^2/(4c)$,
  \begin{equation}\label{eq:plt23-G-over-X-c}
    \frac GX
    \le\frac{(m-2)^2}{2c(m-1)m}.
  \end{equation}
  Now $\delta_{\tilde p}\le1/2$ gives $c\ge1-2^{-(m-2)/2}$.
  For $m\ge9$, $2^{(m-2)/2}\ge m-2$, and
  \[
    \frac1{m-2}\le\frac{3m-4}{(m-1)m}.
  \]
  Hence
  \[
    c\ge
  ...
  From \eqref{eq:plt23-G-over-X} and $P\le m-2$,
  \begin{equation}\label{eq:plt23-G-over-X-z}
    \frac GX\le\frac{2z}{m}.
  \end{equation}
  For $Y$, \eqref{eq:plt23-QL-Delta}, \eqref{eq:plt23-coeff-norm}, and the first term of the sum in \eqref{eq:plt23-KL-def} give
  \begin{align*}
    Y
    &=(1-\tilde M)\sqrt{\frac{2\tilde p}{\tilde p^2+1+\tilde M^2}}\,
      \frac{M_m}{\Lambda^{m-2}}\\
    &\ge
    m(1-\tilde M)(\tilde p-\tilde M)\tilde p^{m-3}
    \sqrt{\frac2{\delta_{\tilde p}+2}},
  \end{align*}
  where $\sqrt{2\tilde p/(\tilde p^2+1+\tilde M^2)}\ge\sqrt{2/(\delta_{\tilde p}+2)}$ by \eqref{eq:plt23-Delta-upper-sigma}.
  Consequently,
  \begin{equation}\label{eq:plt23-2G-over-Y}
    \frac{2G}{Y}
    \le
    \frac{2P}{m}
    \frac{\sqrt{(\delta_{\tilde p}+2)/2}}
         {(1-\tilde M)(\tilde p-\tilde M)\tilde p^{m-4}}.
  \end{equation}
  Put $y=\sqrt{\delta_{\tilde p}}$, so $0<y\le1/\sqrt2$.
  Since $\tilde M\le y$ by \eqref{eq:plt23-zeta-v-consequences} and $x\mapsto(1-x)(\tilde p-x)$ is decreasing on $[0,y]$, \Cref{lem:plt23-product} gives
  \begin{align}
   (1-\tilde M)(\tilde p-\tilde M)\tilde p^{m-4}
   &\ge
   (1-y)(1-y+y^2)(1+y^2)^{m-4}\notag\\
   &\ge
   \frac{m-2}{m}\sqrt{1+\frac{y^2}{2}}
   =\frac{m-2}{m}\sqrt{\frac{\delta_{\tilde p}+2}{2}}.
   \label{eq:plt23-product-applied}
  \end{align}
  Also
  \[
    c=1-\delta_{\tilde p}^{(m-2)/2}\ge\frac{m-2}{m}.
  \]
  Indeed, $\delta_{\tilde p}^{(m-2)/2}\le2^{-(m-2)/2}\le2/m$ for $m\ge9$.
  Therefore
  \begin{equation}\label{eq:plt23-P-bound}
    \frac P{m-2}=1-\frac{cz}{m-2}
    \le1-\frac{z}{m}
    =\frac{m-z}{m}.
  \end{equation}
  ...
  \]
  which is \eqref{eq:plt23-low-v-both}, because $\widehat R_m/\widehat Q_m=G/X$ and $\widehat R_m/\widehat L_m=G/Y$.
\end{proof}

\subsection{The range \texorpdfstring{$1/2\le\delta_{\tilde p}<1$}{1/2<=delta\_p<1}}

The second half is even more direct.
Here the admissibility condition $q>1/3$ forces $\tilde M$ to be sufficiently close to $\sqrt{\delta_{\tilde p}}$; this is exactly what compensates for the apparently weak factors in $\widehat Q_m$ and $\widehat L_m$.

Throughout this subsection $1/2\le\delta_{\tilde p}<1$.
Put
\begin{equation}\label{eq:plt23-zeta-p-def}
  \zeta_p:=\frac{\tilde M^2}{\delta_{\tilde p}},
  \qquad
  \delta_{\tilde q}=\frac{\delta_{\tilde p}(1-\zeta_p)}{\tilde p},
\end{equation}
the first the same ratio as $\zeta_q$ but over the other gap, and satisfying $0<\zeta_p<1$ by \eqref{eq:plt23-zeta-v-consequences}, the second because $\tilde q=(1+\tilde M^2)/\tilde p$ gives $\delta_{\tilde q}=(\delta_{\tilde p}-\tilde M^2)/\tilde p$.
Since $q=(1+\tilde M^2)\Lambda/\tilde p=(1+\tilde M^2)/(\tilde p^2+1+\tilde M^2)$, the hypothesis $q>1/3$ is $\tilde p^2+1+\tilde M^2<3(1+\tilde M^2)$, which with $\tilde p=1+\delta_{\tilde p}$ and $\tilde M^2=\delta_{\tilde p}\zeta_p$ is
\[
  (1+\delta_{\tilde p})^2+1+\delta_{\tilde p}\zeta_p<3(1+\delta_{\tilde p}\zeta_p),
\]
so
\[
  \zeta_p>\frac{\delta_{\tilde p}^2+2\delta_{\tilde p}-1}{2\delta_{\tilde p}}\ge \delta_{\tilde p}^2.
\]
The last inequality follows from
\[
  \delta_{\tilde p}^2+2\delta_{\tilde p}-1-2\delta_{\tilde p}^3
  =(1-\delta_{\tilde p})(1+\delta_{\tilde p})(2\delta_{\tilde p}-1)\ge0.
\]
Hence
\begin{equation}\label{eq:plt23-vtau-square}
  (\delta_{\tilde p}+\zeta_p)^2\le\zeta_p(1+\delta_{\tilde p})^2=\zeta_p\tilde p^2,
\end{equation}
for
\[
  \zeta_p\tilde p^2-(\delta_{\tilde p}+\zeta_p)^2
  =(1-\zeta_p)(\zeta_p-\delta_{\tilde p}^2)\ge0.
\]

\begin{proposition}[The high-$\delta_{\tilde p}$ quadratic bound]\label{prop:plt23-high-v-A}
  Suppose $1/2\le\delta_{\tilde p}<1$.
  Then
  \begin{equation}\label{eq:plt23-high-v-A}
    \widehat R_m\le\widehat Q_m.
  \end{equation}
\end{proposition}

\begin{proof}
  Again the claim is immediate if $\widehat R_m\le0$, so assume \eqref{eq:plt23-F-positive}.
  From \eqref{eq:plt23-QL-Delta}, \eqref{eq:plt23-zeta-p-def}, \eqref{eq:plt23-coeff-norm}, and \eqref{eq:plt23-KA-def},
  \[
    X
    =\frac{2\tilde p^2(1-\zeta_p)}
          {(\tilde p^2+1+\tilde M^2)(\delta_{\tilde p}+\zeta_p)^2}
     \left(
       \frac{2\tilde M^{m-2}}{\delta_{\tilde p}}+mS_m
     \right).
  \]
  ...
  \begin{equation}\label{eq:plt23-X-high-v-lower}
    X>
    \frac{2mS_m(1-\zeta_p)}{\zeta_p\tilde p(\delta_{\tilde p}+2)}.
  \end{equation}

  The positivity of $\widehat R_m$ supplies a useful lower bound on $1-\zeta_p$.
  From \eqref{eq:plt23-F-formula},
  \[
    (1+\tilde M^2)\tilde q^{m-2}<1+\tilde M^{m},
  \]
  so
  \[
    \tilde q^{m-2}
    <1-\frac{\tilde M^2}{1+\tilde M^2}\bigl(1-\tilde M^{m-2}\bigr).
  \]
  On the other hand, Bernoulli gives $\tilde q^{m-2}=(1-\delta_{\tilde q})^{m-2}\ge1-(m-2)\delta_{\tilde q}$.
  Comparing the two inequalities, and substituting \eqref{eq:plt23-zeta-p-def}, yields
  \[
    (m-2)(1+\tilde M^2)(1-\zeta_p)
    >\zeta_p\tilde p\bigl(1-\tilde M^{m-2}\bigr).
  \]
  Therefore \eqref{eq:plt23-X-high-v-lower} gives
  \begin{equation}\label{eq:plt23-X-high-v-final}
    X>
    \frac{2mS_m}{(m-2)(1+\tilde M^2)(\delta_{\tilde p}+2)}
    \bigl(1-\tilde M^{m-2}\bigr).
  \end{equation}

  By \eqref{eq:plt23-F-T} and \eqref{eq:plt23-T-bound-two},
  \begin{equation}\label{eq:plt23-G-high-v}
    G<T_m<(m-2)\bigl(1-\tilde M^{m-1}\bigr).
  \end{equation}
  For $0\le x<1$,
  \begin{equation}\label{eq:plt23-ratio-powers}
    \frac{1-x^{m-2}}{1-x^{m-1}}\ge\frac{m-2}{m-1}.
  \end{equation}
  Indeed, after clearing denominators this is $1-(m-1)x^{m-2}+(m-2)x^{m-1}\ge0$, whose derivative is $-(m-2)(m-1)x^{m-3}(1-x)\le0$ and whose value at $x=1$ is zero.
  Combining \eqref{eq:plt23-X-high-v-final}, \eqref{eq:plt23-SN-AMGM}, and \eqref{eq:plt23-ratio-powers}, and using $1+\tilde M^2\le\tilde p$ from \eqref{eq:plt23-zeta-v-consequences}, it is enough for $X\ge G$ that
  \begin{equation}\label{eq:plt23-high-v-condition}
    \tilde p^{(m-5)/2}
    \ge\frac{(m-2)(\delta_{\tilde p}+2)}{m}.
  \end{equation}
  For $m\ge11$, the left side is at least $(3/2)^3=27/8>3$, since $\tilde p\ge3/2$, whereas the right side is less than $3$.
  For $m=9$, \eqref{eq:plt23-high-v-condition} is
  \[
    (1+\delta_{\tilde p})^2\ge\frac{7(\delta_{\tilde p}+2)}9.
  \]
  It holds at $\delta_{\tilde p}=1/2$, and the difference has derivative $2(1+\delta_{\tilde p})-7/9>0$ thereafter.
  Thus $G\le X$, which is exactly \eqref{eq:plt23-high-v-A}.
\end{proof}

We next prove the bound for $\widehat L_m$.
...
\begin{lemma}[A geometric-sum bound]\label{lem:plt23-geometric-sum}
  Let $m\ge9$ be odd and $1/\sqrt2\le x<1$.
  Then
...
\begin{proof}
  ...
  Suppose first $k\ge4$, equivalently $m\ge11$.
  Since $x<1$,
  \[
  ...
  It remains to take $k=3$, that is $m=9$.
  Define
  \[
  ...
  \[
    \frac{481}{1280}>0.
  \]
  Hence the claim holds also for $m=9$.
\end{proof}

\begin{proposition}[The high-$\delta_{\tilde p}$ linear bound]\label{prop:plt23-high-v-B}
  Suppose $1/2\le\delta_{\tilde p}<1$.
  Then
  \begin{equation}\label{eq:plt23-high-v-B}
    2\widehat R_m\le\widehat L_m.
  \end{equation}
\end{proposition}

\begin{proof}
  The claim is trivial if $\widehat R_m\le0$, so assume \eqref{eq:plt23-F-positive}.
  From \eqref{eq:plt23-QL-Delta}, \eqref{eq:plt23-coeff-norm}, \eqref{eq:plt23-KL-def}, and \eqref{eq:plt23-Delta-upper-sigma},
  \[
    Y\ge
    m(1-\tilde M)(\tilde p-\tilde M)\tilde p^{m-3}
    \sqrt{\frac2{\delta_{\tilde p}+2}}.
  ...
  \begin{equation}\label{eq:plt23-high-v-B-sufficient}
    \frac{m}{2(m-2)}
    \sqrt{\frac2{\delta_{\tilde p}+2}}
    (\tilde p-\tilde M)\tilde p^{m-3}
    \ge
    \sum_{j=0}^{m-2}\tilde M^j,
  \end{equation}
  where we cancelled the positive factor $1-\tilde M$ using $1-\tilde M^{m-1}=(1-\tilde M)\sum_{j=0}^{m-2}\tilde M^j$.

  For fixed $\delta_{\tilde p}$, the left side of \eqref{eq:plt23-high-v-B-sufficient} decreases with $\tilde M$, while the right side increases.
  Since $\tilde M^2<\delta_{\tilde p}$ by \eqref{eq:plt23-zeta-v-consequences}, it is enough to put
  \[
    x:=\sqrt{\delta_{\tilde p}},
    \qquad
    \tilde M=x,
    \qquad
    \tilde p=1+x^2.
...
\subsection{Completion of the bound \texorpdfstring{$R_m\le\Omega_m\psi(\xi,\rho)$}{Rm <= Omega\_m psi}}

We can now finish without any further subdivision of the admissible region.

\begin{proposition}[The bound $R_m\le\Omega_m\psi(\xi,\rho)$]\label{prop:plt23-scalar}
  For every admissible pair $(q,\Lambda)$ in the intermediate-density region $1/3<q<1/2$, and every odd $m\ge9$, the inequality \eqref{eq:plt23-scalar-target} holds.
\end{proposition}

\begin{proof}
  If $\widehat R_m=R_m/\Lambda^m\le0$, the claim is immediate.
  Assume $\widehat R_m>0$, and recall $\theta=\widehat Q_m/\widehat L_m$.
  By \Cref{prop:plt23-reduction} it is enough to prove $\widehat R_m\le\widehat L_m\,d(\theta)$.

  Suppose first $0<\delta_{\tilde p}\le1/2$.
  \Cref{prop:plt23-low-v} gives
  \[
    \frac{\widehat R_m}{\widehat L_m}
    \le
    \min\left\{
      \frac\theta2,
  ...
  \]
  Thus $\widehat R_m\le\widehat L_m\,d(\theta)$ in this range.

  Now suppose $1/2\le\delta_{\tilde p}<1$.
  ...
  Hence $\widehat R_m\le\widehat L_m\,d(\theta)$ again, and \Cref{prop:plt23-reduction} proves \eqref{eq:plt23-scalar-target}.
\end{proof}

\subsection{Completion of the intermediate-density region}

We state explicitly how the preceding argument reconnects to the graphon proof.
Assume
\[
  \frac12<p<\frac23,
  \qquad
...
Otherwise there is a unique eigenvalue $\Lambda>q$.
...
\begin{proof}[Proof of \Cref{thm:main}]
  ...
  Otherwise there is a unique eigenvalue $\Lambda>q$, and \Cref{prop:plt23-splitting-lower-bound,thm:plt23-huber-elim} reduce the problem to \eqref{eq:plt23-scalar-target}, which is proved in \Cref{prop:plt23-scalar}.
