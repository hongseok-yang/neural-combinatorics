# Compact Atlas157 certificate

Status (2026-09-22): **verified**. The fresh 89-file build passed in **26m52s** (1611.563 seconds), with **9.20 GiB** peak process-tree private memory. The installed-source recheck passed too, bringing combined verification to **27m00s** (1619.626 seconds).
...
`p**2*(2*p - 1)**2*(3*p - 2)`. With `p=2/3+(1/3)*s`, the checked identity is

```
sum of positive flag-density blocks
  = 7776000000000000 * (t(H157,W) - (s*(s + 2)**2*(2*s + 1)**2/81)).
```
...
The row has 89 source files, totaling 9,753,056 bytes. It uses 250
prebuilt shared project modules. Source closure, hashes, measured build
costs, and the axiom audit are retained under
`lean/verification_runs/atlas157/`. See
[the verification record](../lean/docs/ATLAS157_VERIFICATION_PROGRESS.md).
