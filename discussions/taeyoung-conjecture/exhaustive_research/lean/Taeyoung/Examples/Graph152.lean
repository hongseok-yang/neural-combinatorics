...
/-!
# Atlas 152: verified Turán-local counterexample

graph6: `EhNG`.  The edge-list definition below is the Lean graph;
the graph6 string is stable external metadata until a verified decoder exists.

The witness is the five-step fractional-fibre perturbation of `T2`, a rational step
...
namespace Taeyoung.Examples.Graph152
...
theorem iso_adj (a b : Fin 6) :
    graph.Adj (relabel a) (relabel b) ↔ (Taeyoung.Methods.Negative.graph152).Adj a b := by
  revert a b
  decide

/-- The method representative is this Atlas graph, up to relabelling. -/
def iso : (Taeyoung.Methods.Negative.graph152) ≃g graph where
  toEquiv := relabel
...
theorem status : ViolatesLowerBound graph :=
  ViolatesLowerBound.of_iso iso (Taeyoung.Methods.Negative.violatesLowerBound_152)

end Taeyoung.Examples.Graph152
