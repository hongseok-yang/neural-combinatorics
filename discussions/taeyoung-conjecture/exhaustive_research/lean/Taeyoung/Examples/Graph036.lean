...
/-!
# Atlas 36: verified rooted triangle-tree example

graph6: `DJc`.  The edge-list definition below is the Lean graph;
...
namespace Taeyoung.Examples.Graph036
...
theorem iso_adj (a b : Fin 5) :
    graph.Adj (relabel a) (relabel b) ↔
      (Taeyoung.Methods.RootedTriangleTree.q1Graph).Adj a b := by
  revert a b
  decide

/-- The certificate representative is this Atlas graph, up to relabelling. -/
def iso : (Taeyoung.Methods.RootedTriangleTree.q1Graph) ≃g graph where
  toEquiv := relabel
...
theorem status : SatisfiesLowerBound graph :=
  SatisfiesLowerBound.of_iso iso
    (Taeyoung.Methods.RootedTriangleTree.q1_satisfiesLowerBound)

end Taeyoung.Examples.Graph036
