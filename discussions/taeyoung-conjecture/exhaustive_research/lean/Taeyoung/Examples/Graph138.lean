...
/-!
# Atlas 138: verified page-rooted book-leaf example

graph6: `Ez`_`.  The edge-list definition below is the Lean graph;
...
namespace Taeyoung.Examples.Graph138

open Taeyoung

def graph : SimpleGraph (Fin 6) :=
  graphFromEdges 6 [(0, 1), (0, 2), (0, 4), (1, 2), (1, 3), (1, 5), (2, 3), (2, 5)]
...
theorem iso_adj (a b : Fin 6) :
    graph.Adj (relabel a) (relabel b) ↔
      (Taeyoung.Methods.PageBook.book138).Adj a b := by
  revert a b
  decide

/-- The certificate representative is this Atlas graph, up to relabelling. -/
def iso : (Taeyoung.Methods.PageBook.book138) ≃g graph where
  toEquiv := relabel
...
theorem status : SatisfiesLowerBound graph :=
  SatisfiesLowerBound.of_iso iso
    (Taeyoung.Methods.PageBook.satisfiesLowerBound_138)

end Taeyoung.Examples.Graph138
