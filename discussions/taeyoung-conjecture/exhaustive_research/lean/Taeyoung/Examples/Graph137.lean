...
/-!
# Atlas 137: verified page-paw branch example

graph6: `EjsG`.  The edge-list definition below is the Lean graph;
...
namespace Taeyoung.Examples.Graph137

open Taeyoung

def graph : SimpleGraph (Fin 6) :=
  graphFromEdges 6 [(0, 1), (0, 4), (1, 2), (1, 3), (1, 4), (2, 3), (3, 4), (4, 5)]
...
theorem iso_adj (a b : Fin 6) :
    graph.Adj (relabel a) (relabel b) ↔
      (Taeyoung.Methods.PagePawBranch.bookPage).Adj a b := by
  revert a b
  decide

/-- The certificate representative is this Atlas graph, up to relabelling. -/
def iso : (Taeyoung.Methods.PagePawBranch.bookPage) ≃g graph where
  toEquiv := relabel
...
theorem status : SatisfiesLowerBound graph :=
  SatisfiesLowerBound.of_iso iso
    (Taeyoung.Methods.PagePawBranch.satisfiesLowerBound_bookPage)

end Taeyoung.Examples.Graph137
