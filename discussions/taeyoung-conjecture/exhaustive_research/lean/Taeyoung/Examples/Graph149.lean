...
/-!
# Atlas 149: verified tensor-Turán counterexample

graph6: `Ele_`.  The edge-list definition below is the Lean graph;
the graph6 string is stable external metadata until a verified decoder exists.

The witness is `tensorTuran 4 5`, the categorical product of two balanced
complete multipartite graphons, of edge density
`p = (1 - 1/4)(1 - 1/5) = 3/5`.  The chromatic number is
3, so admissibility asks only `p ≥ 1/2`.  The two sides are

```
t = χ_H(4)·χ_H(5) / 20^6 = 6579/400000,
Φ = (1 - p)^6·χ_H(1/(1-p))   = 57/3125,
```
...
namespace Taeyoung.Examples.Graph149

open Taeyoung Taeyoung.Methods.Negative

def graph : SimpleGraph (Fin 6) :=
  graphFromEdges 6 [(0, 1), (0, 3), (0, 4), (0, 5), (1, 2), (2, 3), (2, 5), (3, 4)]
...
theorem surj_3 : surjCount graph 3 = 30 := by decide +kernel
theorem surj_4 : surjCount graph 4 = 288 := by decide +kernel
...
theorem count (k : ℕ) :
    properAssignmentCount graph k = 30 * k.choose 3 + 288 * k.choose 4 + 840 * k.choose 5 + 720 * k.choose 6 := by
...
/-- `χ_H(4) = 408`, the first tensor factor. -/
theorem count_4 : properAssignmentCount graph 4 = 408 := by
  rw [count]
  decide

/-- `χ_H(5) = 2580`, the second tensor factor. -/
theorem count_5 : properAssignmentCount graph 5 = 2580 := by
  rw [count]
  decide
...
theorem status : ViolatesLowerBound graph := by
  have hp : (1 - 1 / ((4 : ℕ) : ℝ)) * (1 - 1 / ((5 : ℕ) : ℝ)) =
      3 / 5 := by
    norm_num
  refine violatesLowerBound_of_tensor graph chromPoly chromNum 4 5 ?_ ?_
...
end Taeyoung.Examples.Graph149
