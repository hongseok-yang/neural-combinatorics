...
/-!
# Atlas 206: verified Turán-local counterexample

graph6: `E~nW`.  The edge-list definition below is the Lean graph;
the graph6 string is stable external metadata until a verified decoder exists.

The witness is the one-diagonal perturbation of `T3`, a rational step
...
namespace Taeyoung.Examples.Graph206
...
theorem iso_adj (a b : Fin 6) :
    graph.Adj (relabel a) (relabel b) ↔ (Taeyoung.Methods.Negative.graph206).Adj a b := by
  revert a b
  decide

/-- The method representative is this Atlas graph, up to relabelling. -/
def iso : (Taeyoung.Methods.Negative.graph206) ≃g graph where
  toEquiv := relabel
...
theorem status : ViolatesLowerBound graph :=
  ViolatesLowerBound.of_iso iso (Taeyoung.Methods.Negative.violatesLowerBound_206)

end Taeyoung.Examples.Graph206
