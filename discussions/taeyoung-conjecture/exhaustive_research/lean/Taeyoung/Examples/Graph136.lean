...
/-!
# Atlas 136: verified forest-cone example

graph6: `Ejt?`.  The edge-list definition below is the Lean graph;
...
namespace Taeyoung.Examples.Graph136

open Taeyoung

def graph : SimpleGraph (Fin 6) :=
  graphFromEdges 6 [(0, 1), (0, 4), (1, 2), (1, 3), (1, 4), (1, 5), (2, 3), (3, 4)]
...
theorem iso_adj (a b : Fin 6) :
    graph.Adj (relabel a) (relabel b) ↔
      (Taeyoung.Methods.Link.coneGraph Taeyoung.Methods.ForestCone.p4Isolated).Adj a b := by
  revert a b
  decide

/-- The certificate representative is this Atlas graph, up to relabelling. -/
def iso : (Taeyoung.Methods.Link.coneGraph Taeyoung.Methods.ForestCone.p4Isolated) ≃g graph where
  toEquiv := relabel
...
theorem status : SatisfiesLowerBound graph :=
  SatisfiesLowerBound.of_iso iso
    (Taeyoung.Methods.ForestCone.satisfiesLowerBound_136)

end Taeyoung.Examples.Graph136
