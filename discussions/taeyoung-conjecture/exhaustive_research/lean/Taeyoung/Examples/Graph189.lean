...
/-!
# Atlas 189: verified tensor-Turán counterexample

graph6: `E^NG`.  The edge-list definition below is the Lean graph;
the graph6 string is stable external metadata until a verified decoder exists.

The witness is `tensorTuran 5 5`, the categorical product of two balanced
complete multipartite graphons, of edge density
`p = (1 - 1/5)(1 - 1/5) = 16/25`.  The chromatic number is
3, so admissibility asks only `p ≥ 1/2`.  The two sides are

```
t = χ_H(5)·χ_H(5) / 25^6 = 97344/9765625,
Φ = (1 - p)^6·χ_H(1/(1-p))   = 97888/9765625,
```
...
namespace Taeyoung.Examples.Graph189

open Taeyoung Taeyoung.Methods.Negative

def graph : SimpleGraph (Fin 6) :=
  graphFromEdges 6 [(0, 2), (0, 3), (0, 5), (1, 2), (1, 3), (1, 5), (2, 3), (2, 4), (3, 4), (4, 5)]
...
theorem surj_4 : surjCount graph 4 = 168 := by decide +kernel
theorem surj_5 : surjCount graph 5 = 600 := by decide +kernel
...
theorem count (k : ℕ) :
    properAssignmentCount graph k = 12 * k.choose 3 + 168 * k.choose 4 + 600 * k.choose 5 + 720 * k.choose 6 := by
...
/-- `χ_H(5) = 1560`, the first tensor factor. -/
theorem count_5 : properAssignmentCount graph 5 = 1560 := by
  rw [count]
  decide
...
theorem status : ViolatesLowerBound graph := by
  have hp : (1 - 1 / ((5 : ℕ) : ℝ)) * (1 - 1 / ((5 : ℕ) : ℝ)) =
      16 / 25 := by
    norm_num
  refine violatesLowerBound_of_tensor graph chromPoly chromNum 5 5 ?_ ?_
  · rw [admissibleDensity, hp]; norm_num
  · rw [homDensity_tensorTuran, hp,
      chromaticTarget_of_ne_one _
        (by norm_num : (16 : ℝ) / 25 ≠ 1)]
    rw [count_5]
    simp only [Fintype.card_fin, Finset.sum_range_succ, Finset.sum_range_zero,
...
end Taeyoung.Examples.Graph189
