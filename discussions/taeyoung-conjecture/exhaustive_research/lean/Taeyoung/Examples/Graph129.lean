...
/-!
# Atlas 129: verified tensor-Turán counterexample

graph6: `EheO`.  The edge-list definition below is the Lean graph;
the graph6 string is stable external metadata until a verified decoder exists.

The witness is `tensorTuran 6 8`, the categorical product of two balanced
complete multipartite graphons, of edge density
`p = (1 - 1/6)(1 - 1/8) = 35/48`.  The chromatic number is
3, so admissibility asks only `p ≥ 1/2`.  The two sides are

```
t = χ_H(6)·χ_H(8) / 48^6 = 1171205/10616832,
Φ = (1 - p)^6·χ_H(1/(1-p))   = 14084455/127401984,
```

and `t < Φ`, so this graph refutes the catalogue proposition.

**Where the values of `χ_H` come from.**  Not from `decide`: `χ_H(8)` alone
would be `8^6 = 262144` functions.  Only the surjective counts are
...
namespace Taeyoung.Examples.Graph129
...
theorem surj_3 : surjCount graph 3 = 42 := by decide +kernel
theorem surj_4 : surjCount graph 4 = 384 := by decide +kernel
theorem surj_5 : surjCount graph 5 = 960 := by decide +kernel
...
theorem count (k : ℕ) :
    properAssignmentCount graph k = 42 * k.choose 3 + 384 * k.choose 4 + 960 * k.choose 5 + 720 * k.choose 6 := by
...
/-- `χ_H(6) = 13080`, the first tensor factor. -/
theorem count_6 : properAssignmentCount graph 6 = 13080 := by
  rw [count]
  decide

/-- `χ_H(8) = 103152`, the second tensor factor. -/
theorem count_8 : properAssignmentCount graph 8 = 103152 := by
  rw [count]
  decide
...
theorem status : ViolatesLowerBound graph := by
  have hp : (1 - 1 / ((6 : ℕ) : ℝ)) * (1 - 1 / ((8 : ℕ) : ℝ)) =
      35 / 48 := by
    norm_num
  refine violatesLowerBound_of_tensor graph chromPoly chromNum 6 8 ?_ ?_
  · rw [admissibleDensity, hp]; norm_num
  · rw [homDensity_tensorTuran, hp,
      chromaticTarget_of_ne_one _
        (by norm_num : (35 : ℝ) / 48 ≠ 1)]
    rw [count_6, count_8]
    simp only [Fintype.card_fin, Finset.sum_range_succ, Finset.sum_range_zero,
...
end Taeyoung.Examples.Graph129
