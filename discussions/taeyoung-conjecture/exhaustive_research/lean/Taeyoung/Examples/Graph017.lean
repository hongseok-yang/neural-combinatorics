...
import Taeyoung.Methods.PureChordal.Certificates.N4

/-!
# Atlas 17: verified pure-chordal example
...
`Certificates.G4_1`: 2 bags, every maximal clique of size 3.
...
namespace Taeyoung.Examples.Graph017

open Taeyoung

def graph : SimpleGraph (Fin 4) :=
  graphFromEdges 4 [(0, 1), (0, 2), (0, 3), (1, 2), (2, 3)]
...
theorem iso_adj (a b : Fin 4) :
    graph.Adj (relabel a) (relabel b) ↔
      (Taeyoung.Methods.PureChordal.Certificates.G4_1.graph).Adj a b := by
  revert a b
  decide

/-- The certificate representative is this Atlas graph, up to relabelling. -/
def iso : (Taeyoung.Methods.PureChordal.Certificates.G4_1.graph) ≃g graph where
  toEquiv := relabel
...
theorem status : SatisfiesLowerBound graph :=
  SatisfiesLowerBound.of_iso iso
    (Taeyoung.Methods.PureChordal.Certificates.G4_1.decomp.satisfiesLowerBound
      (by norm_num))

end Taeyoung.Examples.Graph017
