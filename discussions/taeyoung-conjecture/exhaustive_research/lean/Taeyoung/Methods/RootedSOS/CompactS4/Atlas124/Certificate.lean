import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.BlockSum0
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.BlockSum1
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.BlockSum2
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.BlockSum3
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.BlockSum4
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.Contraction
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.Interval
...
theorem graphon_bound (W : Graphon Ω μ) (hp : (1 : Real)/2 ≤ cliqueDensity 2 W) :
    target124 (cliqueDensity 2 W) ≤ homDensity graph124 W := by
  ...
  have ht := target124_interval s
  rw [hp'] at ht
  have ht' : 16*target124 (cliqueDensity 2 W) = s+2*s^2+4*s^3+6*s^4+3*s^5 := by
    rw [ht]
    ring
  nlinarith

theorem satisfiesLowerBound_124 : Taeyoung.SatisfiesLowerBound graph124 :=
  satisfiesLowerBound_124_of_bound (fun W hp => graphon_bound W hp)

#print axioms satisfiesLowerBound_124
