import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas124.IntervalChecks0130
...
theorem group_polynomial_eval (g : Fin 143) (s : Real) :
    (∑ j : Fin 6, (polynomialCoefficient g j : Real)*s^j.1) =
      16*((1+s)/2)^(S4Classification.groupKey g).2 *
        ((groupTotal g 0 : Real)+(groupTotal g 1 : Real)*s+
          (groupTotal g 2 : Real)*s^2+(groupTotal g 3 : Real)*s*(1-s)) := by
  rw [show polynomialCoefficient g = intervalCoefficient (S4Classification.groupKey g).2
      (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
      (groupTotal g 2 - groupTotal g 3) from rfl]
  rw [intervalCoefficient_eval _ (isolated_bound g)]
  push_cast
  ring

theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 74 then 16*10368000000000000 else 0) -
      (if r = 0 then 10368000000000000*(s+2*s^2+4*s^3+6*s^4+3*s^5) else 0) := by
  by_cases h : r = 74 <;> by_cases h0 : r = 0 <;>
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    16*(∑ g : Fin 143,
      ((groupTotal g 0 : Real)+(groupTotal g 1 : Real)*s+
        ...
        (((1+s)/2)^(S4Classification.groupKey g).2 * f (representative g))) =
      10368000000000000*(16*f 74-(s+2*s^2+4*s^3+6*s^4+3*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
