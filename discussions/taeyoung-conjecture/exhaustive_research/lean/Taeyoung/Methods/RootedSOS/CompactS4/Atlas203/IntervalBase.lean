...
import Taeyoung.Methods.RootedSOS.QuarterIntervalCoefficients
...
theorem target_core : S4Classification.coreGraph6 141 = graph203 :=
...
def polynomialCoefficient (g : Fin 143) (j : Fin 6) : Int :=
  quarterIntervalCoefficient (S4Classification.groupKey g).2
    (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
    (groupTotal g 2 - groupTotal g 3) j

def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 141 then (if j = 0 then 256*2592000000000000 else 0) else 0) -
  (if r = 0 then 2592000000000000 *
    (if j = 0 then (3) else if j = 1 then (19) else if j = 2 then (54) else if j = 3 then (94) else if j = 4 then (71) else (15)) else 0)
