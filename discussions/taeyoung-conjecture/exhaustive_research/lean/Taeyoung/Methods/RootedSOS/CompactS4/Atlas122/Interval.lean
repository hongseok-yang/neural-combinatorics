import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.IntervalChecks0130
...
theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 72 then 16*10368000000000000 else 0) -
      (if r = 0 then 10368000000000000*(s+2*s^2+4*s^3+6*s^4+3*s^5) else 0) := by
  by_cases h : r = 72 <;> by_cases h0 : r = 0 <;>
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    16*(∑ g : Fin 143,
      ...
      10368000000000000*(16*f 72-(s+2*s^2+4*s^3+6*s^4+3*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
