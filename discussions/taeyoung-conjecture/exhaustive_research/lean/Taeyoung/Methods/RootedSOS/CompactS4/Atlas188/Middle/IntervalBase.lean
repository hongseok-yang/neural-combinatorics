...
import Taeyoung.Methods.RootedSOS.AffineIntervalCoefficients
...
theorem target_core : S4Classification.coreGraph6 132 = graph188 :=
...
def polynomialCoefficient (g : Fin 143) (j : Fin 6) : Int :=
  affineIntervalCoefficient 9 1 15 (S4Classification.groupKey g).2
    (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
    (groupTotal g 2 - groupTotal g 3) j

def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 132 then (if j = 0 then 759375*2332800000000000000 else 0) else 0) -
  (if r = 0 then 2332800000000000000 *
    (if j = 0 then (-729) else if j = 1 then (4050) else if j = 2 then (2565) else if j = 3 then (45) else if j = 4 then (285) else (34)) else 0)
