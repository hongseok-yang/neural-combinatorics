...
theorem target_core : S4Classification.coreGraph6 120 = graph174 :=
...
def polynomialCoefficient (g : Fin 143) (j : Fin 6) : Int :=
  affineIntervalCoefficient 7 1 12 (S4Classification.groupKey g).2
    (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
    (groupTotal g 2 - groupTotal g 3) j

def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 120 then (if j = 0 then 248832*33094220544000000000000 else 0) else 0) -
  (if r = 0 then 33094220544000000000000 *
    (if j = 0 then (-238) else if j = 1 then (1954) else if j = 2 then (2132) else if j = 3 then (68) else if j = 4 then (154) else (26)) else 0)
