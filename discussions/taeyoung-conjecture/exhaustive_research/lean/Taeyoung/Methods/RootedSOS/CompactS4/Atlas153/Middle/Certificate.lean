import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.BlockSum0
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.BlockSum1
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.BlockSum2
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.BlockSum3
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.BlockSum4
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.Contraction
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.Interval
...
theorem graphon_bound (W : Graphon Ω μ)
    (hp : (3 : Real)/5 ≤ cliqueDensity 2 W) (hp1 : cliqueDensity 2 W ≤ (2 : Real)/3) :
    target153 (cliqueDensity 2 W) ≤ homDensity graph153 W := by
  let s := (15*cliqueDensity 2 W-9)/1
  have hs : 0 ≤ s := by dsimp [s]; linarith
  have hs1 : s ≤ 1 := by dsimp [s]; linarith
  have hp' : (9+1*s)/15 = cliqueDensity 2 W := by dsimp [s]; ring
  ...
  have ht : 759375*target153 (cliqueDensity 2 W) = (-729)*s^0 + (6480)*s^1 + (5670)*s^2 + (900)*s^3 + (165)*s^4 + (14)*s^5 := by
    rw [← hp']
    unfold target153
    ring
  nlinarith
