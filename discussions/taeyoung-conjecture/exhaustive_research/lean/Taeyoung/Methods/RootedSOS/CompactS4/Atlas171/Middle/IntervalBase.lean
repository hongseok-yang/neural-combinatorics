...
theorem target_core : S4Classification.coreGraph6 117 = graph171 :=
...
def polynomialCoefficient (g : Fin 143) (j : Fin 6) : Int :=
  affineIntervalCoefficient 15 1 24 (S4Classification.groupKey g).2
    (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
    (groupTotal g 2 - groupTotal g 3) j

def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 117 then (if j = 0 then 7962624*2566080000000000000000 else 0) else 0) -
  (if r = 0 then 2566080000000000000000 *
    (if j = 0 then (-2430) else if j = 1 then (45198) else if j = 2 then (19764) else if j = 3 then (2556) else if j = 4 then (426) else (22)) else 0)
