...
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.Coloring
...
theorem target_core : S4Classification.coreGraph6 125 = graph181 :=
...
def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 125 then (if j = 0 then 81*7776000000000000 else 0) else 0) -
  (if r = 0 then 7776000000000000 *
    (if j = 0 then (0) else if j = 1 then (2) else if j = 2 then (13) else if j = 3 then (30) else if j = 4 then (28) else (8)) else 0)
