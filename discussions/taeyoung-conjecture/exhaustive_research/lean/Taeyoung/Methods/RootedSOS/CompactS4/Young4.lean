import Taeyoung.Methods.RootedSOS.CompactS4.Young4Rows00
import Taeyoung.Methods.RootedSOS.CompactS4.Young4Rows13
import Taeyoung.Methods.RootedSOS.CompactS4.Young4Rows26
import Taeyoung.Methods.RootedSOS.SparseFlagCoefficients

namespace Taeyoung.Methods.RootedSOS.CompactS4.Young4

theorem code_identity (i j : Fin 32) : codeIdentity i j := by
  by_cases h : i.1 < 13
  · let k : Fin 13 := ⟨i.1-0, by omega⟩
    have hi : (⟨0+k.1, by omega⟩ : Fin 32) = i := by
      apply Fin.ext
      dsimp [k]
      omega
  ...
  · let k : Fin 13 := ⟨i.1-13, by omega⟩
    have hi : (⟨13+k.1, by omega⟩ : Fin 32) = i := by
      apply Fin.ext
      dsimp [k]
      omega
    simpa only [hi] using code_rows_13 k j
  by_cases h : i.1 < 32
  · let k : Fin 6 := ⟨i.1-26, by omega⟩
    have hi : (⟨26+k.1, by omega⟩ : Fin 32) = i := by
      apply Fin.ext
      dsimp [k]
      omega
...
theorem pulled_exact (g : Fin 143) (i j : Fin 32) :
...
theorem pair_basis_density
    ...
    (i j : Fin 32) (W : Taeyoung.Graphon Ω μ) :
