import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.IntervalChecks0130
...
theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 125 then 81*7776000000000000 else 0) -
      (if r = 0 then 7776000000000000*((0)*s^0 + (2)*s^1 + (13)*s^2 + (30)*s^3 + (28)*s^4 + (8)*s^5) else 0) := by
  by_cases h : r = 125 <;> by_cases h0 : r = 0 <;>
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    81*(∑ g : Fin 143,
      ...
      7776000000000000*(81*f 125-((0)*s^0 + (2)*s^1 + (13)*s^2 + (30)*s^3 + (28)*s^4 + (8)*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
