import Taeyoung.Methods.RootedSOS.CompactS4.Atlas157.BlockSum0
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas157.BlockSum1
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas157.BlockSum2
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas157.BlockSum3
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas157.BlockSum4
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas157.Contraction
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas157.Interval
...
theorem graphon_bound (W : Graphon Ω μ) (hp : (2 : Real)/3 ≤ cliqueDensity 2 W) :
    target157 (cliqueDensity 2 W) ≤ homDensity graph157 W := by
  ...
  have ht := target157_interval s
  rw [hp'] at ht
  have ht' : 81*target157 (cliqueDensity 2 W) = (0)*s^0 + (4)*s^1 + (20)*s^2 + (33)*s^3 + (20)*s^4 + (4)*s^5 := by
    rw [ht]
    ring
  nlinarith

theorem satisfiesLowerBound_157 : Taeyoung.SatisfiesLowerBound graph157 :=
  satisfiesLowerBound_157_of_bound (fun W hp => graphon_bound W hp)

#print axioms satisfiesLowerBound_157
