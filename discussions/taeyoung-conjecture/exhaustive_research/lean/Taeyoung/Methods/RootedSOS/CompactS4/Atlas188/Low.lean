import Taeyoung.Methods.RootedSOS.CompactS4.Atlas188.Coloring

/-! The admissible range `[1/2, 3/5]` of Atlas 188, where the catalogue target is
already nonpositive and no certificate is needed. The compact S4 pieces cover
`[3/5, 2/3]` and `[2/3, 1]`. -/

namespace Taeyoung.Methods.RootedSOS.CompactS4.Atlas188.Low

open MeasureTheory Taeyoung

variable {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} [IsProbabilityMeasure μ]

/-- `target188 p = p * (2p - 1) * (17p³ - 33p² + 22p - 5)` is nonpositive on
`[1/2, 3/5]`. The cubic factor is strictly increasing, because

    (17p³ - 33p² + 22p - 5) + 1/125 = (p - 3/5) * (17 * (p - 57/85)² + 287/425),
...
theorem target_nonpos {p : Real} (hp : (1 : Real)/2 ≤ p) (hp1 : p ≤ (3 : Real)/5) :
  ...
  have hsq : (0 : Real) ≤ 17 * (p - 57/85)^2 := by positivity
  have hQ : 17*p^3 - 33*p^2 + 22*p - 5 ≤ 0 := by
    nlinarith [mul_nonneg hu hsq]
  have h0 : (0 : Real) ≤ p := by linarith
  have h2 : (0 : Real) ≤ 2*p - 1 := by linarith
  have hprod : (0 : Real) ≤ p * (2*p - 1) * (-(17*p^3 - 33*p^2 + 22*p - 5)) :=
    mul_nonneg (mul_nonneg h0 h2) (by linarith)
  unfold target188
  nlinarith [hprod]

/-- The Atlas 188 catalogue bound on `[1/2, 3/5]`, in the statement shape the row
...
theorem graphon_bound (W : Graphon Ω μ)
  ...
  le_trans (target_nonpos hp hp1) (homDensity_nonneg graph188 W)

#print axioms graphon_bound

end Taeyoung.Methods.RootedSOS.CompactS4.Atlas188.Low
