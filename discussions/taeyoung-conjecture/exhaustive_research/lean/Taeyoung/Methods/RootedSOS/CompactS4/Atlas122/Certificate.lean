import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.BlockSum0
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.BlockSum1
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.BlockSum2
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.BlockSum3
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.BlockSum4
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.Contraction
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.Interval
...
theorem graphon_bound (W : Graphon Ω μ) (hp : (1 : Real)/2 ≤ cliqueDensity 2 W) :
    target122 (cliqueDensity 2 W) ≤ homDensity graph122 W := by
  ...
  have ht := target122_interval s
  rw [hp'] at ht
  have ht' : 16*target122 (cliqueDensity 2 W) = s+2*s^2+4*s^3+6*s^4+3*s^5 := by
    rw [ht]
    ring
  nlinarith

theorem satisfiesLowerBound_122 : Taeyoung.SatisfiesLowerBound graph122 :=
  satisfiesLowerBound_122_of_bound (fun W hp => graphon_bound W hp)

#print axioms satisfiesLowerBound_122
