...
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas168.Coloring
...
theorem target_core : S4Classification.coreGraph6 114 = graph168 :=
...
def polynomialCoefficient (g : Fin 143) (j : Fin 6) : Int :=
  affineIntervalCoefficient 3 1 6 (S4Classification.groupKey g).2
    (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
    (groupTotal g 2 - groupTotal g 3) j

def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 114 then (if j = 0 then 7776*186624000000000000 else 0) else 0) -
  (if r = 0 then 186624000000000000 *
    (if j = 0 then (0) else if j = 1 then (0) else if j = 2 then (108) else if j = 3 then (-36) else if j = 4 then (36) else (20)) else 0)
