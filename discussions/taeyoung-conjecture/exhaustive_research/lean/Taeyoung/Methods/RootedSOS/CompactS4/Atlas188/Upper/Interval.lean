import Taeyoung.Methods.RootedSOS.CompactS4.Atlas188.Upper.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas188.Upper.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas188.Upper.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas188.Upper.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas188.Upper.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas188.Upper.IntervalChecks0130
...
theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 132 then 243*23328000000000000 else 0) -
      (if r = 0 then 23328000000000000*((2)*s^0 + (17)*s^1 + (38)*s^2 + (61)*s^3 + (91)*s^4 + (34)*s^5) else 0) := by
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    243*(∑ g : Fin 143,
      ...
      23328000000000000*(243*f 132-((2)*s^0 + (17)*s^1 + (38)*s^2 + (61)*s^3 + (91)*s^4 + (34)*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
