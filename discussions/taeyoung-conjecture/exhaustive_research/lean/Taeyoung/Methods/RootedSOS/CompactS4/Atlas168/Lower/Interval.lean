import Taeyoung.Methods.RootedSOS.CompactS4.Atlas168.Lower.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas168.Lower.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas168.Lower.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas168.Lower.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas168.Lower.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas168.Lower.IntervalChecks0130
...
theorem group_polynomial_eval (g : Fin 143) (s : Real) :
    (∑ j : Fin 6, (polynomialCoefficient g j : Real)*s^j.1) =
      7776*((3+1*s)/6)^(S4Classification.groupKey g).2 *
        ((groupTotal g 0 : Real)+(groupTotal g 1 : Real)*s+
          (groupTotal g 2 : Real)*s^2+(groupTotal g 3 : Real)*s*(1-s)) := by
  rw [show polynomialCoefficient g = affineIntervalCoefficient 3 1 6 (S4Classification.groupKey g).2
      (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
      (groupTotal g 2 - groupTotal g 3) from rfl]
  rw [affineIntervalCoefficient_eval 3 1 6 _ (by decide) (isolated_bound g)]
...
theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 114 then 7776*186624000000000000 else 0) -
      (if r = 0 then 186624000000000000*((0)*s^0 + (0)*s^1 + (108)*s^2 + (-36)*s^3 + (36)*s^4 + (20)*s^5) else 0) := by
  by_cases h : r = 114 <;> by_cases h0 : r = 0 <;>
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    7776*(∑ g : Fin 143,
      ((groupTotal g 0 : Real)+(groupTotal g 1 : Real)*s+
        ...
        (((3+1*s)/6)^(S4Classification.groupKey g).2 * f (representative g))) =
      186624000000000000*(7776*f 114-((0)*s^0 + (0)*s^1 + (108)*s^2 + (-36)*s^3 + (36)*s^4 + (20)*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
