...
import Taeyoung.Methods.RootedSOS.ThirdDegreeFiveCoefficients
...
theorem target_core : S4Classification.coreGraph6 80 = graph130 :=
...
def polynomialCoefficient (g : Fin 143) (j : Fin 6) : Int :=
  thirdDegreeFiveCoefficient (S4Classification.groupKey g).2
    (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
    (groupTotal g 2 - groupTotal g 3) j

def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 80 then (if j = 0 then 243*23328000000000000 else 0) else 0) -
  (if r = 0 then 23328000000000000 *
    (if j = 0 then (8) else if j = 1 then (44) else if j = 2 then (86) else if j = 3 then (73) else if j = 4 then (28) else (4)) else 0)
