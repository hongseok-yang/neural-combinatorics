import Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.IntervalChecks0130
...
theorem group_polynomial_eval (g : Fin 143) (s : Real) :
    (∑ j : Fin 6, (polynomialCoefficient g j : Real)*s^j.1) =
      7962624*((15+1*s)/24)^(S4Classification.groupKey g).2 *
        ((groupTotal g 0 : Real)+(groupTotal g 1 : Real)*s+
          (groupTotal g 2 : Real)*s^2+(groupTotal g 3 : Real)*s*(1-s)) := by
  rw [show polynomialCoefficient g = affineIntervalCoefficient 15 1 24 (S4Classification.groupKey g).2
      (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
      (groupTotal g 2 - groupTotal g 3) from rfl]
  rw [affineIntervalCoefficient_eval 15 1 24 _ (by decide) (isolated_bound g)]
...
theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 117 then 7962624*2566080000000000000000 else 0) -
      (if r = 0 then 2566080000000000000000*((-2430)*s^0 + (45198)*s^1 + (19764)*s^2 + (2556)*s^3 + (426)*s^4 + (22)*s^5) else 0) := by
  by_cases h : r = 117 <;> by_cases h0 : r = 0 <;>
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    7962624*(∑ g : Fin 143,
      ((groupTotal g 0 : Real)+(groupTotal g 1 : Real)*s+
        ...
        (((15+1*s)/24)^(S4Classification.groupKey g).2 * f (representative g))) =
      2566080000000000000000*(7962624*f 117-((-2430)*s^0 + (45198)*s^1 + (19764)*s^2 + (2556)*s^3 + (426)*s^4 + (22)*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
