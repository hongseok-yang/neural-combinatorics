...
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas151.Coloring
...
theorem target_core : S4Classification.coreGraph6 99 = graph151 :=
...
def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 99 then (if j = 0 then 7776*186624000000000000 else 0) else 0) -
  (if r = 0 then 186624000000000000 *
    (if j = 0 then (0) else if j = 1 then (0) else if j = 2 then (216) else if j = 3 then (0) else if j = 4 then (24) else (16)) else 0)
