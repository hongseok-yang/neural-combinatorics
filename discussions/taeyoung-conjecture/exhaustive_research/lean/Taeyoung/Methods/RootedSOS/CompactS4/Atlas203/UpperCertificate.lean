import Taeyoung.Methods.RootedSOS.CompactS4.Atlas203.BlockSum0
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas203.BlockSum1
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas203.BlockSum2
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas203.BlockSum3
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas203.BlockSum4
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas203.Contraction
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas203.Interval
...
theorem upper_graphon_bound (W : Graphon Ω μ) (hp : (3 : Real)/4 ≤ cliqueDensity 2 W) :
    target203 (cliqueDensity 2 W) ≤ homDensity graph203 W := by
  let s := 4*cliqueDensity 2 W-3
  have hs : 0 ≤ s := by dsimp [s]; linarith
  have hs1 : s ≤ 1 := by
    have h := cliqueDensity_le_one 2 W
    dsimp [s]
    linarith
  have hp' : (3+s)/4 = cliqueDensity 2 W := by dsimp [s]; ring
  ...
  have ht := target203_interval s
  rw [hp'] at ht
  have ht' : 256*target203 (cliqueDensity 2 W) = (3)*s^0 + (19)*s^1 + (54)*s^2 + (94)*s^3 + (71)*s^4 + (15)*s^5 := by
    rw [ht]
    ring
  nlinarith
