import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.BlockSum0
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.BlockSum1
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.BlockSum2
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.BlockSum3
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.BlockSum4
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.Contraction
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas181.Interval
...
theorem graphon_bound (W : Graphon Ω μ) (hp : (2 : Real)/3 ≤ cliqueDensity 2 W) :
    target181 (cliqueDensity 2 W) ≤ homDensity graph181 W := by
  ...
  have ht := target181_interval s
  rw [hp'] at ht
  have ht' : 81*target181 (cliqueDensity 2 W) = (0)*s^0 + (2)*s^1 + (13)*s^2 + (30)*s^3 + (28)*s^4 + (8)*s^5 := by
    rw [ht]
    ring
  nlinarith

theorem satisfiesLowerBound_181 : Taeyoung.SatisfiesLowerBound graph181 :=
  satisfiesLowerBound_181_of_bound (fun W hp => graphon_bound W hp)

#print axioms satisfiesLowerBound_181
