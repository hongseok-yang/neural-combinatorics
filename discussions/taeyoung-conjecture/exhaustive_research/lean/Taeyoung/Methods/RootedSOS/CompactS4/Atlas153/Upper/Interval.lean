import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.IntervalChecks0130
...
theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 101 then 243*11664000000000000 else 0) -
      (if r = 0 then 11664000000000000*((4)*s^0 + (34)*s^1 + (76)*s^2 + (68)*s^3 + (47)*s^4 + (14)*s^5) else 0) := by
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    243*(∑ g : Fin 143,
      ...
      11664000000000000*(243*f 101-((4)*s^0 + (34)*s^1 + (76)*s^2 + (68)*s^3 + (47)*s^4 + (14)*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
