import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.BlockSum0
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.BlockSum1
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.BlockSum2
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.BlockSum3
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.BlockSum4
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.Contraction
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Upper.Interval
...
theorem graphon_bound (W : Graphon Ω μ)
    (hp : (2 : Real)/3 ≤ cliqueDensity 2 W) (hp1 : cliqueDensity 2 W ≤ (1 : Real)/1) :
    target153 (cliqueDensity 2 W) ≤ homDensity graph153 W := by
  let s := (3*cliqueDensity 2 W-2)/1
  have hs : 0 ≤ s := by dsimp [s]; linarith
  have hs1 : s ≤ 1 := by dsimp [s]; linarith
  have hp' : (2+1*s)/3 = cliqueDensity 2 W := by dsimp [s]; ring
  ...
  have ht : 243*target153 (cliqueDensity 2 W) = (4)*s^0 + (34)*s^1 + (76)*s^2 + (68)*s^3 + (47)*s^4 + (14)*s^5 := by
    rw [← hp']
    unfold target153
    ring
  nlinarith
