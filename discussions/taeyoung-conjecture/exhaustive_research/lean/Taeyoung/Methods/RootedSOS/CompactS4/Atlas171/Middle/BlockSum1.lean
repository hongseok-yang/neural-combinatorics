...
namespace Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.Block01
...
theorem density_nonneg (W : Taeyoung.Graphon Ω μ) (s : Real) : 0 ≤ density W s := by
  have h := integer_flag_density_nonneg (order := 2) (n := 102)
...
end Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.Block01

namespace Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.Block06
open Finset MeasureTheory
variable {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} [IsProbabilityMeasure μ]
...
theorem density_nonneg (W : Taeyoung.Graphon Ω μ) (s : Real) : 0 ≤ density W s := by
  have h := integer_flag_density_nonneg (order := 1) (n := 52)
    S4Flags.flagLabelGraph S4Flags.flagBranchNeighbors
    (fun a i => (Young31.TInt a i : Real))
    (fun ui k => N (finProdFinEquiv ui) k) (fun i j => G i j)
    (fun ui vj => H (finProdFinEquiv ui) (finProdFinEquiv vj))
...
#print axioms density_nonneg
end Taeyoung.Methods.RootedSOS.CompactS4.Atlas171.Middle.Block06
