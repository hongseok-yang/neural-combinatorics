import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas153.Middle.IntervalChecks0130
...
theorem group_polynomial_eval (g : Fin 143) (s : Real) :
    (∑ j : Fin 6, (polynomialCoefficient g j : Real)*s^j.1) =
      759375*((9+1*s)/15)^(S4Classification.groupKey g).2 *
        ((groupTotal g 0 : Real)+(groupTotal g 1 : Real)*s+
          (groupTotal g 2 : Real)*s^2+(groupTotal g 3 : Real)*s*(1-s)) := by
  rw [show polynomialCoefficient g = affineIntervalCoefficient 9 1 15 (S4Classification.groupKey g).2
      (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
      (groupTotal g 2 - groupTotal g 3) from rfl]
  rw [affineIntervalCoefficient_eval 9 1 15 _ (by decide) (isolated_bound g)]
...
theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 101 then 759375*6065280000000000000 else 0) -
      (if r = 0 then 6065280000000000000*((-729)*s^0 + (6480)*s^1 + (5670)*s^2 + (900)*s^3 + (165)*s^4 + (14)*s^5) else 0) := by
  by_cases h : r = 101 <;> by_cases h0 : r = 0 <;>
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    759375*(∑ g : Fin 143,
      ((groupTotal g 0 : Real)+(groupTotal g 1 : Real)*s+
        ...
        (((9+1*s)/15)^(S4Classification.groupKey g).2 * f (representative g))) =
      6065280000000000000*(759375*f 101-((-729)*s^0 + (6480)*s^1 + (5670)*s^2 + (900)*s^3 + (165)*s^4 + (14)*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
