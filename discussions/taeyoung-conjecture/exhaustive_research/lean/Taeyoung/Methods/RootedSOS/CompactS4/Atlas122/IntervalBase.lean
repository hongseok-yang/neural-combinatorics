...
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas122.Coloring
...
theorem target_core : S4Classification.coreGraph6 72 = graph122 :=
...
def polynomialCoefficient (g : Fin 143) (j : Fin 6) : Int :=
  intervalCoefficient (S4Classification.groupKey g).2
    (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
    (groupTotal g 2 - groupTotal g 3) j

def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 72 then (if j = 0 then 16*10368000000000000 else 0) else 0) -
  (if r = 0 then 10368000000000000 *
    (if j = 0 then 0 else if j = 1 then 1 else if j = 2 then 2
      else if j = 3 then 4 else if j = 4 then 6 else 3) else 0)
