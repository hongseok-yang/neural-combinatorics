import Taeyoung.Methods.RootedSOS.CompactS4.Atlas185.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas185.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas185.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas185.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas185.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas185.IntervalChecks0130
...
theorem group_polynomial_eval (g : Fin 143) (s : Real) :
    (∑ j : Fin 6, (polynomialCoefficient g j : Real)*s^j.1) =
      81*((2+s)/3)^(S4Classification.groupKey g).2 *
        ((groupTotal g 0 : Real)+(groupTotal g 1 : Real)*s+
          (groupTotal g 2 : Real)*s^2+(groupTotal g 3 : Real)*s*(1-s)) := by
  rw [show polynomialCoefficient g = thirdIntervalCoefficient (S4Classification.groupKey g).2
      (groupTotal g 0) (groupTotal g 1 + groupTotal g 3)
      (groupTotal g 2 - groupTotal g 3) from rfl]
  rw [thirdIntervalCoefficient_eval _ (isolated_bound g)]
  push_cast
  ring

theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 129 then 81*7776000000000000 else 0) -
      (if r = 0 then 7776000000000000*((0)*s^0 + (4)*s^1 + (14)*s^2 + (24)*s^3 + (29)*s^4 + (10)*s^5) else 0) := by
  by_cases h : r = 129 <;> by_cases h0 : r = 0 <;>
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    81*(∑ g : Fin 143,
      ...
      7776000000000000*(81*f 129-((0)*s^0 + (4)*s^1 + (14)*s^2 + (24)*s^3 + (29)*s^4 + (10)*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
