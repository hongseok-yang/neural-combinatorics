import Taeyoung.Methods.RootedSOS.CompactS4.Atlas147.Lower.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas147.Lower.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas147.Lower.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas147.Lower.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas147.Lower.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas147.Lower.IntervalChecks0130
...
theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 95 then 7776*15552000000000000 else 0) -
      (if r = 0 then 15552000000000000*((0)*s^0 + (0)*s^1 + (108)*s^2 + (36)*s^3 + (36)*s^4 + (12)*s^5) else 0) := by
  by_cases h : r = 95 <;> by_cases h0 : r = 0 <;>
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    7776*(∑ g : Fin 143,
      ...
      15552000000000000*(7776*f 95-((0)*s^0 + (0)*s^1 + (108)*s^2 + (36)*s^3 + (36)*s^4 + (12)*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
