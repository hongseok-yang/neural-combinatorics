import Taeyoung.Methods.RootedSOS.CompactS4.Atlas174.Middle.BlockSum0
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas174.Middle.BlockSum1
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas174.Middle.BlockSum2
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas174.Middle.BlockSum3
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas174.Middle.BlockSum4
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas174.Middle.Contraction
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas174.Middle.Interval
...
theorem graphon_bound (W : Graphon Ω μ)
    (hp : (7 : Real)/12 ≤ cliqueDensity 2 W) (hp1 : cliqueDensity 2 W ≤ (2 : Real)/3) :
    target174 (cliqueDensity 2 W) ≤ homDensity graph174 W := by
  let s := (12*cliqueDensity 2 W-7)/1
  have hs : 0 ≤ s := by dsimp [s]; linarith
  have hs1 : s ≤ 1 := by dsimp [s]; linarith
  have hp' : (7+1*s)/12 = cliqueDensity 2 W := by dsimp [s]; ring
  ...
  have ht : 248832*target174 (cliqueDensity 2 W) = (-238)*s^0 + (1954)*s^1 + (2132)*s^2 + (68)*s^3 + (154)*s^4 + (26)*s^5 := by
    rw [← hp']
    unfold target174
    ring
  nlinarith
