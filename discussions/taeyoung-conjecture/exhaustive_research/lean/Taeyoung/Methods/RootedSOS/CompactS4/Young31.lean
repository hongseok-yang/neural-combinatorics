import Taeyoung.Methods.RootedSOS.CompactS4.Young31Rows00
import Taeyoung.Methods.RootedSOS.CompactS4.Young31Rows13
import Taeyoung.Methods.RootedSOS.CompactS4.Young31Rows26
import Taeyoung.Methods.RootedSOS.CompactS4.Young31Rows39
import Taeyoung.Methods.RootedSOS.SparseFlagCoefficients

namespace Taeyoung.Methods.RootedSOS.CompactS4.Young31

theorem code_identity (i j : Fin 52) : codeIdentity i j := by
  by_cases h : i.1 < 13
  · let k : Fin 13 := ⟨i.1-0, by omega⟩
    have hi : (⟨0+k.1, by omega⟩ : Fin 52) = i := by
      apply Fin.ext
      dsimp [k]
      omega
  ...
  · let k : Fin 13 := ⟨i.1-13, by omega⟩
    have hi : (⟨13+k.1, by omega⟩ : Fin 52) = i := by
      apply Fin.ext
      dsimp [k]
      omega
    simpa only [hi] using code_rows_13 k j
  by_cases h : i.1 < 39
  · let k : Fin 13 := ⟨i.1-26, by omega⟩
    have hi : (⟨26+k.1, by omega⟩ : Fin 52) = i := by
      apply Fin.ext
      dsimp [k]
      omega
    simpa only [hi] using code_rows_26 k j
  by_cases h : i.1 < 52
  · let k : Fin 13 := ⟨i.1-39, by omega⟩
    have hi : (⟨39+k.1, by omega⟩ : Fin 52) = i := by
      apply Fin.ext
      dsimp [k]
      omega
    simpa only [hi] using code_rows_39 k j
  omega

theorem pulled_exact (g : Fin 143) (i j : Fin 52) :
...
theorem pair_basis_density
    ...
    (i j : Fin 52) (W : Taeyoung.Graphon Ω μ) :
