...
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas199.Coloring
...
theorem target_core : S4Classification.coreGraph6 140 = graph199 :=
...
def targetCoefficient (r : Fin 143) (j : Fin 6) : Int :=
  (if r = 140 then (if j = 0 then 81*7776000000000000 else 0) else 0) -
  (if r = 0 then 7776000000000000 *
    (if j = 0 then (0) else if j = 1 then (4) else if j = 2 then (8) else if j = 3 then (15) else if j = 4 then (38) else (16)) else 0)
