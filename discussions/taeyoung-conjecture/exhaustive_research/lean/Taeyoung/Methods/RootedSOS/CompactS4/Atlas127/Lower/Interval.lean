import Taeyoung.Methods.RootedSOS.CompactS4.Atlas127.Lower.IntervalChecks0000
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas127.Lower.IntervalChecks0026
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas127.Lower.IntervalChecks0052
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas127.Lower.IntervalChecks0078
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas127.Lower.IntervalChecks0104
import Taeyoung.Methods.RootedSOS.CompactS4.Atlas127.Lower.IntervalChecks0130
...
theorem target_coefficient_eval (r : Fin 143) (s : Real) :
    (∑ j : Fin 6, (targetCoefficient r j : Real)*s^j.1) =
      (if r = 77 then 7776*186624000000000000 else 0) -
      (if r = 0 then 186624000000000000*((0)*s^0 + (0)*s^1 + (216)*s^2 + (72)*s^3 + (24)*s^4 + (8)*s^5) else 0) := by
  by_cases h : r = 77 <;> by_cases h0 : r = 0 <;>
...
theorem group_identity (s : Real) (f : Fin 143 → Real) :
    7776*(∑ g : Fin 143,
      ...
      186624000000000000*(7776*f 77-((0)*s^0 + (0)*s^1 + (216)*s^2 + (72)*s^3 + (24)*s^4 + (8)*s^5)*f 0) := by
  calc
    _ = ∑ g : Fin 143,
