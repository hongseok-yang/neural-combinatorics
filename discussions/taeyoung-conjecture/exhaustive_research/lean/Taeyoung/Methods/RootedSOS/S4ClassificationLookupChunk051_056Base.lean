...
import Taeyoung.Methods.RootedSOS.S4ClassificationLookups051_056Data
...
def encodedRow (index : Fin 6) : Nat :=
  (S4ClassificationLookups051_056Data.data[index.1]?).getD 0 |>.natAbs
...
def labelUnion (index : Fin 6) : Nat :=
  (labelUnions[51 + index.1]?).getD 0

def lookupCell (index : Fin 6) (left right : Fin 16) : Nat :=
  encodedRow index / cellBase ^ (16 * left.1 + right.1) % cellBase

def witnessGroup (index : Fin 6) (left right : Fin 16) :
    Fin 143 :=
  ⟨lookupCell index left right % groupBase, Nat.mod_lt _ (by decide)⟩

def witnessPermutation (index : Fin 6) (left right : Fin 16) :
    List (Fin 6) :=
...
def lookupWitnessValid (index : Fin 6)
