...
**Ported verbatim** from the odd-cycle project,
`discussions/goodman-style-bound/lean/OddCycleBound/RegionII/Certificate/Bernstein.lean`,
with only the namespace changed.  The file is self-contained: it imports
Mathlib and nothing else, so the port carries no dependency on that project.

Several catalogue notes certify a scalar lemma by exhibiting the nonnegative
coefficients of a polynomial in a Bernstein basis — `k4_two_edge_tail.tex` an
`8×7` matrix, `triangle_adjacent_leaf_tail.tex` a degree-12 list and an `8×7`
matrix.  Both of those rows were closed by finding a shorter algebraic
certificate instead (see `Methods/K4Tail/Scalar.lean` and `Methods/AdjTail.lean`),
so nothing in the project consumes this file *yet*.  It is here so that the next
note whose Bernstein data does not simplify can be transcribed directly rather
than re-derived.

The usage pattern is: state the target polynomial, exhibit a
`BernsteinCertificate n` with exact `ℚ` coefficients, discharge `check = true`
by `decide +kernel` — kernel reduction, **not** `native_decide`, so the axiom
audit is unaffected — and connect the two by an ordinary `ring` identity.  Then
`sound_for` concludes nonnegativity on `[0,1]^n`.  A box `[a,b]×[c,d]` is
handled by rescaling each coordinate affinely before applying it.
...
the Bernstein basis on the unit cube.  Thus no external acceptance result is a
premise of a mathematical theorem.
-/

namespace Taeyoung.Methods.Certificate
...
end Taeyoung.Methods.Certificate
