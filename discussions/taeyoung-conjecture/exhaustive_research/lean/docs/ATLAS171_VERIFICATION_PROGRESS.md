# Atlas171 compact verification record

Atlas171 is **verified** (2026-09-23). The fresh **98-file** build passed in **42m16s** (2536.266 seconds), at **11.88 GiB** peak process-tree private memory. The installed-source recheck brings combined verification to **42m24s** (2544.079 seconds).
...
- `verification_runs/atlas171/fresh_row_1/manifest.json` and `summary.json`.
- `verification_runs/atlas171/fresh_row_1/catalogue_compile.log`.
- `verification_runs/atlas171/installed_example_1.json` and `.log`.
- `verification_runs/atlas171/completed_row.json`.
...
seconds, of which `Low.lean` took 7.8) and `merged_block_pilot_1` (the heaviest
module `Middle.Block01` alone, 307.0 seconds at 11.90 GiB). The interval,
contraction and right modules were not piloted separately: they are within a
few hundred bytes of Atlas188's, whose measurements were taken two hours
earlier in `verification_runs/atlas188/interval_components_1` and in that row's
accepted run. Those figures projected this row at about 2,540 seconds, against
the 2,536.266 measured.

See [the certificate explanation](../../notes/atlas171_compact_certificate.md).
