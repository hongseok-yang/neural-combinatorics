# Atlas157 compact verification record

Atlas157 is **verified** (2026-09-22). The fresh 89-file build passed in **26m52s** (1611.563 seconds), with **9.20 GiB** peak process-tree private memory. The installed-source recheck passed too, bringing combined verification to **27m00s** (1619.626 seconds).
...
The example and classification are updated; Atlas169 is next.

The full interval is `[2/3,1]`. The row contains 89 files and
9,753,056 source bytes. Every row file was rebuilt sequentially against
...
- `verification_runs/atlas157/fresh_row_1/summary.json` and `manifest.json`.
- `verification_runs/atlas157/installed_example_1.json` and its axiom log.
- `verification_runs/atlas157/completed_row.json`.

See [the certificate explanation](../../notes/atlas157_compact_certificate.md).
