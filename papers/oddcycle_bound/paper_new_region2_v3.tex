...
We now express $\widehat R_m$, $\widehat Q_m$, and $\widehat L_m$ in the normalized variables $\tilde q$, $\tilde M$, and $\tilde p$.
...
For $\widehat Q_m$ and $\widehat L_m$ we need two further ingredients.
The first is that $\Lambda$ itself does not disappear from them.
Multiplying $\Omega_m$ by $\xi$ cancels the factor $(1-2\Lambda)^2/(4\Lambda^2)$ in \eqref{eq:plt23-C-xi-rho}, and multiplying $\Omega_m$ by $\rho$ cancels $M_m(\Lambda-M)$, so that
\[
...
\]
Substituting $\Lambda-q=\Lambda\delta_{\tilde q}$ and $\Lambda-M=\Lambda(1-\tilde M)$ turns these into
\begin{equation}\label{eq:plt23-AB-formulas}
  ...
  =\delta_{\tilde q}(1-\tilde M)\sqrt{2\Lambda}\,
   \frac{M_m}{\Lambda^{m-2}},
\end{equation}
in which $\Lambda$ and $1-2\Lambda$ still occur.
Writing
\[
...
\end{equation}
Two bounds on $\Delta$ will be used repeatedly.
First,
...
The second ingredient is the two coefficients of \Cref{prop:plt23-splitting-lower-bound}.
...
Dividing \eqref{eq:plt23-frontier-chart} by $\delta_{\tilde q}$ gives $\zeta_q\ge\tilde q>3/4$, and substituting \eqref{eq:plt23-zeta-v-inverse} into it gives
...
The third is a common factor: each of $\widehat R_m$, $\widehat Q_m$, and $\widehat L_m$ carries a factor $\delta_{\tilde q}$, by \eqref{eq:plt23-F-T} and \eqref{eq:plt23-AB-formulas}, so we set
\[
...
\begin{equation}\label{eq:plt23-GXY-ratios}
  \frac{\widehat R_m}{\widehat Q_m}=\frac GX,
  \qquad
  \frac{\widehat R_m}{\widehat L_m}=\frac GY.
\end{equation}
...
\begin{proof}
  ...
  \]
  Since $\zeta_q>3/4$ and $\tilde p\le3/2$,
  \[
    z>\frac12.
  \]
  ...
  \begin{align*}
    X
    &=\frac{2\Lambda^3\delta_{\tilde q}}{(1-2\Lambda)^2}\,\widehat{\Lambda}_m\\
    &=\frac{2\tilde p^2(1+z)}
            {\Delta\,\delta_{\tilde p}\,(\delta_{\tilde p}+\tilde pz)^2}\,\widehat{\Lambda}_m.
  \end{align*}
  Indeed,
  \[
  ...
  \]
  By \eqref{eq:plt23-KA-def} and \eqref{eq:plt23-KA-sum}, $\widehat{\Lambda}_m\ge m\delta_{\tilde p}S_m$.
  ...
  \]
  which is positive because $z\ge1/2$, hence $2\tilde pz\ge1$, while $\delta_{\tilde p}^2(\delta_{\tilde p}+2)\le5/8$.
  ...
  \begin{align*}
    Y
    &=(1-\tilde M)\sqrt{2\Lambda}\,\widehat M_m\\
    &\ge
...
\[
  ...
  \qquad 0<\zeta_p<1,
\]
the same ratio as $\zeta_q$ but over the other gap.
...
\begin{proof}
  ...
  From \eqref{eq:plt23-AB-formulas}, \eqref{eq:plt23-Delta-identities}, $\delta_{\tilde q}=\delta_{\tilde p}(1-\zeta_p)/\tilde p$, and \eqref{eq:plt23-KA-def}--\eqref{eq:plt23-KA-sum},
  \[
    X
  ...
  Comparing the two inequalities, and substituting $\delta_{\tilde q}=\delta_{\tilde p}(1-\zeta_p)/\tilde p$ and $\tilde M^2=\delta_{\tilde p}\zeta_p$, yields
  \[
  ...
  \end{equation}
  For $m\ge11$, the left side is at least $(3/2)^3=27/8>3$, whereas the right side is less than $3$.
...
\begin{proof}
  The claim is trivial if $\widehat R_m\le0$, so assume $\widehat R_m>0$.
  ...
  Since $\tilde M^2<\delta_{\tilde p}$, it is enough to put
  \[
