...
\begin{enumerate}[label=\textbf{Section \arabic*.},leftmargin=4.6em,start=2]
  ...
  Writing $F_m=R_m/\Lambda^m$, what has to be proved is that $F_m\le\mathcal B\,d(\theta)$ holds in that region for every odd $m\ge9$.
  ...
  \begin{itemize}[leftmargin=1.6em,topsep=2pt,itemsep=1pt]
    \item For $v\le1/2$: $2F_m\le\mathcal A$ and $F_m/\mathcal A+2F_m/\mathcal B\le2$ (\Cref{prop:plt23-low-v}).
    \item For $v\ge1/2$: $F_m\le\mathcal A$ and $2F_m\le\mathcal B$ (\Cref{prop:plt23-high-v-A,prop:plt23-high-v-B}).
  \end{itemize}
  In both halves the first bound settles $\theta\le1/2$ and the second settles $\theta\ge1/2$, which gives $F_m\le\mathcal B\,d(\theta)$ (\Cref{prop:plt23-scalar}).
  All four bounds are proved by elementary analysis; the only inputs are two upper bounds for $F_m$, one bound for a product of elementary factors, and one comparison between a geometric sum and a power.
\end{enumerate}
...
Dividing both sides of \eqref{eq:plt23-scalar-target} by $\Lambda^m$ and separating the two terms of the dual expression, define
\begin{equation}\label{eq:plt23-AB-theta}
  ...
  \theta:=\rho\xi=\frac{\mathcal A}{\mathcal B},
  \qquad
  F_m:=\frac{R_m}{\Lambda^m},
\end{equation}
of which the first three are positive.
Keeping $\mathcal A$ and $\mathcal B$ simultaneously, rather than choosing one value of $\lambda$ in \eqref{eq:plt23-psi-dual} and then splitting the parameter region according to which value was chosen, already reduces \eqref{eq:plt23-scalar-target} to a single inequality.

\begin{proposition}[Reduction to a normalized inequality]\label{prop:plt23-reduction}
  Let $m\ge9$ be odd, let $q$ and $\Lambda$ satisfy \eqref{eq:plt23-range} and \eqref{eq:plt23-Lambda-above-q}, and let
  \begin{equation}\label{eq:plt23-dtheta}
    ...
    \end{cases}
  \end{equation}
  If $F_m\le\mathcal B\,d(\theta)$, then \eqref{eq:plt23-scalar-target} holds.
\end{proposition}
...
The remaining subsections verify the hypothesis $F_m\le\mathcal B\,d(\theta)$, and for that we express $F_m$, $\mathcal A$, and $\mathcal B$ in variables adapted to the problem.
...
\]
By \eqref{eq:plt23-M-Lambda-order} and \Cref{lem:plt23-forced-variance},
\[
...
\[
  ...
  1<\tilde p<2.
\]
We also need the consequence of the upper bound \eqref{eq:plt23-alpha-ceiling-poly} on $\Lambda$.
Since $p+q=1$, $q=\tilde q\Lambda$, and $p=(1+\tilde M^2)\Lambda/\tilde q$,
\[
  \Lambda=\frac{\tilde q}{1+\tilde q^2+\tilde M^2},
  \qquad
  q=\frac{\tilde q^2}{1+\tilde q^2+\tilde M^2},
\]
so \eqref{eq:plt23-alpha-ceiling-poly} is equivalent to
\begin{equation}\label{eq:plt23-frontier-chart}
  \tilde M^2\ge \tilde q(1-\tilde q)=\tilde qu,
\end{equation}
while $q>1/3$ is equivalent to $1+\tilde q^2+\tilde M^2<3\tilde q^2$, that is, to
...
The bound \eqref{eq:plt23-frontier-chart} becomes
\[
  \zeta(\zeta+v)\ge1.
\]
In particular,
...
Since $M=\tilde M\Lambda$, $q=\tilde q\Lambda$, and $p=\tilde p\Lambda=(1+\tilde M^2)\Lambda/\tilde q$, the definition $R_m=\Lambda^m+M^m-pq^{m-1}$ gives
...
Put
\[
  T_m:=(m-2)-\zeta\bigl(\tilde q^{m-2}-\tilde M^{m-2}\bigr).
\]
Because $1-\tilde q^{m-2}\le(m-2)(1-\tilde q)=(m-2)u$ and $\tilde M^2=\zeta u$,
\begin{equation}\label{eq:plt23-F-T}
  F_m\le uT_m.
\end{equation}
Thus $F_m\le0$ already proves \eqref{eq:plt23-scalar-target}, because $\psi(\xi,\rho)\ge0$.
...
\[
  \widehat{\Lambda}_m:=\frac{\Lambda_m}{\Lambda^{m-2}},
  \qquad
  \widehat M_m:=\frac{M_m}{\Lambda^{m-2}}.
\]
...
For brevity set
\[
  S_m:=\sum_{j=0}^{(m-3)/2}\tilde p^{2j}.
\]
There are $(m-1)/2$ summands and their geometric mean is $\tilde p^{(m-3)/2}$.
...
\]
Substituting $\Lambda-q=\Lambda u$, $\Lambda-M=\Lambda(1-\tilde M)$, $\Lambda_m=\Lambda^{m-2}\widehat{\Lambda}_m$, and $M_m=\Lambda^{m-2}\widehat M_m$ turns these into
\begin{equation}\label{eq:plt23-AB-formulas}
  \mathcal A
  =\frac{2\Lambda^3u^2}{(1-2\Lambda)^2}\,\widehat{\Lambda}_m,
  \qquad
  \mathcal B
  =u(1-\tilde M)\sqrt{2\Lambda}\,\widehat M_m.
\end{equation}
Each of $F_m$, $\mathcal A$, and $\mathcal B$ carries a factor $u$, so we also set
\[
  G:=\frac{F_m}{u},
  \qquad
  X:=\frac{\mathcal A}{u},
...
\begin{equation}\label{eq:plt23-GXY-ratios}
  \frac{F_m}{\mathcal A}=\frac GX,
  \qquad
  \frac{F_m}{\mathcal B}=\frac GY.
\end{equation}
...
\begin{lemma}[Defect bounds]\label{lem:plt23-T-bounds}
  ...
  \begin{align}
    T_m
    &\le (m-2)(1+v)-\zeta\bigl(1-v^{(m-2)/2}\bigr),
...
\begin{proof}
  For the first inequality, Bernoulli gives $\tilde q^{m-2}=(1-u)^{m-2}\ge1-(m-2)u$.
  Also $\tilde M^2<v$ by \eqref{eq:plt23-zeta-v-consequences}, hence $\tilde M^{m-2}<v^{(m-2)/2}$ and $\zeta u=\tilde M^2<v$.
  Therefore
  \begin{align*}
    T_m
    &=(m-2)-\zeta\bigl(\tilde q^{m-2}-\tilde M^{m-2}\bigr)\\
    &\le (m-2)-\zeta+(m-2)\zeta u+\zeta\tilde M^{m-2}\\
    &\le (m-2)(1+v)-\zeta\bigl(1-v^{(m-2)/2}\bigr).
  \end{align*}
  ...
  \[
    \zeta\bigl(\tilde q^{m-2}-\tilde M^{m-2}\bigr)>(m-2)\tilde M^{m-1},
  \]
...
\begin{proposition}[The low-$v$ bounds]\label{prop:plt23-low-v}
  ...
  \begin{align}
    ...
    \frac{F_m}{\mathcal A}+2\frac{F_m}{\mathcal B}&\le2.
    \label{eq:plt23-low-v-both}
  \end{align}
\end{proposition}

\begin{proof}
  If $F_m\le0$, both inequalities are immediate.
  ...
  \begin{equation}\label{eq:plt23-G-low-v}
    ...
    P:=(m-2)-cz.
  \end{equation}
  Since $F_m>0$, the left side is positive, so $P>0$.
  ...
  \begin{align*}
    X
    &=\frac{2\Lambda^3u}{(1-2\Lambda)^2}\,\widehat{\Lambda}_m\\
    &=\frac{2\tilde p^2(1+z)}
            {\Delta\,v\,(v+\tilde pz)^2}\,\widehat{\Lambda}_m.
  \end{align*}
  Indeed,
  \[
    v^2+\tilde M^2
  ...
  \]
  By \eqref{eq:plt23-KA-def} and \eqref{eq:plt23-KA-sum}, $\widehat{\Lambda}_m\ge mvS_m$.
  ...
  \end{equation}
  Now $v\le1/2$ gives $c\ge1-2^{-(m-2)/2}$.
  ...
  \begin{align}
   ...
   &\ge
   \frac{m-2}{m}\sqrt{1+\frac{y^2}{2}}
   =\frac{m-2}{m}\sqrt{\frac{v+2}{2}}.
   \label{eq:plt23-product-applied}
  \end{align}
  Also
  \[
    c=1-v^{(m-2)/2}\ge\frac{m-2}{m}.
  \]
  Indeed, $v^{(m-2)/2}\le2^{-(m-2)/2}\le2/m$ for $m\ge9$.
  Therefore
...
\begin{proof}
  Again the claim is immediate if $F_m\le0$, so assume \eqref{eq:plt23-F-positive}.
  ...
  \begin{equation}\label{eq:plt23-X-high-v-lower}
    X>
    \frac{2mS_m(1-\tau)}{\tau\tilde p(v+2)}.
  \end{equation}

  The positivity of $F_m$ supplies a useful lower bound on $1-\tau$.
...
\begin{proof}
  The claim is trivial if $F_m\le0$, so assume $F_m>0$.
...
\begin{proof}
  If $F_m=R_m/\Lambda^m\le0$, the claim is immediate.
  Assume $F_m>0$, and recall $\theta=\mathcal A/\mathcal B$.
  By \Cref{prop:plt23-reduction} it is enough to prove $F_m\le\mathcal B\,d(\theta)$.
  ...
  \]
  Thus $F_m\le\mathcal B\,d(\theta)$ in this range.
  ...
  Hence $F_m\le\mathcal B\,d(\theta)$ again, and \Cref{prop:plt23-reduction} proves \eqref{eq:plt23-scalar-target}.
\end{proof}
