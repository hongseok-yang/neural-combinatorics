...
\subsection{A global consequence of the dual formula}

We retain all notation from \Cref{thm:plt23-huber-elim} and \Cref{prop:plt23-huber-dual}, so that the remaining scalar target is \eqref{eq:plt23-scalar-target}, where
\[
  \psi(\xi,\rho)
  =\max_{0\le \lambda\le 1}
    \left\{\lambda\xi-\frac{\lambda^2}{4(\rho+\lambda)}\right\}.
\]
The point of the next normalization is to keep the two natural scales in this dual expression simultaneously, rather than choosing one value of $\lambda$ and then splitting the parameter region according to which value was chosen.
...
\begin{equation}\label{eq:plt23-AB-theta}
  ...
  \theta:=\rho\xi=\frac{\mathcal A}{\mathcal B}.
\end{equation}
All three quantities are positive.
Dividing the dual objective by $\xi$ gives
\begin{align}
  ...
  \label{eq:plt23-dual-drop-xi}
\end{align}
The inequality holds because $\theta+\lambda\xi\ge\theta$, so replacing the denominator by $\theta$ makes the subtracted term larger.
Put
\begin{equation}\label{eq:plt23-dtheta-def}
  d(\theta)
  :=\max_{0\le\lambda\le1}
       \left\{\lambda-\frac{\lambda^2}{4\theta}\right\}.
\end{equation}
The maximizer is $\lambda=2\theta$ when $2\theta\le1$, and $\lambda=1$ when $2\theta\ge1$.
Hence
\begin{equation}\label{eq:plt23-dtheta-formula}
  d(\theta)
  =
  \begin{cases}
...
\end{equation}
Consequently,
\begin{equation}\label{eq:plt23-master-dual-lower}
  \frac{\Omega_m\psi(\xi,\rho)}{\Lambda^m}
  \ge \mathcal B\,d(\theta).
\end{equation}
The rest of the proof shows directly that $R_m/\Lambda^m$ is at most the right side of \eqref{eq:plt23-master-dual-lower}.

\subsection{Dimensionless variables}\label{subsec:plt23-variables}
...
\begin{equation}\label{eq:plt23-sigma-a-ell}
...
\]
so
\begin{equation}\label{eq:plt23-basic-domain}
  0<\ell<a<1,
  \qquad
  1<\sigma<2.
\end{equation}
...
\[
  D:=1+a^2+\ell^2,
\]
one has $\Lambda=a/D$ and $q=a^2/D$.
Therefore the frontier ceiling is equivalent to
\begin{equation}\label{eq:plt23-frontier-chart}
  \ell^2\ge a(1-a)=au.
\end{equation}
Moreover, $q>1/3$ gives $D<3a^2$.
Since $D\ge1+a$ by \eqref{eq:plt23-frontier-chart},
\[
  3a^2-a-1>0,
\]
and hence
\begin{equation}\label{eq:plt23-a-threequarters}
...
\end{equation}
Indeed, $\ell^2=\zeta u$, while $\sigma=(1+\ell^2)/a$ and $a=1-u$ give $1+v=(1+\zeta u)/(1-u)$, which is equivalent to the first identity in \eqref{eq:plt23-zeta-v-inverse}; the other two follow immediately.
...
\begin{equation}\label{eq:plt23-zeta-v-consequences}
  ...
  \ell^2<v.
\end{equation}
The last inequality follows directly from $\ell^2=\zeta v/H_\zeta$ and $H_\zeta>\zeta$.

It is also useful to express $\Lambda$ without $a$.
Put
\begin{equation}\label{eq:plt23-Delta-def}
  \Delta:=\sigma^2+1+\ell^2.
\end{equation}
Since $p+q=1$, $p=\sigma\Lambda$, and $q=a\Lambda=(1+\ell^2)\Lambda/\sigma$,
\begin{equation}\label{eq:plt23-Delta-identities}
  \Lambda=\frac{\sigma}{\Delta},
  \qquad
  q=\frac{1+\ell^2}{\Delta},
  \qquad
  1-2\Lambda=\frac{v^2+\ell^2}{\Delta}.
\end{equation}
...
\begin{equation}\label{eq:plt23-Delta-upper-sigma}
  \Delta<\sigma(v+2),
\end{equation}
for this is equivalent to $1+\ell^2<\sigma$, that is, to $\ell^2<v$.
Second, since $q>1/3$,
\begin{equation}\label{eq:plt23-Delta-upper-three}
  \Delta<3(1+\ell^2).
\end{equation}
Finally, $q<\Lambda$ and $p=\sigma\Lambda$ imply
\begin{equation}\label{eq:plt23-Lambda-lower}
  1=p+q<(\sigma+1)\Lambda=(v+2)\Lambda,
  \qquad\text{so}\qquad
  \Lambda>\frac1{v+2}.
\end{equation}

\subsection{Normalized defect and the two scales}

Define the normalized defect
\begin{equation}\label{eq:plt23-F-def}
  F_N:=\frac{R_m}{\Lambda^m}.
\end{equation}
Since $m=N+2$, $M=\ell\Lambda$, $q=a\Lambda$, and $p=\sigma\Lambda=(1+\ell^2)\Lambda/a$, the definition $R_m=\Lambda^m+M^m-pq^{m-1}$ gives
...
\end{equation}
Because $1-a^N\le N(1-a)=Nu$ and $\ell^2=\zeta u$,
...
\begin{equation}\label{eq:plt23-normalized-coeffs}
...
\begin{align}
  ...
  &
  K_\Lambda
  &:=\frac{\sigma^{N+1}-1}{\sigma+1},
  \label{eq:plt23-KA-def}\\
  \widehat M_N
  &=2\ell^N+(N+2)K_M,
  &
  K_M
  &:=\frac{\sigma^{N+1}-\ell^{N+1}}{\sigma+\ell}.
  \label{eq:plt23-KL-def}
\end{align}
Since $N+1$ is even,
\begin{align}
  K_\Lambda
  &=v\sum_{j=0}^{(N-1)/2}\sigma^{2j},
...
\end{align}
For brevity set
\begin{equation}\label{eq:plt23-SN-def}
  S_N:=\sum_{j=0}^{(N-1)/2}\sigma^{2j}.
\end{equation}
There are $(N+1)/2$ summands and their geometric mean is $\sigma^{(N-1)/2}$.
Hence the arithmetic--geometric mean inequality gives
\begin{equation}\label{eq:plt23-SN-AMGM}
  S_N\ge \frac{N+1}{2}\sigma^{(N-1)/2}.
\end{equation}

The definitions \eqref{eq:plt23-C-xi-rho}--\eqref{eq:plt23-rho-def} of $\Omega_m$, $\xi$, and $\rho$ give closed forms for the two quantities in \eqref{eq:plt23-AB-theta}.
...
\begin{equation}\label{eq:plt23-Omega-products}
...
\begin{equation}\label{eq:plt23-Omega-rho-xi-squared}
  \Omega_m\rho\xi^2=\frac{2\Lambda^3\Lambda_m(\Lambda-q)^2}{(1-2\Lambda)^2}.
\end{equation}
Substituting $\Lambda-q=\Lambda u$, $\Lambda-M=\Lambda(1-\ell)$, $\Lambda_m=\Lambda^N\widehat{\Lambda}_N$, $M_m=\Lambda^N\widehat M_N$, and $m=N+2$ turns these into
\begin{equation}\label{eq:plt23-AB-formulas}
  \mathcal A
  =\frac{2\Lambda^3u^2}{(1-2\Lambda)^2}\,\widehat{\Lambda}_N,
  \qquad
  \mathcal B
  =u(1-\ell)\sqrt{2\Lambda}\,\widehat M_N.
\end{equation}

Finally define
\begin{equation}\label{eq:plt23-GXY-def}
  G:=\frac{F_N}{u},
  \qquad
  X:=\frac{\mathcal A}{u},
  \qquad
...
\begin{proof}
  For the first inequality, Bernoulli gives $a^N=(1-u)^N\ge1-Nu$.
  Also $\ell^2<v$ by \eqref{eq:plt23-zeta-v-consequences}, hence $\ell^N<v^{N/2}$ and $\zeta u=\ell^2<v$.
  Therefore
  \begin{align*}
    T_N
    &=N-\zeta(a^N-\ell^N)\\
    &\le N-\zeta+N\zeta u+\zeta\ell^N\\
    &\le N(1+v)-\zeta\bigl(1-v^{N/2}\bigr).
  \end{align*}

  For the second inequality, $D<3a^2$ gives
  \[
    \ell^2<2a^2-1<(2a-1)^2,
  \]
  so $\ell<2a-1$, equivalently
  \begin{equation}\label{eq:plt23-a-minus-ell}
    a-\ell>1-a=u.
  \end{equation}
  ...
  \[
    ...
    >Nu\ell^{N-1}.
  \]
  Multiplying by $\zeta=\ell^2/u$ yields
  \[
    \zeta(a^N-\ell^N)>N\ell^{N+1},
  \]
...
\begin{proof}
  ...
  \begin{equation}\label{eq:plt23-hprime}
    h'(y)
    =\frac{y\,\mathcal Q(y)}
           {4(y-1)(y^2-y+1)},
    \qquad
  ...
  \begin{equation}\label{eq:plt23-Hcal-def}
    \mathcal H_N(y)
    :=\frac{N+2}{N}
  ...
  \begin{equation}\label{eq:plt23-Psi-small-y}
    \Psi_N(y)
    \ge
  ...
  For odd $N\ge7$,
  \begin{equation}\label{eq:plt23-Hcal-ratio}
  ...
  \begin{equation}\label{eq:plt23-Psi-seven}
...
\begin{proof}
  ...
  Put
  \begin{equation}\label{eq:plt23-z-c-def}
    z:=\frac\zeta\sigma,
    \qquad
    c:=1-v^{N/2}.
  \end{equation}
  Since $\zeta>3/4$ and $\sigma\le3/2$,
  \begin{equation}\label{eq:plt23-z-half}
    z>\frac12.
  \end{equation}
  Because $\zeta=\sigma z$ and $1+v=\sigma$, \eqref{eq:plt23-zeta-v-inverse} becomes
  \begin{equation}\label{eq:plt23-low-v-uell}
    u=\frac{v}{\sigma(1+z)},
    \qquad
    \ell^2=\frac{vz}{1+z}.
  \end{equation}
  ...
  From \eqref{eq:plt23-AB-formulas}, \eqref{eq:plt23-Delta-identities}, and \eqref{eq:plt23-low-v-uell},
  \begin{align}
    X
    &=\frac{2\Lambda^3u}{(1-2\Lambda)^2}\,\widehat{\Lambda}_N\notag\\
    &=\frac{2\sigma^2(1+z)}
            {\Delta\,v\,(v+\sigma z)^2}\,\widehat{\Lambda}_N.
    \label{eq:plt23-X-low-v}
  \end{align}
  Indeed,
  \[
    v^2+\ell^2
    =v\left(v+\frac{z}{1+z}\right)
    =\frac{v(v+\sigma z)}{1+z}.
  \]
  ...
  \begin{equation}\label{eq:plt23-X-low-v-1}
    X
    >\frac{2\sigma(N+2)S_N(1+z)}
            {(v+2)(v+\sigma z)^2}.
  \end{equation}
  We claim
  \begin{equation}\label{eq:plt23-low-v-denominator}
    (v+2)(v+\sigma z)^2
    \le2\sigma^3z(1+z).
  \end{equation}
  The difference between the right and left sides is
  \[
    v\sigma^2z^2+2\sigma z-v^2(v+2),
  \]
  which is positive because $z\ge1/2$, hence $2\sigma z\ge1$, while $v^2(v+2)\le5/8$.
  Thus \eqref{eq:plt23-X-low-v-1} implies
  \begin{equation}\label{eq:plt23-X-low-v-2}
    X\ge\frac{(N+2)S_N}{\sigma^2z}.
  \end{equation}
  ...
  \begin{equation}\label{eq:plt23-G-over-X}
    \frac GX
    \le
    \frac{\sigma^3zP}{(N+2)S_N}
    \le
    \frac{2\sigma^{3-k}zP}{(N+1)(N+2)}
    \le
    \frac{2zP}{(N+1)(N+2)}.
  \end{equation}
  ...
  \begin{align}
    Y
    &=(1-\ell)\sqrt{2\Lambda}\,\widehat M_N\notag\\
    &\ge
    (N+2)(1-\ell)(\sigma-\ell)\sigma^{N-1}
    \sqrt{\frac2{v+2}},
    \label{eq:plt23-Y-low-v}
  \end{align}
  ...
  \begin{equation}\label{eq:plt23-2G-over-Y}
    ...
    \frac{2P}{N+2}
    \frac{\sqrt{(v+2)/2}}
         {(1-\ell)(\sigma-\ell)\sigma^{N-2}}.
  \end{equation}
  Put $y=\sqrt v$, so $0<y\le1/\sqrt2$.
  Since $\ell\le y$ and $x\mapsto(1-x)(\sigma-x)$ is decreasing on $[0,y]$, \Cref{lem:plt23-product} gives
  \begin{align}
   (1-\ell)(\sigma-\ell)\sigma^{N-2}
   &\ge
   (1-y)(1-y+y^2)(1+y^2)^{N-2}\notag\\
  ...
  \end{align}
  Also
  \begin{equation}\label{eq:plt23-c-lower}
  ...
  \begin{equation}\label{eq:plt23-2G-over-Y-final}
    \frac{2G}{Y}
    \le\frac{2(N+2-z)}{N+2}.
  \end{equation}
...
Here the admissibility condition $q>1/3$ forces $\ell$ to be sufficiently close to $\sqrt v$; this is exactly what compensates for the apparently weak factors in $\mathcal A$ and $\mathcal B$.

Put
\begin{equation}\label{eq:plt23-tau-def}
  \tau:=\frac{\ell^2}{v},
  \qquad 0<\tau<1.
\end{equation}
Since $\ell^2=v\tau$, the inequality \eqref{eq:plt23-Delta-upper-three} gives
\[
  (1+v)^2+1+v\tau<3(1+v\tau),
\]
so
\begin{equation}\label{eq:plt23-tau-lower}
...
\begin{proof}
  ...
  From \eqref{eq:plt23-AB-formulas}, \eqref{eq:plt23-Delta-identities}, $u=v(1-\tau)/\sigma$, and \eqref{eq:plt23-KA-def}--\eqref{eq:plt23-KA-sum},
  \begin{equation}\label{eq:plt23-X-high-v}
    X
    =\frac{2\sigma^2(1-\tau)}{\Delta(v+\tau)^2}
     \left(
       \frac{2\ell^N}{v}+(N+2)S_N
     \right).
  \end{equation}
  ...
  \begin{equation}\label{eq:plt23-X-high-v-lower}
    X>
    \frac{2(N+2)S_N(1-\tau)}{\tau\sigma(v+2)}.
  \end{equation}

  The positivity of $F_N$ supplies a useful lower bound on $1-\tau$.
  From \eqref{eq:plt23-F-formula},
  \[
    (1+\ell^2)a^N<1+\ell^{N+2},
  \]
  so
  \begin{equation}\label{eq:plt23-aN-upper}
    a^N
    <1-\frac{\ell^2}{1+\ell^2}(1-\ell^N).
  \end{equation}
  On the other hand, Bernoulli gives $a^N=(1-u)^N\ge1-Nu$.
  Comparing the two inequalities, and substituting $u=v(1-\tau)/\sigma$ and $\ell^2=v\tau$, yields
  \begin{equation}\label{eq:plt23-one-minus-tau}
    N(1+\ell^2)(1-\tau)
    >\tau\sigma(1-\ell^N).
  \end{equation}
  ...
  Combining \eqref{eq:plt23-X-high-v-final}, \eqref{eq:plt23-SN-AMGM}, and \eqref{eq:plt23-ratio-powers}, and using $1+\ell^2\le\sigma$, it is enough for $X\ge G$ that
...
The only auxiliary fact is a simple comparison between a geometric sum and a power of $1+x^2$.
...
\begin{proof}
  ...
  \begin{equation}\label{eq:plt23-geometric-lhs}
    \frac3{10}C^2.
  \end{equation}
  On the other hand, since $1+x\le2$ and $y\ge1/2$,
  \begin{equation}\label{eq:plt23-geometric-rhs}
...
\begin{proof}
  ...
  \begin{equation}\label{eq:plt23-Y-high-v}
    Y\ge
    (N+2)(1-\ell)(\sigma-\ell)\sigma^{N-1}
    \sqrt{\frac2{v+2}}.
  \end{equation}
  ...
  \begin{equation}\label{eq:plt23-high-v-B-sufficient}
    ...
    \sum_{j=0}^N\ell^j,
  \end{equation}
  where we cancelled the positive factor $1-\ell$ using $1-\ell^{N+1}=(1-\ell)\sum_{j=0}^N\ell^j$.

  For fixed $v$, the left side of \eqref{eq:plt23-high-v-B-sufficient} decreases with $\ell$, while the right side increases.
  Since $\ell^2<v$, it is enough to put
  \[
    x:=\sqrt v,
    \qquad
    \ell=x,
...
\begin{proof}
  ...
  \begin{equation}\label{eq:plt23-low-v-min}
    \frac{F_N}{\mathcal B}
    \le
    \min\left\{
      \frac\theta2,
  ...
  \begin{equation}\label{eq:plt23-dtheta-compare}
    1-\frac1{4\theta}-\frac{2\theta}{1+2\theta}
    =\frac{2\theta-1}{4\theta(1+2\theta)}\ge0.
  \end{equation}
  Thus in all cases
  \begin{equation}\label{eq:plt23-F-Bd}
    F_N\le\mathcal B\,d(\theta).
  \end{equation}

  Now suppose $1/2\le v<1$.
  \Cref{prop:plt23-high-v-A,prop:plt23-high-v-B} give
  \begin{equation}\label{eq:plt23-high-v-min}
    \frac{F_N}{\mathcal B}
    \le\min\{\theta,1/2\}.
  \end{equation}
  If $\theta\le1/2$, the right side is at most $\theta=d(\theta)$.
  If $\theta\ge1/2$, it is at most $1/2\le d(\theta)$.
  Hence \eqref{eq:plt23-F-Bd} again holds.

  Finally, \eqref{eq:plt23-master-dual-lower} gives
  \[
    F_N
    \le\mathcal B\,d(\theta)
    \le\frac{\Omega_m\psi(\xi,\rho)}{\Lambda^m}.
  \]
  Multiplying by $\Lambda^m$ proves \eqref{eq:plt23-scalar-target}.
\end{proof}
