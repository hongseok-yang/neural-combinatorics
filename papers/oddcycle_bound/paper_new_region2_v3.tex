...
Unlike \eqref{eq:plt23-F-formula}, these are not yet written in the tilde variables: they retain $\Lambda$ and $1-2\Lambda$, and the two coefficients $\Lambda_m/\Lambda^{m-2}$ and $M_m/\Lambda^{m-2}$ of \Cref{prop:plt23-splitting-lower-bound}.
...
\end{equation}
These clear $\Lambda$ from $\widehat Q_m$, where it occurs rationally.
In $\widehat L_m$ it occurs as $\sqrt{2\Lambda}$, which no such identity removes, and which we bound below instead, using
\begin{equation}\label{eq:plt23-Delta-upper-sigma}
  \Delta<\tilde p(\delta_{\tilde p}+2),
\end{equation}
which is equivalent to $1+\tilde M^2<\tilde p$, that is, to $\tilde M^2<\delta_{\tilde p}$; with $\Lambda=\tilde p/\Delta$ it gives
\begin{equation}\label{eq:plt23-Lambda-lower}
  \Lambda>\frac1{\delta_{\tilde p}+2}.
\end{equation}
Also, since $q>1/3$,
...
For the two coefficients, a direct substitution into the definitions of $k_m$, $\Lambda_m$, and $M_m$ gives
\begin{align}
  \frac{\Lambda_m}{\Lambda^{m-2}}
...
\end{align}
There are $(m-1)/2$ summands in $S_m$ and their geometric mean is $\tilde p^{(m-3)/2}$, so the arithmetic--geometric mean inequality gives
...
\begin{proof}
  ...
  From \eqref{eq:plt23-Qhat-formula}, \eqref{eq:plt23-Delta-identities}, and \eqref{eq:plt23-low-v-u-Mtilde},
  \begin{align*}
    X
    &=\frac{2\Lambda^3\delta_{\tilde q}}{(1-2\Lambda)^2}\,
      \frac{\Lambda_m}{\Lambda^{m-2}}\\
    &=\frac{2\tilde p^2(1+z)}
  ...
  \end{equation}
  For $Y$, \eqref{eq:plt23-Lhat-formula}, \eqref{eq:plt23-KL-def}, and the first term of the sum in \eqref{eq:plt23-KL-sum} give
  \begin{align*}
    Y
    &=(1-\tilde M)\sqrt{2\Lambda}\,
      \frac{M_m}{\Lambda^{m-2}}\\
    &\ge
...
\begin{proof}
  ...
  From \eqref{eq:plt23-Qhat-formula}, \eqref{eq:plt23-Delta-identities}, \eqref{eq:plt23-zeta-p-def}, and \eqref{eq:plt23-KA-def}--\eqref{eq:plt23-KA-sum},
  \[
    X
...
\begin{proof}
  ...
  From \eqref{eq:plt23-Lhat-formula}, \eqref{eq:plt23-KL-def}, \eqref{eq:plt23-KL-sum}, and \eqref{eq:plt23-Lambda-lower},
  \[
    Y\ge
    m(1-\tilde M)(\tilde p-\tilde M)\tilde p^{m-3}
    \sqrt{\frac2{\delta_{\tilde p}+2}}.
