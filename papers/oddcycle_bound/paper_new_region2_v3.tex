...
\begin{proposition}[Reduction to a normalized inequality]\label{prop:plt23-reduction}
  ...
  \begin{equation}\label{eq:plt23-dtheta}
    ...
    \end{cases}
  \end{equation}
  If $F_N\le\mathcal B\,d(\theta)$, then \eqref{eq:plt23-scalar-target} holds.
\end{proposition}
...
The remaining subsections verify the hypothesis $F_N\le\mathcal B\,d(\theta)$, and for that we express $F_N$, $\mathcal A$, and $\mathcal B$ in variables adapted to the problem.
Set
\[
  N:=m-2,
\]
...
Therefore \eqref{eq:plt23-alpha-ceiling-poly} is equivalent to
...
\begin{proof}
  ...
  \[
    ...
    \qquad (N\ge7).
  \]
  Thus it is enough to prove the claim for $N=7$, where \eqref{eq:plt23-Psi-lower} gives
  \[
