...
an asymptotic two-parameter form}
\author{}
\date{}
...
\cite{dense}. The argument below is conditional on its Theorem~1.1 and
...
\[
 G_m(p)=p^m-p(1-p)^{m-1}.
\]
For a bounded measurable kernel $F$ we use the cut norm
...
change from line to line. No constant is tracked. Explicit admissible
values for all of them are computed in \cite{explicit}.
...
two-colour inequality of \cite[Theorem~1.3]{dense} dominates the
...
The third step is purely combinatorial and constructive: it uses no
regularity lemma and no removal lemma, and its only smallness threshold
...
Recall from \cite[Section~3]{dense} that for $d\geq0$, $h_d$ denotes the
...
Let $\lambda>0$. With $\rho_{n,m}$ as in \cite[Eq.~(5.7)]{dense}, the
Gamma representation \cite[Eq.~(5.15)]{dense}, specialised to $r=1$,
reads
\[
 P_{m,1}(q;\lambda)
...
\cite[Eq.~(5.18)]{dense}, again with $r=1$, expresses
...
\cite[Proposition~5.6]{dense} shows that each of these quantities has
...
\begin{remark}
The bound $c_m\geq2^{4-m}$ recorded in the proof is very lossy. The
true order of magnitude is
$c_m=\tfrac{9m}{7}\bigl(\tfrac23\bigr)^{m}\bigl(1+O((3/4)^m)\bigr)$,
and for $m=3,5$ one has $c_3=2$ and $c_5=143/144$; see
\cite[Lemma~3.1]{explicit}. Only $c_m>0$ is used below.
\end{remark}

Following \cite[Eq.~(6.1)]{dense}, we write, for an arbitrary graphon
$W$ of edge density $p$ with $U=1-W$ and $q=1-p$,
\begin{equation}\label{eq:Phi-definition}
 \Phi_m(W)=t(C_m,W)+t(C_m,U)-t(P_{m-1},U)-G_m(p),
\end{equation}
so that \cite[Theorem~1.3]{dense} states $\Phi_m(W)\geq0$ for
$p\geq2/3$, and \cite[Eq.~(6.3)]{dense} is the gap decomposition
...
\cite[Section~2]{dense}. The excursion expansion
\cite[Eq.~(3.10)]{dense} reads
\[
...
\cite[Lemma~2.2 and Eq.~(2.3)]{dense}, is supported on $[-1/2,1/2]$ and
has total mass
\[
 \mu(\R)=\norm{g}_2^2=\int_0^1\bigl(d_W(x)-p\bigr)^2\,dx .
\]
Since $p\geq2/3$ gives $q\leq1/3$, all the integrands are nonnegative by
\cite[Section~5]{dense}. Discarding every term with $r\geq2$ and applying
...
approximations of \cite[Lemma~2.1 and Appendix~A]{dense}, so that
...
\cite[Theorem~1.3]{dense}: the gap in the two-colour inequality is
...
\cite[Proposition~6.1]{dense}. Retaining every term gives the full
...
Let $h$ be a real unit eigenfunction for an eigenvalue of modulus
$\varrho$. Since $U\geq0$,
\[
...
multiplicity, so that $\sum_i\lambda_i^2=\int U^2\leq q$ and
...
$\int_0^1(d_U-q)^2$. As $p\geq2/3$, \cite[Theorem~1.1]{dense} gives
$\Gamma\geq0$.
...
\end{equation}
the left inequality being \cite[Theorem~1.1]{dense} applied to
...
constant term is $G_m(p)$ by \cite[Eq.~(1.1)]{dense}.
...
\begin{thebibliography}{EXP}
\bibitem[D]{dense}
T.~Kim, J.~Baek, S.~Im, J.~Lee, and H.~Yang,
\emph{Goodman-type lower bounds for odd cycles I: The dense regime},
manuscript.
\bibitem[E]{explicit}
\emph{Quantitative stability for the odd-cycle Goodman bound},
companion note with explicit constants.
\end{thebibliography}
