...
\[
  \begin{aligned}
    \Tr(T^m)
    ={}&\Tr\bigl((\Pi_cT)^m\bigr)
       +\Tr\bigl((\Pi_0T)^m\bigr)\\
       &+\sum_{\substack{(s_1,\ldots,s_m)\in\{c,0\}^m\\
...
We group the contributions involving both spaces by their number $r$ of passages from the constant space into the zero-average space and back, called \emph{excursions}.
Subtracting the path-density correction then expresses the difference between the two sides of \eqref{eq:intro-commonality} as a weighted sum of values of explicit polynomials $P_{m,r}(1-p;\lambda_1,\ldots,\lambda_r)$.
All weights are nonnegative, and the variables $\lambda_i$ lie in $[-1/2,1/2]$.
It suffices to prove that each polynomial is nonnegative for every tuple of variables in this interval, for $1\le r\le(m-1)/2$.

Third, we reduce these multivariable inequalities to one-variable inequalities.
An averaging identity shows that it suffices to consider the case where all variables are equal to a common value $\ell$.
The remaining task is therefore to prove
\[
  P_{m,r}(1-p;\ell,\ldots,\ell)\ge0
  \qquad\text{for }-\frac12\le\ell\le\frac12.
\]
