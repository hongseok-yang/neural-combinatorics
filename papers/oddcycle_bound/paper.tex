...
First, we approximate the graphons $W$ and $1-W$ by step graphons and represent them by complementary symmetric matrices.
We write each matrix in blocks relative to the constant vector and its orthogonal complement.
The constant blocks are the edge densities $p$ and $1-p$, while the blocks on the orthogonal complement have opposite signs.
Since $m$ is odd, the trace terms involving only these latter blocks cancel in the sum $\tC mW+\tC m{1-W}$.
...
Subtracting the analogous expansion of the path-density correction produces explicit symmetric polynomials $P_{m,r}(1-p;\lambda_1,\ldots,\lambda_r)$, where the variables $\lambda_i$ represent eigenvalues of the complementary graphon's nonconstant block and lie in $[-1/2,1/2]$.
...
This is the only stage at which $p\ge2/3$ is required; the strengthened two-colour inequality can fail for $1/2<p<2/3$, so the companion paper takes a different route in that range.
