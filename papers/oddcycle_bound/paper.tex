...
\begin{abstract}
  ...
  \]
  At each density $p=1-1/k$ with $k\ge3$, equality is attained by the balanced complete $k$-partite graphon.
  ...
  We also prove, for $p\ge2/3$, the stronger inequality
  \[
    \tC mW+\tC m{1-W}
    \ge p^m+(1-p)^m+\tP{m-1}{1-W}-(1-p)^{m-1},
  \]
  where $P_{m-1}$ is the path with $m-1$ edges.
  The path-density inequality gives $\tP{m-1}{1-W}\ge(1-p)^{m-1}$, so the final two terms form a nonnegative correction.
  Omitting this correction recovers the extended-commonality inequality of Kim and Lee~\cite{kim2024extended}.
