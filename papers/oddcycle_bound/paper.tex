...
Third, we reduce these multivariable inequalities to one-variable inequalities.
...
\]
Although the number $r$ of variables can grow with $m$, this is a polynomial inequality in just one variable for each choice of $m$, $r$, and $p$.

Finally, we establish the one-variable inequalities using an ODE argument.
An integral representation of $P_{m,r}(1-p;\ell,\ldots,\ell)$ establishes its nonnegativity when $\ell\le0$ (\Cref{lem:pge23-ell-negative}).
...
It suffices to show that, for each fixed $z>0$, the expectation over $Y$ satisfies
\[
  \E_Y\!\left[\rho_{m-2r,m}\!\left(1-p+\frac{\ell}{z}Y\right)\right]\ge0.
\]
Writing $F_j(b)=\E(Y-b)^{2j}$ for integers $j\ge1$, this follows for $p\ge2/3$ from the bound
\[
...
\]
The ODE shows that any interval on which $(r+j)F_j(b)+(3b/2)F_j'(b)$ is negative must contain a particular point depending only on $r$ and $j$.
An integral estimate establishes nonnegativity at this point, ruling out all such intervals and proving the bound for every $b\ge0$.
This establishes the one-variable polynomial inequalities; the implication from the bound on $F_j$ to positivity of the expectation is the only stage that needs $p\ge2/3$.
It follows that the polynomials in the second step are nonnegative, completing the comparison required by the first step and proving \Cref{thm:commonality}.
