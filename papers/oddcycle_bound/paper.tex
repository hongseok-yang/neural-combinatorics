...
The trace formula $\tC mW=\sum_i\lambda_i^m$ expresses the cycle density as a sum of eigenvalue powers.
For odd $m$, negative eigenvalues contribute negatively, so obtaining a lower bound requires controlling their contribution.
Our proof combines the trace formulas for $W$ and $1-W$ so that certain terms cancel, and then shows that the remaining terms cover the path-density correction in \eqref{eq:intro-commonality}.

First, we identify the cancellation.
Every function splits into its constant average and a remainder with integral zero.
The operators associated with the two graphons satisfy
\[
  (\Tker Wf)(x)+(\Tker{1-W}f)(x)=\int_0^1 f(y)\dd y.
\]
Thus, on an input with integral zero, their outputs have opposite signs.
To use this observation in the trace formula, we expand each operator power by separating the constant and zero-average components after every multiplication.
Terms that use only zero-average components cancel between the two traces because $m$ is odd.
Terms that use only constant components contribute $p^m+(1-p)^m$.
The remaining terms involve transitions between the two types of components.
We justify these expansions by first approximating the graphons by functions that are constant on a finite grid of rectangles; their operators can then be represented by finite matrices.

Second, we organize the remaining terms and compare them with the path-density correction.
A passage from the constant component into the zero-average component and back is called an \emph{excursion}.
We group terms by the number $r$ of such passages and sum over their possible lengths.
