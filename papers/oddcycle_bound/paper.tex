...
\Cref{sec:excursions} develops the excursion expansion and reduces the strengthened two-colour theorem to the inequalities $P_{m,r}\ge0$.
...
Equality in the strengthened two-colour theorem forces constant degrees; equality in the cycle--path comparison then forces the complement to be a union of equal cliques.
...
With $U=1-W$ and $q=1-p$, extend the gap in \eqref{eq:commonality-gap} to arbitrary graphons by setting
...
\begin{proof}
  ...
  For $r=(m-1)/2$, the degree $n=m-2r$ in \eqref{eq:Pmr} is one, so
  \[
    P_{m,r}(q;\lambda_1,\ldots,\lambda_r)
  ...
  Retaining this single term gives
  \[
  ...
  Conversely, if $W$ is $p$-regular, the constant functions and their orthogonal complement are invariant under both $\Tker W$ and $\Tker U$.
  On the constants the eigenvalues are $p$ and $q$, while on the orthogonal complement the two operators are negatives of each other.
  Since $m$ is odd, their traces therefore satisfy
  \[
    \tC mW+\tC mU=p^m+q^m.
  \]
  Also, $\Tker U\one=q\one$ gives $\tP{m-1}U=q^{m-1}$.
  Substituting in \eqref{eq:equality-gap} yields $\Phi_m(W)=0$.
\end{proof}
...
\begin{proof}
  ...
  Weighted Cauchy--Schwarz gives, for $f\in L^2[0,1]$,
  \[
    |\Tker Uf(x)|^2
    \le q\int_0^1 U(x,y)|f(y)|^2\dd y,
  \]
  and integration shows that $\norm{\Tker Uf}_2\le q\norm f_2$.
  Hence every eigenvalue $\lambda_i$ of $\Tker U$ lies in $[-q,q]$.
  ...
  In particular, $\Tker U^2=q\Tker U$, so its kernels satisfy
  \begin{equation}\label{eq:equal-neighbourhoods}
    \int_0^1 U(x,z)U(y,z)\dd z=qU(x,y)
    \quad\text{for almost every }(x,y).
  \end{equation}

  Write $N(x)=\{y:U(x,y)=1\}$, which has measure $q$ for almost every $x$.
  By \eqref{eq:equal-neighbourhoods}, adjacent points have the same neighbourhood up to a null set, while nonadjacent points have disjoint neighbourhoods up to a null set.
  Choose $x$ for which these statements hold for almost every $y$, and set $B=N(x)$.
  Then $N(y)=B$ up to a null set for almost every $y\in B$.
  Consequently, $U=1$ almost everywhere on $B\times B$ and $U=0$ almost everywhere between $B$ and its complement.
  Repeating on the remaining set extracts disjoint cliques, each of measure $q$.
  A remainder of positive measure less than $q$ is impossible, since its points still have degree $q$ and no neighbours in the extracted cliques.
  Thus, these cliques partition $[0,1]$ up to a null set, and their number $k$ satisfies $kq=1$.
  Their complement is $W_k$ after relabelling; $p\ge2/3$ implies $k\ge3$.
\end{proof}
...
\begin{proof}
  Otherwise, there would be a sequence $(V_j)_j$ of edge density $p_k$ whose $C_m$-densities tend to the minimum, but with $\delta_\square(V_j,W_k)\ge\varepsilon$ for some fixed $\varepsilon>0$.
  Compactness gives a subsequence converging in cut distance to a graphon $W$.
  Continuity of edge and cycle densities shows that $W$ attains the minimum at edge density $p_k$.
  By \Cref{prop:main-equality}, $W$ is equivalent to $W_k$, contradicting the distance bound.
\end{proof}
