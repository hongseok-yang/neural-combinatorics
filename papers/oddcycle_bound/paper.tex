...
For the triangle, Goodman~\cite{goodman1959on} proved that every graphon of edge density $p$ satisfies
...
At the other edge densities the bound \eqref{eq:intro-goodman} is not attained, and the exact minimum---the subject of the Erd\H os--Rademacher problem~\cite{erdos1962on}---was determined only much later: Razborov~\cite{razborov2008on} found it for the triangle with his flag algebra method, Nikiforov~\cite{nikiforov2010the} for $K_4$, and Reiher~\cite{reiher2016the} for every complete graph, confirming the extremal structure conjectured by Lov\'asz and Simonovits~\cite{lovasz1983on}.
...
Erd\H os~\cite{erdos1962on} conjectured that every complete graph is common, and Burr and Rosta~\cite{burr1980on} that every graph is; Thomason~\cite{thomason1989a} disproved both conjectures, already for $K_4$, and Jagger, \v S\v tov\'i\v cek and Thomason~\cite{jagger1996multiplicities} proved that no graph containing $K_4$ is common.
...
The added quantity is nonnegative: $\tP{m-1}U\ge q^{m-1}$ by the path inequality of Mulholland and Smith~\cite{mulholland1959an}.
