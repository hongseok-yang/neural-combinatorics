...
At each edge density $p=1-1/k$, $k\ge2$, the canonical candidate is the \emph{balanced complete $k$-partite graphon} $W_k$, defined by
\[
  W_k(x,y)=
  \begin{cases}
...
\end{equation}
As $k$ varies, these values lie on the single polynomial $\goal$.
We ask whether this polynomial is a universal lower bound at every edge density, even though it need not be the exact minimum away from $p=1-1/k$.

For $m=3$, this question is answered by Goodman's inequality: every graphon $W$ with $\edens W=p$ satisfies
