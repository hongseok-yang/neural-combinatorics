...
\begin{abstract}
  ...
  \]
  Here $\tC mW$ is the homomorphism density of the $m$-cycle in $W$.
  ...
  The proof combines spectral reduction and excursion generating functions with Dirichlet and Beta averaging, a Laplace--Gamma representation, and an ODE argument for shifted moments.
  Together, these ingredients yield a framework that is uniform in $m$.
...
Although their theorem is stated only at the densities $p=1-1/k$, their flag algebra certificate can be interpolated over real $k\in[3,\infty)$ to give $\tC5W\ge\goodman p5$ for every $p\ge2/3$.
...
We prove \Cref{thm:main} through a stronger two-colour inequality.
...
\paragraph{Proof strategy and reusable ingredients.}
Our proof separates the information specific to the graphon from a universal positivity problem.
We first reduce to step graphons, represent the graphon and its complement by finite matrices, and split the underlying space into the constant direction and its orthogonal complement.
A positive spectral measure supported on $[-1/2,1/2]$ then encodes all nonconstant information.
When we expand matrix powers as words in the two corresponding projections, each passage from the constant direction into the orthogonal complement and back becomes an \emph{excursion}.
Complete homogeneous symmetric polynomials and their generating functions collect these excursions, reducing both main inequalities to the positivity of universal symmetric polynomials integrated against the spectral measure.

The number of spectral variables grows with $m$, so direct multivariate analysis would not yield an argument uniform in the cycle length.
The first key simplification is a \emph{Dirichlet averaging identity}: an $r$-variable excursion polynomial equals the average of its one-variable diagonal specialization evaluated at a random convex combination of the $r$ inputs.
It follows that the minimum over the spectral cube occurs on the diagonal.
Generating-function identities then express the diagonal polynomial as an expectation under a symmetric Beta distribution.
The Dirichlet and Beta laws are standard tools in Bayesian statistics, and Dirichlet laws also underpin Bayesian nonparametrics.
Here their moment identities serve as algebraic averaging operators, turning a high-dimensional problem into a one-dimensional integral inequality.

On the positive half of the spectral interval, however, the resulting Beta integrand can change sign.
An inverse Laplace transform resolves this obstacle: it decouples the variables and converts the integral into expectations of a scalar polynomial under a Gamma distribution.
After centering at $1/2$, positivity reduces to a uniform comparison between odd and even shifted Gamma moments.
We establish this comparison by applying the Gamma Stein identity, deriving a second-order ODE for the even moment, and analyzing its possible zero crossings.
The assumption $p\ge2/3$ enters only in this final shifted-Gamma step, pinpointing why the companion paper needs a different argument below the threshold.

The resulting modular chain---spectral reduction, excursion generating functions, probabilistic diagonalization, and ODE-based moment control---is a second contribution of the paper.
Although the individual ingredients are classical, their combination provides a new route that keeps the odd length $m$ symbolic throughout, rather than treating one fixed cycle at a time.
We expect this architecture to be useful in other fixed-density extremal problems whose pattern densities admit compatible trace expansions.
...
\Cref{sec:short-cycles} illustrates the reduction for $C_3,C_5,C_7$, and \Cref{sec:excursion-positivity} proves the required positivity for all odd cycles.
