...
Finally, we reduce the one-variable inequalities to a comparison of expected odd and even powers.
An integral representation establishes nonnegativity when $\ell\le0$.
For $\ell>0$, the integrand can have both signs, so its positive and negative contributions must be compared after integration.
A further integral identity reduces the required nonnegativity to a bound comparing expected odd powers of a shifted random variable with the next even powers.
We prove this bound using a differential equation.
The resulting estimate establishes the one-variable inequalities for $p\ge2/3$; this is the only stage that needs this density assumption.
