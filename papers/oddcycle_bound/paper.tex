...
For odd $m$, negative eigenvalues contribute negatively to each trace, so obtaining this bound requires controlling their contribution.
We analyze the two traces together: the relationship between $W$ and $1-W$ makes certain terms in their expansions cancel, leaving a smaller collection of terms to bound.

First, we show that the excess over $p^m+(1-p)^m$ consists entirely of terms that switch between constant and zero-average components.
To expand $\Tker W^m$, consider its action on an arbitrary function $f$.
Split $f$ into its constant average and a remainder with integral zero, and apply $\Tker W$ separately to these two pieces.
Split each resulting function again into its constant average and zero-average remainder, then apply $\Tker W$ separately to every piece.
Repeat this procedure through all $m$ applications of $\Tker W$.
After the final application, split each output once more into its constant and zero-average components.
By linearity, the sum of the resulting functions is exactly $\Tker W^m f$.

This gives an expansion of $\Tker W^m$ into terms indexed by the sequence of constant or zero-average components retained during the calculation.
We expand $\Tker{1-W}^m$ in the same way and then take traces.
Since a trace sums diagonal contributions, only sequences that start and finish in the same component space contribute.

Consider first the expansion term that starts with the zero-average component of $f$ and retains the zero-average component of the output after every application.
The identity
\[
  (\Tker Wf)(x)+(\Tker{1-W}f)(x)=\int_0^1 f(y)\dd y
\]
shows that, for a zero-average input, applying $\Tker W$ or $\Tker{1-W}$ gives outputs that are negatives of one another.
Their zero-average components are therefore negatives as well.
Thus, replacing $\Tker W$ by $\Tker{1-W}$ in this sequence introduces a factor of $-1$ at each of the $m$ applications.
The two resulting operators differ by a factor of $(-1)^m$, so their traces cancel because $m$ is odd.

Next, consider the expansion term that starts with the constant component of $f$ and retains the constant component of the output at every stage.
Applying $\Tker W$ to a constant and taking the average of the output multiplies that constant by $p$; the corresponding multiplier for $\Tker{1-W}$ is $1-p$.
Repeating this $m$ times gives trace contributions $p^m$ and $(1-p)^m$, respectively.
Every other sequence that contributes to the traces visits both component spaces and returns to the space in which it started.
The sum of these contributions is therefore exactly the excess over $p^m+(1-p)^m$ that we must bound below by the path-density correction.
We justify the expansions by first approximating the graphons by functions that are constant on a finite grid of rectangles, so that the operators can be represented by finite matrices.

Second, we group the sequences that visit both component spaces and compare their trace contributions with the path-density correction.
A passage from the constant component into the zero-average component and back is called an \emph{excursion}.
We group these contributions by the number $r$ of excursions and sum over the possible lengths of the excursions and the intervening constant portions.
The path density admits a similar expansion, so we can subtract its correction one group at a time.
The resulting gap between the two sides of \eqref{eq:intro-commonality} is a sum of polynomials
$P_{m,r}(1-p;\lambda_1,\ldots,\lambda_r)$ with nonnegative weights.
Here the $\lambda_i$ are eigenvalues of the map obtained by applying $\Tker{1-W}$ to a zero-average function and then subtracting the average of the output.
We prove that these eigenvalues lie in $[-1/2,1/2]$.
The graphon determines the weights, but it is enough to prove that each polynomial is nonnegative throughout this interval in each variable.

Third, we overcome the fact that the number of variables grows with $m$.
These particular polynomials satisfy an averaging identity: their value at any tuple $(\lambda_1,\ldots,\lambda_r)$ is an average of values with all variables equal,
\[
  P_{m,r}(1-p;\ell,\ldots,\ell).
\]
The common value $\ell$ is a randomly chosen weighted average of the original $\lambda_i$, with nonnegative weights summing to one, so it remains in $[-1/2,1/2]$.
Consequently, nonnegativity when all variables are equal implies nonnegativity for every tuple.
This reduces the problem to a polynomial in one variable, regardless of the number of excursions.
A second averaging identity expresses this polynomial as an integral with an explicit nonnegative weight, making it possible to compare its positive and negative contributions.

Finally, we prove that this integral is nonnegative.
When $\ell\le0$, its integrand is nonnegative everywhere.
When $\ell>0$, the integrand can have both signs, so we must show that the positive contribution outweighs the negative one after integration.
We rewrite the integral as a positively weighted combination of expectations in which $\ell$ appears only in the weights.
It is therefore enough to prove that each expectation is nonnegative.
In this representation, the potentially negative terms involve odd powers of a shifted random variable, while the compensating terms involve the next even powers.
These expectations are related by differentiation: varying the shift in an even power produces the preceding odd power.
An integration-by-parts identity for the random variable gives a differential equation for the even-power expectation.
We use this equation to bound the odd-power contribution by the even-power contribution, with constants that make the total nonnegative when $p\ge2/3$.
This is the only step that requires this density assumption.
All four steps apply uniformly in the odd cycle length $m$.
...
The appendices justify the graphon approximation and prove the comparison of expected powers used in the final step.
