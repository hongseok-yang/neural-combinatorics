...
Razborov~\cite{razborov2008minimal} solved the triangle case using flag algebras, Nikiforov~\cite{nikiforov2010number} treated $K_4$, and Reiher~\cite{reiher2016clique} proved the clique density theorem for every complete graph, confirming the extremal structures conjectured by Lov\'asz and Simonovits~\cite{lovasz1983number}. \tk{For extermal structure, we might need to cite papers like \url{https://arxiv.org/abs/1204.2846} and \url{https://arxiv.org/abs/1906.05942}, which characterises the optimal graph, not just number of triangles. }
...
Their proof combines a flag algebra computation with stability \tk{It is a bit misleading, since they only use flag algebra to solve lower bound, and additionally prove stability. flag algebra with stability sounds like flag algebra itself was insufficient, and they closed the gap that flag algebra can't prove with some stability arguments.}.
...
\end{equation}
thereby answering a question of Sidorenko~\cite{sidorenko1989cycles} and proving, in stronger form, a conjecture of Behague, Morrison and Noel~\cite{behague2025common}. \tk{The conjecture only appears in the first version in arXiv of this paper, which does not appear in the most recent or published version.}
...
The path inequality of Mulholland and Smith~\cite{mulholland1959inequality} gives $\tP{m-1}U\ge q^{m-1}$, so the correction term is nonnegative and \eqref{eq:intro-commonality} indeed strengthens extended commonality. \tk{The citation seems to be correct, but Blakely-Roy is more well-known, considered as independent work.}
...
We prove the stronger two-colour inequality through four reductions, each of which replaces the problem by a simpler positivity statement.

First, we approximate the graphons $W$ and $U=1-W$ by step graphons and represent them by complementary symmetric matrices.
We decompose each matrix into a part that records its edge density and a remaining, nonconstant part.
The nonconstant parts for $W$ and $U$ have opposite signs, so their purely nonconstant contributions cancel when the two odd-cycle densities $\tC mW$ and $\tC mU$ are added.
This leaves a finite matrix expansion in which the prescribed edge density has been separated from the rest of the graphon.

Second, we organize the surviving terms.
Each term can be broken into departures from the constant part of the matrix and returns to it; we call these pieces \emph{excursions}.
We group the terms by their number $r$ of excursions and sum each group with a generating function.
This produces an explicit symmetric polynomial $P_{m,r}(q;\lambda_1,\ldots,\lambda_r)$ for each $r$, where the variables $\lambda_i$ represent eigenvalues of the nonconstant part and range over $[-1/2,1/2]$.
The stronger two-colour theorem is then reduced to proving $P_{m,r}\ge0$ on $[-1/2,1/2]^r$ for every $r$.

Third, we reduce each of these multivariable inequalities to one variable.
A Dirichlet averaging identity reduces the problem to the diagonal case $\lambda_1=\cdots=\lambda_r=\ell$.
A second identity, using a symmetric Beta distribution, writes the resulting one-variable polynomial as a one-dimensional integral.
Together, these two averaging steps reduce every inequality $P_{m,r}\ge0$ to a one-variable integral inequality.

Finally, we prove this integral inequality.
We split the problem according to the sign of $\ell$.
For $\ell\le0$, nonnegativity follows directly from the Beta representation.
For $\ell>0$, the integrand can change sign, so a pointwise argument is no longer available.
We instead apply a Laplace transform to rewrite the integral in terms of expectations under a Gamma distribution, and then use Gamma integration by parts together with an ODE argument to prove the required nonnegativity.
These transformations allow us to control the integral as a whole despite the changing sign of its integrand.
This is the only stage at which $q\le1/3$, or equivalently $p\ge2/3$, is required; the strengthened two-colour inequality can fail for $1/2<p<2/3$, so the companion paper takes a different route in that range.

The proof thus follows the chain
\[
  \begin{gathered}
    \text{graphon inequality}
    \longrightarrow \text{complementary matrices}
    \longrightarrow \text{excursion polynomials }P_{m,r}\\
    \longrightarrow \text{a one-variable integral}
    \longrightarrow \text{Gamma expectations and an ODE}.
  \end{gathered}
\]
The reductions keep the odd length $m$ symbolic rather than treating one cycle at a time.
In particular, the complementary-matrix cancellation, excursion generating functions, and Dirichlet reduction may be useful in other fixed-density problems with similar matrix expansions.

\paragraph{Organization.}
In \Cref{sec:matrix-reduction}, we carry out the graphon-to-matrix reduction and record the relevant nonconstant matrix data through eigenvalues and nonnegative weights.
...
The appendices justify the graphon approximation and prove the estimate used to control the Gamma expectations in the final step.
