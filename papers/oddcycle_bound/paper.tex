...
\begin{abstract}
  ...
  We establish its analogue for every odd cycle in the dense regime: in the language of graph limits, for every odd integer $m\ge3$, every graphon $W$ of edge density $\edens W=p\ge2/3$ satisfies
  \[
...
We allow this bound to be nonoptimal between the distinguished densities $p=1-1/k$, but require equality at each of them, where the balanced complete $k$-partite graph is the canonical extremal example.
...
For a finite graph $H$ with vertex set $\{1,\ldots,v\}$ and edge set $E(H)$, the \emph{homomorphism density} of $H$ in $W$ is
\[
...
The canonical examples at the edge densities $p=1-1/k$ are the \emph{balanced complete multipartite graphons}.
For an integer $k\ge2$, write $W_k$ for the graphon defined by
\[
  W_k(x,y)=
  \begin{cases}
