...
We formulate the problem using graphons, which encode limits of dense graphs and express asymptotic edge and subgraph densities as analytic quantities~\cite{lovasz2006limits,lovasz2012large}.
...
\end{equation}
This calculation suggests the polynomial $\goal$ as a lower bound at every edge density, including those where it need not give the exact minimum.
...
The Blakley--Roy inequality~\cite{blakley1965holder} gives $\tP{m-1}U\ge q^{m-1}$.
...
Rearranging \eqref{eq:intro-commonality} and using $q^m-q^{m-1}=-pq^{m-1}$, we obtain
\[
  \tC mW-\bigl(\goal\bigr)\ge\tP{m-1}U-\tC mU\ge0.
\]

\paragraph{Proof strategy: four reductions.}
The proof of the two-colour inequality has four steps.

First, we approximate the graphons $W$ and $U=1-W$ by step graphons and represent them by complementary symmetric matrices.
We write each matrix in blocks relative to the constant vector and its orthogonal complement.
The constant blocks are the edge densities $p$ and $q$, while the blocks on the orthogonal complement have opposite signs.
Since $m$ is odd, the trace terms involving only these latter blocks cancel in the sum $\tC mW+\tC mU$.
The remaining terms describe how the constant and nonconstant subspaces interact.

Second, we organize these terms into \emph{excursions}: departures from the constant subspace into its orthogonal complement, followed by returns.
We group the terms by their number $r$ of excursions and sum each group with a generating function.
Subtracting the analogous expansion of the path-density correction produces explicit symmetric polynomials $P_{m,r}(q;\lambda_1,\ldots,\lambda_r)$, where the variables $\lambda_i$ represent eigenvalues of the complementary graphon's nonconstant block and lie in $[-1/2,1/2]$.
The difference between the two sides of \eqref{eq:intro-commonality} is a sum of integrals of these polynomials against nonnegative measures.
It therefore suffices to prove $P_{m,r}\ge0$ on $[-1/2,1/2]^r$ for $1\le r\le(m-1)/2$.

Third, we reduce each of these multivariable inequalities to one variable.
A Dirichlet averaging identity expresses each polynomial as an average of its diagonal values, reducing positivity to the case $\lambda_1=\cdots=\lambda_r=\ell$.
...
Thus positivity in arbitrarily many spectral variables reduces to a one-variable integral inequality.

Finally, we prove this integral inequality by considering the sign of $\ell$.
...
We use a Laplace integral representation to express it as a positive weighted integral of expectations under a Gamma distribution.
Gamma integration by parts and an ODE argument give a uniform bound comparing odd and even moments, which makes these expectations nonnegative.
...
\[
  ...
  \end{gathered}
\]
All four steps apply uniformly in the odd length $m$.
...
\Cref{sec:matrix-reduction} carries out the graphon-to-matrix reduction and encodes the nonconstant block and its interaction with the constant vector in a positive spectral measure.
