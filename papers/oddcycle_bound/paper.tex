...
\begin{abstract}
  ...
  Goodman's classical triangle inequality is the prototype for this question.
  We establish its analogue for every odd cycle in the dense regime: for every odd integer $m\ge3$, every graphon $W$ of edge density $\edens W=p\ge2/3$ satisfies
  \[
    \tC mW\ge p^m-p(1-p)^{m-1}.
  \]
  At every density $p=1-1/k$ with $k\ge3$, the balanced complete $k$-partite graphon attains equality; we do not claim optimality at other densities.
  Bennett, Dudek, Lidick\'{y} and Pikhurko~\cite{bennett2020minimizing} proved the corresponding $C_5$ result at these discrete densities, and our theorem extends it to every odd cycle while giving a single uniform bound throughout $p\ge2/3$.
  This is the first of two papers establishing the bound for all edge densities.
  A companion paper treats $1/2<p<2/3$, while for $p\le1/2$ the inequality is immediate because its right-hand side is nonpositive.
  ...
  The proof combines spectral reduction and excursion generating functions with Dirichlet and Beta averaging, a Laplace--Gamma representation, and an ODE argument for shifted moments; the resulting framework is uniform in $m$.
  This proof has been formally verified in Lean~4; the code is available at \url{ADDLINKLATER}.
\end{abstract}

\section{Introduction}\label{sec:intro}

A central question in extremal graph theory asks how dense a graph can be while avoiding a prescribed subgraph $H$.
...
These theorems locate the point at which copies of $H$ become unavoidable, but they do not determine how many copies are forced at each density above that point.
...
For complete graphs, this quantitative problem is the classical Erd\H os--Rademacher or clique-density problem~\cite{erdos1962number}.
...
The exact minimum at every density was determined much later.
...
This line of work determines an exact, piecewise-defined extremal curve.
...
\end{equation}
Before this work, balanced complete multipartite graphons were known to be extremal for only two odd cycle lengths.
Goodman's bound settles $C_3$.
...
Dirichlet and Beta laws are standard tools in Bayesian statistics, with Dirichlet laws also underlying Bayesian nonparametrics; here their moment identities serve as algebraic averaging operators that turn a high-dimensional problem into a one-dimensional integral inequality.
...
We establish this comparison by applying the Gamma Stein identity, deriving a second-order ODE for the even moment, and analyzing its zero crossings.
...
This modular chain---spectral reduction, excursion generating functions, probabilistic diagonalization, and ODE-based moment control---is a second contribution of the paper.
