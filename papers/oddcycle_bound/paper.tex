...
We separate functions $f : \mathbb{R} \to \mathbb{R}$ into their constant averages $c = \int f(x)dx$ and zero-average remainders $f_0 = f - c$.
This writes each trace as a sum of contributions from repeated applications of the operator that stay within constant functions, stay within zero-average functions, or pass between these two spaces.
In the sum of the two traces, the constant contributions give $p^m+(1-p)^m$, while the contributions confined to zero-average functions cancel for odd $m$.
The desired inequality therefore reduces to showing that the total contribution from passages between the two spaces is at least the path-density correction.
