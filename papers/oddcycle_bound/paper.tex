...
\begin{abstract}
  ...
  The spectral viewpoint reveals a useful cancellation between a graphon and its complement, probabilistic averaging reduces polynomial inequalities in arbitrarily many spectral variables to a one-variable integral inequality, and the ODE controls the remaining competition between positive and negative terms.
  Together, these tools give a single argument that is uniform in $m$.
  The lower-bound theorem has also been formally verified in Lean~4; the code is available at \url{ADDLINKLATER}.
\end{abstract}
...
\begin{theorem}[Strengthened extended commonality]\label{thm:commonality}
  ...
  \begin{equation}\label{eq:intro-commonality}
    \tC mW+\tC m{1-W}\ge p^m+(1-p)^m+\bigl[\tP{m-1}{1-W}-(1-p)^{m-1}\bigr].
  \end{equation}
\end{theorem}

The Blakley--Roy inequality~\cite{blakley1965holder} gives $\tP{m-1}{1-W}\ge(1-p)^{m-1}$.
This path inequality was proved independently, and earlier, by Mulholland and Smith~\cite{mulholland1959inequality}.
Thus, the path-density correction is nonnegative, and \eqref{eq:intro-commonality} strengthens extended commonality.
Exchanging $W$ and $1-W$ gives the corresponding statement for $p\le1/3$.
For $p\ge2/3$, equality holds in \eqref{eq:intro-commonality} exactly when $W$ has constant degree $p$, meaning that $\int_0^1 W(x,y)\dd y=p$ for almost every $x$ (\Cref{prop:regularity-gap}).
This includes the constant graphon $W\equiv p$ and, when $p=1-1/k$ with $k\ge3$, $W_k$.

This stronger statement immediately implies \Cref{thm:main}.
Since $0\le1-W\le1$, deleting one edge from the $m$-cycle gives $\tC m{1-W}\le\tP{m-1}{1-W}$.
Rearranging \eqref{eq:intro-commonality} and using $(1-p)^m-(1-p)^{m-1}=-p(1-p)^{m-1}$, we obtain
\[
  \tC mW-\bigl(\goal\bigr)\ge\tP{m-1}{1-W}-\tC m{1-W}\ge0.
\]

\paragraph{Proof strategy: four reductions.}
The trace formula expresses the two cycle densities as
\[
  \tC mW=\Tr(\Tker W^m),
  \qquad
  \tC m{1-W}=\Tr(\Tker{1-W}^m).
\]
Each trace is the sum of the $m$th powers of the corresponding operator's eigenvalues.
Our aim is to show that the sum of these two traces exceeds $p^m+(1-p)^m$ by at least the path-density correction in \eqref{eq:intro-commonality}.
We obtain this bound through four reductions, all uniform in the odd cycle length $m$.

First, we isolate the contribution that must be bounded.
For a real-valued function $f\in L^2[0,1]$, write
\[
  f=c\one+f_0,\qquad
  c=\int_0^1 f(x)\dd x,\qquad
  \int_0^1 f_0(x)\dd x=0,
\]
where $\one$ is the constant function equal to one.
Define the operators
\[
  \Pi_{\mathrm c} f=c\one,\qquad \Pi_0 f=f_0,
\]
which retain the constant and zero-average components, respectively.
For either $T=\Tker W$ or $T=\Tker{1-W}$, the trace decomposes as
\[
    \Tr(T^m)
    =  \Tr\bigl((\Pi_{\mathrm c}T)^m\bigr)
       +\Tr\bigl((\Pi_0T)^m\bigr)
       +\sum_{\substack{(s_1,\ldots,s_m)\in\{\mathrm c,0\}^m\\
                         \text{not all }s_i\text{ equal}}}
       \Tr\bigl(\Pi_{s_1}T\cdots\Pi_{s_m}T\bigr).
\]
The first term retains only the constant component, the second retains only the zero-average component, and the final sum includes every sequence using both components.
For $\Tker W$ and $\Tker{1-W}$, the first contributions sum to $p^m+(1-p)^m$, while the second contributions cancel for odd $m$.
It therefore remains to show that the two sums involving both components together are at least the path-density correction.
We carry out this decomposition using finite-matrix representations of graphons that are constant on a finite grid of rectangles; an approximation argument gives the result for general graphons.

Second, we reduce this comparison to polynomial nonnegativity.
For a step graphon, the trace decomposition from the first step gives the following expression for the difference between the two sides of \eqref{eq:intro-commonality}.
\[
  \begin{aligned}
    & \Bigl(\tC mW+\tC m{1-W}-p^m-(1-p)^m\Bigr)
      - \Bigl[\tP{m-1}{1-W}-(1-p)^{m-1}\Bigr]\\
    &\quad {} =
      \Biggl(\sum_{\substack{(s_1,\ldots,s_m)\in\{\mathrm c,0\}^m\\
                            \text{not all }s_i\text{ equal}}}
      \Tr\bigl(\Pi_{s_1}\Tker W\cdots\Pi_{s_m}\Tker W\bigr)
      +\sum_{\substack{(s_1,\ldots,s_m)\in\{\mathrm c,0\}^m\\
                             \text{not all }s_i\text{ equal}}}
      \Tr\bigl(\Pi_{s_1}\Tker{1-W}\cdots\Pi_{s_m}\Tker{1-W}\bigr)\Biggr)\\
    &\quad\qquad\qquad {} - \Bigl[\tP{m-1}{1-W}-(1-p)^{m-1}\Bigr]\\
    &\quad {} =
      \sum_{r=1}^{(m-1)/2}
      \int_{[-1/2,1/2]^r}
      P_{m,r}(1-p;\lambda_1,\ldots,\lambda_r)
      \prod_{i=1}^r\dd\mu(\lambda_i).
  \end{aligned}
\]
The first equality records the outcome of the first reduction.
The second expresses the remaining trace sums, after subtracting the path-density correction, in terms of explicit polynomials.
Here $\mu$ is a finite nonnegative measure determined by $W$, supported on $[-1/2,1/2]$, and constructed in \Cref{sec:matrix-reduction}; the $P_{m,r}$ are symmetric polynomials defined in \eqref{eq:Pmr}.
The index $r$ counts passages from the constant component into the zero-average component and back, called \emph{excursions}.
The measure supplies nonnegative weights, so it suffices to prove, for every $1\le r\le(m-1)/2$, that
\begin{equation}\label{eq:intro-polynomial-positive}
  P_{m,r}(1-p;\lambda_1,\ldots,\lambda_r)\ge0
  \qquad\text{for every }(\lambda_1,\ldots,\lambda_r)\in[-1/2,1/2]^r.
\end{equation}

Third, we reduce \eqref{eq:intro-polynomial-positive} to a one-variable inequality.
Choose nonnegative weights $\Theta_1,\ldots,\Theta_r$, summing to one, uniformly at random.
These weights have the Dirichlet$(1,\ldots,1)$ distribution.
Set $L=\sum_{i=1}^r\Theta_i\lambda_i$.
The Dirichlet averaging identity (\Cref{lem:pge23-dirichlet}) gives
\[
  P_{m,r}(1-p;\lambda_1,\ldots,\lambda_r)
  =\E\bigl[P_{m,r}(1-p;L,\ldots,L)\bigr].
\]
Since $L\in[-1/2,1/2]$ whenever all the $\lambda_i$ lie in this interval, it suffices to prove
\begin{equation}\label{eq:intro-diagonal-positive}
  P_{m,r}(1-p;\ell,\ldots,\ell)\ge0
  \qquad\text{for every }\ell\in[-1/2,1/2].
\end{equation}
Although the number $r$ of variables can grow with $m$, \eqref{eq:intro-diagonal-positive} is a polynomial inequality in just one variable for each choice of $m$, $r$, and $p$.

Finally, we prove \eqref{eq:intro-diagonal-positive} using an ODE argument.
An integral representation of $P_{m,r}(1-p;\ell,\ldots,\ell)$ establishes \eqref{eq:intro-diagonal-positive} when $\ell\le0$ (\Cref{lem:pge23-ell-negative}).
For $\ell>0$, let $Y$ and $Z$ be independent Gamma random variables with shapes $r$ and $m-r$, respectively, both with rate $1$.
The Gamma expectation formula \eqref{eq:pge23-gamma-expectation} gives
\[
  P_{m,r}(1-p;\ell,\ldots,\ell)
  =\binom{m-r-1}{r}
    \E\!\left[\rho_{m-2r,m}\!\left(1-p+\frac{\ell}{Z}Y\right)\right],
\]
where $\rho_{m-2r,m}$ is the polynomial defined in \eqref{eq:pge23-rho}.
To prove \eqref{eq:intro-diagonal-positive} for $\ell>0$, it suffices to show that, for each fixed $z>0$, the expectation over $Y$ satisfies
\begin{equation}\label{eq:intro-expectation-positive}
  \E_Y\!\left[\rho_{m-2r,m}\!\left(1-p+\frac{\ell}{z}Y\right)\right]\ge0.
\end{equation}
Writing $F_j(b)=\E(Y-b)^{2j}$ for integers $j\ge1$, \eqref{eq:intro-expectation-positive} follows for $p\ge2/3$ from the bound
\begin{equation}\label{eq:intro-moment-bound}
  (r+j)F_j(b)+\frac{3b}{2}F_j'(b)\ge0
  \qquad(j\ge1,\ b\ge0),
\end{equation}
as shown in the proof of \Cref{prop:pge23-gamma-positive}.
The function $F_j$ satisfies the differential equation
\begin{equation}\label{eq:intro-moment-ode}
  bF_j''(b)+(b-r-2j+1)F_j'(b)-2jF_j(b)=0.
\end{equation}
If the left-hand side of \eqref{eq:intro-moment-bound} were negative for some $b\ge0$, the ODE \eqref{eq:intro-moment-ode} would force the same expression to be negative at a particular point determined only by $r$ and $j$.
An integral estimate shows that this expression is nonnegative at that point, giving a contradiction and proving the moment bound \eqref{eq:intro-moment-bound} for every $b\ge0$.

The moment bound ensures that the expectation over $Y$ in \eqref{eq:intro-expectation-positive} is nonnegative; this is the only step requiring $p\ge2/3$.
Averaging over $Z$ then proves the one-variable polynomial inequality \eqref{eq:intro-diagonal-positive} for $\ell>0$, completing the fourth reduction.
The Dirichlet averaging identity extends this nonnegativity to the multivariable polynomials in \eqref{eq:intro-polynomial-positive}.
These polynomials are integrated against nonnegative weights in the second step, so the required trace comparison follows, proving \Cref{thm:commonality}.
The deduction preceding this overview then gives \Cref{thm:main}.

\paragraph{Organization.}
\Cref{sec:matrix-reduction} sets up the finite-matrix representation and identifies the spectral quantities used in the proof.
\Cref{sec:excursions} develops the excursion expansion and reduces \Cref{thm:commonality} to the inequalities $P_{m,r}\ge0$.
\Cref{sec:short-cycles} works out this reduction for $C_3$, $C_5$, and $C_7$, while \Cref{sec:excursion-positivity} proves the inequalities for every odd cycle.
\Cref{sec:equality-stability} identifies the equality cases and deduces qualitative stability.
We close with further questions in \Cref{sec:conclusion}.
...
$\edens W=p\ge2/3$, as in \Cref{thm:main,thm:commonality}. Write
\[
  U=1-W,
  \qquad
  q=1-p.
...
The assumption $p\ge2/3$ will be needed only for the positivity argument in \Cref{sec:excursion-positivity}; the matrix identities and excursion expansions developed before that point hold at every edge density.

To prove \Cref{thm:main,thm:commonality} by finite matrix calculations, we first restrict attention to \emph{step graphons}: graphons that are constant on each product of two parts of a finite measurable partition of $[0,1]$.
...
The following lemma justifies this restriction: proving the two inequalities in those theorems for all such graphons, with no bound on the number of parts, proves them for arbitrary graphons as well.
...
\label{lem:step-reduction}
  ...
  are constant on each rectangle $I_a\times I_b$, where
  $I_1,\ldots,I_N$ partition $[0,1]$ into intervals of length $1/N$
  and $N$ ranges over all positive integers.
\end{lemma}
...
Thus, if the two inequalities hold for every graphon in this sequence, they also hold for $W$ by taking limits.
...
\begin{equation}\label{eq:moments}
  ...
  \mu(\R)=\norm g^2.
\end{equation}
Thus, the quantities $g^{\mathsf T}A^jg$ are moments of a single nonnegative measure $\mu$ supported on $[-1/2,1/2]$.
In the next section, we use this representation to express the difference between the two sides of \Cref{thm:commonality} as a sum of repeated integrals of polynomials against $\mu$.
Since $\mu$ is nonnegative, proving that these polynomials are nonnegative will establish the theorem.

\section{Excursion expansions}
\label{sec:excursions}

\Cref{sec:matrix-reduction} expresses cycle and path densities through finite matrices and represents the quantities $g^{\mathsf T}A^jg$ as moments of the measure $\mu$.
We now combine these two descriptions to express the difference between the two sides of \eqref{eq:intro-commonality} as a sum of polynomial integrals against $\mu$ (\Cref{lem:polynomial-reduction}).
Since $\mu$ is nonnegative, this reduces the proof of \Cref{thm:commonality} to polynomial nonnegativity.
\Cref{sec:short-cycles,sec:excursion-positivity} establish that nonnegativity, proving \Cref{thm:commonality} and hence the odd-cycle bound in \Cref{thm:main}.
...
For example, the projection pattern
\[
  P\,\underbrace{QQ}_{\text{one $Q$-block}}\,P
\]
corresponds to the matrix segment $P M_U Q M_U Q M_U P$.
...
We next derive the corresponding formula for path densities, which supplies the correction term in \eqref{eq:intro-commonality}.
...
\begin{proof}
  For $k=0$ and $k=1$, the formula gives $x_0=e^{\mathsf T}e=1$ and
  $x_1=e^{\mathsf T}M_Ue=q$, respectively, with an empty sum over $r$.
  Now let $k\ge2$. Since $Pe=e$ and $e^{\mathsf T}P=e^{\mathsf T}$,
  we may insert $P$ at both ends of the product defining $x_k$.
  ...
  \]
  Associate each summand with the word $PR_1\cdots R_{k-1}P$.
  ...
  To evaluate the summand, use the same block identities as in the proof
  ...
  The endpoints fix the starting position, so each tuple contributes
  once, with no choice of a marked boundary or cyclic placement.
  Summing the displayed weights over these tuples therefore gives
...
We now combine the cycle and path formulas to express the difference between the two sides of \eqref{eq:intro-commonality} as polynomial integrals against $\mu$.
The following lemma completes the reduction announced at the start of this section.
...
\label{lem:polynomial-reduction}
  ...
  Thus, $\Phi_m\ge0$ implies \Cref{thm:main,thm:commonality}.
  In particular, it suffices to prove that each $P_{m,r}$ is nonnegative
  on $[-1/2,1/2]^r$.
\end{lemma}
...
\begin{proof}
  ...
  \begin{equation}\label{eq:commonality-gap}
    ...
    =p^m+q^m-q^{m-1}+\Phi_m.
  \end{equation}
  Recalling $x_{m-1}=\tP{m-1}U$, this identifies $\Phi_m$ as the
  difference between the two sides of \eqref{eq:intro-commonality}.
  In particular, $\Phi_m\ge0$ gives
  \[
    \tC mW+\tC mU
  ...
  To deduce the bound for $\tC mW$ alone, compare the cycle density of
  $U$ with its path density. Since $0\le U\le1$, removing the closing
  factor $U(x_m,x_1)$ from the cycle-density integrand can only increase
  it. The remaining factors describe the path with $m-1$ edges, so
  ...
  Finally, $\mu$ is nonnegative and supported on $[-1/2,1/2]$ by
  \Cref{sec:matrix-reduction}.
  If each $P_{m,r}$ is nonnegative on $[-1/2,1/2]^r$, every integral in
  \eqref{eq:Phi-expansion} is nonnegative, and hence $\Phi_m\ge0$.
  This proves both inequalities for step graphons;
  \Cref{lem:step-reduction} then extends them to arbitrary graphons.
\end{proof}
...
Fix an odd integer $m\ge3$ and $1\le r\le(m-1)/2$. Then, $n=m-2r$ is a positive odd integer.
...
\[
  \int_0^\infty
  \frac{s^{r-1}}{(\ell+s)^m}\rho_{n,m}(q+s)\dd s\ge0.
\]
...
\begin{lemma}[Laplace--Gamma representation]\label{lem:pge23-laplace-gamma}
  Let $Y\sim\GammaLaw(r,1)$, where the parameters denote shape and rate.
  If $\ell>0$, then
  ...
  \end{equation}
  Equivalently, if $Z\sim\GammaLaw(m-r,1)$ is independent of $Y$, then
  \begin{equation}\label{eq:pge23-gamma-expectation}
    \widetilde P_{m,r}(q,\ell)
    =\binom{m-r-1}{r}
      \E\!\left[\rho_{n,m}\!\left(q+\frac{\ell}{Z}Y\right)\right].
  \end{equation}
\end{lemma}
...
\begin{proof}
  ...
  To obtain \eqref{eq:pge23-gamma-expectation}, substitute $z=\ell t$
  in this integral. Since $m-r=n+r$, the powers of $\ell$ cancel, and
  the density $z^{m-r-1}e^{-z}/\Gamma(m-r)$ leaves the prefactor
  \[
    \frac{\Gamma(m-r)}{r\Gamma(n)\Gamma(r)}
    =\binom{m-r-1}{r}.
  \]
  This is the expectation formula used in the introduction, with
  $q=1-p$ and $n=m-2r$.
\end{proof}
...
For $z>0$, this is the moment bound \eqref{eq:intro-moment-bound}
after rescaling with $b=1/z$.
...
\begin{proposition}[Shifted-Gamma positivity]\label{prop:pge23-gamma-positive}
  Let $n\ge1$ be odd, let $r\ge1$ be an integer, and set $m=n+2r$.
...
\begin{proof}[Proof of the main theorems]
  ...
  Since $\mu$ is nonnegative, \eqref{eq:Phi-expansion} gives $\Phi_m\ge0$,
...
The excursion expansion gives an explicit lower bound in terms of degree variation.
...
\begin{proof}
  First suppose that $W$ is a step graphon as in \Cref{sec:matrix-reduction}.
  We keep only the top term $r=(m-1)/2$ of the sum \eqref{eq:Phi-expansion}.
  For this $r$ the degree $n=m-2r$ in \eqref{eq:Pmr} equals one, so the two
  complete homogeneous polynomials of degree $n$ are linear,
  \[
    h_1(\rep pr,-\lambda_1,\ldots,-\lambda_r)=rp-\sum_{i=1}^r\lambda_i,
    \qquad
    h_1(\rep qr,\lambda_1,\ldots,\lambda_r)=rq+\sum_{i=1}^r\lambda_i,
  \]
  and the subtracted term is $h_{n-1}(\rep q{r+1},\lambda_1,\ldots,\lambda_r)=h_0=1$.
  The eigenvalues cancel when the two linear forms are added, and $p+q=1$, so the
  integrand for this $r$ is the constant
  \[
    P_{m,r}(q;\lambda_1,\ldots,\lambda_r)
    =\frac mr\bigl(rp+rq\bigr)-1=m-1.
  \]
  All other integrands in \eqref{eq:Phi-expansion} are nonnegative by \Cref{sec:excursion-positivity}.
  Discarding them and integrating the constant $m-1$ against the $r$-fold product of $\mu$ gives
  \[
    \Phi_m(W)\ge(m-1)\mu(\R)^{(m-1)/2}.
  \]
  By the normalization of $g=M_Ue-qe$ and \eqref{eq:moments},
  \[
    \mu(\R)=\norm g^2
    =\int_0^1(\dW(x)-p)^2\dd x,
  \]
  proving \eqref{eq:degree-variance-gap} for step graphons.
  The same-density approximations in \Cref{lem:step-reduction} extend it to every graphon: the densities defining $\Phi_m(W)$ converge, as does the degree variance, which equals $\tP2W-p^2$.
...
\begin{proof}
  ...
  The operator $\Tker U$ is self-adjoint and Hilbert--Schmidt, so the spectral theorem
  writes it as $\sum_i\lambda_iP_i$ over its nonzero eigenvalues, with $P_i$ the
  orthogonal projection onto the eigenspace of $\lambda_i$.
  All these eigenvalues equal $q$, so the sum collapses to
  \[
    \Tker U=qP,
...
For general odd cycle lengths, the only nontrivial range not covered in the present paper is $p\in(1/2,2/3)$; \Cref{prop:short-cycles} already covers this range for $C_3$, $C_5$, and $C_7$.
...
The equality and stability consequences in \Cref{sec:equality-stability} are not included in the formal verification reported here.
...
\begin{proof}[Proof of \Cref{lem:pge23-gamma-moment}]
  ...
  differential equation for $F$. If $H$ were negative anywhere, that
  equation would force $H(b_*)<0$ at an explicit point $b_*$.
  Finally, an integral representation shows that $H(b_*)\ge0$ by comparing
  two local maxima, giving a contradiction.
  ...
  If $H(b_*)=0$, then $H'(b_*)=0$. Differentiating the formula for $H'$
  above and using \eqref{eq:pge23-gamma-ODE} gives
  \[
    H''(b_*)=\frac{r+4j}{b_*}F(b_*)>0.
  \]
  Thus, if $b_*$ is a zero of $H$, it is a strict local minimum and
  cannot be an endpoint of an interval on which $H$ is negative.
  Now
  \[
    H(0)=(r+j)\E Y^{2j}>0,
    \qquad
  ...
  If $H$ were negative somewhere, the maximal interval of negativity containing that point
  ...
  so this interval would contain $b_*$ and give $H(b_*)<0$.
  To rule out this possibility, it remains only to prove
