...
\begin{abstract}
  ...
  Goodman's classical triangle inequality provides the prototype for this question.
  We establish its analogue for every odd cycle in the dense regime.
  In the language of dense graph limits, for every odd integer $m\ge3$, every graphon $W$ of edge density $\edens W=p\ge2/3$ satisfies
  \[
    \tC mW\ge p^m-p(1-p)^{m-1}.
  \]
  Here $\tC mW$ denotes the $C_m$-density of $W$.
  At every density $p=1-1/k$ with an integer $k\ge2$, the balanced complete $k$-partite graphon attains equality; we do not claim optimality at other densities.
  Bennett, Dudek, Lidick\'{y} and Pikhurko~\cite{bennett2020minimizing} proved the corresponding $C_5$ result at these discrete densities for $k\ge3$, and our theorem extends it to every odd cycle while giving a single uniform bound throughout $p\ge2/3$.
  ...
  \]
  For odd cycles with $p\ge2/3$, we strengthen their inequality by adding an explicit nonnegative correction term to the right-hand side.
...
Extremal graph theory begins with a threshold version of this question: how dense can a graph be while avoiding a prescribed subgraph $H$?
The Erd\H os--Stone--Simonovits theorem~\cite{erdos1946structure,erdos1966limit} answers this question asymptotically.
If $\chi(H)=r+1$, then every $H$-free graph has edge density at most $1-1/r+o(1)$.
The supersaturation theorem of Erd\H os and Simonovits~\cite{erdos1983supersaturated} goes further: exceeding this threshold by a fixed amount forces a positive density of copies of $H$.
These theorems locate the threshold at which copies of $H$ become unavoidable, but they do not determine how many copies are forced at each density above it.
The fixed-density problem studied here asks for this quantitative refinement: among graphs of edge density $p$, how small can the density of $H$ be?

Odd cycles are a particularly natural family for this problem.
They are precisely the obstructions to bipartiteness, and bipartite graphs can have edge density as large as $1/2$.
Accordingly, the Erd\H os--Stone--Simonovits threshold for every odd cycle is $1/2$.
Below this threshold all odd cycles can be avoided; above it, every fixed odd cycle occurs with positive density.
The question is therefore not whether odd cycles occur, but how many cycles of each odd length are forced as $p$ varies above $1/2$.

For complete subgraphs, this quantitative refinement leads to the classical Erd\H os--Rademacher or clique-density problem~\cite{erdos1962number}.
Goodman~\cite{goodman1959sets} found a simple polynomial lower bound for triangles that is sharp at the densities of balanced complete multipartite graphs, although not between them.
Determining the exact, piecewise-defined extremal curve required much deeper ideas.
...
For odd cycles, we pursue a complementary goal.
Rather than determine the exact minimum at every $p$, we seek a single low-complexity polynomial bound, with no case distinctions in $p$, that works uniformly for every odd cycle length.
We allow the bound to be nonoptimal between the distinguished densities $p=1-1/k$, but require it to be exact at each of them, where the balanced complete $k$-partite graph is the canonical extremal example.

We formulate this problem in the language of graphons, the natural limit objects for dense graphs.
...
For an integer $k\ge2$, define the \emph{balanced complete $k$-partite graphon} by
\[
  W_k(x,y)=
  \begin{cases}
  ...
  \end{cases}.
\]
It has edge density $p=1-1/k$.
Cycle densities have a useful spectral interpretation~\cite{lovasz2012large}.
If $\Tker W$ is the integral operator on $L^2[0,1]$ with kernel $W$ and eigenvalues $\lambda_i$, then
\[
  \tC mW=\Tr(\Tker W^m)=\sum_i\lambda_i^m
  \qquad(m\ge3).
\]
For $W_k$, the eigenvalue in the constant direction is $p$, while $-1/k$ has multiplicity $k-1$ and all other eigenvalues vanish.
Consequently, for odd $m$,
\begin{equation}\label{eq:intro-kpartite-value}
  \tC m{W_k}=p^m+(k-1)\Bigl(-\frac1k\Bigr)^m=\goal.
\end{equation}
Thus the single polynomial $\goal$ passes through the odd-cycle densities of every balanced complete multipartite graphon.
Our central question is whether it remains a universal lower bound between these distinguished densities, even when it is not the exact minimum.
The trace formula also suggests a way to seek a proof that treats the cycle length $m$ symbolically rather than one value at a time.

For the triangle, Goodman proved that every graphon of edge density $p$ satisfies
\begin{equation}\label{eq:intro-goodman}
  \tC3W\ge2p^2-p=\goodman p3.
\end{equation}
Before this work, extremality of the balanced complete $k$-partite graphon at the distinguished densities $p=1-1/k$, with $k\ge3$, was known for only $C_3$ and $C_5$.
Goodman's bound settles the case of $C_3$.
For $C_5$, Bennett, Dudek, Lidick\'y and Pikhurko~\cite{bennett2020minimizing} proved that, for every integer $k\ge3$, a graph of edge density $1-1/k$ contains asymptotically at least as many copies of $C_5$ as the balanced complete $k$-partite graph.
By the correspondence between graphs and graphons~\cite{lovasz2006limits,lovasz2012large}, their theorem says that $W_k$ minimizes the $C_5$-density among graphons of edge density $1-1/k$.
Their proof is a flag algebra computation together with a stability analysis.
Although their theorem is stated only at the discrete densities $p=1-1/k$, the same flag algebra certificate gives $\tC5W\ge\goodman p5$ throughout $p\ge2/3$ when its parameter $k=1/(1-p)$ is allowed to vary continuously.

This two-paper series establishes the corresponding bound for every odd cycle and every edge density.
The present paper treats the dense regime $p\ge2/3$.
The companion paper proves the remaining nontrivial range $1/2<p<2/3$; for $p\le1/2$, the bound is immediate because its right-hand side is nonpositive.
...
For $m=3$, \Cref{thm:main} follows from Goodman's bound \eqref{eq:intro-goodman}; the new content begins with $m=5$.
The proof in the companion paper for $1/2<p<2/3$ uses different methods, and we discuss the reason for this division in \Cref{sec:conclusion}.

By \eqref{eq:intro-kpartite-value}, equality holds in \Cref{thm:main} at $p=1-1/k$ for every integer $k\ge3$.
...
For $k\ge3$, the corollary follows from \Cref{thm:main} and \eqref{eq:intro-kpartite-value}.
For $k=2$, it holds because $W_2$ is bipartite, so $\tC m{W_2}=0$, whereas $\tC mW\ge0$ for every graphon.
The corollary extends the theorem of Bennett, Dudek, Lidick\'y and Pikhurko from $m=5$ to every odd cycle length.

We prove \Cref{thm:main} through a stronger inequality that treats $W$ and its complement together.
This two-colour formulation is not merely a formal strengthening: adding the two odd-cycle densities creates the cancellation on which our proof rests.
The relevant framework is \emph{commonality}.
A graph $H$ with $e(H)$ edges is common if
\[
  \dens HW+\dens H{1-W}\ge2^{1-e(H)}
\]
for every graphon $W$.
Equivalently, among all $2$-colourings of the edges of a large complete graph, the uniform random colouring asymptotically minimizes the number of monochromatic copies of $H$.
Goodman's theorem implies that the triangle is common.
Erd\H os~\cite{erdos1962number} conjectured that every complete graph is common, while Burr and Rosta~\cite{burr1980ramsey} conjectured that every graph is common.
Thomason~\cite{thomason1989disproof} disproved both conjectures with $K_4$, and Jagger, \v{S}\v{t}ov\'{\i}\v{c}ek and Thomason~\cite{jagger1996multiplicities} later proved that no graph containing $K_4$ is common.
Cycles nevertheless form an important positive family: Sidorenko~\cite{sidorenko1989cycles} proved that every cycle is common.

Kim and Lee~\cite{kim2024extended} established the following sharper statement, called \emph{extended commonality}, for every path or cycle $H$ and every graphon $W$ of edge density $p$:
...
Since $p^e+(1-p)^e\ge2^{1-e}$, extended commonality implies commonality.

For odd cycles with $p\ge2/3$, we strengthen \eqref{eq:intro-extended-commonality} by adding an explicit nonnegative term to its right-hand side.
...
The correction term is nonnegative because the path inequality of Mulholland and Smith~\cite{mulholland1959inequality} gives $\tP{m-1}U\ge q^{m-1}$.
Thus \eqref{eq:intro-commonality} strengthens \eqref{eq:intro-extended-commonality} for odd cycles whenever $p\ge2/3$.
...
The main theorem follows immediately from this stronger statement.
...
\paragraph{Proof strategy: from a graphon to one variable.}
We begin by approximating an arbitrary graphon by step graphons; this reduces the problem to finite matrices without changing the limiting densities.
The constant vector singles out the prescribed edge density, while its orthogonal complement records how the graphon deviates from being constant.
For the complementary graphon $U=1-W$, this decomposition has a particularly useful form.
Because $m$ is odd, the terms that remain entirely in the orthogonal complement occur with opposite signs in $\tC mW$ and $\tC mU$ and therefore cancel when the two cycle densities are added.
This cancellation explains why the stronger two-colour statement is easier to attack than the one-colour inequality alone.

The terms that remain move from the constant direction into its orthogonal complement, spend some number of steps there, and then return.
We call each such departure and return an \emph{excursion}.
Instead of tracking the exponentially many terms in the matrix expansion one by one, we group them by their number $r$ of excursions.
A generating function collects all the terms with the same $r$; its coefficients are complete homogeneous symmetric polynomials.
The commonality gap---the left-hand side of \Cref{thm:commonality} minus its right-hand side---then becomes a sum of integrals of polynomials $P_{m,r}(q;\lambda_1,\ldots,\lambda_r)$ against a positive measure supported on $[-1/2,1/2]$, where the $\lambda_i$ are the nonconstant spectral parameters in this decomposition.
The measure records the sizes of the graphon's nonconstant spectral components, whereas the polynomials depend only on $m$, $r$, and the edge density.
...
A second moment identity, now for a symmetric Beta variable, rewrites the diagonal value as a one-dimensional integral of an explicit polynomial.
In this way, the graphon inequality has been reduced to a scalar sign problem.

One final obstacle remains.
For a nonpositive spectral value, the integrand in the Beta representation is nonnegative pointwise.
For a positive spectral value, it can change sign, so pointwise estimates cannot finish the proof.
A Laplace-transform identity separates the spectral parameter from the integration variable and rewrites the whole integral as a positive mixture of expectations under a Gamma distribution.
After centering at $1/2$, the desired sign follows from a uniform comparison between an odd shifted Gamma moment and the next even moment.
We prove that comparison using the Gamma integration-by-parts identity, also known as the Gamma Stein identity.
It yields a second-order ODE for the even moment.
The ODE shows that any interval of negativity would have to contain one particular critical point; a final integral comparison proves positivity there and rules out such an interval.

This last comparison is also where the density threshold appears.
Writing $q=1-p$, the assumption $p\ge2/3$ says $q\le1/3$; it gives exactly the amount of separation from $1/2$ needed to absorb the odd moment into the even one.
...
$W$ of edge density $p\ge1-1/(\chi(H)-1)$ satisfies
\[
  \dens HW\ge G_H(p).
\]
The definition is motivated by the balanced complete multipartite graphons:
at $p=1-1/k$,
\[
