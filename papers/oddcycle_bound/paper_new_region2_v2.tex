...
\begin{abstract}
  ...
  Two canonical dual witnesses settle its quadratic and linear branches.
  The only finite residue is a pair of exact univariate Bernstein certificates at $m=9$.
...
\begin{enumerate}[label=\textbf{Section \arabic*.},leftmargin=4.6em,start=2]
  ...
  \]
  Two choices of $\lambda$ give the lower bounds (\Cref{cor:plt23-two-witnesses}),
  \begin{align*}
  ...
  \[
    ...
    \zeta(\zeta+v)\ge1
  \]
  (\Cref{subsec:plt23-chart}).
  What has to be proved is that $R_m\le \Omega_m\psi(\xi,\rho)$ holds in that region for every odd $m\ge9$.
  ...
  \begin{itemize}[leftmargin=1.6em,topsep=2pt,itemsep=1pt]
    ...
    \begin{itemize}[leftmargin=1.4em,topsep=1pt,itemsep=0pt]
      \item $v\ge5/8$, for every odd $m\ge9$ (\Cref{lem:plt23-linear-large-v});
      \item $v\le5/8$ and $\zeta\ge m-2$ (\Cref{lem:plt23-linear-high-zeta});
      \item $\zeta\le m-2$ and $m\ge11$ (\Cref{lem:plt23-linear-low-zeta,lem:plt23-J-growth});
      \item $m=9$ with $\zeta\le7$ and $v\le5/8$ (\Cref{lem:plt23-N7-small-v,lem:plt23-N7-middle-v}).
    \end{itemize}
  \end{itemize}
...
The following theorem evaluates it and bounds it from below by a quantity depending only on $q$, $\Lambda$, $M$ and $m$.

\tk{TODO (exposition), deferred: state this idea before the theorem, and rewrite the proof around it. Everything on the left of \eqref{eq:plt23-huber-payment}, namely $\gamma$ and $\norm{g_s}$, and with them $a_\phi$, $b$, $\norm{\phi_\perp}$ and $u_\gamma$, depends on the eigenfunction $\phi$, whereas $\Omega_m$, $\xi$ and $\rho$ depend only on the eigenvalues $\Lambda$ and $M$, together with $q$ and $m$. The theorem is the passage from eigenfunction data to eigenvalue data, and every step of the proof eliminates one of those eigenfunction quantities. Only the case $u_\gamma>0$ is written that way at present; the cases $\phi_\perp=0$ and $u_\gamma\le0$ perform the same eliminations without saying so. The sentence above already carries part of this and should be absorbed.}
...
\begin{proof}
  We first minimize \eqref{eq:plt23-min-over-Sbar} in the variable $\overline{\norm{g_s}}$.
  ...
  Suppose for the moment that $\overline\gamma\ge0$.
  Then \eqref{eq:plt23-gamma-upper-bound} gives
  \[
    2\overline\gamma a_\phi+2\Lambda b
    \le a_\phi^2-2\Lambda=1-2\Lambda-b^2-\norm{\phi_\perp}^2
    \le1-2\Lambda-b^2,
  \]
  so, since $a_\phi\ge\sqrt{2\Lambda}$ by \eqref{eq:plt23-aphi-lower},
  \begin{equation}\label{eq:plt23-b-gamma-budget}
    b^2+2\Lambda b+2\overline\gamma\sqrt{2\Lambda}\le1-2\Lambda,
  \end{equation}
  where the assumed hypothesis $\overline\gamma\ge0$ is what makes $2\overline\gamma a_\phi\ge2\overline\gamma\sqrt{2\Lambda}$.

  We can use this inequality to obtain two bounds on $\overline\gamma$ and $b$ separately.
  First, since $\phi$ was chosen so that $b\ge0$, the first two terms on the left of \eqref{eq:plt23-b-gamma-budget} are nonnegative, so
  \begin{equation}\label{eq:plt23-gamma-range}
    0\le\overline\gamma\le\frac{1-2\Lambda}{2\sqrt{2\Lambda}}.
  \end{equation}
  Second, viewing \eqref{eq:plt23-b-gamma-budget} as a quadratic inequality in $b$ gives
  \[
  ...
  \begin{equation}\label{eq:plt23-beta-rational}
    \beta(\overline\gamma)
    =\frac{1-2\Lambda-2\overline\gamma\sqrt{2\Lambda}}
    {\sqrt{(1-\Lambda)^2-2\overline\gamma\sqrt{2\Lambda}}+\Lambda}
    \le\frac{1-2\Lambda-2\overline\gamma\sqrt{2\Lambda}}{2\Lambda},
  \end{equation}
  holds, where the equality comes from rationalizing the square root, using $(1-\Lambda)^2-\Lambda^2=1-2\Lambda$, and the inequality holds since the numerator is nonnegative and the denominator is at least $2\Lambda$.
  Set
  ...
  \end{equation}
  Here $\overline v$ lies in $[0,1]$ by \eqref{eq:plt23-gamma-range}, the interval over which the minimum in \eqref{eq:plt23-psi-def} is taken, and the last identity follows by the definitions \eqref{eq:plt23-C-xi-rho} and \eqref{eq:plt23-rho-def}.
  Substituting in \eqref{eq:plt23-beta-rational}, we obtain the inequality that holds under the assumption $\overline\gamma\ge0$,
  \begin{equation}\label{eq:plt23-gamma-beta-v}
    \overline\gamma\beta(\overline\gamma)
    \le\frac{(1-2\Lambda)^2\overline v(1-\overline v)}{4\Lambda\sqrt{2\Lambda}}.
  \end{equation}

  Now suppose first that $\phi_\perp=0$.
  Then the constraint $b\overline\gamma\ge\kappa$ of \eqref{eq:plt23-one-d-t-zero} simplifies to
  \[
    b\overline\gamma\ge\kappa
    =\frac{(\Lambda-q)a_\phi^2+(\Lambda-M)\norm{\phi_\perp}^2}{2a_\phi}
    =\frac{(\Lambda-q)a_\phi}{2}>0,
  \]
  where $\Lambda>q$ by \eqref{eq:plt23-M-Lambda-order} and $a_\phi=\ip\one{|\phi|}>0$.
  ...
  With the upper bound $b\le\beta(\overline\gamma)$ of \eqref{eq:plt23-beta-gamma}, we have
  \[
  ...
  Now assume $\phi_\perp\ne0$.
  Suppose first that $u_\gamma\le0$.
  Then every $(\overline\gamma,\overline{\norm{g_s}})\in\overline S$ satisfies $\overline\gamma\le0$, which implies $\kappa-b\overline\gamma\ge\kappa\ge0$ together with \eqref{eq:plt23-gs-inner-lower} and $b\ge0$, giving the lower bound on the right side of \eqref{eq:plt23-one-d-t}:
  \[
    \min_{\overline\gamma\le u_\gamma}
    \left\{
    \Lambda_m\overline\gamma^2
  ...
  \]
  which is equivalent to $\bigl((\Lambda-q)a_\phi^2-(\Lambda-M)\norm{\phi_\perp}^2\bigr)^2\ge0$.
  ...
  Also, since $2\Lambda\le a_\phi^2\le1$ by \eqref{eq:plt23-aphi-lower} and \eqref{eq:plt23-absphi-normalization},
  \begin{equation}\label{eq:plt23-H-lower}
    \kappa\ge\frac{(\Lambda-q)\sqrt{2\Lambda}}{2}
    +\frac{(\Lambda-M)\norm{\phi_\perp}^2}{2}.
  \end{equation}
  ...
  As discussed, we will now eliminate the dependence on the eigenfunction, namely $\norm{\phi_\perp}$ and $b$ in the objective and $u_\gamma$ in the constraint, in this order, to match the independence of the final lower bound from those quantities.

  In the lower bound \eqref{eq:plt23-H-lower} for $\kappa$, only the second term involves $\phi_\perp$.
  Writing $t=\norm{\phi_\perp}^2>0$ and
  \[
    w=\frac{(\Lambda-q)\sqrt{2\Lambda}}{2}-b\overline\gamma
  \]
  for the first term less $b\overline\gamma$, that bound reads $\kappa-b\overline\gamma\ge w+(\Lambda-M)t/2$.
  Since $x\mapsto\pos x^2$ is nondecreasing,
  \[
    \frac{M_m}{\norm{\phi_\perp}^2}\pos{\kappa-b\overline\gamma}^2
    \ge M_m\,\frac{\pos{w+(\Lambda-M)t/2}^2}{t},
  \]
  and the right side involves $\phi_\perp$ only through $t$.
  Bounding it below by its infimum over all $t>0$ therefore removes $\phi_\perp$, and for every real $w$ that infimum is
  \begin{equation}\label{eq:plt23-K-AMGM}
    \inf_{t>0}\frac{\pos{w+(\Lambda-M)t/2}^2}{t}=2(\Lambda-M)\pos w.
  \end{equation}
  Indeed, for $w\ge0$ this is the arithmetic--geometric mean inequality, with equality at $t=2w/(\Lambda-M)$; for $w<0$, both sides vanish after choosing $0<t\le-2w/(\Lambda-M)$.
  Hence the second term of the objective is at least $2M_m(\Lambda-M)\pos w$.

  In this bound $b$ appears only in $w$, and $b\overline\gamma\le\overline\gamma\beta(\overline\gamma)$ because $\overline\gamma\ge0$ and $b\le\beta(\overline\gamma)$ by \eqref{eq:plt23-beta-gamma}, so
  \[
    \pos w\ge
    \pos{\frac{(\Lambda-q)\sqrt{2\Lambda}}{2}
    -\overline\gamma\beta(\overline\gamma)}.
  \]
  Finally, \eqref{eq:plt23-gamma-range} bounds $\overline\gamma$ by $(1-2\Lambda)/(2\sqrt{2\Lambda})$, and enlarging the range of a minimum only decreases it.
  Together with \eqref{eq:plt23-min-over-Sbar} and \eqref{eq:plt23-one-d-t}, these three steps give
  \begin{equation}\label{eq:plt23-one-d-gamma}
    \Lambda_m\gamma^2+M_m\norm{g_s}^2
    \ge
    \min_{0\le\overline\gamma\le(1-2\Lambda)/(2\sqrt{2\Lambda})}
    \left\{
    \Lambda_m\overline\gamma^2+2M_m(\Lambda-M)
    \pos{\frac{(\Lambda-q)\sqrt{2\Lambda}}{2}
    -\overline\gamma\beta(\overline\gamma)}
    \right\}.
  \end{equation}
  It remains to rewrite \eqref{eq:plt23-one-d-gamma} in the variable $\overline v$ of \eqref{eq:plt23-v-def}, which runs over $[0,1]$ as $\overline\gamma$ runs over $[0,(1-2\Lambda)/(2\sqrt{2\Lambda})]$.
  For the second term of the objective, \eqref{eq:plt23-gamma-beta-v} gives
  \[
    \frac{(\Lambda-q)\sqrt{2\Lambda}}{2}
    -\overline\gamma\beta(\overline\gamma)
  ...
  \]
  where the equality collects the two terms using the definition \eqref{eq:plt23-xi-def} of $\xi$ and $1/(4\Lambda\sqrt{2\Lambda})=\sqrt{2\Lambda}/(8\Lambda^2)$.
  The factor in front is positive and $x\mapsto\pos x$ is nondecreasing, so
  \[
    2M_m(\Lambda-M)
    \pos{\frac{(\Lambda-q)\sqrt{2\Lambda}}{2}
    -\overline\gamma\beta(\overline\gamma)}
    \ge2M_m(\Lambda-M)\frac{\sqrt{2\Lambda}\,(1-2\Lambda)^2}{8\Lambda^2}
    \pos{\xi-\overline v+\overline v^2}.
  \]
  For the first term, \eqref{eq:plt23-v-def} gives $\Lambda_m\overline\gamma^2=\Omega_m\rho\overline v^2$, and the definition \eqref{eq:plt23-C-xi-rho} of $\Omega_m$ gives
  \[
    2M_m(\Lambda-M)\frac{\sqrt{2\Lambda}\,(1-2\Lambda)^2}{8\Lambda^2}
    =\Omega_m.
  \]
  Hence the objective of \eqref{eq:plt23-one-d-gamma} is at least $\Omega_m\bigl(\rho\overline v^2+\pos{\xi-\overline v+\overline v^2}\bigr)$ at every $\overline\gamma$ in the range of that minimum, and minimizing over $\overline v\in[0,1]$ gives
  \[
    \Lambda_m\gamma^2+M_m\norm{g_s}^2
    \ge\Omega_m\min_{0\le v\le1}
    \bigl\{\rho v^2+\pos{\xi-v+v^2}\bigr\}
    =\Omega_m\psi(\xi,\rho)
  \]
  by \eqref{eq:plt23-psi-def}, which is \eqref{eq:plt23-huber-payment}.
\end{proof}

It remains to say which pairs $(q,\Lambda)$ can occur.
The eigenfunction $\phi$ restricts them further, through a lower bound on $\norm g$.
...
\begin{proof}
  ...
  The block form \eqref{eq:block-U} gives
  \[
    \ip{|\phi|}{T_U|\phi|}
    =qa_\phi^2+2a_\phi\ip gh+\ip h{Ah},
  \]
  and bounding $\ip gh\le\norm g\norm h$ by Cauchy--Schwarz and $\ip h{Ah}\le\Lambda\norm h^2$ by the maximality of $\Lambda$, \eqref{eq:plt23-absphi-lower} gives
  \[
  ...
  \]
  and $t\mapsto t/(1-t)$ is increasing, so \eqref{eq:plt23-aphi-lower} allows $t$ to be replaced by $2\Lambda$, which is \eqref{eq:plt23-forced-variance}.
  ...
  \]
  With $p=1-q$, the difference of the two sides is $-(1-\Lambda-q)(\Lambda^2+q\Lambda-q)$, and the factor $1-\Lambda-q$ is positive by \eqref{eq:plt23-M-Lambda-order}, so \eqref{eq:plt23-alpha-ceiling-poly} follows.
  The roots of $\Lambda^2+q\Lambda-q$ are $(-q\pm\sqrt{q^2+4q})/2$, whence \eqref{eq:plt23-alpha-ceiling}.

  Finally, because $\Lambda>q$, the right side of \eqref{eq:plt23-forced-variance} is strictly positive.
  Thus $g\ne0$, and the bound \eqref{eq:plt23-HS-bound} gives $\Lambda^2<pq$.
  Consequently $M>0$.
\end{proof}
...
\begin{proof}
  ...
  \]
  is convex in $v$, affine in $\lambda$, and the two domains are compact convex intervals.
  The elementary finite-dimensional minimax theorem (or Sion's theorem) permits the interchange of $\min_v$ and $\max_\lambda$.
...
The function $\psi$ has the quadratic-versus-linear structure of Huber's robust loss~\cite{Huber1964}, and \eqref{eq:plt23-psi-dual} is an interchange of the convex--concave type treated by Sion~\cite{Sion1958}; the special case needed here is the one proved above.
...
\begin{proof}
  In the first case, take $\lambda=2\rho\xi$ in \eqref{eq:plt23-psi-dual}.
  This gives
  \[
    \psi(\xi,\rho)\ge
  ...
  In the second case, take $\lambda=1$.
  Since $\rho>1/(2\xi)$,
  \[
...
\tk{TODO (notation): $N$ defined below collides with the partition size $N$ of \Cref{subsec:block-decomposition} (the $N\times N$ matrices of \Cref{lem:series-expansions}); several later subsection titles use this $N$, so rename one of the two.}
Set
...
\begin{proof}
  The order $0<M<q<\Lambda$ gives $0<\ell<a<1$.
  Since $q=a^2/D>1/3$, we have $D<3a^2$.
  The ceiling $\Lambda^2+q\Lambda-q\le0$, after substitution from \eqref{eq:plt23-chart-inverse-1}, is exactly
  \[
    \ell^2\ge a-a^2=au.
  \]
  Hence $D\ge1+a$.
  Combining this with $D<3a^2$ gives $3a^2-a-1>0$, which is the stated lower bound on $a$.
  Finally, $\sigma=p/\Lambda>1$ because $p>1/2>\Lambda$, while $p<2/3$ and $\Lambda>q>1/3$ give $\sigma<2$.
\end{proof}
...
\begin{equation}\label{eq:plt23-F-T}
  F_N\le uT_N.
\end{equation}
If $T_N\le0$, then $R_m\le0$, and the scalar target \eqref{eq:plt23-scalar-target} is immediate.
We may therefore assume
...
\begin{proof}
  ...
  \[
    a^N\ge1-Nu.
  \]
  Also $\ell^2=\zeta v/H_\zeta<v$, so $\ell^N<v^{N/2}$.
  Since $\zeta u< v$,
  \begin{align*}
    T_N
    &=N-\zeta(a^N-\ell^N)\\
  ...
  \]
  Thus $a-\ell>1-a=u$.
  By convexity of $x\mapsto x^N$,
  \[
    a^N-\ell^N>N(a-\ell)\ell^{N-1}>Nu\ell^{N-1}.
  \]
  Multiplication by $\zeta=\ell^2/u$ gives \eqref{eq:plt23-T-bound-two}.

  Assume now $T_N>0$.
  From \eqref{eq:plt23-T-bound-one},
  ...
  \]
  because $v^r\le1$ and Bernoulli's inequality gives $(1-\theta)^{-r}\ge1+r\theta$.
  Therefore, using \eqref{eq:plt23-T-bound-two} and \eqref{eq:plt23-zeta-upper-Z},
  ...
  \]
  is convex.
  Indeed, with $r=(N+1)/2$, $s=(N+1)/N$, and $d=N+1-\eta$,
  \[
    \frac{f''(\eta)}{f(\eta)}
    =\frac{r(r+1)}{d^2}
    +\frac{2rs}{d\eta}
  ...
  \]
  Substitution in \eqref{eq:plt23-gZ-step} proves \eqref{eq:plt23-T-tangent-bound}.
\end{proof}
...
\begin{equation}\label{eq:plt23-quadratic-branch}
  2\rho\xi\le1.
\end{equation}
We shall prove \eqref{eq:plt23-scalar-target} for every odd $N\ge7$, hence for every odd $m\ge9$.

By \eqref{eq:plt23-witness-quadratic}, discarding the factor $(1+4\xi)/(1+2\xi)>1$, and using \eqref{eq:plt23-KA-def},
...
\end{equation}
Since the summands in \eqref{eq:plt23-KA-sum} have geometric mean $\sigma^{(N-1)/2}$, AM--GM gives
\begin{equation}\label{eq:plt23-KA-AMGM}
  K_\Lambda\ge\frac{N+1}{2}
  v\sigma^{(N-1)/2}.
\end{equation}
Using \eqref{eq:plt23-chart-inverse-1}, \eqref{eq:plt23-chart-d-e}, and $v=u(1+\zeta)/a$, the right side of \eqref{eq:plt23-quadratic-payment-start} is at least
...
\begin{proof}
  ...
  \[
    ...
    >\frac{\zeta+1}{\zeta},
  \]
  while \eqref{eq:plt23-Q-upper} gives $Q<3(\zeta+1)$.
  Multiplication proves \eqref{eq:plt23-M-lower}.
\end{proof}
...
\begin{proof}
  If $T_N\le0$, the claim follows from \eqref{eq:plt23-F-T}.
  Assume $T_N>0$.
  ...
  \end{equation}
  Second, the tangent estimate \eqref{eq:plt23-T-tangent-bound} shows that \eqref{eq:plt23-quad-payment-final} is sufficient whenever
  ...
  It remains to take $N=7$.
  For $v\le9/10$,
  \[
    E_7(v)\ge\frac{96}{49}(1-v^3)(1+v)\ge1.
  \]
  Indeed, $1+v-v^3-v^4$ is concave, so its minimum on $[0,9/10]$ occurs at an endpoint; at $v=9/10$ it equals $5149/10000>49/96$.
  For $v\ge9/10$,
  \[
    H_7(v)\ge\frac{2}{7}\left(\frac{19}{10}\right)^2
    =\frac{361}{350}>1.
  \]
  Thus either \eqref{eq:plt23-E-N} or \eqref{eq:plt23-H-N} holds in every case, proving the proposition.
\end{proof}
...
\end{equation}
By \eqref{eq:plt23-witness-linear}, \eqref{eq:plt23-KL-def}, and $\Lambda-M=\Lambda(1-\ell)$, $\Lambda-q=\Lambda u$,
...
The factor $c_\xi$ may be small, but only where a short power of $\sigma$ compensates for it.
...
\begin{proof}
  ...
  \]
  because $a>3/4$.
  This proves \eqref{eq:plt23-sqrt-compensation}.

  Next we show $\varphi\sigma^{\zeta/4}\ge1$.
  From \eqref{eq:plt23-C-xi-rho}, \eqref{eq:plt23-zeta-v-inverse}, and \eqref{eq:plt23-D-Q},
  ...
  \begin{equation}\label{eq:plt23-key-log-comp}
    \zeta v(1+4\xi)>2(v+2).
  \end{equation}
  Indeed, the left side is larger than $\zeta v+16/3$.
  If $\zeta\ge2$, this is immediate.
  If $\zeta<2$, then
  \[
    v(2-\zeta)<\frac{5}{4}<\frac{4}{3}
  \]
  because $v<1$ and $\zeta>3/4$, which again proves \eqref{eq:plt23-key-log-comp}.
  ...
  \]
  Together with \eqref{eq:plt23-sqrt-compensation}, this proves \eqref{eq:plt23-compensation}.
\end{proof}
...
\begin{lemma}[The regime $\zeta\ge N$]\label{lem:plt23-linear-high-zeta}
  If $\zeta\ge N$, $0<v\le5/8$, and $N\ge7$ is odd, then \eqref{eq:plt23-linear-core} holds.
\end{lemma}
...
\begin{proof}
  ...
  \[
    ...
    \ge(1-y)(1-y+y^2).
  \]
  Also $q<\Lambda$ and $p=(1+v)\Lambda$ imply $\Lambda>1/(2+v)$, while $\varphi>1/2$.
  Hence
  ...
  \end{equation}
  Indeed, the omitted factors $(N+2)/N$ and $(1+y^2)^{N-6}$ are at least $1$.

  We claim that $h$ is decreasing on $(0,4/5]$.
  ...
  \]
  Here the first inequality follows from $y^3(11y^2-20y+10)\ge0$, and the cubic on the right is decreasing on this interval.
  For $1/2\le y\le4/5$, write $y=2/3+t$.
  If $t\ge0$, then $0\le t\le2/15$, and
  \begin{align*}
  ...
  \[
    ...
    <\frac{46}{15};
  \]
  after squaring, the final difference is $52766011/351562500>0$.
  This proves \eqref{eq:plt23-h-sufficient} and hence \eqref{eq:plt23-linear-core}.
\end{proof}

The next estimate handles all $\zeta$ once $v$ is large.

\begin{lemma}[Large $v$]\label{lem:plt23-linear-large-v}
  If $v\ge5/8$ and $N\ge7$ is odd, then \eqref{eq:plt23-linear-core} holds.
\end{lemma}
...
\begin{proof}
  ...
  \begin{equation}\label{eq:plt23-T-geometric-cancel}
    T_N<N(N+1)(1-\ell).
  \end{equation}
  Using \eqref{eq:plt23-cxi-crude}, $\ell\le\sqrt v$, and $\sigma=1+v$, it is enough, after cancelling $1-\ell$, to prove
  \begin{equation}\label{eq:plt23-large-v-target}
    \frac{N+2}{\sqrt{2(2+v)}}
    (1+v-\sqrt v)(1+v)^{N-1}
    \ge N(N+1).
  \end{equation}
  The left side is increasing in $v$ for $v\ge5/8$: the factor $1+v-\sqrt v$ is increasing for $v>1/4$, and
  \[
  ...
  \[
    ...
    \qquad(N\ge7).
  \]
  It remains to check $N=7$, $v=5/8$.
  The estimates $\sqrt{10}<19/6$ and $\sqrt{21}<23/5$ give
  \[
    \frac{9}{\sqrt{21/4}}
    ...
    >56,
  \]
  the final excess being $24369203/6029312$.
  Thus \eqref{eq:plt23-large-v-target} holds.
\end{proof}

\subsection{The regime \texorpdfstring{$\zeta\le N$}{zeta<=N}: the growth lemma}

It remains, except for $N=7$, to handle $\zeta\le N$.
The compensation lemma reduces this to one variable.
...
\begin{proof}
  ...
  \end{equation}
  The numerator equals $6792$ at $N=9$; its derivative is $28889$ there and is increasing afterwards.
  Hence the quadratic has positive global minimum, and $\mathcal L_N(x)>0$ on $[0,1/4]$.
  ...
  \]
  It is therefore enough to take $N=9$.
  ...
  The quadratic $31-104x+(2282/25)x^2$ is decreasing on this interval and has value $2503/625$ at $x=2/5$.
  Thus $\mathcal L_9'''<0$.
  Since $\mathcal L_9''(2/5)>0$, we have $\mathcal L_9''>0$ throughout the interval; the first two inequalities in \eqref{eq:plt23-L9-data} then imply $\mathcal L_9>0$.
  ...
  \]
  condition \eqref{eq:plt23-J-domain} is exactly $x\le\beta_N$.
  Since $N/(N-x^2)>1$, it is enough to prove
  ...
  \[
    ...
    \qquad(x\ge2/5).
  \]
  Thus $P_N$ is convex on the interval and, starting negative, can cross zero at most once.
  Therefore $K_N$ has no interior minimum: it is enough to check $x=2/5$ and $x=\beta_N$.
  ...
  \]
  where Bernoulli's inequality was used in the last step.
  Thus the first endpoint is settled for every odd $N\ge9$.

  For the second endpoint, use $(1-\beta_N)^2=4\beta_N^4/N^2$.
  The function $h_N(x)=1-x-2x^2/N$ is strictly decreasing, and $h_N(\beta_N)=0$.
  ...
  \]
  show that $\beta_9>21/25$, $\beta_{11}>6/7$, and $\beta_N\ge7/8$ for every $N\ge13$.
  When $N=9$, hence
  \begin{align*}
    K_9(\beta_9)
    &>\frac{11}{9}\frac{4}{81}
  ...
  When $N=11$, $\beta_{11}>6/7$, and the coarser estimates $\beta_{11}^4>1/2$, $1+\beta_{11}^3>3/2$, and $1+\beta_{11}^2>17/10$ give
  \[
    K_{11}(\beta_{11})
  ...
  Finally, for $N\ge13$, $\beta_N\ge7/8$.
  Dropping the factors $(N+2)/N$ and $1+\beta_N^3$,
  \[
    K_N(\beta_N)
    \ge\frac{4(7/8)^4}{N^2}
  ...
  \]
  This completes all endpoint checks and the proof of \eqref{eq:plt23-J-ineq}.
\end{proof}

\begin{lemma}[The regime $\zeta\le N$]\label{lem:plt23-linear-low-zeta}
  If $\zeta\le N$ and $N\ge9$ is odd, then \eqref{eq:plt23-linear-core} holds.
\end{lemma}
...
\begin{proof}
  ...
  \end{align}
  Indeed, $(1+\ell)(\sigma-\ell)\ge1+\ell^3$ because $\sigma\ge1+\ell^2$.
  ...
  \end{equation}
  Also $N-1-\zeta/4\ge3N/4-1$.
  Finally, $\ell<2a-1$ from the proof of \eqref{eq:plt23-T-bound-two}, so
  \[
    1-\ell>2u=\frac{2\ell^2}{\zeta}
    \ge\frac{2\ell^2}{N}.
  \]
  Applying \Cref{lem:plt23-J-growth} with $x=\ell$ to \eqref{eq:plt23-linear-comp-expanded} shows that its right side is at least $N$.
  Since $T_N<N$, \eqref{eq:plt23-linear-core} follows.
\end{proof}

\subsection{The exceptional case \texorpdfstring{$N=7$, i.e. $m=9$}{N=7, i.e. m=9}}

The preceding lemmas already cover $N=7$ whenever either $\zeta\ge7$ or $v\ge5/8$.
It remains to prove \eqref{eq:plt23-linear-core} under
...
\begin{lemma}[The small-$v$ part of $N=7$]\label{lem:plt23-N7-small-v}
  Under \eqref{eq:plt23-N7-corner}, if $0<v\le1/4$, then \eqref{eq:plt23-linear-core} holds.
\end{lemma}
...
\begin{proof}
  ...
  \end{equation}
  Let $t=\zeta+v$.
  Formula \eqref{eq:plt23-xi-zeta-v} becomes
  \[
    4v\xi=
  ...
  \]
  Since $t\le7+1/4=29/4$, its maximum is
  \[
    \frac{246413}{17424}<16.
  \]
  This proves \eqref{eq:plt23-xi-small-v}.

  The function $(1+4\xi)/(2+4\xi)$ is increasing in $\xi$, so \eqref{eq:plt23-xi-small-v} gives
  \[
  ...
  If $M(v)$ and $R(v)$ denote respectively the left and right sides of \eqref{eq:plt23-cxi-small-v-poly}, then exact expansion gives
  \begin{align}
  ...
  For the defect, use \eqref{eq:plt23-zeta-simple-lower}.
  From \eqref{eq:plt23-T-bound-one},
  \begin{align*}
    T_7
  ...
  \end{align*}
  because $\sqrt v\le1/2$.
  On the other hand, \eqref{eq:plt23-ell-small-v} and \eqref{eq:plt23-cxi-small-v-poly} show that the right side of \eqref{eq:plt23-linear-core} is at least
  \[
  ...
  \end{align}
  By \Cref{lem:bernstein-P9}, $P_9(s)>0$ on $[0,1/2]$.
  This proves \eqref{eq:plt23-linear-core}.
\end{proof}

\begin{lemma}[The middle-$v$ part of $N=7$]\label{lem:plt23-N7-middle-v}
  Under \eqref{eq:plt23-N7-corner}, if $1/4\le v\le5/8$, then \eqref{eq:plt23-linear-core} holds.
\end{lemma}

\begin{proof}
  We first show $\xi>1/3$.
  With $t=\zeta+v$,
  \[
    \xi=\frac{4}{v}\frac{(t+1)^2}{t^2(t(v+2)+2)}.
  \]
  The quantity $vG(t,v)$, with $G$ as in the proof of \Cref{lem:plt23-N7-small-v}, is increasing in both variables.
  Indeed,
  \[
  ...
  \begin{equation}\label{eq:plt23-y-middle}
    y^2=\frac{7v}{8+v}.
  \end{equation}
  Since $\zeta\le7$, we have $\ell\le y$.
  The first half of the compensation proof, \eqref{eq:plt23-sqrt-compensation}, gives
  \[
    \sqrt{2\Lambda}\ge1-\ell^2\ge1-y^2.
  \]
  Also, $v^{7/2}\le v/2$ on $[1/4,5/8]$, because $v^{5/2}\le(5/8)^{5/2}<1/2$.
  Together with \eqref{eq:plt23-zeta-simple-lower}, \eqref{eq:plt23-T-bound-one} yields
  ...
  We verify that the ratio of the two sides is increasing.
  Solving \eqref{eq:plt23-y-middle} gives
  \[
  ...
  \begin{align}
    ...
    \label{eq:plt23-Q10}
  \end{align}
  The denominator in \eqref{eq:plt23-R-log-derivative} is positive on $[9/20,5/7]$, and \Cref{lem:bernstein-Q10} gives $Q_{10}(y)>0$ there.
  Thus $\mathcal R$ is increasing.

  At $v=1/4$, one has $y_0=\sqrt{7/33}<6/13$.
  Since the factors involving $y$ decrease with $y$,
  \begin{align*}
  ...
  \end{align*}
  This proves \eqref{eq:plt23-middle-target} at the left endpoint.
  The monotonicity of $\mathcal R$ proves it throughout the interval.
\end{proof}
...
\begin{proof}
  For $N\ge9$, if $\zeta\le N$, use \Cref{lem:plt23-linear-low-zeta}.
  If $\zeta\ge N$, use \Cref{lem:plt23-linear-high-zeta} when $v\le5/8$, and \Cref{lem:plt23-linear-large-v} when $v\ge5/8$.

  For $N=7$, the same two broad lemmas cover the cases $\zeta\ge7$ or $v\ge5/8$.
  The remaining corner \eqref{eq:plt23-N7-corner} is covered by \Cref{lem:plt23-N7-small-v,lem:plt23-N7-middle-v}.
  In every case \eqref{eq:plt23-linear-core} holds.
  Combining it with \eqref{eq:plt23-linear-payment-start}, \eqref{eq:plt23-KL-first-term}, and \eqref{eq:plt23-F-T} proves the proposition.
\end{proof}
...
\begin{proof}[Proof of \Cref{thm:main}]
  ...
  Otherwise there is a unique frontier eigenvalue $\Lambda$, and \Cref{prop:plt23-splitting-lower-bound,thm:plt23-huber-elim} reduce the problem to the scalar target \eqref{eq:plt23-scalar-target}.
  If $2\rho\xi\le1$, \Cref{prop:plt23-quadratic-branch} proves that target.
  If $2\rho\xi>1$, \Cref{prop:plt23-linear-branch} proves it.
...
\begin{remark}[What is finite and what is uniform]
  ...
  The intermediate-region argument is uniform in every odd $m\ge9$, except for a small corner at $m=9$, where two fixed univariate polynomials are certified positive in \Cref{app:bernstein}.
...
The witnesses $\lambda=2\rho\xi$ and $\lambda=1$ are the quadratic and saturated linear regimes.
...
For completeness, this appendix records the only two fixed polynomial positivity checks used in the proof.
The Bernstein-basis principle used here is reviewed in Farouki~\cite{Farouki2012}.
We include the coefficients themselves, not merely a reference to a computer calculation.
...
\[
  ...
  \quad(0\le i\le n).
\]
The functions $B^{[a,b]}_{i,n}$ are nonnegative on $[a,b]$ and sum to one.
Hence an expansion
\[
  P(x)=\sum_{i=0}^n \beta_i B^{[a,b]}_{i,n}(x)
\]
with all $\beta_i>0$ proves $P(x)>0$ throughout the interval.
The coefficients can be checked directly as follows.
If
\[
...
\begin{proof}
  Its degree-nine Bernstein coefficients on $[0,1/2]$, in increasing order of the index, are
  \begin{align*}
    &3,
    &&\frac{33}{16},
    &&\frac{17795}{12288},
    ...
    &&\frac{361511}{524288}.
  \end{align*}
  Every numerator and denominator is positive.
  The Bernstein partition-of-unity argument therefore proves the claim.
  Formula \eqref{eq:power-to-bernstein} gives a short exact-arithmetic verification of the displayed list.
\end{proof}

\begin{lemma}[The degree-ten certificate]\label{lem:bernstein-Q10}
  The polynomial $Q_{10}$ in \eqref{eq:plt23-Q10} is positive on $[9/20,5/7]$.
\end{lemma}

\begin{proof}
  Its degree-ten Bernstein coefficients on $[9/20,5/7]$, in increasing order of the index, are
  \begin{align*}
  ...
  Since the Bernstein basis is nonnegative and sums to one, $Q_{10}>0$ on the stated interval.
  The conversion formula \eqref{eq:power-to-bernstein} verifies the list using only rational arithmetic.
\end{proof}

\begin{remark}[Why this is a proof certificate]
  A Bernstein-coefficient list is not a numerical sample.
  Once the exact identity with the Bernstein expansion is checked, positivity at every point of the whole interval follows from the nonnegativity of the basis functions.
  In the present proof no subdivision is used: one interval suffices for each of the two polynomials.
\end{remark}
