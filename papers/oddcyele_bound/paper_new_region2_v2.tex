...
\begin{enumerate}[label=\textbf{Part \arabic*.},leftmargin=3.4em]
  ...
  $\sum_i\lambda_i^2\le pq$ by \Cref{lem:HS-budget}, a contradiction because $2q^2 > pq$ when $q>1/3$.
  ...
  \]
  which with $2\norm g^2\le pq-\Lambda^2$, again from \Cref{lem:HS-budget},
  gives $\Lambda^2+q\Lambda-q\le0$ (\Cref{lem:forced-variance}).
  ...
  \]
  where $C_m$, $\xi$, and $\rho$ depend on $q$, $\Lambda$,
  ...
  The goal $R_m\le C_m\psi(\xi,\rho)$ involves only $q$, $\Lambda$, and $m$.
...
\begin{proof}
  ...
  Next we differentiate both sides of \eqref{eq:dense-beta-h} with respect to $a$.
...
\begin{lemma}[Laplace--gamma representation]\label{lem:dense-gamma-smoothing}
...
\begin{theorem}[The theorem for $p\ge2/3$]\label{thm:dense-region}
...
\Cref{sec:dense-region}, so that a reader interested in one region need not
...
\subsection{A one-sided Schur-complement identity}\label{subsec:shift}

In \Cref{subsec:two-sided} we introduced the generating functions $L_-$ and
...
\begin{equation}\label{eq:shift-factorization}
...
\begin{lemma}[One-sided identity]\label{lem:shift}
  For every odd $m\ge3$,
...
\begin{proof}
  Taking $-\log$ in \eqref{eq:shift-factorization} turns the product into a
  sum of three terms,
  \[
    -\log\det(I-zT_W)=-\log\det(I+zA)-\log(1-pz)+L_-(z),
  \]
  and for odd $m$, \Cref{lem:analytic-reading}\,(2) gives
  \[
    m\,[z^m]\bigl(-\log\det(I+zA)\bigr)=\Tr\bigl((-A)^m\bigr)=-\Tr(A^m),
    \qquad
    m\,[z^m]\bigl(-\log(1-pz)\bigr)=p^m.
  \]
  Together with the third summand this is \eqref{eq:shift}.
  ...
  the denominator on the right of \eqref{eq:X-coeff} are positive.  The
...
\begin{lemma}[Eigenvalues above $q$]\label{lem:no-frontier}
...
\begin{proof}
  ...
  \Cref{lem:HS-budget}.  This proves the first claim.
  ...
  \]
  where we used \Cref{lem:HS-budget} in the last inequality.
  Since the coefficients of $L_-$ are nonnegative, \eqref{eq:shift} gives
  \[
...
\begin{equation}\label{eq:L-def}
  M=\sqrt{pq-\Lambda^2},
\end{equation}
which by \eqref{eq:HS-budget} controls the remaining eigenvalues:
\[
...
\subsection{A lower bound via splitting $g$ along the eigenfunction of
$\Lambda$}\label{subsec:master-defect}
...
\begin{proposition}[Lower bound via splitting $g$ along the eigenfunction of $\Lambda$]\label{prop:master-defect}
  For odd $m$, let
  \begin{align}
    k_m(\lambda)&=\frac{p^{m-1}-\lambda^{m-1}}{p+\lambda},
    \label{eq:km-def}\\
    \Lambda_m&=2M^{m-2}+mk_m(\Lambda),\label{eq:Am-def}\\
    M_m&=2M^{m-2}+mk_m(M),\label{eq:Bm-def}\\
    R_m&=\Lambda^m+M^m-pq^{m-1}.\label{eq:Rm-def}
  \end{align}
  Then
  \begin{align}
    t(C_m,W)-\bigl(p^m-pq^{m-1}\bigr)
    ...
    \\
    {} &\ge -R_m+\Lambda_m\gamma^2+M_m\norm{g_s}^2.\label{eq:master-defect}
  \end{align}
  Moreover,
...
\begin{proof}
  ...
  \begin{align}
    ...
    \label{eq:linear-shift-lower}
  \end{align}
  where the first equality follows from \eqref{eq:X-coeff} and the second equality holds because $m-1$ is even.

  The decomposition \eqref{eq:g-decomp} can be expressed in terms of the spectral measure $\mu$ of $A$ as
  \[
    \mu(\{\Lambda\})=\gamma^2,
    \qquad
    \int_{[-M,M]}\dd\mu(\lambda)=\norm{g_s}^2,
  \]
  so \eqref{eq:linear-shift-lower} can be decomposed as
  \[
    m\int k_m(\lambda)\dd\mu(\lambda)
    =mk_m(\Lambda)\gamma^2
  ...
  \begin{align}
    ...
    \label{eq:trace-payment}
  \end{align}
  where the last equality follows from \eqref{eq:g-decomp}.
  
  It remains to combine the two bounds.  
  Subtracting $p^m-pq^{m-1}$ in \eqref{eq:shift} gives the first line of \eqref{eq:master-defect},
  \[
    t(C_m,W)-\bigl(p^m-pq^{m-1}\bigr)
    =pq^{m-1}-\Tr(A^m)+m[z^m]L_-(z),
  \]
  and substituting \eqref{eq:trace-payment} and \eqref{eq:shift-payment},
  \begin{align*}
    &pq^{m-1}-\Tr(A^m)+m[z^m]L_-(z)
    \\
  ...
  \end{align*}
  which proves \eqref{eq:master-defect}.

  Finally, since $k_m(p)=0$ and $k_m$ is strictly decreasing on $[0,p]$, $k_m$ is positive on $[0,p)$. 
  From \eqref{eq:L-order}, we have $M, \Lambda \in [0, p)$, so both $k_m(\Lambda)$ and $k_m(M)$ are positive.
  Since the other terms in the definitions of $\Lambda_m$ and $M_m$ are also positive, \eqref{eq:AB-order} holds.
\end{proof}

\subsection{Minimizing $\Lambda_m\gamma^2+M_m\norm{g_s}^2$}
\label{subsec:forced-variance}\label{sec:huber}\label{subsec:shape-elim}

The lower bound of \Cref{prop:master-defect} is of use only once $\gamma$ and
$\norm{g_s}$ are controlled, so we now minimize $\Lambda_m\gamma^2+M_m\norm{g_s}^2$
over the values that these can take.  Two lemmas provide the constraints.  The
first bounds $\Lambda$ from above, and the second bounds $\gamma$ from above and
$\norm{g_s}$ from below, both in terms of the decomposition of $|\phi|$ along
$\one$ and $\phi$.

The first inequality restricts the admissible pair $(q,\Lambda)$.

\begin{lemma}[Variance lower bound]\label{lem:forced-variance}
  Let $\phi\perp\one$ be a unit eigenvector with $A\phi=\Lambda\phi$.  Then
...
\begin{proof}
  ...
  \]
  Since $\Lambda$ is the largest eigenvalue of $A$,
  \begin{align*}
    \Lambda
  ...
  $a_\phi^2\ge2\Lambda$; after squaring we obtain
  ...
  $1-2\Lambda>0$, gives
  \[
    (1-\Lambda-q)(\Lambda^2+q\Lambda-q)\le0.
  \]
  The first factor is positive by \eqref{eq:L-order}, so
  \eqref{eq:alpha-ceiling-poly} follows.  Solving the quadratic gives
  \eqref{eq:alpha-ceiling}.  Finally, because $\Lambda>q$, the right side of
  \eqref{eq:forced-variance} is strictly positive.  Thus $g\ne0$, and the
  bound \eqref{eq:HS-budget} gives $\Lambda^2<pq$.  Consequently $M>0$.
\end{proof}

\tk{TODO (naming): ``Huber'' is to be dropped from display text (kept only as the
historical remark below), but the replacement for ``envelope'' is not settled; also
``frontier eigenfunction'' awaits the leading-vs-outlier decision.}
...
\tk{TODO (naming): ``channel'' and ``safe'' are to be dropped; the replacement for the
quantity $\gamma=\ip{g}{\phi}$ (``coupling'' vs ``overlap'') is undecided.}

\begin{lemma}[Direct channel]\label{lem:direct-channel}
  The frontier coupling obeys
  \begin{equation}\label{eq:direct-channel}
    \gamma\le u_\gamma:=
    \frac{z-2\Lambda-2\Lambda b}{2\sqrt z}.
  \end{equation}
\end{lemma}

\begin{proof}
  Since $T_U\phi=\gamma\one+\Lambda\phi$ and
  \[
...
\begin{lemma}[Safe channel]\label{lem:safe-channel}
  ...
  \begin{equation}\label{eq:H-shape}
    \cH=\frac{(\Lambda-q)z+(\Lambda-M)K}{2\sqrt z}.
  \end{equation}
  Then
...
\begin{proof}
  ...
  $k\perp\one,\phi$, and using that every safe eigenvalue is at most $M$,
  we obtain
  \begin{align*}
    \Lambda
...
\begin{equation}\label{eq:Q-shape}
  ...
  \left\{
  \Lambda_m\gamma^2+\frac{M_m}{K}\pos{\cH-b\gamma}^2
  \right\},
\end{equation}
with the natural limiting interpretation when $K=0$.

\tk{TODO (naming): ``shape'' refers to the variables $(a_\phi,b,K)$ of the decomposition
$|\phi|=a_\phi\one+b\phi+k$; shape and profile have both been rejected, and a name for
these variables and for this optimization step is still to be found.}
...
\end{equation}
so $d,f,e>0$.
\tk{TODO (notation): $C_m$ collides with the cycle $C_m$; rename it.}
Put
\begin{equation}\label{eq:C-xi-rho}
  C_m=\frac{M_mf\sqrt{2\Lambda}\,e^2}{4\Lambda^2},
  \qquad
  \xi=\frac{4\Lambda^2d}{e^2},
  \qquad
  \rho=\frac{\Lambda_m}{M_m}\frac{\sqrt\Lambda}{2\sqrt2\,f},
\end{equation}
and define
...
\begin{theorem}[Huber shape elimination]\label{thm:huber-elim}
  ...
  \end{equation}
  is proved for every pair $(q,\Lambda)$ satisfying
...
\begin{proof}
  ...
  \[
    C_m\psi(\xi,\rho)
    \le C_m\xi=M_mfd\sqrt{2\Lambda}\le M_mfd.
  \]
  ...
  \[
    2\gamma\sqrt z+2\Lambda b
    \le z-2\Lambda=e-b^2-K\le e-b^2.
  \]
  Since $z\ge2\Lambda$,
  \begin{equation}\label{eq:b-gamma-budget}
    b^2+2\Lambda b+2\gamma\sqrt{2\Lambda}\le e.
  \end{equation}
  It follows that
  \begin{equation}\label{eq:beta-gamma}
    0\le\gamma\le\frac{e}{2\sqrt{2\Lambda}},
    \qquad
    b\le\beta(\gamma):=
    \sqrt{\Lambda^2+e-2\gamma\sqrt{2\Lambda}}-\Lambda.
  \end{equation}
  Rationalizing the square root,
  \begin{equation}\label{eq:beta-rational}
    \beta(\gamma)
    \le\frac{e-2\gamma\sqrt{2\Lambda}}{2\Lambda}.
  \end{equation}
  Also, since $z\ge2\Lambda$ and $z\le1$,
  \begin{equation}\label{eq:H-lower}
    \cH\ge\frac{d\sqrt{2\Lambda}}{2}+\frac{fK}{2}.
  \end{equation}
  For every real $w$,
  ...
  \begin{equation}\label{eq:one-d-gamma}
    \cQ\ge
    \min_{0\le\gamma\le e/(2\sqrt{2\Lambda})}
    \left\{
    \Lambda_m\gamma^2+2M_mf
    \pos{\frac{d\sqrt{2\Lambda}}{2}-\gamma\beta(\gamma)}
    \right\}.
  \end{equation}
  Set
  \[
    v=\frac{2\gamma\sqrt{2\Lambda}}{e}\in[0,1].
  \]
  By \eqref{eq:beta-rational},
  \[
    \gamma\beta(\gamma)
    \le\frac{e^2v(1-v)}{4\Lambda\sqrt{2\Lambda}}.
  \]
  Therefore
  \[
    \frac{d\sqrt{2\Lambda}}{2}-\gamma\beta(\gamma)
    \ge\frac{\sqrt{2\Lambda}\,e^2}{8\Lambda^2}
    (\xi-v+v^2).
  \]
  Furthermore,
  \[
    \Lambda_m\gamma^2=C_m\rho v^2,
    \qquad
    2M_mf\frac{\sqrt{2\Lambda}\,e^2}{8\Lambda^2}=C_m.
  \]
  ...
  \[
    \Lambda_m\gamma^2=C_m\rho v^2\ge C_m\psi(\xi,\rho).
  \]
...
\begin{corollary}[Quadratic and linear witnesses]\label{cor:two-witnesses}
  ...
  \begin{equation}\label{eq:witness-quadratic}
    C_m\psi(\xi,\rho)
    \ge
    \frac{2\Lambda^3\Lambda_md^2}{e^2}
    \frac{1+4\xi}{1+2\xi}.
  \end{equation}
  If $2\rho\xi>1$, then
  \begin{equation}\label{eq:witness-linear}
    C_m\psi(\xi,\rho)
    >M_mf\sqrt{2\Lambda}\,d
    \frac{1+4\xi}{2+4\xi}.
  \end{equation}
\end{corollary}
...
\begin{proof}
  ...
  \[
    C_m\rho\xi^2=\frac{2\Lambda^3\Lambda_md^2}{e^2}.
  \]
...
\Cref{lem:analytic-reading}); several later subsection titles use this $N$,
...
\begin{lemma}[Bounds for $T_N$]\label{lem:T-bounds}
...
\subsection{The regime \texorpdfstring{$2\rho\xi\le1$}{2 rho xi <= 1}}\label{sec:quadratic}
...
\begin{proposition}[The regime $2\rho\xi\le1$]\label{prop:quadratic-branch}
...
\subsection{The regime \texorpdfstring{$2\rho\xi>1$}{2 rho xi > 1}}\label{sec:linear}
...
\begin{proof}
  ...
  \end{equation}
  Using $\Lambda=a/D$, the square of \eqref{eq:sqrt-compensation} is
  \[
    2a\ge(1+a^2+y)(1-y)^2,
    \qquad y=\ell^2.
  \]
...
\begin{lemma}[The regime $\zeta\ge N$]\label{lem:linear-high-zeta}
...
\begin{lemma}[The regime $\zeta\le N$]\label{lem:linear-low-zeta}
...
\begin{proposition}[The regime $2\rho\xi>1$]\label{prop:linear-branch}
