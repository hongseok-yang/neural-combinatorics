...
in every part:
\[
  |\lambda_i|\le\frac12\quad\text{for every }i \qquad(\Cref{lem:half-norm}),
\]
and
\[
...
\item \Cref{sec:common-operator,sec:dense-region} prove all odd lengths
  ...
  \[
   ...
   =\frac{\ell^{n+r}}{r\Gamma(n)\Gamma(r)}
    \int_0^\infty t^{n+r-1}e^{-\ell t}\,
    \E\,\rho_{n,m}\Bigl(q+\frac Yt\Bigr)\dd t.
  \]
...
\item \Cref{sec:regionII-operator,sec:huber,sec:chart,sec:quadratic,sec:linear}
  prove all odd $m\ge9$ when $1/2<p<2/3$.  The argument in this part is
  spectral.  Only the identity for $W$ from Part~2 is used,
  \[
   t(C_m,W)=p^m-\Tr(A^m)+m[z^m]L_-(z),
  \]
  and every coefficient of $L_-$ is nonnegative (\Cref{lem:shift}, where
  $L_-$ is written $\cL$).  Nothing cancels $\Tr(A^m)$ this time, so it
  must be bounded directly.

  The two lemmas above restrict the spectrum of $A$.  If two eigenvalues
  were larger than $q$, then $\sum_i\lambda_i^2>2q^2$; but $2q^2>pq$
  exactly when $q>1/3$, which contradicts the second lemma.  Hence at most
  one eigenvalue exceeds $q$.  If none does, the theorem is immediate
  (\Cref{lem:no-frontier}), so we may assume that exactly one does and
  write $\alpha$ for it.  The second lemma then also gives
  $|\lambda_i|\le L:=\sqrt{pq-\alpha^2}$ for every other eigenvalue.  An
  upper bound for $\alpha$ comes from its eigenfunction $\phi$: since
  $U\ge0$, replacing $\phi$ by $|\phi|$ can only increase
  $\ip\phi{T_U\phi}$, and expanding $|\phi|$ along $\one$ and $\phi$
  gives
  \[
   \norm g^2\ge\frac{\alpha(\alpha-q)^2}{2(1-2\alpha)},
  \]
  which with the second lemma forces $\alpha^2+q\alpha-q\le0$
  (\Cref{lem:forced-variance}).

  Splitting $g=\gamma\phi+g_s$ into its component along $\phi$ and its
  component in the remaining spectral subspace turns the identity into
  \[
   t(C_m,W)-\bigl(p^m-pq^{m-1}\bigr)
   \ge-R_m+A_m\gamma^2+B_m\norm{g_s}^2,
   \qquad R_m=\alpha^m+L^m-pq^{m-1},
  \]
  where the explicit form of $A_m$ and $B_m$ is given in
  \eqref{eq:Am-def}--\eqref{eq:Bm-def} (\Cref{prop:master-defect}).  It
  therefore suffices to prove $A_m\gamma^2+B_m\norm{g_s}^2\ge R_m$.
  ...
  \]
  and put $K=\norm k^2$, so that $a_\phi^2+b^2+K=1$.  Using $0\le U\le1$
  as above gives (\Cref{lem:direct-channel,lem:safe-channel})
  \[
  ...
  \]
  and Cauchy--Schwarz turns the second into a lower bound on
  ...
  \]
  where $C_m$, $\xi$, and $\rho$ depends on $q$, $\alpha$,
  and $m$.  The theorem in this region is thereby reduced to the scalar
  inequality $R_m\le C_m\psi(\xi,\rho)$.  Writing
  $\pos x=\max_{0\le\lambda\le1}\lambda x$ and interchanging the minimum
  and the maximum, which strong duality permits, gives
  \[
   \psi(\xi,\rho)=\max_{0\le\lambda\le1}
   \left\{\lambda\xi-\frac{\lambda^2}{4(\rho+\lambda)}\right\},
  \]
  and the choices $\lambda=2\rho\xi$ and $\lambda=1$ give the two lower
  bounds used, one for each of the regimes $2\rho\xi\le1$ and
  $2\rho\xi>1$ (\Cref{prop:huber-dual,cor:two-witnesses}).

  What is left is an inequality between explicit quantities depending on
  $q$, $\alpha$, and $m$.  Since $p=1-q$ and $L^2=pq-\alpha^2$, the two
  ratios
  \[
   v=\frac p\alpha-1,
   \qquad
  ...
  \]
  already determine $q$, $p$, $\alpha$, and $L$, so we may use them as the
  two free parameters.  Here $v$ measures how much smaller the leading
  eigenvalue $\alpha$ is than the edge density, and $\zeta$ compares the
  remaining spectrum with the gap $\alpha-q$.  The pairs $(v,\zeta)$ that
  can occur form a region of the plane, cut out by explicit inequalities
  such as $0<v<1$ and $\zeta(\zeta+v)\ge1$, and the same region applies
  every $m$ (\Cref{sec:chart}).  What has to be proved is that
  $R_m\le C_m\psi(\xi,\rho)$ holds at every point of that region and for
  every odd $m\ge9$.
  ...
  \begin{itemize}[leftmargin=1.6em,topsep=2pt,itemsep=1pt]
   ...
   \item The regime $2\rho\xi>1$ (\Cref{sec:linear}):
    ...
    \end{itemize}
  \end{itemize}
  These are all elementary analysis, except the last case, $m=9$, where
  two fixed polynomials have to be shown positive on an interval.  Their
  coefficients in a Bernstein basis are listed as exact rationals in
  \Cref{app:bernstein}, positivity of the coefficients giving positivity
  on the interval.
...
\Cref{app:bernstein} are exact rational certificates.  Separately from the
proof, \Cref{sec:transferable} discusses the possible applicability of the
methods used here to other problems in extremal combinatorics.
...
notation $[z^m]F(z)$, the exponent of $z$ recording the length of a walk or
cycle.  Here $z$ is always a \emph{real} number in an explicitly given
interval $(-r,r)$, every $F$ below is the sum of a power series converging
on that interval, and
\begin{equation}\label{eq:coeff-def}
   [z^m]F:=\frac{F^{(m)}(0)}{m!}
\end{equation}
is the corresponding Taylor coefficient. 

Two elementary facts about convergent power series are used throughout.
First, the coefficients are determined by the function: if $F$ and $G$ are
sums of power series converging on $(-r,r)$ and $F=G$ there, then
$[z^m]F=[z^m]G$ for every $m$, by \eqref{eq:coeff-def}.  Every identity
below is therefore obtained by ordinary linear algebra for all small real
$z$, and the coefficient of interest is then read off.  The restriction to
small $|z|$ only keeps the series convergent; it imposes no condition on the
graphon.  Second, sums and products of such series are again such series,
with coefficients given by the Cauchy product, and so is a quotient $F/G$
with $G(0)\ne0$, on a possibly smaller interval.

By \Cref{lem:step-reduction} we may work with a step graphon, whose
operator is a finite matrix; this is made explicit in
\Cref{sec:common-operator}.  All generating functions below are built from
such matrices by the three constructions in the next lemma.

\begin{lemma}[Taylor expansions used below]\label{lem:analytic-reading}
Let $X$ be a real symmetric matrix with eigenvalues $\lambda_1,\ldots,
\lambda_N$ and spectral radius $\rho(X)=\max_i|\lambda_i|$, and let $z$ be
real with $|z|<1/\rho(X)$.
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.6em]
\item \emph{(Resolvent.)}  The matrix $I-zX$ is invertible and
  \[
   (I-zX)^{-1}=\sum_{j\ge0}z^jX^j,
  \]
  the series converging absolutely.  Hence, for fixed vectors $u,v$,
  \[
   [z^j]\ip u{(I-zX)^{-1}v}=\ip u{X^jv}.
  \]
\item \emph{(Log-determinant.)}  One has $\det(I-zX)>0$ and
  \[
   -\log\det(I-zX)=\sum_{j\ge1}\frac{\Tr(X^j)}{j}z^j,
  \]
  so that $m\,[z^m]\bigl(-\log\det(I-zX)\bigr)=\Tr(X^m)$.
\item \emph{(Outer logarithm.)}  If $Y$ is the sum of a power series
  converging on $(-r,r)$, with $Y(0)=0$ and $|Y|\le\theta$ there for
  some $\theta<1$, then
  \[
   -\log\bigl(1-Y(z)\bigr)=\sum_{k\ge1}\frac{Y(z)^k}{k}
   \qquad(|z|<r),
  \]
  and the right side is again a convergent power series on $(-r,r)$.
  Since $Y(0)=0$, the term $Y^k$ has a zero of order at least $k$ at
  the origin, so only the terms with $k\le m$ affect $[z^m]$.
\end{enumerate}
\end{lemma}

\begin{proof}
(1)  Diagonalize $X=Q\Lambda Q^{\!\top}$ with $Q$ orthogonal.  Each
$1-z\lambda_i$ is nonzero, so $I-zX$ is invertible.  Since
$\norm{X^j}_{\mathrm{op}}=\rho(X)^j$, the series is dominated by the
geometric series $\sum_j(|z|\rho(X))^j$; multiplying its partial sums by
$I-zX$ telescopes to $I-z^{J+1}X^{J+1}\to I$.

(2)  Here $\det(I-zX)=\prod_i(1-z\lambda_i)$, and each factor is positive
because $|z\lambda_i|<1$.  Taking real logarithms and using
$-\log(1-w)=\sum_{j\ge1}w^j/j$ for $|w|<1$,
\[
 -\log\det(I-zX)
 =-\sum_{i=1}^N\log(1-z\lambda_i)
 =\sum_{j\ge1}\frac{\sum_i\lambda_i^j}{j}z^j
 =\sum_{j\ge1}\frac{\Tr(X^j)}{j}z^j,
\]
the interchange of the finite sum over $i$ with the sum over $j$ being
justified by absolute convergence.

(3)  Apply $-\log(1-w)=\sum_{k\ge1}w^k/k$ at $w=Y(z)$, which is legitimate
since $|Y(z)|\le\theta<1$; the series is dominated by
$\sum_k\theta^k/k<\infty$.  The final statement holds because
$[z^j]Y^k=0$ for $j<k$.
\end{proof}

The matrices occurring below are $T_W$, $T_U$, and the compression $A$ of
\eqref{eq:block-U}, all symmetric.  For any kernel $0\le K\le1$, the finite
matrix $T_K$ of \Cref{sec:common-operator} satisfies
\[
 \sum_i\lambda_i(T_K)^2=\Tr(T_K^2)
 =\int_{[0,1]^2}K(x,y)^2\dd x\dd y\le1,
\]
so $\rho(T_W)\le1$ and $\rho(T_U)\le1$, and items~(1) and~(2) apply for
$|z|<1$; for $A$ one has $\rho(A)\le1/2$ by \Cref{lem:half-norm}.  The
composite functions $R_\pm$, $L_\pm$ of \Cref{sec:dense-region} and $\cL$ of
\Cref{sec:regionII-operator} are obtained from these by item~(1), a quotient,
and item~(3); \Cref{lem:analytic-disc} shows that the single interval
$|z|<1/2$ serves for all of them, with $\theta=1/12$ in item~(3).

The smallest instance shows the whole mechanism.  Take $X=T_W$ and $m=3$.
Item~(2) gives, for $|z|<1$, the convergent expansion
\[
 -\log\det(I-zT_W)
 =\Tr(T_W)\,z+\frac{\Tr(T_W^2)}2z^2+\frac{\Tr(T_W^3)}3z^3+\cdots,
\]
whence
\[
 3\,[z^3]\bigl(-\log\det(I-zT_W)\bigr)=\Tr(T_W^3)=t(C_3,W)
\]
by \eqref{eq:finite-trace-cycle}.  Every later identity has this shape: a
determinant is factorized by Schur complementation for small real $z$, the
factorization is turned into a sum of logarithms by item~(2), and one Taylor
coefficient is extracted.  Readers who prefer formal generating functions
lose nothing: passing to the coefficient sequence is an injective ring
homomorphism into $\R[[z]]$ which matches items~(1)--(3) with their formal
counterparts, so each identity below may be read in either sense.
...
half and $-1$ on the second, then $T_Uf=-\tfrac12f$, and hence
$\ip f{T_Uf}=-\tfrac12\norm f^2<0$.
...
We first make explicit the finite-dimensional setting in which all subsequent
determinants are taken.  By \Cref{lem:step-reduction}, for each fixed odd
cycle length it is enough to prove the desired inequality for a symmetric
step graphon.  Thus, throughout the determinant-based argument, fix a finite
measurable partition
\[
   [0,1]=B_1\sqcup\cdots\sqcup B_N
\]
into sets of positive measure such that $W$, and hence $U=1-W$, is
constant on each rectangle $B_a\times B_b$.  After the inequality has been
proved in this setting, \Cref{lem:step-reduction} passes it to an arbitrary
graphon.
...
\eqref{eq:step-operator-action} is zero, so $T_Kf=0$.  Hence all nonzero
action of $T_K$ takes place on the finite-dimensional space
$\mathcal E$.

From this point on, whenever a determinant involving $T_W$ or $T_U$
appears, it means the ordinary determinant of the restriction to
$\mathcal E$; no determinant of an operator on the whole infinite-dimensional
space $L^2([0,1])$ is being used.  The trace has the same finite-matrix
interpretation.  Indeed, in the orthonormal basis
$e_a=\one_{B_a}/\sqrt{|B_a|}$, the matrix of $T_K|_{\mathcal E}$ has
entries
\[
...
\begin{equation}\label{eq:finite-trace-cycle}
\begin{aligned}
 \Tr(T_K^r)
...
\end{aligned}
\end{equation}
Thus the finite matrix trace is exactly the cycle-density quantity that the
proof needs.

The constant function $\one$ lies in $\mathcal E$ and has norm one,
because the underlying probability space has total mass one.  Set
\[
   \mathcal E_0=\mathcal E\cap\one^\perp.
\]
Then $\mathcal E=\R\one\oplus\mathcal E_0$ is an orthogonal decomposition.
Relative to this finite-dimensional decomposition, write the complement
operator as
...
\end{equation}
Here $g\in\mathcal E_0$, $g^*$ denotes the linear functional
$h\mapsto\ip g h$, and $A$ is a self-adjoint linear map on the
finite-dimensional space $\mathcal E_0$, equivalently a real symmetric
matrix in an orthonormal basis.  This $A$ is precisely the nonzero part of the
mean-zero compression $P_{\one^\perp}T_UP_{\one^\perp}$: that larger
compression vanishes on $\mathcal E^\perp$.
...
\[
 \ip {f_+}{T_Uf_+}\le\left(\int f_+\right)^2,
 \qquad
 \ip {f_-}{T_Uf_-}\le\left(\int f_-\right)^2,
 \qquad
 \ip {f_+}{T_Uf_-}
 \le\left(\int f_+\right)\left(\int f_-\right),
\]
...
\[
 ...
 =-2a^2\ge-\frac12.
\]
Finally, $A=P_{\mathcal E_0}T_UP_{\mathcal E_0}$ acts on $\mathcal E_0$.
Thus, for every $f\in\mathcal E_0$, we have $P_{\mathcal E_0}f=f$ and hence
\[
   \ip f{Af}=\ip f{T_Uf}.
\]
Moreover, $A$ is self-adjoint, so its operator norm is the largest absolute
value of its Rayleigh quotient:
\[
 \norm A_{\mathrm{op}}
 =\sup_{\substack{f\in\mathcal E_0\\ \norm f=1}}
    \bigl|\ip f{Af}\bigr|.
\]
The upper and lower bounds just proved therefore give
$\bigl|\ip f{Af}\bigr|\le\tfrac12$ for every unit $f\in\mathcal E_0$, and
so $\norm A_{\mathrm{op}}\le\tfrac12$, as claimed.
\end{proof}
...
Because $A$ is a finite real symmetric matrix, it has an orthogonal
eigenspace decomposition
\[
   A=\sum_i\lambda_iP_i,
\]
where the distinct $\lambda_i$ are its eigenvalues and $P_i$ is the
orthogonal projection onto the corresponding eigenspace.  Define the finite
positive measure
\begin{equation}\label{eq:mu-def}
   \mu=\sum_i\norm{P_i g}^2\,\delta_{\lambda_i},
\end{equation}
where $\delta_{\lambda_i}$ denotes a unit point mass at $\lambda_i$.
Thus $\mu$ assigns to each
eigenvalue the nonnegative weight measuring how much of $g$ lies in its
eigenspace.  For any function $F$ defined on the finite set of eigenvalues,
...
\]
By \Cref{lem:half-norm}, every $\lambda_i$ lies in
$[-1/2,1/2]$, so $\mu$ is supported on that interval.

For real $|z|<2$, the function
$F_z(\lambda)=(1-z\lambda)^{-1}$ is bounded and continuous on this
interval.  Taking $F=F_z$ therefore gives
...
The next lemma exhibits a single interval on which every generating function
of \Cref{sec:dense-region,sec:regionII-operator} is a convergent power
series, so that the coefficient extraction \eqref{eq:coeff-def} is available
for all of them at once.  The two parameter choices used later are
$(\varepsilon,c)=(-1,p)$ and $(\varepsilon,c)=(+1,q)$.

\begin{lemma}[A common interval of convergence]\label{lem:analytic-disc}
Let $c\in[0,1]$ and $\varepsilon\in\{+1,-1\}$, and put
\[
 R_\varepsilon(z)=\ip g{(I-\varepsilon zA)^{-1}g}
   =\int\frac{\dd\mu(\lambda)}{1-\varepsilon\lambda z},
 \qquad
 Y(z)=\frac{z^2R_\varepsilon(z)}{1-cz}.
\]
Then $R_\varepsilon$ and $Y$ are sums of power series converging for real
$|z|<1/2$, where they satisfy
\[
 |R_\varepsilon(z)|\le\tfrac43\norm g^2\le\tfrac16,
 \qquad
 |Y(z)|\le\tfrac1{12}.
\]
Consequently $-\log\bigl(1-Y(z)\bigr)=\sum_{k\ge1}Y(z)^k/k$ for $|z|<1/2$.
\end{lemma}

\begin{proof}
The matrix $\varepsilon A$ is symmetric with $\rho(\varepsilon A)\le1/2$ by
\Cref{lem:half-norm}, so $R_\varepsilon(z)=\sum_{j\ge0}\varepsilon^j
\ip g{A^jg}z^j$ converges for $|z|<2$ by
\Cref{lem:analytic-reading}\,(1).  The total mass of $\mu$ is
$\mu(\R)=\sum_i\norm{P_ig}^2=\norm g^2$, and \Cref{lem:HS-budget} gives
$\norm g^2\le pq/2\le1/8$.  For $|z|<1/2$ and $\lambda\in\supp\mu$ we have
$|\varepsilon\lambda z|<1/4$, hence $1-\varepsilon\lambda z>3/4$ and
\[
 |R_\varepsilon(z)|
 \le\frac{\mu(\R)}{3/4}
 =\frac43\norm g^2\le\frac16.
\]
Since $c\in[0,1]$ we also have $1-cz\ge1-|z|>1/2$, so $Y$ is again a
convergent power series on $|z|<1/2$, being $z^2R_\varepsilon(z)$ times the
geometric series $\sum_{j\ge0}c^jz^j$, and
\[
 |Y(z)|\le\frac{|z|^2\cdot\frac16}{1/2}
 \le\frac{\frac14\cdot\frac16}{1/2}=\frac1{12}.
\]
The last assertion is \Cref{lem:analytic-reading}\,(3) with $Y(0)=0$ and
$\theta=1/12$.
\end{proof}
...
\end{align}
By \Cref{lem:analytic-disc}, applied with $(\varepsilon,c)=(-1,p)$ and with
$(\varepsilon,c)=(+1,q)$, all four functions are convergent power series for
$|z|<1/2$.  For odd $m$, put
\begin{equation}\label{eq:dense-Sm}
   S_m=m[z^m]\bigl(L_-(z)+L_+(z)\bigr),
\end{equation}
the coefficients being taken at $z=0$ in the sense of
\eqref{eq:coeff-def}.
...
\begin{proof}
All operators in this proof are the finite matrices on $\mathcal E$
described in \Cref{sec:common-operator}.  In particular, every determinant
and trace below is an ordinary finite-matrix determinant or trace.
The plan is to encode the cycle densities of $W$ and $U$ in two
determinant power series and then add those series.  Their mean-zero blocks
will be $-A$ and $A$, so the terms containing an odd power of $A$
will cancel.

We first derive the block matrix for $W$.  The operator $T_1$ associated
with the constant kernel $1$ sends $a\one+h$, with
$h\in\mathcal E_0$, to
\[
   T_1(a\one+h)=\left(\int(a\one+h)\right)\one=a\one,
\]
because $\int h=0$.  Thus $T_1$ has block matrix
$\left(\begin{smallmatrix}1&0\\0&0\end{smallmatrix}\right)$.  Since
$W=1-U$, we have $T_W=T_1-T_U$, and \eqref{eq:block-U} gives
\[
 T_W=
...
We next compute the two determinants by a block elimination.  Recall the
following elementary identity.  If $D$ is invertible, then
\begin{equation}\label{eq:block-determinant-identity}
 \det\begin{pmatrix}\alpha&v^*\\v&D\end{pmatrix}
 =\det(D)\bigl(\alpha-v^*D^{-1}v\bigr).
\end{equation}
The scalar $\alpha-v^*D^{-1}v$ is called the \emph{Schur complement} of
the block $D$.  To see the identity directly, multiply on the left by a
block matrix of determinant one:
\[
 \begin{pmatrix}1&-v^*D^{-1}\\0&I\end{pmatrix}
 \begin{pmatrix}\alpha&v^*\\v&D\end{pmatrix}
 =\begin{pmatrix}\alpha-v^*D^{-1}v&0\\v&D\end{pmatrix}.
\]
The matrix on the right is block triangular, so its determinant is the
product of its diagonal-block determinants, proving
\eqref{eq:block-determinant-identity}.  The blocks $I\pm zA$ used below are
invertible for $|z|<2$ by \Cref{lem:analytic-reading}\,(1), since
$\rho(A)\le1/2$; in particular they are invertible on the interval
$|z|<1/2$ of \Cref{lem:analytic-disc}, on which all identities of this
proof hold between convergent power series.

For $T_W$, the block matrix above gives
\[
 I-zT_W=
 \begin{pmatrix}
  1-pz&zg^*\\
 ...
 \end{pmatrix}.
\]
Apply \eqref{eq:block-determinant-identity} with
$D=I+zA$ and $v=zg$.  Since
$R_-(z)=\ip g{(I+zA)^{-1}g}$, we obtain
\begin{align*}
 \det(I-zT_W)
 &=\det(I+zA)\bigl(1-pz-z^2R_-(z)\bigr)\\
 &=\det(I+zA)(1-pz)
...
\[
 ...
 \end{pmatrix},
\]
so using $D=I-zA$ and the definition of $R_+(z)$ gives
\begin{align*}
 \det(I-zT_U)
 &=\det(I-zA)\bigl(1-qz-z^2R_+(z)\bigr)\\
 &=\det(I-zA)(1-qz)
   \left(1-\frac{z^2R_+(z)}{1-qz}\right).
\end{align*}

It remains to extract the cycle traces from these factorizations.  For a
finite self-adjoint matrix $X$, choose an orthonormal eigenbasis and denote
the eigenvalues by $\lambda_i$.  In that basis, $I-zX$ is diagonal with
diagonal entries $1-z\lambda_i$, while $X^j$ is diagonal with entries
$\lambda_i^j$.  Therefore
\[
\begin{aligned}
 -\log\det(I-zX)
 &=-\sum_i\log(1-z\lambda_i)\\
 &=\sum_{j\ge1}\frac{\sum_i\lambda_i^j}{j}z^j
 =\sum_{j\ge1}\frac{\Tr(X^j)}{j}z^j.
\end{aligned}
\]
Taking minus the logarithm of the two determinant factorizations therefore
gives
\begin{align*}
 -\log\det(I-zT_W)
 ={}&-\log\det(I+zA)-\log(1-pz)+L_-(z),\\
 -\log\det(I-zT_U)
 ={}&-\log\det(I-zA)-\log(1-qz)+L_+(z).
\end{align*}
We now add these identities and extract $[z^m]$.  The left sides,
multiplied by $m$, give
\[
   \Tr(T_W^m)+\Tr(T_U^m)
   =t(C_m,W)+t(C_m,U).
\]
Here the equality with the cycle densities is exactly
\eqref{eq:finite-trace-cycle}.
The scalar logarithms satisfy
\[
   -\log(1-pz)=\sum_{j\ge1}\frac{p^j}{j}z^j,
   \qquad
   -\log(1-qz)=\sum_{j\ge1}\frac{q^j}{j}z^j,
\]
so their contribution, after multiplication by $m$, is $p^m+q^m$.
For the compressed block $A$, the coefficient of $z^m$ in
$-\log\det(I-zA)$ is $\Tr(A^m)/m$, whereas its coefficient in
$-\log\det(I+zA)$ is $(-1)^m\Tr(A^m)/m$.  These two coefficients cancel
because $m$ is odd.  Finally, the two remaining logarithms contribute
\[
   m[z^m]\bigl(L_-(z)+L_+(z)\bigr)=S_m.
\]
Combining these contributions proves \eqref{eq:dense-cycle-sum}.

To obtain the second displayed identity in the lemma, rearrange the first:
\begin{align*}
 t(C_m,W)-\bigl(p^m-pq^{m-1}\bigr)
 ...
 &=q^{m-1}+S_m-t(C_m,U),
\end{align*}
using $p+q=1$.  This is \eqref{eq:dense-defect}.
\end{proof}
