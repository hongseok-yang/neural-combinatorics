...
\begin{equation}\label{eq:dense-path-gf}
  \sum_{j\ge0}x_jz^j=\frac1{1-qz-z^2R_+(z)}.
\end{equation}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \\
    {} &=\sum_{r\ge1}\frac mr [z^{m-2r}]\frac{R_-(z)^r}{(1-pz)^r}.
  \end{align*}
  ...
  \[
    ...
    =\int\!\cdots\!\int\frac1{(1-qz)^{r+1}}
    \prod_{i=1}^r\frac1{1-\lambda_iz}
    \prod_{i=1}^r\dd\mu(\lambda_i),
  \]
...
\begin{proof}
  ...
  \[
    ...
    =\frac1{(1-az)^k}\prod_{i=1}^r\frac1{1-b_iz}
  ...
  and therefore
  \begin{equation}\label{eq:dense-P-in-h}
    \begin{aligned}
      P_{m,r}(q;\lambda_1,\ldots,\lambda_r)
      ={}&\frac mr\sum_{j=0}^{n}\binom{n-j+r-1}{r-1}
...
\begin{equation}\label{eq:dense-n}
  n=m-2r.
\end{equation}
Then $n\ge1$ is odd.  Let $X\sim\BetaLaw(r,r)$, and for $x\in[0,1]$
set
...
\begin{lemma}[Beta formula]\label{lem:dense-beta}
  ...
  \begin{align}
    ...
    \left[
    \frac mn\E\bigl(V_X^n+W_X^n\bigr)
...
\begin{proof}
  ...
  Next we differentiate \eqref{eq:dense-beta-h} with respect to the common
  variable $a$.  On the left, differentiating the generating function
  ...
  Now take $\lambda_1=\cdots=\lambda_r=\ell$ in the definition
  \eqref{eq:dense-Pmr} of $P_{m,r}$ and apply the two formulas to its three
  terms.  With $(a,b)=(q,\ell)$ and with $(a,b)=(p,-\ell)$, the linear form
  $aX+b(1-X)$ is $V_X$ and $W_X$ respectively, so
  \eqref{eq:dense-beta-h} gives
  \[
    h_n(q^{\times r},\ell^{\times r})=\binom{n+2r-1}{2r-1}\E V_X^n,
    \qquad
    h_n(p^{\times r},(-\ell)^{\times r})=\binom{n+2r-1}{2r-1}\E W_X^n,
  \]
  while \eqref{eq:dense-beta-h-deriv} with $(a,b)=(q,\ell)$ gives
  \[
    h_{n-1}(q^{\times(r+1)},\ell^{\times r})
    =\binom{n+2r-1}{2r-1}\frac nr\E\bigl[XV_X^{n-1}\bigr].
  \]
  Substituting these into
  $\widetilde P_{m,r}(q,\ell)
  =\frac mr\bigl[h_n(p^{\times r},(-\ell)^{\times r})
  +h_n(q^{\times r},\ell^{\times r})\bigr]
  -h_{n-1}(q^{\times(r+1)},\ell^{\times r})$
  and using $\frac mr=\frac nr\cdot\frac mn$ proves
  \eqref{eq:dense-beta-first}.

  For \eqref{eq:dense-beta-second}, put
  \[
    u=q+\ell\frac{1-x}{x},
  \]
  so that $V_x=xu$ and, since $p=1-q$, also $W_x=x(1-u)$.  Then
  $V_x^n+W_x^n=x^n\bigl(u^n+(1-u)^n\bigr)$ and $xV_x^{n-1}=x^nu^{n-1}$,
  whence
  \[
    \frac mn\bigl(V_x^n+W_x^n\bigr)-xV_x^{n-1}
    =x^n\left[\frac mn\bigl(u^n+(1-u)^n\bigr)-u^{n-1}\right]
    =x^n\rho_{n,m}(u).
  \]
  Taking $x=X$ and expectations gives \eqref{eq:dense-beta-second}.

  For $\ell>0$, write \eqref{eq:dense-beta-second} out with the beta density,
  \[
  ...
  \]
  which maps $x\in(0,1]$ onto $s\in[0,\infty)$ and turns the argument of
  $\rho_{n,m}$ into $q+s$.  Since $n+2r=m$,
  \[
    x^{n+r-1}(1-x)^{r-1}\dd x
    =-\ell^{n+r}\frac{s^{r-1}}{(\ell+s)^{n+2r}}\dd s
    =-\ell^{n+r}\frac{s^{r-1}}{(\ell+s)^{m}}\dd s,
  \]
  the sign being absorbed by the reversal of the limits of integration.
  Together with
  \[
