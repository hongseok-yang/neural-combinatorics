...
series.  Finally, if we write $\ord F$ for the least $j$ with $c_j\ne0$,
then the series
\[
...
Applying this to $F(\lambda)=\lambda^j$ and summing the series of
\Cref{lem:analytic-reading}\,(1) coefficientwise gives
\begin{equation}\label{eq:resolvents-common}
 \ip g{(I-zA)^{-1}g}
 =\int_{-1/2}^{1/2}\frac{\dd\mu(\lambda)}{1-\lambda z},
\end{equation}
an identity of formal power series, the right side denoting
...
\end{align}
The two inner series have zero constant term, so $L_\pm$ are well defined
formal power series.  For odd $m$, put
...
invertible over $\R[[z]]$ by \Cref{lem:analytic-reading}\,(1), their
constant term being $I$.
