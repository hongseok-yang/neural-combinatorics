...
eigenfunction of \texorpdfstring{$\Lambda$}{Lambda}}\label{subsec:master-defect}
...
\end{equation}
Specifically, \Cref{lem:direct-channel} bounds $\gamma$ from above and
\Cref{lem:safe-channel} bounds $\norm{g_s}$ from below.  Both are linear in
$\gamma$ and $\norm{g_s}$, and $\Lambda_m\gamma^2+M_m\norm{g_s}^2$ is a
convex quadratic in them by \eqref{eq:AB-order}, so the minimization in
\eqref{eq:overapprox} is convex.
...
$\phi$, left open in \Cref{subsec:master-defect}, so that $b\ge0$.
Note that
...
\begin{equation}\label{eq:phi-parts}
...
\eqref{eq:phi-parts}.  Thus
\begin{equation}\label{eq:aphi-lower}
  a_\phi^2\ge2\Lambda.
\end{equation}

\begin{lemma}[Upper bound for $\gamma$]\label{lem:direct-channel}
...
\begin{proof}
  ...
  \begin{equation}\label{eq:direct-pairing}
    \ip{\phi_+}{T_U\phi}
  ...
  \]
  using $\gamma=\ip g\phi$ from \eqref{eq:g-decomp} and $A\phi=\Lambda\phi$.
  Hence, by \eqref{eq:phi-parts},
  \[
    \ip{\phi_+}{T_U\phi}
  ...
  Substituting into \eqref{eq:direct-pairing} gives \eqref{eq:direct-channel}.
\end{proof}

\begin{lemma}[Lower bound for $\norm{g_s}$]\label{lem:safe-channel}
...
\begin{proof}
  ...
  Hence
  \begin{equation}\label{eq:absphi-lower}
    \ip{|\phi|}{T_U|\phi|}\ge\ip\phi{T_U\phi}=\Lambda.
  \end{equation}

  We expand the left side of \eqref{eq:absphi-lower}.  In
  ...
  \]
  which is \eqref{eq:safe-channel}. 
  The consequence follows from \eqref{eq:safe-channel} and Cauchy--Schwarz,
  $\norm{g_s}\norm{\phi_\perp}\ge\ip{g_s}{\phi_\perp}\ge\cH-b\gamma$.
\end{proof}

By \eqref{eq:direct-channel} and \eqref{eq:gs-safe-lower}, the pair
$(\gamma,\norm{g_s})$ lies in
...
\begin{theorem}[Reduction to $\psi(\xi,\rho)$]\label{thm:huber-elim}
  Let
  \begin{align}
...
\begin{proof}
  We first evaluate the minimum in \eqref{eq:Q-shape} in the variable $s$.
  ...
  \begin{equation}\label{eq:one-d-t}
  ...
  side of \eqref{eq:one-d-t} is at least
  \[
    M_m\frac{\cH^2}{\norm{\phi_\perp}^2}.
  \]
  By \eqref{eq:safe-channel},
  \[
    \frac{\cH^2}{\norm{\phi_\perp}^2}
  ...
  the other hand, taking $v=0$ in \eqref{eq:psi-def} gives
  \[
    \Omega_m\psi(\xi,\rho)
  ...
  Now suppose $u_\gamma>0$.  A minimizer in \eqref{eq:one-d-t} may be taken
  with $\gamma\ge0$: replacing a negative $\gamma$ by $0$ decreases both
  terms.  The bound \eqref{eq:direct-channel} gives
  \[
  ...
  choosing $0<t\le-2w/(\Lambda-M)$.  Using \eqref{eq:one-d-t},
  ...
  Finally suppose $\phi_\perp=0$.  The constraint in \eqref{eq:Sbar} then
...
\begin{proof}
  ...
  \]
  and $t\mapsto t/(1-t)$ is increasing, so \eqref{eq:aphi-lower} allows $t$ to
  be replaced by $2\Lambda$, which is \eqref{eq:forced-variance}.

  For the second claim, \eqref{eq:HS-budget} gives
  $2\norm g^2\le pq-\Lambda^2$, so \eqref{eq:forced-variance} and
  $1-2\Lambda>0$ give
  \[
  ...
  positive by \eqref{eq:L-order}, so \eqref{eq:alpha-ceiling-poly} follows.
  The roots of $\Lambda^2+q\Lambda-q$ are $(-q\pm\sqrt{q^2+4q})/2$, whence
  \eqref{eq:alpha-ceiling}.

  Finally, because $\Lambda>q$, the right side of \eqref{eq:forced-variance}
  is strictly positive.  Thus $g\ne0$, and the bound \eqref{eq:HS-budget}
  gives $\Lambda^2<pq$.  Consequently $M>0$.
\end{proof}

Together, \Cref{thm:huber-elim} and \Cref{lem:forced-variance} show that
\eqref{eq:main-goal} follows once
\begin{equation}\label{eq:scalar-target}
  R_m\le \Omega_m\psi(\xi,\rho)
\end{equation}
is proved for every pair $(q,\Lambda)$ satisfying \eqref{eq:regionII} and
\eqref{eq:alpha-ceiling}.

\begin{proposition}[Dual form of $\psi$]\label{prop:huber-dual}
...
robust loss~\cite{Huber1964}, and \eqref{eq:psi-dual} is an interchange of
...
\begin{corollary}[Two lower bounds for $\psi$]\label{cor:two-witnesses}
