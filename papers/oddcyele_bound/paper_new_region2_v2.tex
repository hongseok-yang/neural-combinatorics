...
We now give the explicit form of $\Phi_m$ as an integral of a polynomial in $q$ and the eigenvalues of $A$.
...
It remains to express the terms of \eqref{eq:dense-Pmr} through the
$h_j(\lambda_1,\ldots,\lambda_r)$.  In \eqref{eq:dense-h-gf} the $k$ equal
variables and the remaining ones give separate factors, so
\[
...
\]
and comparing the coefficients of $z^d$ gives
\begin{equation}\label{eq:dense-h-split}
 h_d(a^{\times k},b_1,\ldots,b_r)
 =\sum_{j=0}^d
   \binom{d-j+k-1}{k-1}a^{d-j}h_j(b_1,\ldots,b_r).
\end{equation}
Apply this to the three terms of \eqref{eq:dense-Pmr}, the first two with
$k=r$ and $a=p$ or $q$, the third with $k=r+1$ and $a=q$.  Since $h_j$ is
homogeneous of degree $j$, we have
...
Write \eqref{eq:dense-P-in-h} as
\[
 P_{m,r}(q;\lambda_1,\ldots,\lambda_r)
...
using \eqref{eq:dense-h-diagonal},
\[
 \widetilde P_{m,r}(q,\ell)
 =\sum_{j=0}^nc_j\binom{j+r-1}{r-1}\ell^j.
\]
On the other hand, \eqref{eq:dense-dirichlet-moment} gives
\[
 P_{m,r}(q;\lambda_1,\ldots,\lambda_r)
...
\]
the last equality is \eqref{eq:dense-dirichlet-identity} with $\ell$ replaced by $S$. 

For the second claim, if the $\lambda_i$ lie in $[-1/2,1/2]$ then so does every convex
combination $S$, so the right side of \eqref{eq:dense-dirichlet-identity} is
