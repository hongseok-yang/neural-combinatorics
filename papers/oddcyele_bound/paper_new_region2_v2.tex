...
\begin{proof}
  ...
  It remains to check nonnegativity of coefficients.
  The coefficients of $Y$ are integrals against the measure
  $\mu=\sum_i\norm{P_ig}^2\delta_{\lambda_i}$ of \eqref{eq:mu-def}: for every
  integer $r\ge2$,
  \begin{equation}\label{eq:X-coeff}
    [z^r]Y(z)
    =\int\frac{p^{r-1}-(-\lambda)^{r-1}}{p+\lambda}\dd\mu(\lambda).
  \end{equation}
  By \Cref{lem:half-norm}, $|\lambda|\le1/2<p$, so both the numerator and the
  denominator on the right of \eqref{eq:X-coeff} are positive, and $Y$ has
  nonnegative coefficients.  Since $-\log(1-Y)=\sum_{k\ge1}Y^k/k$, the
  same holds for $L_-$.
\end{proof}
...
\[
  \sum_{\lambda_i\ne\alpha}\lambda_i^2\le pq-\alpha^2=L^2,
\]
so in particular $|\lambda_i|\le L$ for every $\lambda_i\ne\alpha$.

These quantities are ordered as
\begin{equation}\label{eq:L-order}
  0\le L<q<\alpha<\frac{1}{2}<p,
\end{equation}
where $L<q$ because $L^2<pq-q^2=q(1-2q)<q^2$ for $q>1/3$, and
$\alpha<1/2$ comes from \Cref{lem:half-norm}.

\subsection{The master defect inequality}\label{subsec:master-defect}

\tk{TODO (naming): ``master defect inequality'' --- defect, excess, master, and the
weaker ``lower bound in terms of\ldots'' have all been rejected; a direct name for this
subsection, for the proposition below, and for the word ``payment'' used in its proof is
still to be found.}

Since $L_-(z)-Y(z)=\sum_{k\ge2}Y(z)^k/k$ again has nonnegative coefficients,
$[z^m]L_-(z)\ge[z^m]Y(z)$.  Hence \eqref{eq:X-coeff} with $r=m$ gives
\begin{equation}\label{eq:linear-shift-lower}
  m[z^m]L_-(z)
  \ge m\int k_m(\lambda)\dd\mu(\lambda),
  \qquad
  k_m(\lambda)=\frac{p^{m-1}-\lambda^{m-1}}{p+\lambda},
\end{equation}
where $(-\lambda)^{m-1}=\lambda^{m-1}$ because $m$ is odd.

We will apply \eqref{eq:linear-shift-lower} when the eigenvalues in question
lie in an interval $[-L,L]$ with $L<p$, so we compute
$\min_{|\lambda|\le L}k_m(\lambda)$.  For $0\le s<p$,
\[
  k_m(-s)=\frac{p^{m-1}-s^{m-1}}{p-s}
  >\frac{p^{m-1}-s^{m-1}}{p+s}=k_m(s),
\]
so it suffices to minimize over $0\le\lambda\le L$.  There $k_m$ is strictly
decreasing: with $n=m-1$,
\[
  k_m'(\lambda)
  =-\frac{p^n+np\lambda^{n-1}+(n-1)\lambda^n}{(p+\lambda)^2}<0.
\]
The minimum is therefore attained at $\lambda=L$:
...
\begin{equation}\label{eq:g-decomp}
  ...
  \qquad g_s\perp\phi.
\end{equation}
\tk{TODO (notation): rename $A_m$, $B_m$, $R_m$ (and $C_m$ at
...
\subsection{Minimizing $A_m\gamma^2+B_m\norm{g_s}^2$}
...
$\norm{g_s}$ are controlled, so we now minimize $A_m\gamma^2+B_m\norm{g_s}^2$
over the values that these can take.  Two lemmas provide the constraints.  The
first bounds $\alpha$ from above, and the second bounds $\gamma$ from above and
$\norm{g_s}$ from below, both in terms of the decomposition of $|\phi|$ along
$\one$ and $\phi$.

The first inequality restricts the admissible pair $(q,\alpha)$.
...
\end{equation}
so $d,f,e>0$.
\tk{TODO (notation): $C_m$ below collides with the cycle $C_m$; rename together with
