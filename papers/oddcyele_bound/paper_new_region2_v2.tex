...
\begin{enumerate}[label=\textbf{Section \arabic*.},leftmargin=4.6em,start=2]
  \item \emph{The cases $m=3,5,7$.}  We prove these cases separately so that when we consider the general odd $m$ in the later sections, we can assume $m\ge9$.
  ...
  \item 
  In this section we prove lemmas that are used throughout the remaining part
  of this paper.  We first prove that, in proving \eqref{eq:main-goal}, it is
  enough to consider step graphons, so that every computation below is finite
  dimensional.  We then show that the logarithm of a determinant generates
  the cycle densities: for any kernel $K$,
  \[
  ...
  further generating functions $L_-(z)$ and $L_+(z)$, which satisfy the
  following two equalities (\Cref{lem:cycle-identities}):
  \begin{align*}
    t(C_m,U)&=\phantom{-}\Tr(A^m)+q^m+m[z^m]L_+(z),\\
    t(C_m,W)&=-\Tr(A^m)+p^m+m[z^m]L_-(z).
  \end{align*}
  \item In this section, we prove \Cref{thm:main} for the case $p \ge 2/3$. We first eliminate the $\Tr(A^m)$ by addint the two identities:
  \[
  ...
  $\widetilde P_{m,r}(q,\ell)\ge0$ for $\ell\in[-1/2,1/2]$.  Second, once we
  obtain this diagonal polynomial, we can simplify it further to an integral
  against a beta distribution, which for $\ell>0$ becomes
  (\Cref{lem:pge23-beta})
  \begin{align*}
...
We build our series from the finite matrices of
\Cref{subsec:block-decomposition}, using the two expansions in the next lemma.
...
For a first example of these expansions, take $X=T_W$ and $m=3$.  Item~(2)
gives
\[
  -\log\det(I-zT_W)
...
For a second example, expanding $(I-zA)^{-1}=\sum_{j\ge0}z^jA^j$ by
...
\end{equation}
for every $m\ge2$.
One can now simplify this generating function by the
block decomposition \eqref{eq:block-U}, as the following lemma states.
...
\begin{proof}
  ...
  Both determinants decompose along this block structure: if $D$ is
  invertible, then
  ...
  $\left(\begin{smallmatrix}1&-v^*D^{-1}\\0&I\end{smallmatrix}\right)$, whose
  determinant is one:
  \begin{align*}
    \det\begin{pmatrix}\alpha&v^*\\v&D\end{pmatrix}
    ...
    &=\det\begin{pmatrix}\alpha-v^*D^{-1}v&0\\v&D\end{pmatrix}
     =\det(D)\bigl(\alpha-v^*D^{-1}v\bigr),
  \end{align*}
  where the last equality holds since the determinant of a block triangular
  matrix is the product of its diagonal-block determinants.

  To apply \eqref{eq:block-determinant-identity} to $I-zT_U$ and $I-zT_W$,
  one needs to check that the lower right block $I\mp zA$ is invertible, and
  this follows from \Cref{lem:series-expansions}\,(1) since both of these
  terms have $I$ as their constant term.  Taking $D=I\mp zA$ and $v=\mp zg$
  factorizes the two determinants into three parts,
  \begin{align*}
  ...
  \]
  The last summands equal $m[z^m]L_\pm(z)$, which proves
  \eqref{eq:cycle-identity-U} and \eqref{eq:cycle-identity-W}.
\end{proof}

\begin{remark} \label{rem:analytic-reading}
  Alternatively, one may interpret these series as functions.  If a series
...
\begin{proof}
  Adding \eqref{eq:cycle-identity-U} and \eqref{eq:cycle-identity-W} cancels
  the two traces and leaves
  \[
    t(C_m,W)+t(C_m,U)=p^m+q^m+m[z^m]\bigl(L_-(z)+L_+(z)\bigr),
  \]
  which is \eqref{eq:pge23-two-sided-identity} by the definition
  \eqref{eq:pge23-Sm} of $S_m$.
...
\end{equation}
Two of the three terms are already expressed through $q$ and $A$, so it
...
\end{equation}
where the last equality follows from \eqref{eq:resolvents-common}.  The first row of the system shows
...
\begin{proof}
  ...
  \]
  which is the second term in brackets in \eqref{eq:pge23-Pmr}.  The same
  argument for $L_-$ with $R_-(z) = \langle g, (I + zA)^{-1} g \rangle$ produces the first term.
...
\begin{equation}\label{eq:plt23-one-sided-identity}
  t(C_m,W)=p^m-\Tr(A^m)+m[z^m]L_-(z),
\end{equation}
as proven in \Cref{lem:cycle-identities}.  Throughout \Cref{sec:plt23} we
bound these terms directly.

\begin{lemma}[Nonnegative coefficients]\label{lem:plt23-one-sided-identity}
  Every coefficient of $L_-(z)$ is nonnegative.  Moreover,
...
\begin{proof}
  ...
  \]
  are nonnegative, which implies the same for
  $L_-=-\log(1-Y)=\sum_{k\ge1}Y^k/k$ by \eqref{eq:Lminus}.
  ...
  For the second claim, the coefficients of
  $L_-(z)-Y(z)=\sum_{k\ge2}Y(z)^k/k$ are nonnegative as well, so
  \[
    [z^m]L_-(z)\ge[z^m]Y(z)
    =\int\frac{p^{m-1}-(-\lambda)^{m-1}}{p+\lambda}\dd\mu(\lambda)
    =\int\frac{p^{m-1}-\lambda^{m-1}}{p+\lambda}\dd\mu(\lambda),
  \]
  where the first equality follows from \eqref{eq:plt23-Y-coeff} and the
  second holds because $m-1$ is even.
\end{proof}


\subsection{At most one eigenvalue above \texorpdfstring{$q$}{q}}

We first prove the following bound on the sum of the squared eigenvalues of
$A$, in which $\Tr(A^2)=\sum_i\lambda_i^2$.
...
\begin{proof}
  We first bound $m[z^m]L_-(z)$ from below.  Recall that
  \[
    \mu=\sum_i\norm{P_ig}^2\delta_{\lambda_i}
  \]
  by \eqref{eq:mu-def}, which decomposes $\norm g^2$ along the eigenfunctions
  of $A$.  The eigenspace of $\Lambda$ is spanned by $\phi$, so it carries the
  weight $\ip g\phi^2=\gamma^2$, and every other eigenvalue lies in $[-M,M]$
  and carries the weight of $g_s$.  This allows us to write
  \eqref{eq:plt23-g-decomp} in terms of $\mu$ as
  \[
    \mu(\{\Lambda\})=\gamma^2,
    \qquad
...
\begin{equation}\label{eq:plt23-Sbar}
  ...
  \overline{\norm{g_s}}\ge0\bigr\},
\end{equation}
where $u_\gamma, b, \norm{\phi_\perp}$, and $\kappa$ do not depend on
$\gamma$ or $\norm{g_s}$.
Hence
...
\begin{theorem}[Reduction to $\psi(\xi,\rho)$]\label{thm:plt23-huber-elim}
  ...
  \end{align}
  where $\Omega_m$, $\xi$ and $\rho$ are positive by
  \eqref{eq:plt23-M-Lambda-order} and \eqref{eq:plt23-Lambdam-Mm-positive}.
  Then
...
\begin{proof}
  We first minimize \eqref{eq:plt23-min-over-Sbar} in the variable
  $\overline{\norm{g_s}}$.  Since the objective increases in $\overline{\norm{g_s}}$, one should choose $\overline{\norm{g_s}}$ as small as $\overline S$ allows.  When $\phi_\perp\ne0$, one should choose
  $\overline{\norm{g_s}}\norm{\phi_\perp}=\pos{\kappa-b\overline\gamma}$, so that
  ...
  \begin{equation}\label{eq:plt23-one-d-t}
    ...
    \right\}.
  \end{equation}
  When $\phi_\perp=0$, the variable $\overline{\norm{g_s}}$ does not appear in
  the constraint, so it should be $0$, and \eqref{eq:plt23-Sbar} becomes a
  restriction on $\overline\gamma$ alone, $b\overline\gamma\ge\kappa$.  Then
  ...
  Suppose for the moment that $\overline\gamma\ge0$.  Then
  \eqref{eq:plt23-gamma-upper-bound} gives
  \[
  ...
  \end{equation}
  where the assumed hypothesis $\overline\gamma\ge0$ is what makes
  $2\overline\gamma a_\phi\ge2\overline\gamma\sqrt{2\Lambda}$.

  We can use this inequality to obtain two bounds on $\overline\gamma$ and
  $b$ separately.  First, since $\phi$ was chosen so that $b\ge0$, the first
  two terms on the left of
  \eqref{eq:plt23-b-gamma-budget} are nonnegative, so
  \begin{equation}\label{eq:plt23-gamma-range}
    0\le\overline\gamma\le\frac{1-2\Lambda}{2\sqrt{2\Lambda}}.
  \end{equation}
  Second, viewing \eqref{eq:plt23-b-gamma-budget} as a quadratic inequality
  in $b$ gives
  \[
    b^2+2\Lambda b-\bigl(1-2\Lambda-2\overline\gamma\sqrt{2\Lambda}\bigr)\le0,
  \]
  whose discriminant is nonnegative by \eqref{eq:plt23-gamma-range}.  So the
  larger root of this quadratic, which is
  $\sqrt{(1-\Lambda)^2-2\overline\gamma\sqrt{2\Lambda}}-\Lambda$, upper bounds
  $b$:
  ...
  \end{equation}
  holds, where the equality comes from rationalizing the square root, using
  $(1-\Lambda)^2-\Lambda^2=1-2\Lambda$, and the inequality holds since the
  numerator is nonnegative and the denominator is at least $2\Lambda$.
  Set
  ...
  \end{equation}
  Here $\overline v$ lies in $[0,1]$ by \eqref{eq:plt23-gamma-range}, the
  interval over which the minimum in \eqref{eq:plt23-psi-def} is taken, and
  the last identity follows by the definitions
  \eqref{eq:plt23-C-xi-rho} and \eqref{eq:plt23-rho-def}.
  Substituting in \eqref{eq:plt23-beta-rational}, we obtain the inequality
  that holds under the assumption $\overline\gamma\ge0$,
  ...
  Now suppose first that $\phi_\perp=0$.  Then the constraint
  $b\overline\gamma\ge\kappa$ of \eqref{eq:plt23-one-d-t-zero} simplifies to
  \[
    b\overline\gamma\ge\kappa
    =\frac{(\Lambda-q)a_\phi^2+(\Lambda-M)\norm{\phi_\perp}^2}{2a_\phi}
    =\frac{(\Lambda-q)a_\phi}{2}>0,
  \]
  where $\Lambda>q$ by \eqref{eq:plt23-M-Lambda-order} and
  $a_\phi=\ip\one{|\phi|}>0$.  Since $\phi$ was chosen so that $b\ge0$, this
  forces $\overline\gamma>0$, so
  \eqref{eq:plt23-gamma-range}--\eqref{eq:plt23-gamma-beta-v} hold.  With
  the upper bound $b\le\beta(\overline\gamma)$ of \eqref{eq:plt23-beta-gamma}, we have
  \[
    ...
    \le\frac{(1-2\Lambda)^2\overline v(1-\overline v)}{4\Lambda\sqrt{2\Lambda}},
  \]
  which simplifies to $\xi-\overline v+\overline v^2\le0$.  Thus the second
  term in \eqref{eq:plt23-psi-def} is zero at $v=\overline v$, and every pair $(\overline\gamma,\overline{\norm{g_s}})$ in $\overline S$ satisfies
  \[
    \Lambda_m\overline\gamma^2
    ...
    \ge \Omega_m\psi(\xi,\rho),
  \]
  where the first equality is \eqref{eq:plt23-v-def}.  With
  \eqref{eq:plt23-one-d-t-zero} this proves
  \eqref{eq:plt23-huber-payment} in the case $\phi_\perp=0$.

  \tk{Revised up to here.}

  Now assume $\phi_\perp\ne0$.  Suppose first that $u_\gamma\le0$.  Every
  admissible $\overline\gamma$ is nonpositive, so
  $\kappa-b\overline\gamma\ge\kappa$, and the right
  ...
  with $\overline\gamma\ge0$: replacing a negative $\overline\gamma$ by $0$
  decreases both
  terms, so \eqref{eq:plt23-gamma-range}--\eqref{eq:plt23-gamma-beta-v}
  hold.
  ...
  choosing $0<t\le-2w/(\Lambda-M)$.

  We now bound the objective of \eqref{eq:plt23-one-d-t} from below at each
  admissible $\overline\gamma\ge0$.  Abbreviate
  \[
    w=\frac{(\Lambda-q)\sqrt{2\Lambda}}{2}-b\overline\gamma,
  \]
  so that $\kappa-b\overline\gamma\ge w+(\Lambda-M)\norm{\phi_\perp}^2/2$ by
  \eqref{eq:plt23-H-lower}.  Since $x\mapsto\pos x^2$ is nondecreasing and
  $\norm{\phi_\perp}^2>0$,
  \[
    \frac{M_m}{\norm{\phi_\perp}^2}\pos{\kappa-b\overline\gamma}^2
    \ge M_m\,
  ...
  \]
  the last step by \eqref{eq:plt23-K-AMGM} with $t=\norm{\phi_\perp}^2$.  This
  removes $\norm{\phi_\perp}$ from the objective, and $b$ is removed next:
  as $\overline\gamma\ge0$ and $b\le\beta(\overline\gamma)$ by
  \eqref{eq:plt23-beta-gamma}, we have
  $b\overline\gamma\le\overline\gamma\beta(\overline\gamma)$, so
  \[
    \pos w\ge
    \pos{\frac{(\Lambda-q)\sqrt{2\Lambda}}{2}
    -\overline\gamma\beta(\overline\gamma)}.
  \]
  Finally, \eqref{eq:plt23-gamma-range} bounds the admissible
  $\overline\gamma$ by $(1-2\Lambda)/(2\sqrt{2\Lambda})$, and enlarging the
  range of a minimum only decreases it.  Together with
  \eqref{eq:plt23-min-over-Sbar} and \eqref{eq:plt23-one-d-t}, these three
  steps give
  \begin{equation}\label{eq:plt23-one-d-gamma}
    ...
    \right\}.
  \end{equation}
  It remains to rewrite \eqref{eq:plt23-one-d-gamma} in the variable
  $\overline v$ of \eqref{eq:plt23-v-def}, which runs over $[0,1]$ as
  $\overline\gamma$ runs over $[0,(1-2\Lambda)/(2\sqrt{2\Lambda})]$.  For the
  second term of the objective, \eqref{eq:plt23-gamma-beta-v} gives
  \[
    \frac{(\Lambda-q)\sqrt{2\Lambda}}{2}
    -\overline\gamma\beta(\overline\gamma)
  ...
  \]
  where the equality collects the two terms using the definition
  \eqref{eq:plt23-xi-def} of $\xi$ and
  $1/(4\Lambda\sqrt{2\Lambda})=\sqrt{2\Lambda}/(8\Lambda^2)$.  The factor in
  front is positive and $x\mapsto\pos x$ is nondecreasing, so
  \[
    2M_m(\Lambda-M)
    \pos{\frac{(\Lambda-q)\sqrt{2\Lambda}}{2}
    -\overline\gamma\beta(\overline\gamma)}
  ...
  \]
  For the first term, \eqref{eq:plt23-v-def} gives
  $\Lambda_m\overline\gamma^2=\Omega_m\rho\overline v^2$, and the definitions
  \eqref{eq:plt23-C-xi-rho} and \eqref{eq:plt23-rho-def} also give
  \[
    2M_m(\Lambda-M)\frac{\sqrt{2\Lambda}\,(1-2\Lambda)^2}{8\Lambda^2}
    =\Omega_m.
  \]
  Hence the objective of \eqref{eq:plt23-one-d-gamma} is at least
  $\Omega_m\bigl(\rho\overline v^2+\pos{\xi-\overline v+\overline v^2}\bigr)$
  at every admissible $\overline\gamma$, and minimizing over
  $\overline v\in[0,1]$ gives
  \[
    \Lambda_m\gamma^2+M_m\norm{g_s}^2
