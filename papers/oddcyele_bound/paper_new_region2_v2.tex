...
\section{Below the threshold \texorpdfstring{$q\le1/3$}{q<=1/3}: Dirichlet averages and gamma smoothing}
...
\subsection{A two-sided Schur-complement identity and cancellation of the odd trace}
...
\subsection{Completion of the proof for \texorpdfstring{$q\le1/3$}{q<=1/3}}

\begin{theorem}[The theorem for $q\le1/3$]\label{thm:dense-region}
...
\section{Above the threshold \texorpdfstring{$1/3<q<1/2$}{1/3<q<1/2}: the single-outlier reduction for \texorpdfstring{$m\ge9$}{m>=9}}
...
\begin{proof}
  ...
  \end{align*}
  \tk{TODO: this display repeats \Cref{lem:analytic-reading}(2); replace by a
  reference to \eqref{eq:logdet-cycle}.}
