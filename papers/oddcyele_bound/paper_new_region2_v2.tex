...
\begin{proof}
  ...
  $\overline S$ allows, that is, $s\norm{\phi_\perp}=\pos{\kappa-bt}$.  For
  $\phi_\perp\ne0$ this gives $s^2=\pos{\kappa-bt}^2/\norm{\phi_\perp}^2$, and
  \begin{equation}\label{eq:plt23-one-d-t}
    \min_{(t,s)\in\overline S}\bigl(\Lambda_mt^2+M_ms^2\bigr)
    =\min_{t\le u_\gamma}
    \left\{
    \Lambda_mt^2+\frac{M_m}{\norm{\phi_\perp}^2}\pos{\kappa-bt}^2
    \right\};
  \end{equation}
  for $\phi_\perp=0$ the constraint reads $bt\ge\kappa$ instead, and that case
  is treated at the end.  We write $\gamma$ for $t$ again below.

  Assume $\phi_\perp\ne0$.  Suppose first that $u_\gamma\le0$.  Every
  admissible $\gamma$ is nonpositive, so $\kappa-b\gamma\ge\kappa$, and the right
  ...
  reads $b\gamma\ge\kappa$, that is,
  \[
