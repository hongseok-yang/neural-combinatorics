...
the compression $P_{\one^\perp}T_UP_{\one^\perp}$ used there vanishes on
...
\]
so $\bigl|\ip f{Af}\bigr|\le\tfrac12$ for every unit $f\in\mathcal E_0$.
...
\end{equation}
for every $m\ge2$.  Thus the cycle densities of $U$ and of $W$ are the
coefficients of $-\log\det(I-zT_U)$ and $-\log\det(I-zT_W)$, and we expand
these two series along the block decomposition \eqref{eq:block-U}.  Since
$T_W=T_1-T_U$ has block matrix
$\left(\begin{smallmatrix}p&-g^*\\-g&-A\end{smallmatrix}\right)$, as computed
in the proof of \Cref{lem:dense-cancellation} below,
\[
 I-zT_U=
...
\]
The two matrices differ only in the sign of the mean-zero block and in the
scalar entry, which is the reason for treating $W$ and $U$ together.

Eliminating the off-diagonal entries by the Schur complement identity
\eqref{eq:block-determinant-identity} factorizes the determinants into three
parts,
\[
 \det(I-zT_U)=\det(I-zA)\,(1-qz)
   \left(1-\frac{z^2\ip g{(I-zA)^{-1}g}}{1-qz}\right),
\]
and likewise for $W$ with $A$ replaced by $-A$ and $q$ by $p$.  Taking
$-\log$ turns each product into a sum of three series, which we track separately:
\[
\begin{aligned}
 -\log\det(I-zT_U)={}&-\log\det(I-zA)-\log(1-qz)-\log\left(1-\frac{z^2\ip g{(I-zA)^{-1}g}}{1-qz}\right),\\
 -\log\det(I-zT_W)={}&-\log\det(I+zA)-\log(1-pz)-\log\left(1-\frac{z^2\ip g{(I+zA)^{-1}g}}{1-pz}\right).
\end{aligned}
\]
Write
\begin{align}
...
\end{align}
for the two Schur complements, as in \eqref{eq:resolvents-common} with $A$
replaced by $\mp A$, and abbreviate the third summands by
\begin{align}
...
\end{align}
Each inner series $z^2R_\pm(z)/(1-cz)$ has order at least two, so $L_\pm$
are well defined formal power series, as discussed in
\Cref{subsec:analytic-reading}.

The first two summands are already explicit.  For the first,
\Cref{lem:analytic-reading}\,(2) gives, for odd $m$,
\[
...
% TODO: this re-derives the expansion of \Cref{lem:analytic-reading}(2);
