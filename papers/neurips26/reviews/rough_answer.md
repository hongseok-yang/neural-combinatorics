...
UPDATE (2026-07-28): there IS a computable lower bound, from Lubetzky–Zhao's own analysis, and it is tight enough to sandwich the optimum. For a $2$-regular pattern graph, the optimum of the upper-tail problem is at least the convex minorant of $x \mapsto h_q(x^{1/2})$ evaluated at $x = r^2$ (this is the construction already implemented in `sbm_check.ipynb`'s first cell via the Graham-scan minorant). Values for the six cells: $0.460284, 0.725284, 1.049172$ ($q=0.05$) and $0.306561, 0.504311, 0.746006$ ($q=0.10$), for $r=0.4,0.5,0.6$. Combined with the explicit bipodal upper constructions, the true optimum is bracketed within $7\times10^{-4}$-$2.8\times10^{-3}$ in every cell, $h_q(W_{\mathrm{LZ}})$ exceeds even the upper end, and the revised learnt values are provably within $4\times10^{-3}$ of the optimum. The same bound applies verbatim to $C_4$ and $C_5$ (also $2$-regular; the evaluation point is $r^2$ in all three cases). VERIFY before posting: the exact statement/lemma number in Lubetzky–Zhao for the convex-minorant lower bound, so the citation is precise.
