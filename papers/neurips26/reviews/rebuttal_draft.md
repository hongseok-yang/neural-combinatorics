...
To measure reproducibility separately from this hyperparameter search, we repeated the $K_3$ instance at $p=7/9$ ten times under a single fixed configuration during the rebuttal period, using the same setting as in the ablation study.
...
We will apply the same repetition analysis to the open instances $C_5$, $C_7$, and $H_6$ during the discussion phase. 
To quantify sampling error in constraint enforcement, we froze the final $K_3$ network. For each of 1,000 independent batches of size $2^{16}$, we chose the scalar offset that made the batch edge density equal to $p=7/9$ and then evaluated the resulting graphon on a deterministic grid. The grid-evaluated edge-density error had a standard deviation of $1.1\times10^{-3}$ and a maximum absolute value of $3.4\times10^{-3}$. This error decreased as $N^{-1/2}$. For the reported results, we instead solve the offset directly on the deterministic evaluation grid, thereby removing this batch-induced error.
