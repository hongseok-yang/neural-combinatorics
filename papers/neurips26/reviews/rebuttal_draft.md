...
**Robustness and numerical validation.** For each problem instance, we trained eight models. These covered all combinations of two activation functions, sinusoidal and GeLU, and four initial learning rates, with one run for each combination. The paper reports the candidate with the lowest objective value among these eight runs. The selection pool was therefore fixed at eight runs per instance.

To measure how consistently the method recovered the reported solution, we will report how many of the original eight runs found the same structure, up to graphon relabelling. We will also report the distribution of their objective values and their constraint errors on independent samples. Because the original runs varied the activation and learning rate as well as the random initialisation, we are separately running 16 independent initialisations with a fixed training configuration on the \(K_3\) benchmark at \(p=7/9\) and on the key open instances. [TODO: insert the original-sweep recovery rates and the completed fixed-configuration results.]

For the triangle upper-tail comparison, the reported entropy values were obtained by deterministic 64-bit numerical integration rather than Monte Carlo sampling. We are additionally checking convergence across integration resolutions, constraint error, and the paired gap \(h_q(W_{\mathrm{LZ}})-h_q(W_{\mathrm{ours}})\) in all six cells. For each learned bipodal profile, we will also fit an analytically constrained four-parameter model to obtain an explicit independent witness. [TODO: insert resolution audit, residuals, paired gaps, and bipodal-fit results.]
...
**1. Number of runs and robustness.** For each problem instance, we trained eight models. These covered all combinations of two activation functions, sinusoidal and GeLU, and four initial learning rates, with one run for each combination. The paper reports the candidate with the lowest objective value among these eight runs, so the selection pool was fixed at eight runs per instance. To measure consistency, we will report how many of the original eight runs recovered the same structure as the reported candidate, up to graphon relabelling, together with their objective distribution and constraint errors on independent samples. Because these runs varied hyperparameters as well as the random initialisation, we are also running 16 independent initialisations with a fixed training configuration on \(K_3\) at \(p=7/9\) and on the key open instances. [TODO: insert the original-sweep recovery rates and the completed fixed-configuration results.]
...
**Runs.** For each problem instance, we trained eight models, one for every combination of the sinusoidal or GeLU activation and four initial learning rates. The paper reports the lowest-objective candidate among these eight runs. We will report how many of the eight recovered the same structure, together with their objective values and constraint errors on independent samples. To isolate sensitivity to random initialisation from sensitivity to these hyperparameters, we are also running 16 independent initialisations with a fixed training configuration on \(K_3\) at \(p=7/9\) and on the key open instances. [TODO: insert the original-sweep recovery rates and the completed fixed-configuration results.]
