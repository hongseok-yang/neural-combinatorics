...
**Robustness.** Every candidate reported in the submitted paper was selected from exactly eight runs, one for each combination of two activations and four learning rates. These eight runs constitute a hyperparameter search and therefore do not by themselves measure robustness. During the rebuttal period, we conducted additional experiments to test the sensitivity of our method to random initialisation, the random batches used to enforce the density constraint, and the choice of Monte Carlo permutations. In all three cases, the effect on the objective was at most on the order of $10^{-3}$, and the discovered structure was stable. First, we repeated the $K_3$ instance at $p=7/9$ ten times using different random initialisations while holding the activation and learning rate fixed at the values used in the ablation study.
The objective values were highly reproducible, with a mean of $0.435711$ and a 95% confidence interval of $[0.435705, 0.435718]$, an interval only $1.3\times10^{-5}$ wide.
Seven of the ten runs recovered the clean 5-block partition. The remaining three produced five-part solutions with comparable objective values, but in each case the smallest part deviated from an exact block.
Because the 5-block construction is not the unique optimiser, this deviation does not indicate suboptimality.
We will apply the same repetition analysis to the open instances $C_5$, $C_7$, and $H_6$ during the discussion period. Second, to quantify sampling error in constraint enforcement, we froze the final $K_3$ network. For each of 1,000 independent batches of size $2^{16}$, we chose the scalar offset that made the batch edge density equal to $p=7/9$ and then evaluated the resulting graphon on a deterministic grid. The grid-evaluated edge-density error had a standard deviation of $1.1\times10^{-3}$ and a maximum absolute value of $3.4\times10^{-3}$.
Finally, we measured sensitivity to the choice of permutation set $S$ in Eq. (8). Training $C_5$ using only the identity, the two cyclic orderings, or the full set of twelve coset representatives produced objectives that agreed to within $1.6\times10^{-4}$ at all six edge densities. Thus, the choice of permutation set affected neither the discovered solution nor its objective.
Separately, we classified the 11.8% of suboptimal cases in our 175-graph study: 7% converged to a suboptimal local optimum, while 4.8% failed because of NaNs or out-of-memory errors.
We will include the results of these robustness experiments and the failure-mode analysis in the revised version.

**Quality of the learnt large-deviation candidates.** The learnt graphons outperform the Lubetzky–Zhao construction by a genuine margin, not by numerical error: they lie within $4\times10^{-3}$ of a numerically optimised bipodal solution, which we believe is near-optimal.
For the six triangle upper-tail settings $(q,r)\in\{0.05,0.10\}\times\{0.4,0.5,0.6\}$, where $q$ is the baseline Erdős-Rényi edge probability and $r^3$ is the target triangle density, the entropies of the learnt graphons were evaluated by discretising each on an $M\times M$ grid and replacing the graphon integrals with finite sums in 64-bit floating-point arithmetic.
...
**Computational cost.** The running time of our framework scales quadratically in the number of motif vertices, linearly in the Monte Carlo batch size, and quadratically in the network width, and the benchmarks we ran during the rebuttal period are consistent with or better than these rates. 
Specifically, with width $d$, depth $L$, batch size $N$, and symmetry set $\mathcal S$, the dominant per-iteration cost is $O(Nv(H)^2Ld^2+N|\mathcal S|e(H))$, where $v(H)$ and $e(H)$ are the numbers of vertices and edges of $H$. 
In the measurements, doubling the batch size multiplies the time by $1.4$-$1.9$ and doubling the width by $2.4$-$2.8$, below the predicted rates because fixed overheads partially mask the leading term. 
Relative to $K_3$, the times of $C_5$ and $C_7$ grow by factors $3.2$ and $8.4$, close to the vertex-pair ratios $3.3$ and $7.0$, with the mild excess for $C_7$ coming from its larger symmetry set. 
...
Training uses batches of $2^{12}$-$2^{16}$ samples; the $2^{28}$ sample size in the submission was a one-time post-training evaluation budget, not a training cost. A 20,000-epoch run takes about 18 minutes for $K_3$ and under an hour for $C_5$ on the single RTX A5000 workstation GPU that ran every submitted experiment. The method is thus practical for the small~moderate sized motifs representative of many concrete problems in extremal graph theory. We will include the complexity analysis and benchmarks in the revised version.
