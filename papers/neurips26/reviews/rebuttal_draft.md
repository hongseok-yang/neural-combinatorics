...
**Rigorous bounds for the large-deviation candidates.** The Table 6 entropy values are computed by deterministic 64-bit numerical integration, not Monte Carlo sampling, and we now enforce the constraint exactly at the evaluation resolution. For every triangle upper-tail cell, we also compare two checks that do not rely on the neural optimiser. A computable lower bound follows from the convex-minorant bound of Lubetzky and Zhao, while numerical optimisation within the four-parameter bipodal family gives an explicit feasible graphon whose constraint and entropy have closed-form expressions and which therefore supplies an upper bound on the optimum. Across all six cells, these bounds bracket the true optimum within $7\times10^{-4}$ to $2.8\times10^{-3}$. The Lubetzky-Zhao construction lies strictly above the feasible end of the bracket in every cell, including the small-gap $q=0.10$ regime, certifying its suboptimality; the revised learned values lie within $4\times10^{-3}$ of the true optimum. The same lower-bound and explicit-bipodal checks apply to $C_4$ and $C_5$, and explicit feasible bipodal graphons beat the Lubetzky-Zhao construction in all 18 cells across the three pattern graphs.
...
t(C_m,W)\ge p^m-p(1-p)^{m-1}.
$$
For $m=3$ this is Goodman's inequality; the theorem extends it to all odd cycles. The bound is nontrivial for $p>1/2$ and sharp at every balanced complete $k$-partite graphon, so those graphons are minimisers at $p=1-1/k$. The theorem has been formalised completely in Lean 4.
...
Third, motivated by the bipodal outputs for the large-deviation problem, we proved a local structural theorem. For every $d$-regular graph $H$ with $d\ge2$ and every phase-boundary point with $r\ne(d-1)/d$, there is a nontrivial open neighbourhood in which the optimiser on the symmetry-breaking side is unique up to relabelling and bipodal, with parameters and optimal value analytic in $(q,r)$. Consequently, the corresponding conditioned dense random graphs concentrate in cut distance around this bipodal structure. This addresses a central open question about explicit nonconstant variational minimisers and their qualitative structure.

We are preparing manuscripts describing these theorems for submission to mathematics journals. The odd-cycle and chordal theorems are formalised completely in Lean 4; the formalisation of the large-deviation theorem takes established results from the literature and parts of graphon theory as axioms. Together, the theorems demonstrate the intended workflow: the neural framework produces interpretable candidate structures, and separate mathematical analysis converts those structures into theorems. We retain precise candidate language for numerical outputs not covered by these results.
