...
Thus the minimiser is the graphon closest in KL divergence to $W \equiv q$ that explains the observed \emph{excess} (upper tail) or \emph{deficit} (lower tail) in $H$-density, in the same spirit as maximum-entropy modelling and variational inference in ML.
...
For the upper-tail problems, the 2-regular graphs $K_3$, $C_4$, $C_5$ share close edge densities and block sizes in their respective learnt optimisers, suggesting the same optimal graphon, while the star $K_{1,3}$ shows a qualitatively different structure with a much smaller block size; see \Cref{fig:upperdeviation} for the learnt graphons with $q=0.1, r=0.5$ and \Cref{fig:upperdeviation-all} for all upper-tail regimes.
