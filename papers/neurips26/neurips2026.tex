...
Our approach represents $W$ as a neural network $f_\theta$, recasts these graphon optimisations as optimisations over the parameters $\theta$, and applies gradient-based training. A naive instantiation of this idea fails for three reasons. First, known optimal graphons are typically step functions, which standard architectures such as MLPs struggle to represent due to their bias toward smooth functions. Second, each occurrence of $t(\cdot, W)$ involves an intractable high-dimensional integral, and estimating these expectations and their powers efficiently requires problem-specific sampling. Third, the scalar constraints in 	\textbf{(P1)} and 	\textbf{(P3)} are poorly handled by penalty terms, so the architecture must enforce them more directly. The remainder of this section describes the architecture and training procedure that address these three challenges.
...
Fix the number of layers $L \ge 2$, the hidden width $d$, a differentiable activation $\phi : \R \to \R$, and an increasing function $s : [L] \to (0, \infty)$ for scheduling the scale of weight initialisation at each layer $\ell \in [L]$; we consider schedules such as $s(\ell) = \ell$ and $s(\ell) = 2^{\ell-1}$. The network $f_\theta[c]$ has a distinguished scalar bias $c \in \R$ and remaining parameters $\theta$, and is defined for $2 \le \ell \le L$ by
\begin{align*}
...
\end{align*}
Here $\mathit{sm}$ is the sigmoid, and $g^{(\ell)}_\theta : \{\mathbf{x} = (x_1, x_2) \in [0,1]^2 : x_1 \le x_2\} \to \R^d$. The network parameters are
$c \in \R$, $b^{(\ell)} \in \R^d$, $V \in \R^{1 \times d}$, $U^{(\ell)} \in \R^{d \times 2}$, and $V^{(\ell')}, W^{(\ell)} \in \R^{d \times d}$, for $\ell' \in [L-1]$, $\ell \in [L]$. These parameters are initialised i.i.d.\ by
\begin{align*}
...
\end{align*}
for all $i, j \in [d]$, $k \in [2]$. We collect every parameter except $c$ into $\theta$. The use of $\min$ and $\max$ in the definition of $f_\theta[c]$ enforces $f_\theta[c](x_1, x_2) = f_\theta[c](x_2, x_1)$, so $f_\theta[c]$ is symmetric and represents a graphon.
...
For 	\textbf{(P1)}, fix a graph $H$ on $[v(H)]$ and $p \in (0, 1)$. At each iteration, we sample $(N \cdot v(H))$ i.i.d.\ uniform values $\{x_i^{(k)}\}$ on $[0, 1]$, form
