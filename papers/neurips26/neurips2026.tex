...
Discretisation-based computation of $t(H, W)$ on an $M \times M$ grid scales as $O(M^{v(H)})$, which becomes infeasible for motifs with many vertices, such as the Petersen graph; Monte Carlo estimation, while less sensitive to $v(H)$, can suffer from large relative error when the motif density is very small.
