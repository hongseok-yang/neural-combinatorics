...
We demonstrate our framework on two families of graphon optimisation problems that share these characteristics but originate in distinct mathematical contexts: \textbf{(P1)} generalised Tur\'an problems, minimising a homomorphism density under a fixed edge-density constraint; and \textbf{(P2)} the variational problem of \citet{chatterjee2011large}, minimising a relative-entropy functional under a fixed subgraph-density constraint.
On \textbf{(P1)}, our framework rediscovers nontrivial known optima that previously required substantial human effort~\citep{goodman1959on,bollobas1976relations,lovasz1983on,fisher1989lower,razborov2008minimal}, produces candidate minimisers for open instances such as $C_7$-density minimisation, and through a systematic sweep over small graphs, uncovers, for a six-vertex graph, a previously unreported family of extremal structures that do not match the complete $k$-partite pattern of classical examples.
...
Graphons provide a common language for both problem classes studied in this paper: the objective and constraints in each involve graphon homomorphism densities $t(H,W)$ or closely related graphon integrals, and the structure of the optimisers is the central object of interest. The first class arises from extremal graph theory, where optimising over graphons is equivalent to optimising the corresponding asymptotic finite-graph problem~\citep{lovasz2012large}; the second arises from large deviation theory of dense random graphs. We describe each in turn.
...
which say that for every $H$ and $p \leq 1 - 1/(\chi(H) - 1)$, where $\chi(H)$ is the chromatic number of $H$, the optimum is $0$. But for $p > 1 - 1/(\chi(H) - 1)$, the problem is much harder. The problem is completely solved only for cliques: $H = K_r$ for all $r \ge 3$~\citep{reiher2016clique}, with landmark earlier results for $r = 3$~\citep{razborov2008minimal} and $r = 4$~\citep{nikiforov2011number}. For $C_5$, the minimiser is known only at the special values $p = 1 - 1/k$ ($k$ a positive integer)~\citep{bennett2020minimizing}.

\paragraph{Variational problem from large deviation theory.}
Large deviation theory for random graphs addresses two related questions: at what exponential rate does $G(n,q)$ produce an atypically large or small subgraph density, and what does the graph typically look like when such a rare event occurs?
...
Thus the minimiser is the graphon closest in KL divergence to $W \equiv q$ that explains the observed \emph{excess} (upper tail) or \emph{deficit} (lower tail) in $H$-density, in the same spirit as maximum-entropy modelling in ML\@.
...
For the upper-tail problems, the 2-regular graphs $K_3$, $C_4$, $C_5$ share close edge densities and block sizes in their respective learnt optimisers, suggesting the same optimal graphon, while $K_{1,3}$ shows a qualitatively different structure with a much smaller block size; see \Cref{fig:upperdeviation} for the learnt graphons with $q=0.1, r=0.5$ and \Cref{sec:addexperiments} for all upper-tail regimes.
...
\tk{I modified the text to say 'in the upper tail problems for $H = K_3$' instead of 'for $H = K_3$', since the construction by LZ only applies to the upper tail. Also, the `suggesting the same optimal graphon' is a misleading argument, as while the learnt graphons are similar, the numerical optimisation found that they are in fact not the same, just very close.}
