...
Graphons provide a common language for both problem families studied in this paper: the objective and constraints in each involve graphon homomorphism densities $t(H,W)$ or closely related graphon integrals, and the structure of the optimal grpahons is the central object of interest. The first family arises from extremal graph theory, where optimising over graphons is equivalent to optimising the corresponding asymptotic finite-graph problem~\citep{lovasz2012large}; the second arises from the large deviation theory of dense random graphs. We describe each in turn.

\noindent{\bf Constrained density minimisation.}\
The first task is to minimise the homomorphism density of a fixed graph $H$ subject to a fixed edge density $p$. To state the asymptotic problem formally, we phrase it in terms of graph sequences:
\begin{equation}
...
that for every $H$ and $p \leq 1 - 1/(\chi(H) - 1)$, where $\chi(H)$ is the chromatic number of $H$, the optimum is $0$. But for $p > 1 - 1/(\chi(H) - 1)$, the problem is much harder. It is completely solved only for cliques: $H = K_r$ for all $r \ge 3$~\citep{reiher2016clique}, with landmark earlier results for $r = 3$~\citep{razborov2008minimal} and $r = 4$~\citep{nikiforov2011number}. For $C_5$, the minimiser is known only at the special values $p = 1 - 1/k$ ($k$ a positive integer)~\citep{bennett2020minimizing}.

\noindent{\bf Variational problem from large deviation theory.}\ 
The large deviation theory for random graphs addresses two related questions: at what exponential rate does $G(n,q)$ produce an atypically large or small subgraph density, and what does the graph typically look like when such a rare event occurs?
...
Concretely, fix $q \in (0,1)$ and let $h_q(W) \coloneqq \int_{[0,1]^2} \mathrm{KL}(\mathrm{Bern}(W(x,y))\,\|\,\mathrm{Bern}(q))dx dy$, the integrated per-edge KL divergence of $W$ from the base Erd\H{o}s--R\'enyi graphon $W \equiv q$.
...
\begin{table}[t]
  \centering
  \captionof{table}{Relative-entropy $h_q(W)$ at our learnt graphon ($W_{\mathrm{ours}}$) and at the one-clique-on-uniform construction $W_{\mathrm{LZ}}$ of \citet{lubetzky2015replica}, for \textbf{(P2)} with $H = K_3$ on $(q,r) \in \{0.05, 0.1\} \times \{0.4, 0.5, 0.6\}$. Lower is better.\label{tab:largedeviation}}
  {\small
...
We test $(q, r) \in \{0.05, 0.1\} \times \{0.4, 0.5, 0.6\}$ for upper-tail variational problems with $H \in \{K_3, C_4, C_5, K_{1,3}\}$, and $(q, r) \in \{0.7, 0.8, 0.9\} \times \{0.05, 0.1\}$ for lower-tail variational problems with $H = K_3$; in all these regimes the structure of optimisers is, to our knowledge, not fully understood. As a reference for $H = K_3$, we use the one-clique-on-uniform construction $W_{\mathrm{LZ}}$ proposed by \citet{lubetzky2015replica}; \Cref{tab:largedeviation} compares $h_q(W_{\mathrm{LZ}})$ to $h_q(W_{\mathrm{ours}})$ at our learnt graphon $W_{\mathrm{ours}}$ in the upper-tail regime.
...
Quantitatively, in the upper-tail problems for $H = K_3$, our method consistently finds graphons with lower relative entropy than the one-clique-on-uniform construction $W_{\mathrm{LZ}}$, showing that the latter is suboptimal in these regimes; see \Cref{sec:addexperiments} for details.
...
Our work presents a tools for finding structured candidate optima of graphon variational problems, which are of interest in extremal graph theory and the large deviation theory of random graphs. 
Our methods allows highly interpretable candidate optima compared to baselines, which can provides the insights for structure of optima and thus support mathematical conjecture generation and discovery.
Our work does not include any major negative societal impacts.

While our method is effective across a range of graphon variational problems, it is limited to graphon optimisation problems, therefore our method can not be applied to combinatorial problems involving sparse graphs.
While our method suggests highly interpretable candidate optima, it does not provide rigorous guarantees on optimality, and should be used in conjunction with rigorous methods such as flag algebras to verify optimality and to provide certificates.
We utilised either discretisation based computation or Monte-Carlo estimation for computing homomorphism densities when evaluating our methods, both of them can be computationally expensive for large motifs, for instance, discretisation based computation is infeasible for Petersen graph cliques due to high-exponent scaling of einsum used in computation, and Monte Carlo estimation can be prone to high relative error when the motif density is very low.
...
We also report the complete results of the variational problem discussed in \Cref{sec:experiments}, see \Cref{fig:k3upperdeviation,fig:k3lowerdeviation,fig:c4upperdeviation,fig:c4lowerdeviation} for the found graphons.
For the quantitative comparison, the one-clique-on-uniform construction $W_{LZ}$ only applies to the upper tail problem for 2-regular graphs, see \Cref{tab:upperdeviationcycle} for the comparison between the relative entropies of the found graphons and the one-clique-on-uniform construction under constraints $t(C_k, W)\ge r^k$, and \Cref{tab:lowerdeviation} for the upper tail problem for $K_{1,3}$ and the lower tail problem for $K_3$. 

\begin{table}[t]
  \centering
  \captionof{table}{Relative-entropy $h_q(W)$ at our learnt graphon ($W_{\mathrm{ours}}$) and at the one-clique-on-uniform construction $W_{\mathrm{LZ}}$ of \citet{lubetzky2015replica}, for \textbf{(P2)} with $H = C_4, C_5, K_{1,3}$ on $(q,r) \in \{0.05, 0.1\} \times \{0.4, 0.5, 0.6\}$. Lower is better.\label{tab:upperdeviationcycle}}
  {\small
  \begin{tabular}{lcccccc}
    \toprule
    & \multicolumn{3}{c}{$q=0.05$} & \multicolumn{3}{c}{$q=0.10$} \\
    ...
    $h_q(W_{\mathrm{ours}})$, $H=C_5$  & $0.000000$ & $0.000000$ & $1.000000$ & $0.308858$ & $0.506122$ & $0.746845$ \\
    \midrule
    $h_q(W_{\mathrm{ours}})$, $H=K_{1,3}$    & $0.000000$ & $0.000000$ & $1.000000$ & $0.263666$ & $0.484864$ & $0.748724$ \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\begin{table}[t]
  \centering
  \captionof{table}{Relative-entropy $h_q(W)$ at our learnt graphon ($W_{\mathrm{ours}}$) and at the one-clique-on-uniform construction $W_{\mathrm{LZ}}$ of \citet{lubetzky2015replica}, for \textbf{(P2)} with $H = K_3$ on $(q,r) \in \{0.7, 0.8, 0.9\} \times \{0.05, 0.1\}$. Lower is better.\label{tab:lowerdeviation}}
  {\small
  \begin{tabular}{lcccccc}
    \toprule
    & \multicolumn{3}{c}{$q=0.05$} & \multicolumn{3}{c}{$q=0.10$} \\
