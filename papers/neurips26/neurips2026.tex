...
\end{equation}
for graphs $H_1, H_2$ and positive integers $m_1, m_2$. The key question is typically whether this asymptotic optimum is non-negative; if so, the inequality $t(H_1, G)^{m_2} \geq t(H_2, G)^{m_1}$ holds asymptotically for all $G$. Sidorenko's conjecture~\citep{sidorenko1992, sidorenko1993correlation} predicts non-negativity when $H_1$ is bipartite, $H_2 = K_2$, and $(m_1, m_2) = (e(H_1), e(H_2))$, and more systematic studies of general pairs $(H_1, H_2)$ appear in recent work~\citep{conlonlee2024domination, behague2025}.
