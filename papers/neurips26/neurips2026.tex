...
Beyond extremal graph theory, the same machinery applies to graphon variational problems arising in dense random-graph large-deviation theory: minimising a relative-entropy functional under a triangle-density constraint characterises the upper tail of triangle counts in Erd\H{o}s--R\'enyi graphs.
...
\nonumber
\end{align}
where $H_1, \ldots, H_k, F_1, \ldots, F_m$ are graphs and $f : \R^k \to \R$, $g_j : \R^m \to \R$ are real-valued functions. We focus on two extremal-graph-theory cases below; after introducing graphons, we describe a third application that is intrinsically a graphon problem and arises from large-deviation theory. The display above is a schematic finite-$n$ form; when an equality constraint is not exactly attainable for every graph size, we use the graph-sequence formulation below.
...
For (P3), we reuse the constrained training loop with the triangle-density solver targeting $t(K_3, f_\theta[c]) = r^3$, and minimise the Monte Carlo estimate of $h_q$ obtained by averaging $\mathrm{KL}(\mathrm{Bern}(f_\theta[c_\theta](x,y))\,\|\,\mathrm{Bern}(q))$ over a fresh batch of pairs $(x,y)$. Because the sigmoid output lies in $(0,1)$, the KL objective is differentiable in $\theta$. Experiments for this application are in progress and will appear in a later version.
...
We empirically evaluate our method on resolved and unresolved extremal-graph-theory problems that fit (P1) or (P2); experiments for (P3) are deferred to a later version. The goals are to test whether the method (i) can rediscover known optima that required substantial human effort and (ii) can produce mathematically interesting candidates for open problems. We also include an ablation study that assesses the impact of key components of our architecture and training procedure.
