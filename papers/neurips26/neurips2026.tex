...
Our work presents a method for finding structured candidate optima of graphon variational problems, which is of interest to mathematicians working on extremal graph theory and the large deviation theory of random graphs.
...
First, it is tailored to graphon optimisation and therefore does not apply to combinatorial problems on sparse graphs, where the graphon limit is degenerate.
Second, although the candidate optima it produces are highly interpretable, the method itself does not provide rigorous guarantees of optimality; it should therefore be used in conjunction with rigorous techniques such as flag algebras to verify optimality and to provide certificates.
...
For example, discretisation-based computation becomes infeasible for cliques of the Petersen graph due to the high-exponent scaling of the underlying einsum, while Monte Carlo estimation can suffer from large relative error when the motif density is very small.
