...
For the other graphs, we used a set $\mathcal{S}=\{\pi^j : j = 0, 1, \ldots, m-1\}$ generated by a permutation $\pi \in S_{V(G)}$, where $\pi$ and $m$ are chosen so that the set
\[
  \{\{\pi^j(v), \pi^j(w)\} : \{v, w\} \in E(G) \text{ and } j = 0, 1, \ldots, m-1\}
\]
...
\section{Conjecture for odd cycles} \label{sec:oddcycleconjecture}
