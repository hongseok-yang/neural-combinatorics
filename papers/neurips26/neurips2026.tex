...
\]
where $\widehat{t}_{\mathcal{D}}$ denotes the estimator~\eqref{eqn:estimator} evaluated on the tuples in $\mathcal{D}$. We resample $(\mathcal{D}, \mathcal{E})$ at every iteration to mitigate overfitting to particular Monte Carlo samples; reusing the $x_i^{(k)}$ across $\mathcal{D}$ and $\mathcal{E}$ saves forward evaluations of $f_\theta[c]$.
We set $\eta_t$ by cosine-annealing scheduling~\citep{ilya2017sgdr}, which alternates between discovering coarse structure (high learning rate) and refining sharp boundaries (low learning rate). 
