...
\begin{abstract}
Machine learning is opening new directions in mathematical discovery, from automatic provers for Olympiad problems to progress on long-standing conjectures. We propose a neural exploratory framework for \emph{asymptotic extremal problems} in graph theory, which study graph optimisation as the number of vertices tends to infinity. Such problems can often be reformulated as continuous optimisation over \emph{graphons}---symmetric measurable functions $[0,1]^2 \to [0,1]$ that arise as limits of dense graph sequences. We represent graphons by implicit neural networks and optimise graphon objectives by gradient descent. Our design combines three ingredients: a multi-scale sinusoidal residual architecture biased toward sharp, step-like graphons; an embedded solver that enforces a single empirical density constraint and is differentiated by implicit differentiation; and symmetry-aware Monte Carlo estimators for homomorphism densities. On generalised Tur\'an problems and density-domination problems that previously required substantial human effort, our method rediscovers known optimal graphons without human intervention. On open problems of the same kind, it produces candidate optima and candidate inequality violations, including a previously unreported family of extremal structures. We also formulate its application to the graphon variational problem governing upper-tail large deviations for triangle counts in Erd\H{o}s--R\'enyi random graphs. The method produces numerical candidates and structural conjectures rather than certificates, and is intended to complement rigorous tools such as flag algebras.
\end{abstract}
...
We evaluate this framework on a range of constrained and unconstrained graphon optimisation problems. It recovers nontrivial known results, including triangle-density and $5$-cycle-density minimisation under an edge-density constraint~\citep{goodman1959on,bollobas1975relations,lovasz1983on,fisher1989lower,razborov2008minimal}, that previously required substantial human effort. Applied to open problems, the same framework produces candidate minimisers for the $7$-cycle density under the same constraint, candidate violations of clique-density inequalities, and---through a systematic sweep over small graphs---a previously unreported family of candidate extremal structures for a six-vertex graph that does not match the complete $k$-partite pattern of classical examples.
As an additional application, the same constrained graphon optimisation machinery applies to variational problems from dense random-graph large-deviation theory, where minimising a relative-entropy functional under a triangle-density constraint describes the upper tail of triangle counts in Erd\H{o}s--R\'enyi graphs.
...
\nonumber
\end{align}
where $H_1, \ldots, H_k, F_1, \ldots, F_m$ are graphs and $f : \R^k \to \R$, $g_j : \R^m \to \R$ are real-valued functions. We focus on two extremal-graph-theory cases below; after introducing graphons, we also describe a large-deviation application that natively lives in the graphon formulation. The display is a schematic finite-$n$ form; when an equality constraint is not exactly attainable for every graph size, we use the graph-sequence formulation below.
...
\paragraph{Large-deviation variational problem.}
Graphons also provide the natural variational language for dense random-graph large deviations. Fix $q \in (0,1)$, and let $h_q(W) \coloneqq \int_{[0,1]^2} \mathrm{KL}\bigl(\mathrm{Bern}(W(x,y))\,\|\,\mathrm{Bern}(q)\bigr)\,dx\,dy$. The Chatterjee--Varadhan large-deviation principle~\citep{chatterjee2011large,chatterjee2017large} implies that, for $r > q$, $\binom{n}{2}^{-1}\log\Pr[t(K_3, G(n,q)) \ge r^3]$ converges to $-\inf_{W: t(K_3,W) \ge r^3} h_q(W)$, and the typical structure of $G(n,q)$ conditioned on this upper-tail event is described by near-minimisers of the same variational problem~\citep{lubetzky2015replica}. Our method applies directly to the boundary form
\begin{equation}
\label{eqn:large-deviation-problem}
  ...
  t(K_3,W) = r^3,
\end{equation}
which is the equality-constrained problem when the upper-tail optimum lies on the active boundary.

\section{Implicit Neural Representations of Graphons} \label{sec:method}

We tackle the graphon formulations of the two extremal problem classes from \Cref{sec:background}, together with the large-deviation application in~\eqref{eqn:large-deviation-problem}:
\begin{align}
...
\nonumber
\\
\textbf{(P3)}\quad &\min_W\, h_q(W)
\quad \text{subject to } t(K_3, W) = r^3,
\label{eqn:targetproblems}
\end{align}
for graphs $H, H_1, H_2$, reals $p,q,r \in (0,1)$, and positive integers $m_1, m_2$, where $W$ ranges over graphons. Problems (P1) and (P3) have a single scalar equality constraint; (P2) is unconstrained.

Our approach represents $W$ as a neural network $f_\theta$, recasts these graphon programs as optimisation over the parameters $\theta$, and applies gradient-based training. A naive instantiation of this idea fails for three reasons. First, known optimal graphons are typically step functions, which standard architectures such as MLPs struggle to represent due to their bias toward smooth functions. Second, each occurrence of $t(\cdot, W)$ involves an intractable high-dimensional integral, and estimating these expectations and their powers efficiently requires problem-specific sampling; the entropy objective $h_q(W)$ in (P3) is another graphon integral that must be estimated stochastically. Third, the scalar constraints in (P1) and (P3) are poorly handled by penalty terms, so the architecture must enforce them more directly. The remainder of this section describes the architecture and training procedure that address these three challenges.

\paragraph{Architecture.}
We parameterise graphons by a residual MLP that injects a layer-specific sinusoidal encoding of the input at every layer. When used for constrained problems (P1) and (P3), the architecture additionally includes a constraint solver.

Fix the number of layers $L \ge 2$, the hidden width $d$, a differentiable activation $\phi : \R \to \R$, and an increasing scale function $s : [L] \to (0, \infty)$; we consider schedules such as $s(\ell) = \ell$ and $s(\ell) = 2^{\ell-1}$. The network $f_\theta[c]$ has a distinguished scalar bias $c \in \R$ and remaining parameters $\theta$, and is defined for $2 \le \ell \le L$ by
\begin{align*}
...
Second, in constrained problems we determine $c$ via a numerical solver rather than learning it directly. To enforce $t(K_2, f_\theta[c]) = p$ in (P1), we use a batch of $K$ i.i.d.\ samples
\[
...
The same idea applies to any single homomorphism-density constraint of the form $t(F, W)=\tau$: one replaces the edge-density estimator in~\eqref{eqn:solver-specification} by an estimator of $t(F, f_\theta[c])$. Because increasing the scalar bias $c$ increases every edge probability, this empirical constraint remains a monotone scalar equation in $c$. For the large-deviation application (P3), this means sampling triples $(x,y,z)$ and solving the empirical equation
\[
  \frac{1}{|\mathcal{T}|}
  \sum_{(x,y,z)\in\mathcal{T}}
  f_\theta[c](x,y)f_\theta[c](y,z)f_\theta[c](z,x)
  = r^3.
\]
Our present implementation does not exactly enforce multiple simultaneous constraints; extending the solver to vector-valued root finding or projection-based updates is an important direction for future work.
...
\end{equation}
where $H \oplus H'$ is the disjoint union of $H$ and $H'$. Concretely, we expand the polynomial into a linear combination of monomials, rewrite each monomial as a single homomorphism density via~\eqref{eqn:elimination densityproduct}, estimate each resulting density with~\eqref{eqn:estimator}, and combine the estimates linearly. In implementation, each monomial is estimated as a U-statistic by using disjoint vertex-sample tuples for the copies in the disjoint union whenever possible, which avoids reusing the same sampled vertex as two independent variables within a monomial. We use this extension for (P2).
...
\noindent\textit{Large-deviation variational problem.}
For (P3), with parameters $q,r$, we use the same constrained loop with $F=K_3$ and target $r^3$, and minimise the Monte Carlo estimate of $h_q$ obtained by averaging $\mathrm{KL}(\mathrm{Bern}(f_\theta[c_\theta](x,y))\,\|\,\mathrm{Bern}(q))$ over a fresh batch of pairs $(x,y)$. Since the sigmoid output lies in $(0,1)$, the KL objective is differentiable in the network parameters. Experimental results for this application will be reported in a later version.
...
We empirically evaluate our method on resolved and unresolved extremal-graph-theory problems that fit (P1) or (P2) in \Cref{eqn:targetproblems}; experiments for the large-deviation application (P3) will be added in a later version. The goals are to test whether the method (i) can rediscover known optima that required substantial human effort and (ii) can produce mathematically interesting candidates for open problems. We also include an ablation study that assesses the impact of key components of our architecture and training procedure.
...
For $p\in\{4/7,5/8\}$, which satisfy $1-1/k<p<1-1/(k+1)$, the learnt graphons (\Cref{fig:k3-57,fig:k3-62}) capture the core structure of the known optimal $W_p$.
For $p=5/7$, the learnt graphon (\Cref{fig:k3-71}) does not match $W_p$, but its bipartite middle-block structure matches the characterisation of all optimal graphons in~\citep{pikhurko2017asymptotic}.
...
For $K_k$ with $k>3$, we ran $\min_W t(K_k, W)$ s.t.\ $t(K_2, W){=}1{-}1/k$ for $k\in\{4,5,6\}$. Despite the increasing complexity, our method found near-optimal graphons (\Cref{fig:clique}) showing the complete $k$-partite structure.
