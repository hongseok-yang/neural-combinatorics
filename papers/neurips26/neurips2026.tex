...
As there is no known conjectured optimum, we compare our results to the best Petersen-graph density obtained over all hand-crafted graphons $W_1,\ldots,W_4$ and the constant graphon defined in \Cref{sec:handcraftedgraphons}.
We estimated the Petersen homomorphism density for each learnt graphon using a Monte Carlo estimator; the ``Ours (Estimated)'' column in \Cref{tab:petersenresult2} reports 95\% bootstrap confidence intervals $[\ell, u]$, since discretisation-based computation is infeasible for large graphs at high resolution.

\begin{table}[t]
    \centering
  \caption{Homomorphism densities of the Petersen graph under the edge-density constraint $t(K_2, W)=p$, found by our method and the hand-crafted constructions. ``Ours (Estimated)'' is a 95\% bootstrap confidence interval $[\ell, u]$.\label{tab:petersenresult2}}
  {\small
    \begin{tabular}{cll}
         \toprule
...
The next two graphons are constructed by modifying the complete $(k+1)$-partite structure.
We partition the interval $[0,1]$ into $k+1$ groups $X_1, \ldots, X_{k+1}$ with $|X_1| = \cdots = |X_{k+1}| = 1/(k+1)$, and define
\begin{align*}
