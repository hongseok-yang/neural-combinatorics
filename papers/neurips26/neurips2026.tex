...
We evaluate this single framework on three classes of graphon variational problems that share these characteristics but originate in distinct mathematical contexts: \textbf{(P1)} generalised Tur\'an problems---minimising a homomorphism density under a fixed edge-density constraint; \textbf{(P2)} density-domination problems---testing whether one homomorphism-density inequality dominates another; and \textbf{(P3)} the upper-tail variational problem of \citet{chatterjee2011large}---minimising a relative-entropy functional under a fixed triangle-density constraint. On (P1) and (P2), our framework rediscovers nontrivial known optima that previously required substantial human effort~\citep{goodman1959on,bollobas1975relations,lovasz1983on,fisher1989lower,razborov2008minimal}, produces candidate minimisers for open instances such as $7$-cycle density, identifies candidate violations of clique-density inequalities, and---through a systematic sweep over small graphs---uncovers a previously unreported family of extremal structures for a six-vertex graph that does not match the complete $k$-partite pattern of classical examples. On (P3), the same architecture, solver, and training loop apply with only a change of objective and of the constrained motif, providing a uniform handle on a problem class whose optimisers describe the conditional structure of Erd\H{o}s--R\'enyi graphs in their upper-tail regime.
...
We evaluate our framework on instances of (P1), (P2), and (P3) drawn from the extremal-graph-theory and large-deviation literatures. The same architecture, solver, and training loop are used throughout; the only changes between problem classes are which constraint, if any, is enforced and how the objective is estimated. The experiments are organised to demonstrate three capabilities: (i) rediscovering classical optima that previously required substantial human effort, (ii) producing mathematically interesting candidates for open instances, and (iii) extending the same machinery beyond extremal graph theory to upper-tail large deviations. An ablation study at the end isolates the contribution of each architectural component.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccccc}
    ...
    \midrule
    $h_q(W_{\mathrm{LZ}})$    & \TODO{} & \TODO{} & \TODO{} & \TODO{} & \TODO{} & \TODO{} \\
    $h_q(W_{\mathrm{ours}})$  & \TODO{} & \TODO{} & \TODO{} & \TODO{} & \TODO{} & \TODO{} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
