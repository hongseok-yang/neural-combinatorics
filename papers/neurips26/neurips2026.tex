...
For the upper-tail problems, the 2-regular graphs $K_3$, $C_4$, $C_5$ share close edge densities and block sizes in their respective learnt optimisers, suggesting the same optimal graphon, while $K_{1,3}$ shows a qualitatively different structure with a much smaller block size; see \Cref{fig:upperdeviation} for the learnt graphons with $q=0.1, r=0.5$ and \Cref{fig:upperdeviation-all} for all upper-tail regimes.
For the lower-tail problems, we obtain a qualitatively different structure of nearly-bipartite graphons.
Quantitatively, for $H = K_3$ our method consistently finds graphons with lower relative entropy than the one-clique-on-uniform construction $W_{\mathrm{LZ}}$, showing that the latter is suboptimal in these regimes; see \Cref{tab:largedeviation} for details.
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{Learnt graphons for upper-tail variational problems with $q=0.1, r=0.5$ for $H = K_3, C_4, C_5, K_{1,3}$ (left to right). \TODO{To be replaced with version corresponding to other figures. }}
