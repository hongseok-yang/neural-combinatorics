...
On \textbf{(P2)}, the same architecture, solver, and training loop apply with only a change of objective and of the constraint type, providing a uniform handle on a problem class whose optimisers describe the conditional structure of Erd\H{o}s--R\'enyi graphs in both upper-tail and lower-tail regimes.
...
We test $(q, r) \in \{0.05, 0.1\} \times \{0.4, 0.5, 0.6\}$ for upper tail variational problems for graphs $H = K_3, C_4, C_5, K_{1,3}$, and $(q, r) \in \{0.7, 0.8, 0.9\} \times \{0.05, 0.1\}$ for lower tail variational problems for $H = K_3$, which are all regime the structure of optimisers is, to our knowledge, not fully understood. As a reference, we use the one-clique-on-uniform construction $W_{\mathrm{LZ}}$ proposed by \citet{lubetzky2015replica}; \Cref{tab:largedeviation} compares $h_q(W_{\mathrm{LZ}})$ to $h_q$ at our learnt graphon in the upper tail regimes.
Qualitatively, the learnt graphons consistently founds two-block structures for all graphs and both upper and lower tail regimes.
For the upper tail problems, the 2-regular graphs $K_3, C_4, C_5$ shares a close edge densities and the block size, suggesting the same underlying optimisation landscape; while the star $K_{1,3}$ shows a different structure with a much smaller block size, see \Cref{fig:upperdeviation} for the learnt graphon with $q=0.1, r=0.5$ and \Cref{fig:upperdeviation-all} for all upper tail regimes.
For the lower tail problems, we obtain the qualitatively different structure of nearly-bipartite graphons.
Quantitatively, our method consistently finds graphons with lower relative entropy than the one-clique-on-uniform construction, suggesting that the latter is suboptimal in these regimes, see \Cref{tab:largedeviation} for details.
...
\begin{table}[t]
  \centering
  \captionof{table}{Relative-entropy $h_q(W)$ at our learnt graphon ($W_{\mathrm{ours}}$) and at the one-clique-on-uniform construction $W_{\mathrm{LZ}}$ of \citet{lubetzky2015replica}, for \textbf{(P2)} on $(q,r) \in \{0.05, 0.1\} \times \{0.4, 0.5, 0.6\}$. Lower is better.\label{tab:largedeviation}}
  {\small
