...
We test $(q, r) \in \{0.05, 0.1\} \times \{0.4, 0.5, 0.6\}$ for upper-tail variational problems with $H \in \{K_3, C_4, C_5, K_{1,3}\}$, and $(q, r) \in \{0.7, 0.8, 0.9\} \times \{0.05, 0.1\}$ for lower-tail variational problems with $H = K_3$; in all these regimes the structure of optimisers is, to our knowledge, not fully understood. As a reference for $H = K_3$, we use the tripartite construction $W_{\mathrm{LZ}}$ proposed by \citet{lubetzky2015replica}; \Cref{tab:largedeviation} compares $h_q(W_{\mathrm{LZ}})$ to $h_q(W_{\mathrm{ours}})$ at our learnt graphon $W_{\mathrm{ours}}$ in the upper-tail regime.
...
For the quantitative comparison, the tripartite construction $W_{LZ}$ only applies to the upper tail problem for 2-regular graphs, see \Cref{tab:upperdeviationcycle} for the comparison between the relative entropies of the found graphons and the tripartite construction under constraints $t(C_k, W)\ge r^k$, and \Cref{tab:lowerdeviation} for the upper tail problem for $K_{1,3}$ and the lower tail problem for $K_3$. 
